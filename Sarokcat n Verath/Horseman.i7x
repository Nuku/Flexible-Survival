...
lostToHorseman is a number that varies. lostToHorseman is usually 0. [utilize in later updates]
...
To say Horseman loses:
	say "     [if horsefight is not 2]The powerful horseman[else]The muscled equine farmhand[end if] shakes his head slightly to clear it before [if horsefight is not 2]trying to punch[else]charging right at[end if] you again, obviously not knowing when he's beat. His attack is clumsy and easily avoided, and it's easy enough to sidestep and [if horsefight is 0]guide him right into a wall[else]send him crashing to the ground. You hear a solid thud [if horsefight is 0]on impact[else]when his head hits a rock[end if]. The powerful beast groans and collapses[if horsefight is 0] up against the wall.[else if horsefight is 1] in the grass.[else if horsefight is 2].[end if] When he turns [if horsefight is not 0]himself over[else]to rest his head on the wall[end if], you see the stupefied look on his face. He's clearly stunned.";
	say "[beatTheHorseman]";
