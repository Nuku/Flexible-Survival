...
To say butterfly defeat:
	...
	if ButterflyForeplay is False:
		...
		else if breast size of player is greater than 0:
			if a random number from 1 to 100 is less than the libido of the player:
		...
		else if cunts of player is greater than 0 and a random number from 1 to 100 is less than the libido of the player:
			let speech be "";
			if ButterflyRaped is 0:
				now speech is "'You didn't need to be so rough' she protests with tears forming in her eyes. As you approach she fidgets anxiously, worried at what you may do. She ponders escape but still hopes that you may continue where you previously left off.";
			else:
	...
	if ButterflyRaped is 1 or ButterflyRaped is 2:
		...
		if cocks of player is greater than 0:
			...
			if "One Way" is not listed in feats of player:
				...
				say " Strange [one of]erotic tingles[or]cold waves[or]hot flashes[at random] run over your [one of]cock[or]man meat[or]shaft[or]pole[at random] begins to shrink. [if cunts of player is greater than 1]They[else]It[end if] dwindles in size, becoming [descr]. ";
			...
			if cunts of player is not 0 and "One Way" is not listed in feats of player:
				...
				say " Strange [one of]erotic tingles[or]cold waves[or]hot flashes[at random] run over your [one of]cunt[or]pussy[or]vagina[or]cleft[at random] begins to shrink. [if cunts of player is greater than 1]They[else]It[end if] dwindles in size, becoming [descr]. ";
		...
		if cocks of player is greater than 0:
			...
			now ButterflyForeplay is True;
		else if breast size of the player is greater than 0:
...
to say testosterone pill use:
	...
	if cunts of player > 0 and cocks of player > 0:		[HERM]
		...
		follow the cock descr rule;
		say "You can see your [if cocks of player is 1][one of]cock[or]penis[or]shaft[or]maleness[at random][else][one of]cocks[or]malenesses[at random][end if] [one of]engorge[or]swell[or]throb[at random] as [if cocks of player is greater than 1]they gain[else]it gains[end if] in length, becoming [descr]! ";
...
to say butterfly grove scene:
	...
	if ButterflyPregnant is True and cocks of player is 0: [How'd you manage that?]
		...
		say "You gain 1 x testosterone pill!";
		now carried of testosterone pill is 1;
	else if ButterflyRaped is greater than 1 and ButterflyPregnant is True and gestation of ButterflyBaby is 3:
		...
		stop the action;
	else if ButterflyRaped is greater than 1 and ButterflyPregnant is False:
		repeat with y running from 1 to number of filled rows in table of random critters:
			choose row y in table of random critters;
			if name entry is "Butterfly":
				let debit be 0;
		...
		if player_returned_negligee is False and cocks of player is greater than 0 and ButterflyPregnant is False:
			...
			if ButterflyCanGetPregnant is True:
				...
				impregnatebutterfly;
			wait for any key;
		else if player_returned_negligee is False and cunts of player is greater than 0:
		...
		increase perception of player by 2;
		wait for any key;
	else if ButterflyProcreated is True and ButterflyLove is True and ButterflyRaped is 0 and cocks of player is greater than 0:
		say "The butterfly girl is hovering before you. [ButterflyAttire][ButterflyTummy][line break][line break]";
		if waiterhater is 0, wait for any key; [skips waiting if it's not wanted]
		...
		if the player consents:
			if cute panties is owned:
				say "Your naked lover spots something sticking out of your pocket and looks at you with a curious expression. You pull the pair of cute panties out and show them to her. She stretches them out examining them closely, her antennae twitching quizzically, but she remains perplexed. You offer to show her their purpose, and taking the panties back you stoop down and lift one of her legs through the appropriate hole, and then move on to the other leg as she reacts in surprise. Pulling them up her velvet thighs and in to place you realize how lucky it is that they're low-cut since they don't get in the way of her large abdomen. She twists around with her arms in the air to view them from various angles. Still curious she tugs at them clearly not used to the feeling. Your [cunt size desc of player] vagina[if cunts of player is greater than 1]s[end if] [if cunts of player is greater than 1]were[else]was[end if] already becoming damp, but to your surprise the sight of her camel toe takes it up a notch further. With your arm moving instinctually you reach for it, and fondle her through the fabric as she continues to show more interest in the panties than what you're doing.[line break][line break]";
				if waiterhater is 0, wait for any key; [skips waiting if it's not wanted]
				say "As you put an arm around her midriff and suck at her earlobe she awakens to your advance stroking your [skin of player] [if breast size of player is greater than 0][breast size desc of player] boobs[else] chest[end if]. You kneel down on the ground as the butterfly follows pausing a moment first to give you a good close-up of the panties. She sits on your lap as you both kiss passionately and hold each other close. As you fondle her [ButterflyBreastDesc] breasts you wonder quietly how she'd react to a bra. You reach down and pull the crotch of her panties aside exposing her cunt as she peers down still intrigued at this mysterious piece of cloth. You both begin to finger each other's cunts and soon are in the heat of sex as she kneads your [if cunts of player is greater than 1]many [end if]puss[if cunts of player is greater than 1]ies[else]y[end if] fervently. In a moment of pure ecstasy you both cum as her liquid pours over your lap and you spray a sizable load of fluid over her new panties. She climbs off and tries to examine them as best she can before starting to take them off. You enjoy her usual 'au naturale' look returning from behind as she strips. As she continues to examine the sodden panties curiously you offer to take them back, but she immediately moves them out of your reach. Stepping away she holds them up to her face, and breathes in your scent. Delirious with excitement she speeds off still clutching the panties to her face.[line break]";
				say "You lose 1 x cute panties!";
				delete cute panties;
			else:
