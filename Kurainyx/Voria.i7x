...
to say ResolveEvent Plant Takeover:
	...
	say "     Next to the bucket are several potted plants on a stand with a sign indicating that they are of the carnivorous variety. All but one of the displayed plants have wilted, the lone survivor being a sort of Venus flytrap with a dark-blue round, bulbous head and two leaves growing on either side of its stem. With it only being the size of the average human's hand, you feel sorry for the small plant, and you debate if you want to take it back to the abbey garden. On the other hand, you remind yourself that just because the plant looks normal doesn't mean that it's not uninfected.";
	say "[line break]     [bold type]Do you want to take the plant home?[roman type][line break]";
	say "     ([link]Y[as]y[end link]) - Bring the plant back to the abbey.";
	say "     ([link]N[as]n[end link]) - Leave the plant alone.";
	if Player consents:
		LineBreak;
		say "     Carefully, you reach toward the surviving Venus flytrap, taking one last precaution just in case the plant does turn out to be dangerous, but the plant does not budge at all when you pick up the pot holding it. Satisfied with your haul of two bottles of water, albeit dirty ones, and a new green buddy, you exit the garden store and head straight for the abbey. Fortunately, the store is actually located relatively close to your base, and you make it back home without any trouble. Going into the abbey's garden, you transfer the Venus flytrap from its pot and into a nice patch of dirt along the wall. With all the work you have done for this little plant, you figure you might as well name the newest addition to the abbey. It takes a bit for you to come up with something suitable, but you eventually recall a period where you browsed some books in the library out of boredom and learned about a genus of flies named 'Voria', which seems like a fitting name for a fly-eating plant. With Voria settled in its new home, you move on to taking care of other matters while also making a note to check on the plant later.";
		WaitLineBreak;
...
an everyturn rule:
	if (HP of Voria > 0 and HP of Voria < 7) or (HP of Voria > 8 and HP of Voria < 22 and HP of Voria is not 15):
...
to say VoriaDesc:
	if HP of Voria > 0 and HP of Voria < 7:	[Grow Phase 1]
		...
		if Player is Neuter:	[Player has no parts to water Voria]
			...
			say "[line break]     [bold type]Do you want to water Voria?[roman type][line break]";
	...
	else if HP of Voria < 15:	[Grow Phase 2]
	...
	else if HP of Voria < 22:	[Grow Phase 3]
		...
		say "[line break]     [bold type]Do you want to stay and watch what happens to the fox?[roman type][line break]";
...
Description of Carnivorous Plant is "[VoriaDesc]".
...
to say CarnivorousPlantSummon:
	...
	say "     [if Player is not in Grey Abbey Garden]Needing a little help with your exploration, you call out Voria's name. You wait for a few moments, wondering if you were able to reach your plant companion. Eventually, you hear the rustling of leaves, and you turn around to see Voria shambling towards you. Happy to see the plant, you walk over to it and[else]Using both your words and gestures, you tell Voria to follow you. The plant rocks left and right from its patch of dirt until it finally uproots itself and promptly shambles towards you. Glad to have a Voria by your side, you[end if] give it a few pats on its giant, blue head, earning you a quiet purr from the flora.";

to say CarnivorousPlantDismiss:
	move Voria to Grey Abbey Garden;
	say "     Giving Voria a pat on its head, you tell the plant to [if Player is not in Grey Abbey Garden]go back home. Voria rumbles softly as it processes your command, and after a moment, it begins shambling in the direction of the abbey[else]stay here and guard the place. Voria rumbles softly as it processes your command, and after a moment, it shambles toward the patch of dirt, its tentacles burrowing into the ground until the plant is firmly rooted[end if].";
...
to VoriaPostCombat:	[Voria vore scenes after you win a fight against a voreable enemy]	[TO DO: Make adjustments to post combat scenes for herm enemies]
	...
	say "[line break]     [bold type]Do you watch Voria eat its prey?[roman type][line break]";
					...
					if Player consents:
						...
						if title entry is:
							...
							-- "Scissor":
								say "[VoriaPCScissor]";
							-- "Just Watch":
								say "[if VoriaCocks > 0][VoriaPCWatchMale][else][VoriaPCWatchFemale][end if]";
				else:
...
to say VoriaPCPussyPound:	[Player sticks dick in enemy pussy]
	...
	say "     Voria waits patiently for you to take a few moments to bask in your afterglow, but as soon as you catch your breath and pull out, the plant promptly reintroduces the cockhead tentacle into its prey's dripping cunt, slowly pumping in and out of the well-used hole. At the same time, one of Voria's mouth-tentacle goes over to your cock, a tongue-like appendage slipping out of the tiny maw and lapping up the juicy bounty dripping from your shaft. After cleaning you off, Voria takes out the cock-tentacle from your foe, the limb completely clean as all traces of lusty fluids were somehow absorbed into the plant. Voria then moves onto the main course, raising its prey and feeding the rest of them into its maw. Voria rumbles happily after tightly shutting its mouth, relishing in its catch as well as the creamy filling that you added to it. With both you and your plant friend sated, you continue on your explorations.";
