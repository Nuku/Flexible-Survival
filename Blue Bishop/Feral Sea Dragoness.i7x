		...
		if bodyname of player is "Feral Sea Dragon" or bodyname of player is "Feral Sea Dragoness":
			if "Submissive" is listed in feats of player:
				...
				if cocks of player > 0:
					...
					say "     Very suddenly, she cries out, revelling in being rammed [if hp of player > 0]underneath a virile beast such as you[else]by someone regardless of having bested them in combat[end if]. The sound of her voice, along with her clenching hole, is enough to make quick work of you, forcing you to roar out in pleasure as you finally fire off your [cum load size of player] load into the dragoness.";
			...
			If "Touched by Madness" is listed in feats of player and (("Kinky" is listed in feats of player and a random chance of 1 in 3 succeeds) or a random chance of 1 in 4 succeeds) and scalevalue of player < 4 and FSDOPEN < 2 and inasituation is false:
				If FFSDUBM is false:
					...
					say "     Unable to contest her new desires, she begins to grind herself against the sand, her portal tightening against your wholly engulfed frame, pulling you deeper into her slick depths. This overwhelming heat; this hungry, demanding tunnel, it[if cocks of player > 0 or cunts of player > 0]'s more than enough to set you off, almost immediately, sealing your fate when this is all that this dragoness needs to find bliss herself[else] torments you to no end, your fate soon sealed as the dragoness finds her bliss[end if], pulling you into that final, inevitable destination before your senses fade. The dragoness would no doubt recover and move on, the occupancy of her new captive not outwardly apparent to those who might see her large form...";
					now FFSDUBM is true;
				else:
				...
				if ("Kinky" is listed in feats of player and a random chance of 1 in 3 succeeds) or a random chance of 1 in 4 succeeds:
					...
					infect;
			else if cocks of player > 0 and cock length of player > 15 and a random chance of 3 in 4 succeeds:
				...
				else if "More Anal" is listed in feats of player:
...
to say fsdfeeding:
	if fsdfeedmemory is 0:
	...
	else if fsdfeedmemory is 2 and "More Anal" is listed in feats of player:
		...
		say "     You're--rightfully so--taken off guard when you feel your bowels flood with her ill-conceived meal, [if scalevalue of player < 4]your beleaguered hole quickly strained under the excessive flow as you belly quickly bulges with her contents.  She's a bit more liberal with feeding you this time, perhaps to teach you a lesson for trying to avoid getting fed, though you can at least take comfort in not having to taste its foul contents this time.[run paragraph on].[run paragraph on][else]though you're at least large enough to handle the rather lewd assault; however, her lack of familiarity with this tactic shines through when the beast apparently assumes she's underfed you, and only proceeds to unload more and more of the fish through your beleaguered hole, to the point where your belly bulges blatantly with her goods[end if].";
...
to say FSDF_1: [Cunt Pitching]
	...
	if cocks of player > 1: [1 = Yes, cunt. 2 = Shared, ass and cunt. 0 = No.]
		say "     You figure you can use [if cocks of player is 2]both[else]two[end if] of your cocks, shall you? [if cock length of player > 19]You're probably too well-endowed to fit both of them in there, though, so the only choice you have is to stick the other up her ass.[end if]";
		if player consents:
	...
	if player consents:
		say "     Just at the cusp of ecstasy, you wrench your tool[if tempnum2 > 0]s[end if] free of the sea dragoness's tight hole[if tempnum2 is 2]s[end if], grinding against her hide until you're set off, firing your [cum load size of player] against her underside, [if cock width of player > 19]utterly drenching it[else]making an embarrassing mess of it[end if]. It doesn't seem to be enough to set her off, her abused [if tempnum2 is not 2]cunt[else]holes[end if] dribbling against the open air. Should you help her out?";
		if player consents:
		...
		say "     In one final thrust, you [if cockname of player is listed in infections of knotlist]tie[else]hilt[end if] your tool[if tempnum2 is 1]s[end if] within the sea dragoness's tight hole, crying out as you're set off, flooding her womb with your [cum load size of player] load[if cock width of player > 19] and quickly beginning to sputter from her[end if]. This is more than enough to set the beast off, singing out weakly as her cunt squeezes and milks you of all you have to offer her.";
...
to say FSDF_2: [Anal Pitching]
	...
	if cocks of player > 1: [1 = Yes, ass. 2 = Shared, ass and cunt. 0 = No.]
		say "     You figure you can use [if cocks of player is 2]both[else]two[end if] of your cocks, shall you? [if cock length of player > 19]You're probably too well-endowed to fit both of them in her ass, so the only choice you have is to go for both holes.[end if]";
		if player consents:
			if cock length of player > 19:
				now tempnum2 is 2;
			else:
	...
	if player consents:
		say "     Just at the cusp of ecstasy, you wrench your tool[if tempnum2 is 1]s[end if] free of the sea dragoness's tight hole[if tempnum2 is 2]s[end if], grinding against her hide until you're set off, firing your [cum load size of player] against her underside, [if cock width of player > 19]utterly drenching it[else]making an embarrassing mess of it[end if]. It doesn't seem to be enough to set her off, her [if tempnum2 is 2]abused[else]unattended[end if] cunt dribbling against the open air. Should you help her out?";
		if player consents:
			say "     Simple finger? Else you'll eat her out.";
			if player consents:
