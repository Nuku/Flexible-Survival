...
to say FSDF_1: [Cunt Pitching]
	...
	if cocks of player > 1: [1 = Yes, cunt. 2 = Shared, ass and cunt. 0 = No.]
		say "     You figure you can use [if cocks of player is 2]both[else]two[end if] of your cocks. Shall you[if cock length of player < 20]?[else]? You're probably too well-endowed to fit both of them in there, though, so the only choice you have is to stick the other up her ass.[end if]";
		if player consents:
	...
	if player consents:
		...
		say "     In one final thrust, you [if player is knotted]tie[else]hilt[end if] your tool[if tempnum2 is 1]s[end if] within the sea dragoness's tight hole, crying out as you're set off, flooding her womb with your [cum load size of player] load[if cock width of player > 19] and quickly beginning to sputter from her[end if]. This is more than enough to set the beast off, singing out weakly as her cunt squeezes and milks you of all you have to offer her.";
...
to say FSDF_2: [Anal Pitching]
	...
	if cocks of player > 1: [1 = Yes, ass. 2 = Shared, ass and cunt. 0 = No.]
		say "     You figure you can use [if cocks of player is 2]both[else]two[end if] of your cocks. Shall you[if cock length of player < 20]?[else]? You're probably too well-endowed to fit both of them in her ass, so the only choice you have is to go for both holes.[end if]";
		if player consents:
			if cock length of player > 19:
				now tempnum2 is 2;
			else:
