...
instead of sniffing Blot:
	say "     The object smells faintly of fresh synthetics, [if blotintense > 3]tinged with the heady aroma of your sex[otherwise]but otherwise isn't noteworthy[end if].";
...
to say blotlook:
	...
	otherwise:
		blotdecay;
		say "     Standing there quietly in a corner of the stall, the otherwise rustic aesthetic of the area is clashed harshly against a seemingly cartoonish, inflatable horse toy. As far as you're aware the thing is generally harmless, the hole between its legs plainly illustrating its purpose.";
		...
		say "     [italic type][if blotintense < 3]The thing looks completely inanimate.[run paragraph on] You're fairly certain there'll be little to no consequence in using it right now[otherwise if blotintense < 5]The thing appears to be mostly in a mundane state right now, but it seems to subtly become excited as you approach.[run paragraph on] You could probably get away with using it[otherwise if blotintense < 8]The thing seems tinged with a streak of wanton menace.[run paragraph on] You could probably get away with using it, but not for long[otherwise if blotintense < 10]You feel ill at ease near the thing, as though you're being drawn to the tainted object.[run paragraph on] You don't think it'd be particularly wise to use it right now[otherwise]The thing seems to undulate from the corners of your eyes, as though thrumming with an otherworldly need.[run paragraph on] You feel overwhelmingly drawn to the thing, but you'd have to be insane to use it now[end if]...[roman type][line break]";
	say "[line break]";
...
to say blotfuck:
	...
	otherwise:
		blotdecay;
		say "     Are you sure? [if blotintense is -1]Who knows what will happen![otherwise if blotintense > 4]You imagine it won't end well, by this point![end if]";
		if player consents:
			if blotintense is -1:
				...
				otherwise:
					...
					say "     Briefly preparing yourself, [if scalevalue of player < 4]you climb onto the pony's back[otherwise]you crawl on top of the pony[end if], grinding your exposed, eager cunt[sfn] against the smooth, pliable vinyl of the toy's hindquarters. You quickly find yourself getting more pleasure out of this than you expected, driving you to continue.";
			...
			otherwise if blotintense < 3:
				...
				otherwise:
					...
					say "     Briefly preparing yourself, [if scalevalue of player < 4]you climb onto the pony's back[otherwise]you crawl on top of the pony[end if], grinding your exposed, eager cunt[sfn] against the smooth, pliable vinyl of the toy's hindquarters. ";
			...
			otherwise if blotintense < 5:
				...
				otherwise:
					...
					say "     Briefly preparing yourself, [if scalevalue of player < 4]you climb onto the pony's back[otherwise]you crawl on top of the pony[end if], grinding your exposed, eager cunt[sfn] against the smooth, pliable vinyl of the toy's hindquarters. Almost immediately, you feel something wet flood your crotch, engulfing your [cunt size desc of player] cunt[sfn], and caressing you as you writhe.";
				say "     Inflatable object loudly creaking under your[if scalevalue of player > 3] substantive[end if] weight as you start to build up an earnest, the thing eagerly aids your motion, soon sweat-slicked, [bodydesc of player] form writhing against the sticky toy. [if scalevalue of player > 3]Inflating to accommodate your impressive size[otherwise]Subtly vibrating with an otherworld influence[end if], it comes to life under your touch, soon making it difficult for your [bodytype of player] body to move without its aid as it clings to you...";
			otherwise if blotintense < 8:
				say "     As you approach the curious toy, it subtly quivers in response to your mere presence. You make your way around to the rear of the inflatable pony.";
				if cocks of player > 0:
					say "     Its smooth, near-featureless behind exposes your intended point of entry. The hole is already a bit open from your last adventure, or perhaps the soft and yielding material is trying to invite you? It's clearly already lubricated with your sexual fluids, but you could play around with it anyways. Shall you? Else you'll get straight to the point.";
					if player consents:
					...
					otherwise:
						say "     Moving on";
					say ", you [if scalevalue of player < 4]climb onto the pony's back[otherwise]crawl on top of the pony[end if], carefully aiming[if cocks of player > 1] one of[end if] your [cock size desc of player], half-erect dick[smn] for the entrance before forcing its way in. The toy trembles under your weight, [if cock length of player > 11]easily stretching to accommodate the girth of your impressive[otherwise]easily taking your[end if] cock as a latex-like substance begins to engulf it and the entirity of your hindquarters[if cocks of player > 2], along with your other rods[otherwise if cocks of player is 2], along with your other rod[end if]";
					if cunts of player > 0:
						say ". Feeling something wet flood your cunt[sfn][if anallevel is not 1] and ass[end if], [if cunts of player > 1 or anallevel is not 1]they quickly harden into thick, ridged appendages to pound you with[otherwise]it quickly hardens into a thick, ridged appendage to pound you with[end if]";
			...
			otherwise if blotintense < 10:
				...
				say "     Forcibly, you're dragged [if scalevalue of player < 4]on top of the pony's back[otherwise]over of the pony[end if], ";
				if cocks of player > 0:
					say "eagerly encasing[if cocks of player > 1] one of[end if] your [cock size desc of player], half-erect dick[smn][if cocks of player > 2] before taking the rest as well[otherwise if cocks of player is 2] before taking the other as well[end if].";
					say "     The toy trembles under your weight, easily having consumed your[if cock length of player > 11] impressive[end if] meat as a latex-like substance quickly engulfs[if cockname of player is not listed in infections of internallist] your [ball size][otherwise]the rest of your crotch[end if] as well";
					if anallevel is not 1:
						say ". Soon, you feel the tainted material flood your [if cunts of player > 0]cunt[sfn] and ass, pounding your holes with a quickly-expanding, quickly-hardening series of distinctly ribbed implements[otherwise]your ass, pounding your hole with a quickly expanding, quickly-hardening series of distinctly ribbed implements[end if]";
			...
			otherwise:
				...
				if cocks of player > 0:
					say "your partially-aroused, [cock size desc of player] cock[if cocks of player > 1]s are[otherwise] is[end if] firmly gripped as they immediately and firmly pumped";
					if anallevel is not 1:
		...
		otherwise:
			...
			if keychar in lower case exactly matches the text "s" or keychar in lower case exactly matches the text "1" or keychar in lower case exactly matches the text "return" or keychar in lower case matches the text "struggle":
				...
				if struggleatt < boundsegment:
					...
					blotlust;
				otherwise:
					say "      [if blotintense < 3]Easily[otherwise if blotintense < 5]With relative ease[otherwise if blotintense < 8]Eventually[otherwise]Finally[end if], you manage to wrench yourself free and dismount the possessed toy,  [if blotintense < 3]though it quickly regresses to a state of complete inanimacy[otherwise if blotintense < 5]gradually cooldoing down and becoming inanimate once more[otherwise if blotintense < 8]quivering for a moment as it slowly begins to fill the void you left[otherwise if blotintense < 10]the mess of a thing trembling in distress over its now-missing rider[otherwise]the thing practically crawling over to be one with you once more, forcing you to retreat even further[end if].";
					say "     You clean yourself off and prepare to go about your business once more. [if blotintense > 5]It's likely best that you leave the vinyl object alone until it 'cools down' a bit[otherwise]The vinyl object seems relatively harmless, for now at least[end if]...";
					say "[line break]";
					cleanboundmemory;
					wait for any key;
					now trixieexit is 1;
...
to blotprogress:
	...
	otherwise if blotintense is 5:
		...
		if cocks of player > 0:
			say ". The undulating material pumps your [cock size desc of player] dick[smn] with such force that you rock against the toy with each motion, slicked by your own cum. [if cunts of player > 0 and anallevel is not 1]Moreover, you feel something large and hard force its way into your cunt[sfn] and ass, the shape shifting around until it appear satisfied with a bulging, ribbed tool to rail you with.[otherwise if cunts of player > 0]Moreover, you feel something large and hard force its way into your cunt[sfn], the shape shifting around until it appear satisfied with a bulging, ribbed tool to rail you with.[otherwise if anallevel is not 1]Moreover, you feel something large and hard force its way into your ass, the shape shifting around until it appear satisfied with a bulging, ribbed tool to rail you with.[end if]";
		otherwise:
			say ". The undulating material grinds against your [cunt size desc of player] cunt[sfn] with such force that you rock against the toy with each motion, slicked by your own honey. [if anallevel is not 1]Moreover, you feel something large and hard force its way into your holes, the shape shifting around until it appear satisfied with a bulging, ribbed tool to rail you with[otherwise]Moreover, you feel something large and hard force its way into your hole[sfn], the shape shifting around until it appear satisfied with a bulging, ribbed tool to rail you with[end if].";
	...
	otherwise if blotintense is 10:
		say "     With one final pull, you're drawn entirely into the body of the inflatably pony. Body adjusted until it fits as much as a suit as it is a new prison. Completely unable to move, your vision is limited to the translucent eyes of the vinyl construct. All you can hear is your pained panting and the wet sound of your continued, unrelenting abuse.";
		say "     Outwardly, the Pony appears to have regressed to its inanimate state, though betrayed by your loud, muffled moans[if cocks of player > 0], its occasional twitching, and your throbbing, material-coated cock[smn] hanging between its legs[otherwise] and its occasional twitching[end if]. It's difficult to maintain even the slightest mental clarity at this point, the prospect of being milked by this toy eternally very close to a reality!";
...
to say blotbinddesc:
	...
	otherwise if blotintense < 8:
		...
		if cocks of player > 0:
			say "having engulfed your cock[smn] [if cunts of player > 0 and anallevel is not 1]while pounding your cunt[sfn] and ass with large and distinctly moulded implements[otherwise if cunts of player > 0]while pounding your cunt[sfn] with large and distinctly moulded implements[otherwise if anallevel is not 1]while pounding your ass with large and distinctly moulded implements[otherwise]within its tainted, vinyl grasp[end if]";
		otherwise:
			say "pounding your cunt[sfn][if anallevel is not 1] and ass[end if] with large and distinctly moulded implements";
	...
	otherwise if blotintense < 10:
		...
		if cocks of player > 0:
			say "having engulfed your cock[smn] [if cunts of player > 0 and anallevel is not 1]while relentlessly pounding your cunt[sfn] and ass with large and distinctly moulded implements[otherwise if cunts of player > 0]while relentlessly pounding your cunt[sfn] with large and distinctly moulded implements[otherwise if anallevel is not 1]while relentlessly pounding your ass with large and distinctly moulded implements[otherwise]within its powerful, vinyl grasp[end if]";
		otherwise:
	...
	otherwise:
		...
		if cocks of player > 0:
			say "having engulfed your cock[smn] [if cunts of player > 0 and anallevel is not 1]while relentlessly pounding your cunt[sfn] and ass with large and distinctly moulded implements[otherwise if cunts of player > 0]while relentlessly pounding your cunt[sfn] with large and distinctly moulded implements[otherwise if anallevel is not 1]while relentlessly pounding your ass with large and distinctly moulded implements[otherwise]within its overwhelming, vinyl grasp[end if]";
		otherwise:
			say "relentlessly pounding your cunt[sfn][if anallevel is not 1] and ass[end if] with large and distinctly moulded implements";
...
to blotbindorgasm:
	if blotintense < 3:
		...
		if cocks of player > 0:
			say "cock pumps your [cum load size of player] load into the toy[if cocks of player > 2], its brothers staining its hind legs[otherwise if cocks of player is 2], its brother staining its hind legs[end if]. [if cock width of player > 23]Briefly bulging to contain your impressive release, it seems to recede and acclimate to the torrent[otherwise]Eagerly, it seems to consume your release, motion further slicked by it[end if]";
		otherwise:
			say "cunt[sfn] ache[sfv] against the toy, staining it with your juices";
		say ". Trembling subtly in the wake of this, it seems to feed off your [if cocks of player > 0]cum[otherwise]ecstasy[end if], [if blotintense < 2]gradually changing[otherwise]close to changing[end if] as you're compelled to continue riding it.";
	otherwise if blotintense < 5:
		say "     Finally overtaken by bliss, your ";
		if cocks of player > 0:
			say "cock[smn] pump[smv] your [cum load size of player] load into the toy, engulfing vinyl milking it for as long as it can. [if cock width of player > 23]Briefly bulging to contain your impressive release, it seems to recede and acclimate to the torrent[otherwise]Eagerly, it seems to consume your release, motion further slicked by it[end if]";
		otherwise:
			say "cunt[sfn] ache[sfv] against the toy, engulfing vinyl prolonging your bliss for as long as it can";
		say ". Trembling further in the wake of this, it's further empowered by your [if cocks of player > 0]cum[otherwise]ecstasy[end if], [if blotintense < 4]gradually changing[otherwise]close to changing[end if] as you're driven to continue riding it.";
	otherwise if blotintense < 8:
		say "     Finally overtaken by bliss, your ";
		if cocks of player > 0:
			say "cock[smn] pump[smv] your [cum load size of player] load into the toy, engulfing vinyl milking it for as long as it can[if cunts of player > 0 and anallevel is not 1] while pounding your holes[otherwise if cunts of player > 0] while pounding your hole[sfn][otherwise if anallevel is not 1] while pounding your hole[end if]. [if cock width of player > 23]Briefly bulging to contain your impressive release, it seems to revel in the torrential flood[otherwise]Eagerly, it seems to consume your release, motion further slicked by it[end if]";
		otherwise:
			say "cunt[sfn] ache[sfv] against the toy, pounding vinyl milking you for as long as it can";
		say ". Groaning under you in the wake of this, it's further empowered by your [if cocks of player > 0]cum[otherwise]ecstasy[end if], [if blotintense < 7]gradually changing[otherwise]close to changing[end if] as you're forced to continue riding it...";
	otherwise if blotintense < 10:
		...
		say ". Visibly trembling under you, it's even further empowered by your [if cocks of player > 0]cum[otherwise]ecstasy[end if], [if blotintense < 9]gradually changing[otherwise]close to changing[end if] as you're forced to continue riding it...";
	otherwise:
		say "     Sent crashing into bliss, your ";
		if cocks of player > 0:
			say "cock[smn] pump[smv] your [cum load size of player] load into your prison, vinyl material milking it for as long as it can[if cunts of player > 0 and anallevel is not 1] while pounding your holes[otherwise if cunts of player > 0] while pounding your hole[sfn][otherwise if anallevel is not 1] while pounding your hole[end if]";
			if cock width of player > 23:
...
when play ends:
	if bodyname of player is "Blotted":
		...
		say "     Apparently, the pony is still 'Rideable'[if cocks of player > 0 and anallevel is not 3 and cunts of player is 0], especially with your cock[smn] exposed, like they are[end if]. Customers from far and wide come to make use of you, now subject to the whims of the pony and these complete strangers, who you can only barely glimpse at through your prison's eyes.";
