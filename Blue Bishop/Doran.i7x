...
to say DoranSexMenu: [Doran Sex Protocol]
	...
	if DoranRoleplayInSession > 0: [Is a Roleplay In Session?]
		...
		if anallevel is 3 and DoranRoleIntensity is 0 and DoranRimmingStatus is not 1:
			...
			now sortorder entry is 2;
			now description entry is "Service Doran's tail-hole";
		[]
...
to say DoranDom_RidingBehind: [Riding Behind]
	...
	say "     [if DoranRoleIntensity is 0]Commanding[else]Inviting[end if] your touch, [ghe] gives you some free reign to fondle and grope [ghis] behind. [gchis] powerful hindquarters yielding slightly against your shameless caress, [ghe] firmly presses against your digits, encouraging you to continue feeling [ghim] up. [gchis] haunches are ample enough that you need to part them to get an unobstructed view of [ghis] [if DoranInternal is true]smooth, scaled crotch[else if DoranBallModStatus is 0]ample balls and sheath[else]over-sized balls and sheath[end if][if DoranBallModStatus > 0], strings of precum oozing from [ghis] barely-aroused dick[end if]. [gchis] hard scales barely hide the softer flesh of [ghis] anal ring.";
...
to say DoranSub_SelfMilking: [Self-milking]
	...
	say "     Even as [ghe] draws directly from the source some of the heady fluid dribbles down to soak [ghis] [if DoranInternal is true]crotch[else]over-sized ballsack[end if], air full with the beast's [gmasculine] aroma. Perhaps you should go in and assist Doran? Else, you'll watch from a distance.";
	if player consents:
...
to say DoranModInternalMenu: [Internal/External Genitals]
	...
	if DoranInternal is true:
		say ", leaf-side up. [gche] wheels around to give you a clear view of [ghis] nethers, groaning softly as a pair of[if DoranBallModStatus > 0] over-sized[end if] balls bulge out from between [ghis] legs, filling out nicely while an ill-fitting sheath grows out from [ghis] scaled slit, barely able to contain [ghis][if DoranBallModStatus > 0] perpetually-drooling[end if] cock. The growth gradually subsides until [ghe]'s left with a full set of exposed genitals.";
		now DoranInternal is false;
	else:
		say ", dragon-side up. [gche] wheels around to give you a clear view of [ghis] nethers, groaning softly as [ghis] [if DoranBallModStatus > 0] over-sized[end if] balls slowly recede between [ghis] legs, until both they and [ghis] sheath shrink down completely, leaving a smooth[if DoranBallModStatus > 0], though still perpetually-oozing,[end if] crotch. You can see his cock peeking slightly from his newly formed genital slit.";
