...
to say dordom_7: [Riding Player] [BODY MODDED]
	...
	otherwise:
		say "...!' With one final, firm descent upon your tool, this concluding [if dorroleint is 0]command[otherwise]endeavour[end if] is more than enough to make you both cry out, your [cum load size of player] load rapidly pumping into [ghis] depths, [ghis] own[if dorballmod > 0], torrential flood of[end if] cum [if dorroleint is 0]aimed to paint you with a lewd lack of restraint[otherwise]firing wildly into the open air[end if]. [if cock width of player > 24]Forcibly, [ghe] pulls your hands against [ghis] belly, making you feel [ghis] form as it visibly bloats to contain your massive load[otherwise if player is knotted]Moaning loudly, [ghe] remains tied to you[otherwise]Moaning loudly, [ghe] keep your tool enveloped entirely within [ghis] depths[end if], eager to contain every last drop of your seed.";
		if dorvorreg is 2 and scalevalue of player < 4:
...
to say dorsub_1: [Anal Catching] [BODY MODDED]
	...
	otherwise:
		say "     [gche] shows absolutely no difficulty in taking your member[if player is knotted], even tying with it almost immediately[end if]. [gche] doesn't seem to mind your ease, revelling in being [if dorroleint is 4]shamelessly used[otherwise]ridden by[end if] you, supple abyss almost overwhelming in how [if dorroleint is 4]obligingly[otherwise]assertively[end if] it milks the organ.";
	...
	if player consents:
		...
		otherwise:
			say ".";
	otherwise:
		say "     Succumbing to your overwhelming urges, [if player is knotted and cock length of player > 17]you make one final push, tying yourself with the beast before you cry out[otherwise if player is knotted]you soon cry out, tying your cock within [ghis] fleshy abyss one last time[otherwise]you soon cry out, hilting your cock within [ghis]  fleshy abyss[end if], gouts of your thick, [if cock width of player > 24]voluminous[otherwise]virile[end if] seed flooding [ghis] depths[if cocks of player > 2], its brothers staining [ghis] behind and crotch[otherwise if cocks of player is 2], its brother staining [ghis] behind and crotch[end if]";
