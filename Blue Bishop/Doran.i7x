...
to say DoranVictory_Anal: [anal sex]
	...
	say "     'Oh, m-my victor is so [if scalevalue of player < 4]t-tight[else if player is twistcapped]s-skilled[else]f-firm[end if].' His discourse is mixed with deep moans, elicited as you build a motion of riding the dragon's powerful dick, [if player is male]your own bouncing in the open air[else]cunt[sfn] dripping against the open air[end if], staining the dragon's scales with [if cocks of player > 1 or cunts of player > 1]their[else]its[end if] slick fluids[if scalevalue of player < 3], stretched to the limits of your ability to contain the organ[end if].";
...
to say DoranSexMenu: [Doran Sex Protocol]
	...
	if DoranRoleplayInSession > 0: [Is a Roleplay In Session?]
		...
		while Trixieexit is 0:
			...
			say "(4) [if player is female][link]Ride [ghim][as]4[end link][else][italic type]Female-specific interaction[roman type][end if] [if cunts of player > 0 and cunt length of player < 8][italic type]- Too small![roman type][end if][line break]";
...
to say DoranAutoSexProtocol:
	if DoranRole is 1 and a random chance of 1 in 2 succeeds:
		...
		say "[DoranDom_CumWorship]"; [Cum Worship]
	else if anallevel > 1 and (cunts of player is 0 or anallevel is 3) and ((cunts of player is 0 and ((anallevel is 3 and a random chance of 2 in 3 succeeds) or (anallevel is 2 and a random chance of 1 in 2 succeeds))) or (player is female and a random chance of 1 in 4 succeeds)) and (scalevalue of player > 1 or player is twistcapped):
		say "[DoranDom_Anal]"; [anal sex scene]
...
to say DoranDom_Vaginal: [Cunt Sex] [BODY MODDED]
	...
	if scalevalue of player < 4 and DoranRoleIntensity is 0 and a random chance of 1 in 2 succeeds:
		...
		if player is male or cunts of player > 1:
...
to say DoranDom_Oral: [Oral Sex] [BODY MODDED]
	...
	if scalevalue of player > 3:
		if DoranRoleIntensity is 0:
			...
			say ". Detecting your need, the [gdragon] exploits this opportunity to return the favor, head moving to offer [ghis] own tongue's affection in turn, scaled [if cocks of player > 1]maw enveloping one of them[else if player is male]maw enveloping it[else if cunts of player > 1]lips embracing one of them[else]lips embracing it[end if].";
...
to say DoranDom_CuntHandjob: [Cunt-Focused Handjob] [BODY MODDED]
	...
	if DoranRoleIntensity is 0 and ((DoranAutofired is true and a random chance of 1 in 2 succeeds) or (DoranAutofired is false and a random chance of 1 in 5 succeeds)):
		...
		if DoranVoreStatus is 2 and scalevalue of player < 4:
			if player is male and cock width of player > 20 and DoranRoleIntensity is 0, now DoranPlayerCumFilled is true;
...
to say DoranSub_CockOral: [Cock Oral] [BODY MODDED]
	...
	If DoranTitModStatus > 0:
		...
		if player consents:
			now tempnum is 1;
			say "     Grinning slightly, [ghe] pulls free of your dick, now slicked by [ghis] saliva, before shifting to encompass [if cocks of player > 2]two of[else if cocks of player is 2]both of[else]one of[end if] your dick[smn] between [ghis] vast crevasse. [gchis] ample, scaled cleavage yielding around your rigid length[smn], [ghe] starts to build up a motion against [itthemm], attending tongue slicking [ghis] efforts with a mix of [ghis] saliva and your[if cock width of player > 24] abundant[end if] precum.";
		else:
			now tempnum is 0;
	else:
		now tempnum is 0;
		...
		if player consents:
			...
			if DoranRoleIntensity is 4:
				...
				if player consents:
					...
					say "[if cock width of player > 24]. [gche] doesn't seem even remotely phased by the volume you surrender to [ghim], reveling in the sheer excess[end if].";
...
to say DoranSub_CatchCunt: [Cunt Catching] [BODY MODDED]
	...
	if DoranRoleIntensity is 4:
		...
		say ", more that enough to bring [ghis] organ to its full size. Not feeling eager to subject yourself to the ride dry, you make some effort to slick the member beforehand, [if DoranBallModStatus > 0]not that it's difficult with the amount of precum [ghe]'s constantly oozing[else if scalevalue of player > 3]finding little difficulty in achieving your goal[else], though you're not quite big enough to get the job done, the [gdragon] bending in to help you with [ghis] own[end if]. It's not long before it's ready, dripping with an aching need.";
	if scalevalue of player < 4:
		say "     Now ready, you climb atop the [gdragon]'s scaled undercarriage, carefully aiming the dripping rod for [if cunts of player > 1]one of your exposed cunts[else]your exposed cunt[end if].";
...
to say DoranNeutral_69: [BODY MODDED]
	...
	while Trixieexit is 0:
		...
		if tempnum is 1:
			...
			if player is male and (((player is female or anallevel is 3) and a random chance of 1 in 2 succeeds) or (cunts of player is 0 and anallevel < 3)):
...
to say DoranRP_VirileBeast: [Doran RP: The Virile Beast Query]
	...
	if DoranRole > 0 and anallevel is 1 and ((player is female and cunt length of player < 5) or cunts of player is 0):
		say ", additionally,";
	else if anallevel is 1 and ((player is female and cunt length of player < 5) or cunts of player is 0):
...
to say DoranRP_KnightMenu: [Doran RP: The Knight Query]
	...
	if player consents:
		...
		if anallevel is 1 and ((player is female and cunt length of player < 5) or cunts of player is 0):
