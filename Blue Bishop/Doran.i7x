...
to say doranlook:
	...
	if dorfirsttalk is false:
		...
		say ".";
		say "     [italic type]Doran presently regards you as [if dorgenreg is true]female[else]male[end if][if doransgender is true] and themself as female[end if]. [gchis] sexual demeanour remains [if dorrolereg is 0]domineering[else if dorrolereg is 2]submissive[else]reasonable[end if] towards you, as you've commanded[if dorroleint is 0 and dorrolereg is 0], the severity of which being intense[else if dorroleint is 4 and dorrolereg is 2], the severity of which being intense[else if dorrolereg is not 1], the severity of which being fairly restrained[end if].[roman type]";
...
to say dorsexprot: [Doran Sex Protocol]
	...
	if dorrpis > 0: [Is a Roleplay In Session?]
	...
	else if dorrolereg is 0 or (dorrolereg is 1 and dorneutroll is 1): [Dom sex pool]
		...
		else if anallevel is 3 and dorroleint is 0 and (("Kinky" is listed in feats of player and a random chance of 1 in 5 succeeds) or (a random chance of 1 in 8 succeeds)) and dorrjd is not 1:
			say "[dordom_6]"; [rimjob scene]
		else if dorballmod > 0 and dorinternal is false and dorroleint is 0 and (("Kinky" is listed in feats of player and a random chance of 1 in 4 succeeds) or (a random chance of 1 in 7 succeeds)):
			say "[dordom_9]"; [Ball Worship]
		else if dorballmod > 0 and (("Kinky" is listed in feats of player and a random chance of 1 in 3 succeeds) or (a random chance of 1 in 6 succeeds)):
		...
		while trixieexit is 0:
			...
			say "(8) [if dorballmod > 0][link]Self Milking[as]8[end link][else][italic type]Bodymod-Specific interaction[roman type][end if][line break]";
			say "[line break]";
	...
	if dorwassnub is false:
		...
		else if dorrolereg is 2 and dorroleint < 4:
			increase dorroleint by 1;
		else if dorrolereg is 1:
			if dorroleint > 2:
				decrease dorroleint by 1;
			else if dorroleint < 2:
				increase dorroleint by 1;
...
to say dordom_1: [Anal Sex] [BODY MODDED]
	...
	if anallevel is 3 and a random chance of 2 in 3 succeeds:
		...
		if scalevalue of player < 4:
			...
			say "little";
		say " resistance in its movement through your bowels, each shoving advance of the thing forcing another gasp from your lips. [gche] subjects you to this ritual for a [if dorroleint is 0]considerable[else]fair[end if] amount of time, leaving your ass a slick and [if dorroleint is 0]significantly[end if] loosened up mess before [ghe]'s finally satisfied[if dorroleint is 0], so aroused by this abuse that you could climax off of it alone, if not for the [gdragon]'s keen eye tempering restraint[end if].";
	if dorroleint is 0:
	...
	if scalevalue of player < 4 and dorroleint is 0 and a random chance of 1 in 2 succeeds:
		...
		say "     '[one of]My [dorgr] is so very small, I wouldn't want to make them too uncomfortable, just yet[or]This way, I get a better view of my [dorgr], as they squirm[or]For such a little [dorgr], they certainly know how to make my dick disappear[at random].' [gche] teases you, much in response to your preceding chain of moans as [ghe] relegates you to a mere sex toy for [ghis] perverse desire, forcibly riding you up and down [ghis] [if dorballmod > 0]sputtering[else]throbbing[end if], draconic length[if scalevalue of player < 3], visibly bulging in order to contain [ghim][end if]. Pleading for [ghis] reciprocation, [ghe] can't help but at least somewhat oblige [ghis] ['][dorgr]['], a talon moving to [if cocks of player > 1]stroke one of your exposed cocks, no doubt rendered hard from the[else if cocks of player is 1]stroke your exposed cock, no doubt rendered hard from the[else if cunts of player > 1]tease one of your dripping, unattended portals, no doubt aching for need following this[else]tease your dripping, unattended portal, no doubt aching for need following this[end if] ordeal.";
		say "     [gche] grows increasingly determined to impale you with [ghis] knot. Your hole [if player is twistcapped]only briefly[else]somewhat[end if] stubborn about it before [ghe] finally makes headway, causing you to slowly cry out in the wake of each subsequent inch before you're finally rewarded the token reprieve of engulfing the whole of [ghis] tool. This development, however, fails to make [ghim] yield for longer than a couple seconds, as you soon find your thighs slapping against the beast's [if dorinternal is true]smooth crotch[else]voluminous sack[end if] at a rapidly increasing pace.";
		if libido of player > 35:
			...
			if cocks of player > 0:
				...
				say "puss[if cunts of player > 1]ies'[else]y's[end if] gushing forcibly strained against the[if scalevalue of player < 4] immense[end if] intrusion, staining the [gdragon]'s crotch with your sexual fluids.";
		say "     Shifting back onto [ghis] feet, [ghe] offers slight care in pulling you free of [ghis] dick, knot audibly popping from your stretched hole before planting you on the ground once more, soon followed by [if dorballmod > 0]an excessive[else]a[end if] flood of the beast's thick, heady ejaculate thereafter. Feeling particularly devious, [ghe] revels in your pitiful state, aiming [ghis] [if dorballmod > 0]perpetually[else]still[end if] cum-oozing dick at your head and upper torso to [if dorballmod > 0]drench[else]sully[end if] them as well.";
	else:
		...
		say "!' [gchis] remark strained against [ghis] increasing fervor, each powerful thrust sending a tremor through your[if scalevalue of player < 4] diminutive,[end if] [bodytype of player] body. [gche] grows increasingly determined to impale you with [ghis] knot. Your hole [if player is twistcapped or scalevalue of player > 3]only briefly[else]somewhat[end if] stubborn about it before [ghe] finally makes headway, causing you to slowly cry out the wake of each subsequent inch before you're finally rewarded the token reprieve of engulfing the whole of [ghis] tool. This development, however, fails to make [ghim] yield for longer than a couple seconds, as you quickly feel [ghis] [if dorinternal is true]smooth crotch[else if dorballmod > 0]massive sack[else]voluminous sack[end if] slap against your [if cock width of player > 0 and player is not internal and dorinternal is false]own[else if scalevalue of player < 4 and dorinternal is false]thighs[else]crotch[end if] in rapid succession.";
		if libido of player > 35:
			say "     The powerful dick's assault is more than enough to make you almost immediately cry out, ";
			if cocks of player > 0:
			...
			say ". Doran would doubtlessly [if dorroleint is 0]chide you for so easily succumbing[else]remark on how easily you succumbed[end if] if [ghe] wasn't so wrapped up in matters of [ghis] own, approaching climax, forcing [ghis] crotch firm against you a final time before roaring out, gout after overwhelming, continuous gout of cum flooding your bowels, ";
			if dorballmod > 0:
				say "belly [if scalevalue of player < 4]immediately and distinctly[else]quickly[end if] bloating to abide [ghis] massive, torrential load, until the -- almost seemingly absurd -- release starts to sputter from your [if scalevalue of player < 4]over[end if]stuffed hole.";
			else:
				say "[if scalevalue of player < 3]belly visibly bloating to abide [ghis] load[else]quickly leaking from your stuffed hole when you can't contain any more[end if].";
		...
		if dorroleint is 0:
			say ". Feeling particularly devious, [ghe] revels in your pitiful state, aiming [ghis] [if dorballmod > 0]perpetually[else]still[end if] cum-oozing dick at your[if scalevalue of player < 4] head and upper[end if] torso to [if dorballmod > 0]drench[else]sully[end if] them as well.";
		else:
			say ".";
	if dorroleint is 0:
	...
	if dorvorreg is 2 and scalevalue of player < 4:
		...
		say "     '[one of]Well done[or]Many thanks[or]My thanks[at random], [if dorroleint is 0][one of]my [dorgr] takes their [gdragon] like a good slut[or]I always enjoy filling my [dorgr] with their [gdragon]'s cum[or]I look forward to using my [dorgr] in the future[at random][else][one of]my [dorgr] treats their [gdragon] well[or]I always enjoy my [dorgr]'s attention[or]I look forward to having more fun with my [dorgr] in the future[at random][end if].' Following [ghis] adulation [ghe] [if dorroleint is 0]nonetheless appears rather fond of leaving you as a mess, letting you off to clean yourself[else]helps to clean you off, tongue's slick affection shared with you[end if] before resting back at the campfire once more.";
...
to say dordom_2: [Cunt Sex] [BODY MODDED]
	...
	if scalevalue of player < 4 and dorroleint is 0 and a random chance of 1 in 2 succeeds:
		...
		say "     [gche] grows increasingly determined to impale you with [ghis] knot. Your hole [if cunt length of player > 13]only briefly[else]somewhat[end if] stubborn about it before [ghe] finally makes headway, causing you to slowly cry out the wake of each subsequent inch before you're finally rewarded the token reprieve of engulfing the whole of [ghis] tool. This development, however, fails to make [ghim] yield for longer than a couple seconds, as you quickly find your thighs slapping against the beast's [if dorinternal is true]smooth crotch[else]voluminous sack[end if] at a rapidly increasing pace.";
		if libido of player > 35:
			say "     The powerful dick's assault is more than enough to make you almost immediately cry out, ";
			if cocks of player > 0:
		...
		say "!' [gchis] remark strained against [ghis] increasing fervour, each powerful thrust sending a tremor through your[if scalevalue of player < 4] diminutive,[end if] [bodytype of player] body. [gche] grows increasingly determined to impale you with [ghis] knot. Your hole [if cunt length of player > 13]only briefly[else]somewhat[end if] stubborn about it before [ghe] finally makes headway, causing you to slowly cry out in the wake of each subsequent inch before you're finally rewarded the token reprieve of engulfing the whole of [ghis] tool. This development, however, fails to make [ghim] yield for longer than a couple seconds, as you quickly feel [ghis] [if dorinternal is true]smooth crotch[else if dorballmod > 0]massive sack[else]voluminous sack[end if] slap against your [if cock width of player > 0 and player is not internal and dorinternal is false]own[else if scalevalue of player < 4]thighs[else]crotch[end if] in rapid succession.";
		if libido of player > 35:
			say "     The powerful dick's assault is more than enough to make you almost immediately cry out, ";
			if cocks of player > 0:
...
to say dordom_3: [Oral Sex] [BODY MODDED]
	if dorauto is true:
		...
		say "     '[if dorroleint is 0][one of]I think my [dorgr]'s best suited for sucking off their [gdragon][or]My dick's eager for my [dorgr]'s attention[or]I know my [dorgr] longs for a taste of their [gdragon][at random][else][one of]I think it's only fair that my [dorgr] offers me some oral satisfaction[or]I want my [dorgr] to grant their [gdragon]'s dick some much-needed attention[or]I long for the touch of my [dorgr]'s lips[at random][end if]";
		say ".' With [ghis] [if dorroleint is 0]command appointed[else]request made[end if] you approach [ghim], [if scalevalue of player > 3]head [end if][if dorroleint is 0]eagerly [end if]descending";
	say " upon the [gdragon]'s muscular, scaled undercarriage. Well-toned frame heaving with each excited breath, you're given ample view of the beast's[if dorrolereg is 0] enticing[end if] cock[if dorballmod > 0 and dorinternal is false] and oversized balls[end if], already peeking from it's [if dorinternal is true]slit[else]sheath[end if] with barely containable anticipation[if dorballmod > 0], dripping as it's perpetually want to do[end if]. [if dorrolereg is 0]Immediately[else]Soon enough[end if] your lips descend upon its exposed tip, tongue's affection inviting its growth within their embrace, much to [ghis] moaning approval.";
	say "     '[one of]Mmm[or]Ah[or]Yes[at random], [if dorroleint is 0][one of]my [dorgr] is such a good little slut[or]my [dorgr] can't resist the sight of dick[or]I don't think anybody would make a better pet than my [dorgr][at random][else][one of]my [dorgr]'s tongue feels so nice[or]I wish only for more, my [dorgr][or]please don't stop, my [dorgr][at random][end if].' Compelled to work harder, your [if scalevalue of player > 3]tongue shifts to attend[else]digits rise to caress[end if] [ghis] [if dorinternal is true]organ's ill-fitting home[else if dorballmod > 0]greatly oversized, scaled ballsack[else]ponderous, scaled ballsack[end if], further inviting [ghis] dick's countenance, until it hangs fully exposed before you";
	if scalevalue of player > 3:
		say ". [if dorroleint is 0]Entranced by the tool, you're inclined to immediately plunge it down your maw, rapidly[else]Lips slowly enveloping the head, you descend upon its length, gradually building a motion until you're eagerly[end if] bobbing against";
	else:
	...
	say " the member with a wild fervour. Your senses are flooded with the taste and smell of the [gdragon]'s thick and salty sexual fluids as [if dorballmod > 0]you're forced to gulp down the stuff[else]they drool[end if] from [ghis] organ[if dorroleint is 0], more than enough incentive to fuel your hunger further[end if].";
	...
	if scalevalue of player > 3:
		...
		say "     'M-[dorgr]...!' [if dorroleint is not 0]Doran's remark muffled against your flesh as you[else]Doran mutters under [ghis] breath before your affections[end if] are richly rewarded, the creature suddenly going rigid as your maw's flooded with surge after successive, [if dorballmod > 0]torrential[else]voluminous[end if] surge of the beast's carnal batter. [if dorroleint is 0 and dorballmod > 0]Quickly overwhelmed by the sheer volume, the [gdragon]'s persistent teasing is eventually enough[else if dorroleint is 0]Driven by your overwhelming need to consume as much as you can, the [gdragon]'s persistent teasing is eventually enough[else if dorballmod > 0]Quickly overwhelmed by the sheer volume, the [gdragon]'s reciprocation is more than enough[else]The [gdragon]'s reciprocation is more than enough[end if] to drive you to bliss, [if dorroleint is 0]your own sexual fluids left as a puddle on the floor[else]eagerly consuming your own flood of sexual fluids[end if].";
	else:
		if dorroleint is 0:
		...
		say "     'M-[dorgr]...!' Doran mutters under [ghis] breath before your affections are richly rewarded, the creature suddenly going rigid as you're painted with surge after successive, [if dorballmod > 0]torrential[else]voluminous[end if] surge of the beast's carnal batter";
		if dorballmod > 0:
	...
	if dorroleint is 0:
		...
		else if (scalevalue of player < 3 and a random chance of 1 in 2 succeeds) or dorballmod > 0:
		...
		if dorvorreg is 2 and scalevalue of player < 4:
			...
			say "     '[one of]Ah[or]Yes[or]Mm[at random], [one of]my [dorgr] treats their [gdragon] well[or]I don't think a [gdragon] could be happier with any other [dorgr][or]my [dorgr] certainly knows how make a mess of things[at random]!' [gchis] regard jovial in the wake of your completed task, [ghe] sets back down next to the campfire, allowing you to recover and go about your business.";
...
to say dordom_4: [Cock-Focused Handjob] [BODY MODDED]
	if dorauto is true:
		...
		say "     Perceptive as Doran is, you're barely given the time to [if dorroleint is 0]offer yourself[else]ask for sex[end if] before [ghe]'s on you, your[if scalevalue of player < 4] smaller,[end if] [bodytype of player] form grabbed and [if dorroleint is 0]pulled[else]guided[end if] to [ghis] place of rest before [ghis] shameless talons expose yourself before [ghim].";
		if dorroleint is 0:
	...
	say " for but a moment, the [gdragon]'s talons [if dorroleint is 0]firmly[else]gently[end if] wrap around [if cocks of player > 1]one of your [cock size desc of player] cocks[else]your [cock size desc of player] cock[end if], second talon holding you in place as [ghe] [if dorroleint is 0]roughly[else]carefully[end if] caresses your exposed length, driving you hard before the beast's intent gaze.";
	...
	if dortitmod > 0 and a random chance of 4 in 5 succeeds:
		...
		if cocks of player > 1:
			...
			say "[ghe] [if cock length of player > 17]frequently comments on how impressive your asset is[else if cock length of player < 6 and dorroleint is 0]often teases you over how inadequate your asset is[else]offering cooing approval of your asset[end if] as [ghe] proceeds to squeeze and pump against you, fuelled by a twisted determination.";
	else:
		say "[if dorroleint is 0][ghe] uses [ghis] snout to keep you pinned down[else]slathering you with [ghis] saliva[end if] as [ghis] other talon moves [if cocks of player > 2]to one of your other members, now attending two of them[else if cocks of player is 2]to your other member, now attending both of them[else if cunts of player > 1]to one of your cunts, briefly teasing its entrance as your member remains attended to in tandem[else if cunts of player is 1]your cunt, briefly teasing its entrance as your member remains attended to in tandem[else if anallevel is not 1]your anal ring, briefly teasing its entrance as your member remains attended to in tandem[else]to offer its reassuring caress along the length of your legs[end if]. [gchis] endeavours quickly rise in fervour, ";
		if cocks of player > 1:
			say "[if cock length of player > 17]frequently commenting on how impressive your assets are[else if cock length of player < 6 and dorroleint is 0]often teasing you over how inadequate your assets are[else]offering cooing approval of your assets[end if] as [ghe] proceeds to milk them, fuelled by a twisted determination.";
		else if cunts of player > 0:
			say "[if cock length of player > 17]frequently praising you for the size of your dick[else]offering cooing approval to your assets[end if] as [ghe] proceeds to support [ghis] endeavours by sinking [ghis] digit past the supple folds of your flesh, [if cunt length of player < 7]slight, strained grunts escaping [ghis] lips as [ghe] contends with your tight hole[else]meeting only brief resistance as you envelope the irreverent intrusion[end if]";
		else:
			say "[if cock length of player > 17]frequently commenting on how impressive your asset is[else if cock length of player < 6 and dorroleint is 0]often teasing you over how inadequate your asset is[else]offering cooing approval of your asset[end if] as";
	...
	if dorroleint is 0 and ((dorauto is true and a random chance of 1 in 2 succeeds) or (dorauto is false and a random chance of 1 in 5 succeeds)):
		...
		if cocks of player is 1 and (cunts of player > 0 or anallevel is not 1) and titfuck is 0:
			...
			say ".";
		say "     Slowly pulling [ghis] talons free of you as things die down, you're given some opportunity to survey [if dorroleint is 0]the devastation[else]what[end if] [ghe] has wrought, yourself a worn out and[if cock width of player > 24] massively[end if] cum-stained mess";
		if dorroleint is 0:
			...
			say ".";
			say "     Giving you a moment to rest, [ghis] maw descends upon you a second time, meeting you with [ghis] thick tongue's caress as [ghe] endeavours to clean [if titfuck is 1]both you and [ghim]self[else]you[end if] up in silence. [if cock width of player > 24]This ritual persists for a fair amount of time, considering the substantial mess you've made[else]His work need only take a moment before [ghe]'s complete[end if], leaving you with only a thin veneer of saliva.";
		if dorvorreg is 2 and scalevalue of player < 4:
			if cock width of player > 20 and dorroleint is 0, now dorvorpcf is true;
			say "     '[one of]Mm[or]Hm[or]Heh[at random], [if dorroleint is 0][one of]I could learn a thing or two from my [dorgr] about being such a good[or][dorgr] certainly knows a thing or two about being an obedient[at random] little [one of]toy[or]plaything[or]slut[at random][else][one of]I enjoy playing with my [dorgr][or]my [dorgr] is such fun[or]my [dorgr] should have seen the look on their face[at random][end if]!' Satisfied though [ghe] may be, [ghis] tongue continues to caress along your smaller, [bodydesc of player] frame, a hungry look in [ghis] eyes. '[one of]Ah[or]Oh[at random] [one of]But we're not done here yet[or]my [dorgr]'s fun is just starting[or]don't think I'll let them off that easy[at random]...'";
			dorvore;
		else:
			say "     '[one of]Mm[or]Hm[or]Heh[at random], [if dorroleint is 0][one of]I could learn a thing or two from my [dorgr] about being such a good[or][dorgr] certainly knows a thing or two about being an obedient[at random] little [one of]toy[or]slut[or]plaything[at random][else][one of]I enjoy playing with my [dorgr][or]my [dorgr] is such fun[or]my [dorgr] should have seen the look on their face[at random][end if]!' Satisfied, [ghe] lets you off before [ghe] struts back to [ghis] place of rest next to the fire, watching you with slight amusement as you get back up and go about your business.";
...
to say dordom_5: [Cunt-Focused Handjob] [BODY MODDED]
	...
	say ", the [gdragon]'s talons [if dorroleint is 0]firmly[else]gently[end if] descend upon [if cunts of player > 1]one of your [cunt size desc of player] cunts[else]your [cunt size desc of player] cunt[end if], second talon holding you in place as [ghe] [if dorroleint is 0]firmly[else]carefully[end if] caresses its supple lips, your subsequent arousal making you wet before the beast's piercing gaze.";
	...
	if dorroleint is 0:
		...
		say "     '[one of]I think my [dorgr] likes that[or]Mayhaps we kick this into high gear[or]One could say that my [dorgr]'s fun is just starting[at random], yes?' Tongue's";
	say " affection rising to slide against your[if scalevalue of player < 4] diminutive,[end if] [bodytype of player] torso[if breast size of player > 0] and along the curvature of your [short breast size desc of player] breasts[end if], [if dorroleint is 0][ghe] uses [ghis] snout to keep you pinned down[else]slathering you with [ghis] saliva[end if] as [ghis] other talon moves [if cocks of player > 1]to one of your dicks, claws now attending it in tandem[else if cocks of player is 1]to you dick, now attending to it in tandem[else if cunts of player > 2]to one of your other portals, now attending to the two of them in tandem[else if cunts of player > 1]your other portal, now attending to both of them in tandem[else if anallevel is not 1]your anal ring, briefly teasing its entrance as your feminine portal remains attended to in tandem[else]to offer its reassuring caress along the length of your legs[end if]. [gchis] endeavours quickly rise in fervour, ";
	if cocks of player > 0:
		say "[if cunt length of player < 7]slight, strained grunts escaping [ghis] lips[else]meeting only brief resistance[end if] as [ghe] forces [ghis] digit past the supple folds of your flesh, pumping your dick all the while in [ghis] endeavour to milk them, fuelled by a twisted determination.";
	else:
		...
		if cunts of player is 1 and anallevel is not 1:
			say ", [ghis] endeavours to penetrate your ass [if cunt length of player < 7 and scalevalue of player < 4 and player is not twistcapped]equally[else if scalevalue of player < 4 and player is not twistcapped]noticably more[else if cunt length of player > 5]not nearly as[else]just as[end if] difficult";
	say ".";
	if dorroleint is 0 and ((dorauto is true and a random chance of 1 in 2 succeeds) or (dorauto is false and a random chance of 1 in 5 succeeds)):
		...
		say " sexual fluids, [if cocks of player > 0]each gout of jizz strained and prolonged by [ghis] intrusive talon's expert milking of your stuffed portal[else]your orgasm prolonged for as long as the [gdragon] can manage[end if]. Slowly pulling [ghis] talons free of you, you're given some opportunity to survey [if dorroleint is 0]the devastation[else]what[end if] [ghe] has wrought, yourself a worn out";
		if cocks of player > 0:
		...
		if dorvorreg is 2 and scalevalue of player < 4:
			...
			say "     '[one of]Mm[or]Hm[or]Heh[at random], [if dorroleint is 0][one of]I could learn a thing or two from my [dorgr] about being such a good[or][dorgr] certainly knows a thing or two about being an obedient[at random] little [one of]toy[or]plaything[or]slut[at random][else][one of]I enjoy playing with my [dorgr][or]my [dorgr] is such fun[or]my [dorgr] should have seen the look on their face[at random][end if]!' Satisfied, [ghe] lets you off before [ghe] struts back to [ghis] place of rest next to the fire, watching you with slight amusement as you get back up and go about your business.";

to say dordom_6: [Rimjob] [BODY MODDED]
	if dorauto is true:
		say "     You find yourself suddenly pounced upon, [if scalevalue of player < 4]your smaller, [bodytype of player] form pinned to the ground with little effort[else]pinning you to the ground with little effort[end if]. Agitated in demeanour, the otherwise silent Doran casually wheels around and gives you a rather telling view of [ghis] ass. [if scalevalue of player < 4]Large, s[else]S[end if]hapely curves exposed before you, [ghe] shows no restraint in immediately pressing your face firmly between [ghis] cheeks.";
	...
	if scalevalue of player < 4:
		...
		say "     You continue to service [ghis] anal ring in the hopes of finish [ghim] off, the sounds of [ghis] final groans muffled by your confines, [ghis] supple ring throbbing against your tongue, [bodydesc of player] form soon [if dorballmod > 1]hosed by[else]flooded with[end if] the warmth of [ghis][if dorballmod > 1] torrential,[end if] virile cum as it [if dorballmod > 1]sputters[else]oozes[end if] all over you. Satisfied though [ghe] may be, [ghe] seems intent to remain on top of you for but a moment longer, licking both [ghim] and yourself clean of the[if dorballmod > 1] massive[end if] mess [ghe]'s made before finally getting off.";
	else:
		say "     '[one of]Nng[or]Mnnf[or]A-ah[at random], [one of]my [dorgr] is so very good at pleasing their [gdragon][or]keep going, my [dorgr][or]my [dorgr] really loves their [gdragon]'s ass[at random]...!' Your substantive, slick intrusion very effective at getting the [gdragon] off, you can feel the [gdragon]'s dick [if dorballmod > 0]sputtering[else]oozing[end if] pre against your [bodytype of player] torso. In the wake of [ghis] rising need, [ghis] endeavours to pleasure you become increasingly erratic, as do your own.";
...
to say dordom_7: [Riding Player] [BODY MODDED]
	if dorauto is true:
		...
		say "     The [gdragon] appears particularly generous in the wake of your countenance, grinning as [ghe] [if dorroleint is 0]pushes[else]sets[end if] you on your back, [if scalevalue of player < 4]pulling your smaller, [bodytype of player] form closer[else]adjusting your [bodytype of player] form before [ghim][end if], revealing your [cock size desc of player] dick[smn] before prying eyes.";
		say "     '[one of]Heh[or]Ooh[or]Yes[at random], [if scalevalue of player < 4][one of]my [dorgr] is so very big for someone so very small[or]thank goodness my [dorgr] is willing to overlook the inconvenience of being so well-endowed[or]my little [dorgr] is full of such big surprises[at random][else][one of]my [dorgr] is so very big, in more ways than one[or]no doubt my [dorgr] is quite proud of their equipment[or]I never cease to be impressed by my [dorgr][at random][end if]...' [gche] continues to grin, a greedy look about [ghim] as [ghis] talons move to stroke [if cocks of player > 2]two of your impressive tools[else if cocks of player is 2]both of your impressive tools[else]your impressive tool[end if], '[if dorroleint is 0][one of]I think they'd like to be used by their [gdragon][or]Oh, and they're so eager to be used[or]And always eager to be exploited like the toy they are[at random][else][one of]I'd be remiss not to make use of them[or]I'm certain they wouldn't mind if I take a ride[or]Just relax, I'll take it from here[at random][end if]";
	...
	if dorroleint is 0 and ((dorauto is false and a random chance of 1 in 3 succeeds) or (dorauto is true and a random chance of 2 in 3 succeeds)):
		...
		say "     Brief, strained grunts illustrative of [ghis] endeavours, [ghe] shows [if dorroleint is 0]almost no[else]some[end if] restraint in forcing the organ within [ghis] fleshy confines, [if dorroleint is 0]eager to exploit you for [ghis] own personal pleasure[else]his prior lubrication diminishing some of the strain [ghis] work puts on you[end if]";
		if dorroleint is 0:
			...
			say ". [if scalevalue of player < 4]The [gdragon] takes some care not to crush you under [ghis] weight, an endeavour quickly becoming difficult for [ghim] to maintain[else]The [gdragon] keeps you gently pinned under [ghis] weight[end if]";
	...
	if dorroleint is 0 and ((dorauto is false and a random chance of 1 in 5 succeeds) or (dorauto is true and a random chance of 2 in 3 succeeds)):
		...
		if dorvorreg is 2 and scalevalue of player < 4:
			...
			say "     '[one of]Heh[or]Ha[or]Hm[at random], [one of]my [dorgr] looks good this way[or]I got my [dorgr] good[or]I think my [dorgr] prefers being a messy cum slut[at random]!' Grinning to you as [ghe] slowly recovers, [ghe] chooses to leave you to clean yourself up[if cock width of player > 24 or dorballmod > 0] -- an effort that will doubtlessly take you some time -- [else] [end if]before departing once more to [ghis] place of rest.";
	else:
		say "...!' With one final, firm descent upon your tool, this concluding [if dorroleint is 0]command[else]endeavour[end if] is more than enough to make you both cry out, your [cum load size of player] load rapidly pumping into [ghis] depths, [ghis] own[if dorballmod > 0], torrential flood of[end if] cum [if dorroleint is 0]aimed to paint you with a lewd lack of restraint[else]firing wildly into the open air[end if]. [if cock width of player > 24]Forcibly, [ghe] pulls your hands against [ghis] belly, making you feel [ghis] form as it visibly bloats to contain your massive load[else if player is knotted]Moaning loudly, [ghe] remains tied to you[else]Moaning loudly, [ghe] keep your tool enveloped entirely within [ghis] depths[end if], eager to contain every last drop of your seed.";
		if dorvorreg is 2 and scalevalue of player < 4:
			say "     '[one of]Whew[or]Heavens[or]Goodness[at random], [if dorroleint is 0][one of]That was a great appetizer, now I'm ready for the main course[or]now my [dorgr] gets their inevitable reward[or]my [dorgr] is such a slutty little toy, and soon to be a meal as well[at random][else][one of]We're not finished here just yet, my [dorgr][or]I'm so blessed that my [dorgr] gives me a ride AND a meal[or]I must thank my [dorgr] for that opportunity, and the one that is to follow[at random][end if]!' Taking some time to rest, [ghe] eventually does free you of [ghis] cumbersome weight. [if cock width of player > 24]Groaning slightly as [ghis] overstuffed hole leaks some of your voluminous jizz when [ghe] pulls[else]Pulling[end if] [ghim]self off your now-softening dick, you're soon met with [ghis] tongue's reassuring affection, eager to fill [ghim]self even further...";
			if cock width of player > 24, now dorvornpcf is true;
			dorvore;
		else:
			say "     '[one of]Whew[or]Heavens[or]Goodness[at random], [if dorroleint is 0][one of]I'm rather fond of using my [dorgr] like that[or]no doubt my [dorgr] is happy to be so rewarded with this opportunity[or]my [dorgr] is such a slutty little toy[at random][else][one of]I think my [dorgr] enjoyed that[or]I like taking my [dorgr] out for a ride[or]I must thank my [dorgr] for this opportunity[at random][end if]!' Taking some time to rest, [ghe] eventually does free you of [ghis][if scalevalue of player < 4] cumbersome[end if] weight, [if cock width of player > 24]groaning slightly as [ghis] overstuffed hole leaks some of your voluminous jizz when [ghe] pulls[else]pulling[end if] [ghim]self of your now-softening dick before [if dorroleint is 0]slowly departing back to [ghis] place of rest, leaving you to clean yourself off[else]taking a moment to clean you off, departing back to [ghis] place of rest once [ghe]'s satisfied[end if].";
...
to say dordom_8: [Riding Behind]
	if dorauto is true:
		...
		say "     Offering yourself [if dorroleint is 0]as the [gdragon]'s plaything[else]for a bit of fun with the [gdragon][end if], [ghe] happily hops to [ghis] feet, a playful grin on [ghim] as [ghe] moves in closely to feel you over, setting aside any attire you might have.";
		say "     '[if dorroleint is 0][one of]I think my [dorgr] deserves a little [']gift['] for their obedience[or]I'm feeling a little generous[or]I'm sure my little plaything will enjoy this[at random][else][one of]I think [dorgr]'s [gdragon] is feeling a little needy right now[or]I have an itch only [dorgr] can scratch[or]This'll be fun[at random][end if]...' Once [ghe]'s satisfied, [ghe] pulls away";
	say " before exposing [ghis] ample ass in front of you, large tail hiked up to give you a clear view.";
	say "     [if dorroleint is 0]Commanding[else]Inviting[end if] your touch, [ghe] gives you some free reign to fondle and grope [ghis] behind. [gchis] powerful hindquarters yielding slightly against your shameless caress, [ghe] firmly presses against your digits, encouraging you to continue feeling [ghim] up. [gchis] haunches are ample enough that you need to part them to get an unobstructed view of [ghis] [if dorinternal is true]smooth, scaled crotch[else if dorballmod is 0]ample balls and sheath[else]oversized balls and sheath[end if][if dorballmod > 0], strings of precum oozing from [ghis] barely-aroused dick[end if]. [gchis] hard scales barely hide the softer flesh of [ghis] anal ring.";
	...
	if dorroleint is 0 and a random chance of 1 in 2 succeeds:
		...
		say "     '[if dorroleint is 0][one of]Give me my [dorgr]'s cum, I need it all[or]I must have my [dorgr]'s seed inside me[or]I want what's mine, [dorgr][at random][else][one of]I-I need [dorgr]'s cum[or]I want to feel [dorgr]'s seed inside me[or]P-please, I need it all[at random][end if]!' [gche]'s absolutely desperate for your bliss, and it doesn't take long before [ghe] finally gets what [ghe] wants, letting out a strained cry as you pump your [cum load size of player] load into the [gdragon]'s tight ass. This seems to set [ghim] off as well, painting the ground below you with [ghis][if dorballmod > 0] excessive[end if] cum.";
		...
		if dorvorreg is 2 and scalevalue of player < 4:
			say "     '[one of]Oooh[or]Mmm[or]Aah[at random], [if dorroleint is 0][one of]I love the taste of my useful little [dorgr][or]I think it's time for my [dorgr] to fill me another way[or]My [dorgr] certainly knows how to make their [gdragon] hungry[at random][else][one of][dorgr] is so very tasty[or]I'm not quite full yet[or]it seems I've worked up quite an appetite[at random][end if]...' A wicked grin across [ghis] scaled lips, [ghis] attendance now doubt allowed [ghim] to get a taste of [ghis] meal-to-be...";
			if cock width of player > 24, now dorvornpcf is true;
			dorvore;
		else:
			say "     '[if dorroleint is 0][dorgr] [one of]is so useful to their [gdragon][or]has more than satisfied their [gdragon][or]knows how to attend their [gdragon][at random][else][one of]That was fun[or][dorgr] has my thanks[or]I should be satisfied for now[at random][end if]!' [gche] continues the task of licking you clean, letting you free once [ghe]'s satisfied before [ghe] goes to attending [ghis] own[if dorballmod > 0], considerable[end if] mess.";
...
to say dordom_10: [Cum Worship]
	...
	if a random chance of 1 in 2 succeeds:
		...
		say "     '[one of]J-just a little more[or]M-[dorgr] needs it all[or]Keep s-sucking[at random]...' Slightly trembling against your touch, [ghe] seems finally overtaken by bliss, [ghis] still barely-present immediately overtaking you with a torrent of [ghis] bestial fluid. [if scalevalue of player > 4]After maybe three gulps[else if scalevalue of player is 4]After maybe two gulps[else]You get off maybe one gulp[end if] before it starts sputtering from your maw, forcing it free from your lip's embrace and causing each successive gout to drench your [bodytype of player] torso.";
		say "     There, covered in the [gdragon]'s voluminous jizz, you're able to free and arm and [if cocks of player > 0]jerk yourself off[else]fondle yourself[end if], motion slicked by the abundant fluid. Made so sensitive by your endeavour, it doesn't take much effort to set yourself off, [if cocks of player > 0]your own, [cum load size of player] load mixing with [ghis]s[else]your juices mixing with [ghis] own[end if]. Doran, no doubt amused by the display, allows you to revel in the throes of your ecstasy while laying drenched in [ghis] cum.";
	else:
		say "     '[one of]I think [dorgr] needs a bath[or]Shame [dorgr] didn't bring a bathing suit[or]I'm going to make such a mess of [dorgr][at random]...' Musing over you, the [gdragon]'s impressive dick starts to slip free from its ill-fitting [if dorinternal is true]slit[else]sheath[end if], hard and throbbing above you almost immediately. Gently pinned to the ground by [ghis] hind legs, you remain tantalizingly out of reach of [ghis] [if dorinternal is true]nethers[else]rod[end if], Doran teasing you by its mere presence, eagerly drooling all over your [bodytype of player] body.";
...
to say dorsub_8: [Self-milking]
	say "     You [if dorroleint is 4]order[else]call[end if] Doran over, who happily trots up to you, a slight trail of precum following in [ghis] wake. Somewhat [if dorroleint is 4]imp[else]sheep[end if]ishly, you order Doran to start sucking [ghim]self off and. Ever-obliging, the creature lifts a hind leg to expose [if dorinternal is true]his flush crotch[else]his sheath and oversized sack[end if], partially aroused and oozing.";
	say "     '[one of]Mmm[or]Ah[or]Ooh[at random], [if dorroleint is 4][one of]I hope I can impress [dorgr][or]I'll try my best to entertain [dorgr][or]I see [dorgr] wants a show[at random][else][one of]I hope [dorgr] likes what they see[or]This'll be entertaining[or][dorgr] plays all the funnest games[at random][end if]!' Tongue caressing along [ghis] [if dorinternal is true]slit[else]sheath[end if], [ghe]'s already making a bit of a mess against [ghis] underside and snout, awash in spurts of [ghis] thick precum. [if dorroleint is 4]Not even having to order [ghim][else]With a little suggestion[end if], [ghis] lips envelope [ghis] quickly exposed, pointed cockhead, filling the air with the wet sound of [ghis] exaggerated gulping.";
	say "     Even as [ghe] draws directly from the source some of the heady fluid dribbles down to soak [ghis] [if dorinternal is true]crotch[else]oversized ballsack[end if], air full with the beast's [gmasculine] aroma. Perhaps you should go in and assist Doran? Otherwise, you'll watch from a distance.";
	if player consents:
		say "     [if scalevalue of player < 4]Kneeling down[else]Lowering yourself[end if] next to the [gdragon], you hands first move in to feel around the base of [ghis] now-fully erect cock. Fully hilted within [ghis] maw, your hands are bathed in [ghis] hot breath, saliva and precum-slicked lips pressing against your digits. [gche] pulls back only as far as to allow your wandering touch free reign, the slick and throbbing organ coating your fingers in the stuff.";
		...
		if player consents:
			say "     Lips moving in to meet [ghis] slick [if dorinternal is true]cockbase[else]balls[end if], you're senses are quickly flooded with the dragon's [gmasculine] flavour, bathing you in [ghis] shallow breath. [gche] seems further aroused by your lewd actions, gouts of precum gushing from between [ghis] lips with each throb.";
			now tempnum is 1;
		else:
			now tempnum is 0;
		if dorroleint is 4:
			...
			if player consents:
				...
				say "     It doesn't take much longer before your [gdragon] lets out a loud groan, muffled by the sound of [ghis] orgasm[if dornpbb > 0] loudly[end if] gushing between [ghis] lips";
				if dornpbb > 0:
				...
				say "     '[one of]Ooh[or]Whew[or]Aah[at random], [if dorroleint is 4][one of]I hope [dorgr] was satisfied by the show[or]I hope I did [dorgr] proud[or]I must thank [dorgr] for that[at random][else][one of]I'm sure that was quite a show[or]That was fun[or]That was certainly interesting[at random][end if]!' [if dornpbb > 0]Considerably[else]Somewhat[end if] bloated from the whole ordeal, [ghe] slowly returns to the campfire once everything is taken care off, freeing you to go about your business once more.";
		else: [not max dom -- allow orgasm]
			say "     It doesn't take much longer before your [gdragon] lets out a loud groan, muffled by the sound of [ghis] orgasm[if dornpbb > 0] loudly[end if] gushing between [ghis] lips. Furrowing [ghis] brow, [ghe]'s clearly distracted with keeping up to each powerful barrage, each exaggerated gulp causing [ghis] stomach to gradually expand. [gchis] seed begins to ooze out from between [ghis] lips, [if tempnum is 1]inviting you to taste[else]staining your hands[end if].";
			say "     [gchis] flow eventually does subside, and as it does [ghe] starts to pull [ghim]self free and lick [ghim]self clean, [if tempnum is 1]meeting your continued attendance in a brief kiss[else]attending to your mess afterwards[end if].";
			say "     '[one of]Ooh[or]Whew[or]Aah[at random], [one of]I'm sure that was quite a show[or]That was fun[or]That was certainly interesting[at random]!'  [if dornpbb > 0]Considerably[else]Somewhat[end if] bloated from the whole ordeal, [ghe] slowly returns to the campfire once everything is taken care off, freeing you to go about your business once more.";
		...
		if player consents:
			...
			say "     '[one of]Ooh[or]Whew[or]Aah[at random], [if dorroleint is 4][one of]I hope [dorgr] was satisfied by the show[or]I hope I did [dorgr] proud[or]I must thank [dorgr] for that[at random][else][one of]I'm sure that was quite a show[or]That was fun[or]That was certainly interesting[at random][end if]!' [if dornpbb > 0]Considerably[else]Somewhat[end if] bloated from the whole ordeal, [ghe] slowly returns to the campfire once everything is taken care of, freeing you to go about your business once more.";
		else: [Prolong]
			...
			if dorroleint is 4:
				...
				if dornpbb > 0:
					...
					say ". Furrowing [ghis] brow, [ghe]'s clearly distracted with keeping up to each powerful barrage, each exaggerated gulp causing [ghis] stomach to gradually expand";
				say ". Once [ghis] fervour finally dies down, [ghe] pulls free of [ghis] cum-painted cock before [ghe] starts licking [ghim]self clean of the mess.";
				say "     '[one of]Ooh[or]Whew[or]Aah[at random], [one of]I'm sure that was quite a show[or]That was fun[or]That was certainly interesting[at random]!' [if dornpbb > 0]Considerably[else]Somewhat[end if] bloated from the whole ordeal, [ghe] slowly returns to the campfire once everything is taken care of, freeing you to go about your business once more.";

Section 3.2 - Doran NPC Submissive Sex Pool

to say dorsub_1: [Anal Catching] [BODY MODDED]
	say "     You [if dorroleint is 4]order[else]ask[end if] the [gdragon] to turn away and offer [ghim]self to you. Eager to please, [ghe] wheels around, [if scalevalue of player < 4]lowering [ghim]self to the ground before [end if]hiking [ghis] tail over to leave your entry exposed. Shapely ass revealed before you, [ghe]'s already quite aroused by the prospect of your imminent abuse, partially exposed tool staining the dirt with its[if dorballmod > 0] excessive[end if] pre.";
	say "     '[one of]Ah[or]Oh[or]Mm[or]Very well[at random], [dorgr], [if dorroleint is 4][one of]completely ruin me[or]please abuse me[or]I ache for them[at random][else][one of]don't hold back[or]I'm ready for anything they've got[or]I'm all theirs[at random][end if].' You expose your [cock size desc of player] dick[smn] before the open air, [if cock length of player > 17 or dorroleint is 4]a very eager, needy look in [ghis] eyes at the prospect of getting[else]clearly eager to be[end if] impaled by [if cocks of player > 1]one of those things[else]the thing[end if].";
	...
	if cock length of player > 17:
		...
		say "     '[one of]More, [dorgr].[run paragraph on] I need it all[or]Further, [dorgr], further[at random]!' [ghe] [if dorroleint is 4]beg[else]ask[end if]s, and with a bit of work and pre's slick lubrication [ghis] tight tailpipe grows increasingly willing to take your sizeable tool[if player is knotted], up to the knot[end if], allowing you the freedom to start fucking [ghim] proper.";
	else:
		say "     [gche] shows absolutely no difficulty in taking your member[if player is knotted], even tying with it almost immediately[end if]. [gche] doesn't seem to mind your ease, revelling in being [if dorroleint is 4]shamelessly used[else]ridden by[end if] you, supple abyss almost overwhelming in how [if dorroleint is 4]obligingly[else]assertively[end if] it milks the organ.";
		say "     '[one of]Fuck me, [dorgr][or]Harder, [dorgr], harder[at random]!' [gche] [if dorroleint is 4]plead[else]demand[end if]s, the creature greedy for your throbbing meat as you're compelled to start fucking [ghim] proper.";
...
to say dorsub_2: [Cock Oral] [BODY MODDED]
	say "     [if dorroleint is 4]Without having to say a word[else]A little awkwardly[end if] you set yourself down, exposing your [cock size desc of player], [cock of player] dick[smn] before the [gdragon]. [if dorroleint is 4]Immediately, [ghe]'s upon you, easily understanding what you need of [ghim],[else]Gesturing [ghim] over [ghe] makes [ghis] way to you, clearly eager to please[end if] and looking rather excited by the offering.";
	say "     '[if dorroleint is 4][one of][dorgr], nothing would please their [gdragon] more[or][dorgr], their [gdragon] longs to taste them[or]I see [dorgr] is in need of me[at random][else][one of]I'd be happy to oblige [dorgr][or][dorgr]'s very eager for my affection, I see[or]it certainly looks like [dorgr]'s excited to see me[at random][end if]...' The [gdragon] goes quiet for a moment, [ghis] scaled lips parting to engulf [if cocks of player > 2]the head of one of them, taloned hands moving to caress its brothers[else if cocks of player is 2]the head of one of them, taloned hand moving to caress its brother[else]its head[end if]. Humid breath washing over your lower torso, [if dorroleint is 4][ghe]'s rather gentle in this initial, oral embrace, clearly in love with the taste of your flesh[else]you abide [ghis] thick tongue's initial, teasing query[end if].";
	If dortitmod > 0:
		say "     [gchis] ample breasts press gently against you, perhaps you could [if dorroleint is 4]command[else]ask[end if] [ghim] to make use of them?";
		if player consents:
			now tempnum is 1;
	...
	if dorroleint is 4:
		say "[if cock length of player > 17][dorgr] [one of]never ceases to impress me with their size[or]is so very big and intimidating[or]has such enviable assets[at random][else][one of][dorgr] never ceases to impress me with their taste[or]I hope I can satisfy [dorgr][or]what [dorgr] offers me is such a treat[at random][end if]";
	else:
		say "[if cock length of player > 17][dorgr] [one of]is quite the challenging prospect[or]certainly tests the limits of my ability[or]makes me quite jealous of their size[at random][else][one of]I hope [dorgr] is comfortable[or][dorgr] is in for some fun now[or][dorgr] is going to love this[at random][end if]";
	...
	if dorroleint is not 4 or (dorroleint is 4 and a random chance of 1 in 3 succeeds) or dornpbb > 0:
		say "[ghe] continues on, '...Though, [if dorroleint is 4][one of]I must beg for the opportunity to satisfy myself[or]they make me ache with need[at random][else][one of]my own need is overwhelming[or]this task leaves me quite aroused[at random][end if], might they be so obliging?' You imagine [ghe][']d not be as capable of pleasuring you with [ghis] attention split, shall you oblige [ghis] request? Otherwise you'll ensure [ghis] attention remains solely on you.";
		if player consents:
			say "     You decide to allow [ghim] this opportunity, to which [ghe] seems grateful, hand moving [if cocks of player > 2]away from your spare cocks to attend [ghis] own[else if cocks of player > 1]away from your spare cock to attend [ghis] own[else]over to attend [ghis] own cock[end if] before [if tempnum is 0][ghis] maw envelops your flesh a second time[else][ghe] gets back to work[end if], [ghis] efforts becoming much more deliberate now. Dexterous, slick appendage eager to [if tempnum is 0]wrap around your [cock size desc of player] tool[else]tease and caress along your [cock size desc of player] tool[smn][end if], [ghe] exhibits [if dorroleint is 4]an obsessive[else]a clear[end if] desire to satisfy your need[if cock length of player > 15 and dorroleint is 4], more than eager to hilt your impressive dick within [ghis] supple confines, at expense to [ghis] own comfort[else if cock length of player > 15], frequently pushing the limits of [ghis] endurance to better engulf your impressive dick[end if]. As [ghis] need for self-pleasure rises, so does the pace in which [ghe] [if tempnum is 0]bob[else]grind[end if]s against you become erratic, though it's more than sufficient in bringing you closer to bliss.";
			...
			if player consents:
				...
				if tempnum is 1:
					say "[if cocks of player > 2]your other rods make a further mess of [ghim], dripping down to mix with[else if cocks of player is 2]your other rod makes a further mess of [ghim], dripping down to mix with[else if dorballmod > 0][ghe] wallows in a pool of[else][ghe] wallows in a puddle of[end if] [ghis] own spunk.";
				else:
					say "[if cocks of player > 2]your unattended organs stain the ground, mixing with[else if cocks of player is 2]your unattended organ stains the ground, mixing with[else if dorballmod > 0][ghe] wallows in a pool of[else][ghe] wallows in a puddle of[end if] [ghis] own spunk.";
				say "     '[one of]Ah[or]Alright[or]Yes[at random], [if dorroleint is 4][one of][dorgr] offers me so much[or]I hope I did [dorgr] proud[or][at random][else][one of]I hope [dorgr] is satisfied[or]I hope [dorgr] enjoyed that as much as I did[or]I think [dorgr]'s [gdragon] is finished here[at random][end if].' [gchis] remark punctuating [ghis] finished work, you're quickly cleaned off before the [gdragon] pulls away and lets you go, no doubt so [ghe] can attend to [ghis] own shameful state.";
		else:
			if dornpbb is 1, now dornpbb is 2;
			say "     You decide against it, ";
			if tempnum is 1:
				say "to which [ghe] [if dorroleint is 4]quickly nods in agreement, clearly trying to distance [ghim]self from making the request in the first place[else]obliges with little further thought on the matter[end if] before returning [ghis] attention to your cock[smn]";
			else:
				say "to which [ghe] [if dorroleint is 4]quickly nods in agreement, clearly trying to distance [ghim]self from making the request in the first place before [ghis] maw envelops[else]obliges with little further thought on the matter, maw enveloping[end if] your cock a second time";
			say ", [ghis] efforts becoming much more deliberate now. Dexterous, slick appendage eager to [if tempnum is 0]wrap around[else]tease and caress along[smn][end if] your [cock size desc of player] tool, [ghe] displays [if dorroleint is 4]an obsessive[else]a clear[end if] desire to satisfy your need[if tempnum is 1][else if cock length of player > 15 and dorroleint is 4], more than eager to hilt your impressive dick within [ghis] supple confines, at expense to [ghis] own comfort[else if cock length of player > 15], frequently pushing the limits of [ghis] endurance to better engulf your impressive dick[end if]. [gche] retains a very methodical, calculated pace,[if cocks of player > 2] occasionally switching to the other dicks in [ghis] grip to keep you sensitive to [ghis] tongue's influence,[else if cocks of player > 1] occasionally switching to the other dick in [ghis] grip to keep you sensitive to [ghis] tongue's influence,[end if] which proves effective at rapidly sending you ever-closer to bliss.";
			if dorroleint is 4:
				...
				if player consents:
					...
					say "     '[one of]Ah[or]Alright[or]Yes[at random], [if dorroleint is 4][one of][dorgr] offers me so much[or]I hope I did [dorgr] proud[or][at random][else][one of]I hope [dorgr] is satisfied[or]I hope [dorgr] enjoyed that as much as I did[or]I think [dorgr]'s [gdragon] is finished here[at random][end if].' [gchis] remark punctuating [ghis] finished work, you've been cleaned of your prior mess, the [gdragon] pulling away and letting you off, settling back down next to the fire.";
			else:
				say "     You finally cry out, dick[smn] erupting against the beast's skilled tongue[if tempnum is 1] and chest[end if]. With a disciplined determination, [ghe] milks you dry of every last drop of your [cum load size of player] payload, ";
				if tempnum is 0:
					say "[if cocks of player > 2]talon bunching up at your other cock's head that [ghe] might lick it clean when [ghe] concludes with the first, though [ghe] is not so dexterous that [ghe] can stop some of it from spilling all over the ground[else if cocks of player is 2]talon bunching up at your other cock's head so [ghe] can lick it clean when [ghe] concludes with the first[else]licking it clean once you might offer no more for [ghim][end if]";
		...
		say "[ghis] [if tempnum is 1]tongue caresses along you cock[smn] once more[else]maw envelops your cock a second time[end if] as [ghe] returns to [ghis] work, [ghis] efforts now much more deliberate. Dexterous, slick appendage eager to [if tempnum is 0]wrap around[else]tease and caress along[smn][end if] your [cock size desc of player] tool, [ghe] displays [if dorroleint is 4]an obsessive[else]a clear[end if] desire to satisfy your need[if tempnum is 1][else if cock length of player > 17 and dorroleint is 4], more than eager to hilt your impressive dick within [ghis] supple confines, at expense to [ghis] own comfort[else if cock length of player > 17], frequently pushing the limits of [ghis] endurance to better engulf your impressive dick[end if]. [gche] retains a very methodical, calculated pace,[if cocks of player > 2] occasionally switching to the other dicks in [ghis] grip to keep you sensitive to [ghis] tongue's influence,[else if cocks of player > 1] occasionally switching to the other dick in [ghis] grip to keep you sensitive to [ghis] tongue's influence,[end if] which proves effective at rapidly sending you ever-closer to bliss.";
		if dorroleint is 4:
			say "     The wet sounds of [ghis] attendance filling the air, it doesn't take you long before you reach your limit! You can command [ghim] to let go and free yourself to get off on the [gdragon]'s [if tempnum is 1]voluptuous tits and [end if]obedient face, should you? Else you will give [ghim] what [ghe] clearly wants.";
			if player consents:
				say "     Words strained under the weight of your passion, you order the beast to let go, doing as you command just before your dick[smn] erupt upon [ghis] obliging snout[if tempnum is 1] and abundant chest[end if]. Regarding in demeaning act with a wanton embrace, [ghe] patiently abides being so irreverently painted by your [cum load size of player] load. When all is said and done [ghe]'s naught but an embarrassing mess, eagerly licking your spunk off [ghim].";
				...
				if tempnum is 0:
					...
					say "[ghis] ample clevage considerably slicked by the whole mess";
				say "[if cock width of player > 24].[run paragraph on] [gche] doesn't seem even remotely phased by the volume you surrender to [ghim], revelling in the sheer excess[end if].";
				say "     '[one of]Ah[or]Alright[or]Yes[at random], [if dorroleint is 4][one of][dorgr] offers me so much[or]I hope I did [dorgr] proud[or]It was a great pleasure to serve [dorgr][at random][else][one of]I hope [dorgr] is satisfied[or]I hope [dorgr] enjoyed that as much as I did[or]I think [dorgr]'s [gdragon] is finished here[at random][end if].' [gchis] remark punctuating [ghis] finished work, you've been cleaned of your prior mess, the [gdragon] pulling away and letting you off, settling back down next to the fire.";
		else:
			say "     You finally cry out, dick[smn] erupting against the beast's skilled tongue[if tempnum is 1] and chest[end if]. With a disciplined determination, [ghe] milks you dry of every last drop of your [cum load size of player] payload, ";
			if tempnum is 0:
				say "[if cocks of player > 2]talon bunching up at your other cock's head that [ghe] might lick it clean when [ghe] concludes with the first, though [ghe] is not so dexterous that [ghe] can stop some of it from spilling all over the ground[else if cocks of player is 2]talon bunching up at your other cock's head so [ghe] can lick it clean when [ghe] concludes with the first[else]licking it clean once you might offer no more for [ghim][end if]";
...
to say dorsub_3: [Cunt Oral] [BODY MODDED]
	say "     [if dorroleint is 4]Without having to say a word[else]A little awkwardly[end if] you set yourself down, exposing your [cunt size desc of player] cunt[sfn] before the [gdragon]. [if dorroleint is 4]Immediately, [ghe]'s upon you, easily understanding what you need of [ghim],[else]Gesturing [ghim] over [ghe] makes [ghis] way to you, clearly eager to please[end if] and looking rather excited by the offering.";
	say "     '[if dorroleint is 4][one of][dorgr], nothing would please their [gdragon] more[or][dorgr], their [gdragon] longs to taste them[or]I see [dorgr] is in need of me[at random][else][one of]I'd be happy to oblige [dorgr][or][dorgr]'s very eager for my affection, I see[or]it certainly looks like [dorgr]'s excited to see me[at random][end if]...'Briefly without any further discourse from the [gdragon], [ghis] scaled lips part to embrace [if cunts of player > 1]one of your supple, nether lips[else]your supple, nether lips[end if][if cocks of player is 1], taloned hands moving to caress your exposed dick[smn][else if cunts of player > 2], taloned hands moving to caress its sisters[else if cunts of player is 2], taloned hand moving to caress its sister[end if]. Humid breath washing over your lower torso, [if dorroleint is 4][ghe]'s rather gentle in this initial, oral embrace, no doubt illustrating a particular reverence for your flesh[else]you abide [ghis] thick tongue's initial, teasing query[end if].";
	say "     '[one of]Oh[or]hm[or]Ah[at random], ";
	if dorroleint is 4:
		say "[if cunt length of player > 17][dorgr] [one of]has so much to offer their [gdragon][or]offers quite an intimidating feast of flesh[or]has such enviable assets[at random][else][one of][dorgr] never ceases to impress me with their taste[or]I hope I can satisfy [dorgr][or]what [dorgr] offers me is such a treat[at random][end if]";
	else:
		say "[if cunt length of player > 17][dorgr] [one of]is quite the challenging prospect[or]certainly isn't unwilling to test my ability[or]is quite the impressive offering[at random][else][one of]I hope [dorgr] is comfortable[or][dorgr] is in for some fun now[or][dorgr] is going to love this[at random][end if]";
	say "!' Taking a moment to speak up, ";
	if dorroleint is not 4 or (dorroleint is 4 and a random chance of 1 in 3 succeeds) or dornpbb > 0:
		say "[ghe] continues on, '...Though, [if dorroleint is 4][one of]I must beg for the opportunity to satisfy myself[or]they make me ache with need[at random][else][one of]my own need is overwhelming[or]this task leaves me quite aroused[at random][end if], might they be so generous?' You imagine [ghe][']d not be as capable of pleasuring you with [ghis] attention split, shall you oblige [ghis] request? Otherwise you'll ensure [ghis] attention remains solely on you.";
		if player consents:
			say "     You decide to allow [ghim] this opportunity, to which [ghe] seems grateful, hand moving [if cocks of player > 0]away from your cock[smn] to attend [ghis] own[else if cunts of player > 1]away from your spare portal to attend [ghis] previously neglected cock[else]over to attend [ghis] previously neglected cock[end if] before [ghis] maw envelops your flesh a second time, [ghis] efforts becoming much more deliberate now. Dexterous, slick appendage eager to plunge into your depths, [ghe] exhibits [if dorroleint is 4]an obsessive[else]a clear[end if] desire to satisfy your need[if cunt length of player > 17 and dorroleint is 4], more than eager to push the limits of how far [ghe] can thrust [ghis] tongue upon you, at expense to [ghis] own comfort[else if cunt length of player > 17], frequently pushing the limits of [ghis] ability to thrust [ghis] tongue upon you[end if]. As [ghis] need for self-pleasure rises, so does the pace in which [ghe] tongues you become erratic, though it's more than sufficient in bringing you closer to bliss.";
			if cocks of player > 0:
...
to say dorsub_4: [Cunt Catching] [BODY MODDED]
	say "     You decide to take your [gdragon] for a ride, [if dorroleint is 4]command[else]ask[end if]ing [ghim] to roll on [ghis] back and reveal [ghis] underside for you. [gche] [if dorroleint is 4]instinctively[else]eagerly[end if] complies, laying on the ground once more before [ghis] legs are spread, exposing [ghim]self fully before you. The mere implication of your command is enough to arouse [ghim] slightly, flared cockhead peeking from its scaled [if dorinternal is true]slit[else]sheath[end if].";
	say "     '[dorgr], [if dorroleint is 4][one of]I'm honoured by such a prospect[or]I long to be relegated as a mere toy for their pleasure[or]I am theirs to do with as they please[at random][else][one of]don't hold back[or]I'm eager to be ridden[or]I'm excited by the prospect[at random][end if]";
	if dorroleint is 4:
		...
		if player consents:
			...
			if dorinternal is true:
				...
				say "feel [ghis] [if dorballmod > 0]massive, othersized[else]sizeable, full[end if] sack and bulging sheath. Moans pulled from the [gdragon] with your gentle touch, you lean in further and prepare [ghim] with your tongue's caress";
			say ", more that enough to bring [ghis] organ to its full size. Not feeling eager to subject yourself to the ride dry, you make some effort to slick the member beforehand, [if dorballmod > 0]not that it's difficult with the amount of precum [ghe]'s constantly oozing[else if scalevalue of player > 3]finding little difficulty in achieving your goal[else], though you're not quite big enough to get the job done, the [gdragon] bending in to help you with [ghis] own[end if]. It's not long before it's ready, dripping with an aching need.";
	else:
		say "     Leaning in between [ghis] twin, powerful legs, your hands move to ";
		if dorinternal is true:
			say "stroke is exposed, smooth portal. Moans pulled from the [gdragon] with your gentle touch, you lean in further and prepare [ghim] with your tongue's caress across [ghis] nether lips";
	...
	if scalevalue of player < 4:
		...
		say "     '--[one of]Ah[or]Oh[or]Um[at random], does [dorgr] [one of]need aid[or]require help[at random]? [if dorroleint is 4][one of]Not that I think they can't handle their [gdragon], of course[or]I wouldn't want my size to cause them too much trouble[at random][else][one of]I wouldn't want to hurt them[or]I'll be as slow as is comfortable for them[at random][end if].' Clearly wanting to avoid possibly hurting you, shall your let [ghim]? Else you'll stick to your guns.";
		if player consents:
	...
	say "     '[one of]Oh[or]Nrgh[or]Nnf[at random], [one of]M-[dorgr] is so [if cunt length of player < 12]t-tight[else if cunt length of player < 17]s-skilled[else]f-firm[end if][or]I-I can't hold back, M-[dorgr][or]M-[dorgr][at random]...' [gchis] mutterings are mixed with deep moans, made all the more prevalent as you build a motion of riding the [gdragon]'s powerful dick, [if cocks of player > 1]your own bouncing against the open air, staining the [gdragon]'s scales with their slick fluids[else if cocks of player is 1]your own bouncing against the open air, staining the [gdragon]'s scales with its slick fluids[else if cunts of player > 2]unstuffed cunts aching against the open air, staining the [gdragon]'s scales with their honey[else if cunts of player is 2]unstuffed cunt aching against the open air, staining the [gdragon]'s scales with its honey[else]stuffed portal eagerly milking the beast for what [ghe]'s worth[end if][if scalevalue of player < 3 or cunt length of player < 12] while stretched to the limits of your ability to contain the organ[end if]. [if dorballmod > 0]Each descent causes yet another gout of precum to shoot from [ghim], causing you to flinch.[run paragraph on] [gchis] supply almost seems endless![end if]";
	...
	if player consents:
		...
		say "     [if dorroleint is 4]Insistently, [ghe] goes about licking you clean, even as you pull yourself free [ghe]'s quick to attend to your leaking hole until [ghe]'s satisfied with your appearance, finally letting you off[else]Eventually, you pull yourself free of the [gdragon]'s now-softened organ, stepping off of [ghim][end if].";
		say "     'Oh [dorgr], [if dorroleint is 4][one of]I love seeing them full of their [gdragon]'s seed[or]they're so good at taking their [gdragon]'s load[or]I beg for the opportunity to fill them again[or]I love being inside them[at random][else][one of]they always manage to impress me[or]I envy their skill[or]they know how to handle themself[at random][end if].' Offering [ghis] regards, [ghe] takes a moment to catch [ghis] breath before getting back up and returning to [ghis] spot in the cave. [gche] already goes through a ritual of cleaning [ghim]self of the[if dorballmod > 0] excessive[end if] mess that was doubtlessly made.";
...
to say dorsub_5: [Anal Catching] [BODY MODDED]
	...
	say "     '[one of]Oh[or]Nrgh[or]Nnf[at random], [one of]M-[dorgr] is so [if scalevalue of player < 4]t-tight[else if player is twistcapped]s-skilled[else]f-firm[end if][or]I-I can't hold back, M-[dorgr][or]M-[dorgr][at random]...' [gchis] mutterings are mixed with deep moans, made all the more prevalent as you build a motion of riding the [gdragon]'s powerful dick, [if cocks of player > 1]your own bouncing against the open air, staining the [gdragon]'s scales with their slick fluids[else if cocks of player is 1]your own bouncing against the open air, staining the [gdragon]'s scales with its slick fluids[else]unstuffed cunt[sfn] aching against the open air, staining the [gdragon]'s scales with their honey[end if][if scalevalue of player < 3]while stretched to the limits of your ability to contain the organ[end if]. [if dorballmod > 0]Each descent causes yet another gout of precum to shoot from [ghim], causing you to flinch.[run paragraph on] [gchis] supply almost seems endless![end if]";
	say "     Writhing against the ground, completely lost in the pleasure you offer [ghim], it doesn't take long before [ghe] looks like [ghe]'s about to erupt! It might be ideal to pull out just before then. Shall you, lest you take the full brunt of [ghis] carnal fury?";
	if player consents:
...
to say dorsub_6: [Rimjob] [BODY MODDED]
	say "     You, [if dorroleint is 4]rather bluntly, order[else]somewhat sheepishly, ask[end if] Doran to have [ghim] attend your ass, to which [ghe] unquestioningly obliges. Moving close and rolling onto [ghis] back, the [gdragon] ";
	if dorroleint is 4:
		...
		if player consents:
			...
			if dorroleint is 4:
				...
				if player consents:
					...
					if player consents:
						say "     Unable to hold back any longer, you feel the [gdragon]'s roar of bliss vibrating against your thighs, aiming [ghis] cock toward a nearby wall just before it's [if dorballmod > 0]uttery drenched with massive gouts[else]doused with errant strings[end if] of [ghis] thick, white fluid[if dornpbb > 0], the magnitude of which intensified by [ghis] prior neglect[end if]. [if dorballmod > 0]Taking what seems like forever to subside[else]Gradually subsiding[end if], your attendant pants audibly, bathing you in [ghis] warm breath while [ghe] recovers";
						now tempnum is 0;
					else:
						say "     Unable to hold back any longer, you feel the [gdragon]'s roar of bliss vibrating against your thighs, you're immediately blasted with a[if dorballmod > 0]n overwhelmingly excessive[end if] barrage of the [gdragon]'s voluminous seed[if dornpbb > 0], the magnitude of which intensified by [ghis] prior neglect[end if], painting [if scalevalue of player < 4 or dorballmod > 0]you utterly[else]your torso[end if] with strings of the thick, white fluid, until it [if dorballmod > 0]floods[else]oozes[end if] down to meet your attendant's tongue, who -- though quite exhausted -- nonetheless remains very eager to [if dorballmod > 0]swallow down the excess[else]lap it up[end if][if dorballmod > 0].[run paragraph on] By the time the flow subsides you're practically bathing in [ghis] cum, utterly drenched by the stuff[end if]";
						now tempnum is 1;
					say ". Eventually, ";
					if dorroleint is 4:
						say "you let [ghim] free, [if scalevalue of player < 4]climb[else]gett[end if]ing off [ghim] before [ghe] cleans you of [if tempnum is 1 and dorballmod > 0]the excessive amounts of [ghis][else if tempnum is 1]the remainder of [ghis][else]any remaining[end if]";
					else:
						say "[ghe] [if scalevalue of player < 4]puts you back down[else]pulls out from under you[end if], cleaning you of [if tempnum is 1 and dorballmod > 0]the excessive amounts of [ghis][else if tempnum is 1]the remainder of [ghis][else]any remaining[end if]";
					say " mess.";
					say "     '[if dorroleint is 4][one of]It pleases their [gdragon] that they could tend to them so[or]Oh, my [dorgr]'s ass is positively enviable[or]I could tend to [dorgr] like that for hours[at random][else][one of]I hope I satisfied [dorgr][or]That was certainly a fun little digression[or]Oh, the things [dorgr] asks of me[at random][end if]!' Grinning slyly, [ghe] returns back to [ghis] place of rest once [ghe]'s satisfied with your present state, letting you gather your thoughts and go about your business once more.";
			else:
				...
				if player consents:
					...
					say "     Unable to hold back any longer, you feel the [gdragon]'s roar of bliss vibrating against your thighs, you're immediately blasted with a[if dorballmod > 0]n overwhelmingly excessive[end if] barrage of the [gdragon]'s voluminous seed[if dornpbb > 0], the magnitude of which intensified by [ghis] prior neglect[end if], painting [if scalevalue of player < 4 or dorballmod > 0]you utterly[else]your torso[end if] with strings of the thick, white fluid, until it [if dorballmod > 0]floods[else]oozes[end if] down to meet your attendant's tongue, who -- though quite exhausted -- nonetheless remains very eager to [if dorballmod > 0]swallow down the excess[else]lap it up[end if][if dorballmod > 0].[run paragraph on] By the time the flow subsides you're practically bathing in [ghis] cum, utterly drenched by the stuff[end if]";
					now tempnum is 1;
				say ". Eventually, [ghe] [if scalevalue of player < 4]puts you back down[else]pulls out from under you[end if], cleaning you of [if tempnum is 1 and dorballmod > 0]the excessive amounts of [ghis][else if tempnum is 1]the remainder of [ghis][else]any remaining[end if] mess.";
			...
			if dornpbb is 1, now dornpbb is 2;
			say "     Reveling in this lewd ritual, the [gdragon]'s oozing rod bobs unattended before you, though [ghe] is [if dorroleint is 4]obedient[else]obliging[end if] enough to remain fixated on your ass, each irreverent twitch of [ghis] invading appendage expertly making you twist and writhe in pleasure. [gchis] talons soon move your stroke your previously unattended [if cocks of player > 1]cock[smn][else]portal[sfn][end if] to supplement [ghis] work, intensifying your inevitable march closer to bliss.";
...
to say dorsub_7: [Oral Giving] [BODY MODDED]
	say "     Approaching [ghim], you [if dorroleint is 4]order[else]ask[end if] the [gdragon] to lay on [ghis] back, who's more than eager to oblige. Once Doran is in position, [ghis] [if dorinternal is false and dorballmod > 0]sizeable genitalia is exposed before you, oversized sack and bulging sheath[else if dorinternal is false]sizeable genitalia is exposed before you, ample sack and bulging sheath[else]genital slit is exposed before you, cockhead peeking from its home and[end if] left to your mercy[if dorballmod > 0], already leaking precum[end if]. [if dorroleint is 4][gche][else]It seems [ghe][end if][']ll let you do as you please, so perhaps you could obsess over this lewd offering? Otherwise, you'll keep your distance as you attend to [ghim].";
	if player consents:
		now tempnum is 1;
		...
		if player consents:
			...
			if dorinternal is false:
				...
				say "     Slowly, your hands move to feel [ghis] partially exposed dick, head drooling [if dorballmod > 0]considerably[else]slightly[end if] from your attention as you caress";
		if dorinternal is false:
			...
			say "     Your hand gently pumps along the sheath's length, graced with groans from the [gdragon] and the feel of [ghis] tool rising from [ghis] depths, until [ghis] cock is completely rigid and eager. Precum [if dorballmod > 0]sputtering all over[else]staining[end if] [ghis] torso, it begs for attention.";
		else:
			 say " [ghis] eager flesh. Your hand gently traces along its soft, precum-slicked length, causing the dragon to subtly writhe against the tease. The portal strains to squeeze [ghis] tool through its scaled lips, until [ghis] cock is completely rigid and eager. Precum [if dorballmod > 0]sputtering all over[else]staining[end if] [ghis] torso, it begs for attention.";
	else:
		now tempnum is 0;
		if dorinternal is false:
			say "     Impartially, your hand coils around [ghis] sheath, which is barely able to contain the [gdragon]'s dick, before [if dorroleint is 4]assertively[else]gently[end if] pumping along its length, each successive motion exposing [ghis] tool further, inch by inch. Moans escaping the [gdragon]'s lips, you needn't do this for long until its completely rigid and eager before you, dripping with need.";
		else:
			say "     Your digits firmly caress along the length of [ghis] taut, scaled lips, which seems barely able to contain the [gdragon]'s dick. Length growing more and more out from [ghis] bulging hole, your hand moves to [if dorroleint is 4]firmly[else]gently[end if] pump along its length, each successive motion pulling inch after inch free from its ill-fitting home. Moans escaping the [gdragon]'s maw, you needn't do this for long until its completely rigid and eager before you, dripping[if dorballmod > 0] excessively[end if] with need.";
	...
	if dorroleint is 4:
		...
		if player consents:
			...
			say "     Very well, shall you [if tempnum > 1]shift your attention to [ghis] dick and [else if dorrp_tv1 is 2][else if dorrp_tv1 is 0]move in and [end if]finish this by sucking [ghim] off? Else [ghe][']ll waste [ghis] load against [ghim]self.";
			if player consents:
				say "     Quickly, you [if tempnum > 1]move your way up to engulf [ghis] cockhead within your lips[else if dorrp_tv1 is 2]plunge [ghis] cock as deep down your throat as you're comfortable with[else]move in to engulf [ghis] cockhead within your lips[end if][if scalevalue of player < 4], your small body barely able to contain it[end if], madly [if dorrp_tv2 is 1]milking[else]pumping[end if] the beast. This is more than enough to bring [ghim] over the edge, roaring out weakly as [ghis] dick[if dorrp_tv2 is 1] and anal ring[end if] throb against your touch, unloading gout after successive[if dorballmod > 0], overwhelming[end if] gout into your eager maw[if dornpbb > 0], its volume exacerbated from [ghis] prior neglect[end if]";
				if scalevalue of player < 4:
					say ". [if dorroleint is 4 and dorballmod is 0]Substantial though [ghis] load is, [ghe]'s trained enough restrained to not overwhelm you, allowing your diminutive form to down the thick, potent seed with little difficulty[else if dorballmod > 0]Given how diminutive your form is, [ghis] excessive release is simply too much for you to handle, the potent seed almost immediately sputtering from your hole[else]Given how diminutive your form is, you can barely keep up with all of it, [ghis] thick, potent seed quickly sputtering from your hole[end if].";
				else if dorballmod > 0:
					say ". Even as large as you are, it's a challenge to keep up with [ghis] excessive load[if dorroleint is 4], though the obedient [gdragon] manages to muster enough restraint to let you savour [ghis] thick, potent seed[else], the [gdragon]'s thick, potent seed soon sputtering from your lips[end if].";
				else:
				...
				if dorinternal is false:
					say "     [if tempnum is 2 and dorballmod > 0]Revelling in [ghis] massive orbs[else if tempnum is 2]Revelling in [ghis] substantive orbs[else if dorrp_tv1 is 2]Pulling out to caress your tongue along [ghis] oozing cock[else if dorrp_tv1 is 1]Delighting in [ghis] oozing cock[else]Continuing to watch from afar[end if]";
				else:
					say "     [if tempnum is 2]Revelling in [ghis] precum-soaked cockbase[else if dorrp_tv1 is 2]Pulling out to caress your tongue along [ghis] oozing cock[else if dorrp_tv1 is 1]Delighting in [ghis] oozing cock[else]Continuing to watch from afar[end if]";
				say ", it's not long before your continued [if dorrp_tv2 is 1]milking[else]pumping[end if] makes the beast roar out weakly, [ghis] dick[if dorrp_tv2 is 1] and anal ring[end if] throbbing against your hands, spending gout after successive[if dorballmod > 0], overwhelming[end if] gout against [ghis] face and torso[if dornpbb > 0], its volume exacerbated from [ghis] prior neglect[end if]";
				say ". Eventually, you pull free of the [gdragon], now a spent and panting mess.";
			say "     '[if dorroleint is 4][one of][dorgr]'s [gdragon] immensely appreciates this gift[or]This [gdragon] is gifted with such a generous [dorgr][or]I'm glad [dorgr] appreciates their [gdragon][at random][else][one of]Clearly [dorgr] was feeling a bit generous[or]No doubt [dorgr] likes what the see[or]I hope [dorgr] enjoyed attending to their [gdragon][at random][end if]!' Having very much enjoyed this little ritual, [ghe] eventually moves to clean both you and [ghim]self up, returning back to [ghis] place of rest[if dorroleint is 4] once you'll allow [ghim][end if]. Though [ghe] certainly got a kick out of it, there's no question that relieving [ghim] has left you a bit lust-addled...";
			now dorbb is true;
	else:
		say "shall you [if tempnum > 1]shift your attention to [ghis] dick and[else if dorrp_tv1 is 0]move in and[end if] finish this by sucking [ghim] off? Else [ghe]'ll waste [ghis] load against [ghim]self.";
		if player consents:
			say "     Quickly, you [if tempnum > 1]move your way up[else]move in[end if] to engulf [ghis] cockhead within your lips[if scalevalue of player < 4], your small body barely able to contain it[end if], madly [if dorrp_tv2 is 1]milking[else]pumping[end if] the beast. This is more than enough to bring [ghim] over the edge, roaring out weakly as [ghis] dick[if dorrp_tv2 is 1] and anal ring[end if] throb against your hands, unloading gout after successive[if dorballmod > 0], overwhelming[end if] gout into your eager maw[if dornpbb > 0], its volume exacerbated from [ghis] prior neglect[end if]";
			if scalevalue of player < 4:
				say ". [if dorroleint is 4 and dorballmod is 0]Substantial though [ghis] load is, [ghe]'s trained enough restrained to not overwhelm you, allowing your diminutive form to down the thick, potent seed with little difficulty[else if dorballmod > 0]Given how diminutive your form is, [ghis] excessive release is simply too much for you to handle, the potent seed almost immediately sputtering from your hole[else]Given how diminutive your form is, you can barely keep up with all of it, [ghis] thick, potent seed quickly sputtering from your hole[end if]";
			else if dorballmod > 0:
				say ". Even as large as you are, it's a challenge to keep up with [ghis] excessive load[if dorroleint is 4], though the obedient [gdragon] manages to muster enough restraint to let you savour [ghis] thick, potent seed[else], the [gdragon]'s thick, potent seed soon sputtering from your lips[end if].";
			...
			say "     [if tempnum is 2]Substantive balls churning against your touch[else if dorrp_tv1 is 1]Delighting in [ghis] oozing cock[else]Continuing to watch from a distance[end if], it's not long before your continued [if dorrp_tv2 is 1]milking[else]pumping[end if] makes the beast roar out weakly, [ghis] dick[if dorrp_tv2 is 1] and anal ring[end if] throbbing against your hands, spending gout after successive[if dorballmod > 0], overwhelming[end if] gout against [ghis] face and torso[if dornpbb > 0], its volume exacerbated from [ghis] prior neglect[end if]";
		say ". Eventually, you pull free of the [gdragon], now a spent and panting mess.";
...
to say dorneut_1: [BODY MODDED]
	...
	while trixieexit is 0:
		...
		if calcnumber is 1:
			...
			now trixieexit is 1;
		say "he [gdragon] [if tempnum is 1]rolls onto [ghis] back and invites you to climb on [ghim][else if scalevalue of player < 4]sets you down on the ground before moving over you[else]sets you down on the ground before climbing atop you[end if], such that you offer [ghim] unobstructed access to your exposed hindquarters. In turn, you're presented with [ghis] substantive, partially aroused assets [if tempnum is 1]before[else]above[end if] you, the beast eager to tease by prodding you with its [if dorballmod > 0]perpetually sputtering[else]already oozing[end if] tip.";
		if tempnum is 1:
			...
			if player consents:
				...
				say "     Slowly, you wrap your hand around the dripping rod, gradually pumping the impressive organ until it grows to full attention. Digits quickly [if dorballmod > 0]drowing in[else]made slick with[end if] [ghis] thick, [gmasculine] precum, your";
				now tempnum2 is 2;
		...
		if tempnum is 1:
			...
			if tempnum2 is 2:
				if player consents:
					say "     Lips meeting to embrace [ghis] gushing, flared head, it's more than enough to[if libido of player > 40] finally[end if] set the [gdragon] off, [ghis] roar in bliss muffled against your [bodytype of player] form as your maw is quickly [if dorballmod < 0]overwhelmed[else]flooded[end if] with wave after [if dorballmod > 0]excessive[else]successive[end if] wave of [ghis] [if dorballmod > 0]seemingly endless[else]substantial[end if] load, quickly [if scalevalue of player < 3 or dorballmod > 0]sputtering from your lips[else]making your dizzy with how much you're driven to swallow[end if][if libido of player > 40].[run paragraph on] Eventually things begin to die down and you're given a moment to catch your breath[end if].";
					...
					if player consents:
						...
						say "     You only have to stare down [ghis] barrel for a few more seconds before you [if libido of player > 40] finally[end if] set the [gdragon] off, [ghis] roar in bliss muffled against your [bodytype of player] as you're immediately[if dorballmod > 0] and utterly[end if] drenched with wave after [if dorballmod > 0]excessive[else]successive[end if] wave of [ghis] [if dorballmod > 0]seemingly endless[else]substantial[end if] load[if dorballmod > 0].[run paragraph on] You're quickly dazed by the sheer volume, practically bathing in [ghis] seed[end if][if libido of player > 40].[run paragraph on] Eventually, things begin to die down, and you're given a moment to catch your breath[end if].";
			else:
				if player consents:
					say "     Shall you aim away from you? Else you'll get painted by the stuff.";
					if player consents:
			...
			say "     Unable to hold back any longer the [gdragon] plunges [ghis] gushing rod down your hole as far as you can take, more than enough to[if libido of player > 40] finally[end if] set [ghim] off, , [ghis] roar in bliss muffled against your [bodytype of player] form as your maw is quickly [if dorballmod < 0]overwhelmed[else]flooded[end if] with wave after [if dorballmod > 0]excessive[else]successive[end if] wave of [ghis] [if dorballmod > 0]seemingly endless[else]substantial[end if] load, quickly [if scalevalue of player < 3 or dorballmod > 0]sputtering from your lips[else]making your dizzy with how much you're driven to swallow[end if][if libido of player > 40].[run paragraph on] Eventually things begin to die down and you're given a moment to catch your breath[end if].";
			if dorvorreg is 2, now dorvorpcf is true;
		...
		if dorvorreg is 2 and scalevalue of player < 4:
			...
			say "     '[one of]Oh[or]Hm[or]Yes[at random], [one of]I hope [dorgr] had plenty of fun[or]I hope [dorgr]'s [gdragon] didn't make too much of a mess[or][dorgr] is always fun to play with[at random]!' [if tempnum is 1]Helping you off[else]Climbing off of you[end if], Doran goes about checking you over and cleaning you up before allowing you to gather your things and go about your business once more, the [gdragon] no doubt needing a bit of time to clean up the rest of the mess.";
...
to dorvore: [Invocation of Doran Vore Protocol]
	...
	if a random chance of 1 in 2 succeeds: [Foot-first Variant]
		...
		say "     Intimidating teeth bordering [ghis] is wet, fleshy abyss from the outside world, the [gdragon][if dorroleint is 0]'s inclined to tease you, nipping [ghis] little companion before[else] takes great care to spare you from them as[end if] that same tongue pulls you head-first into [ghis] voracious maw, using a talon to help guide your way. Supple, eager surface caressing your face and [bodydesc of player] torso, the beast revels in subjecting you to this touch in partial darkness.";
	...
	say "     At the conclusion your descent into the Doran's slippery, fleshy prison,[if dorvornpcf is true] a cum-filled mess of your own make as[end if] [ghe] strokes [ghis]--[if scalevalue of player is 3]doubtlessly bugling[else]somewhat bulging[end if]--belly in satisfaction. The supple, yielding confines embrace you in your fetal position, [if dorvornpcf is true]a dizzyingly heady swill of semen and[else]slick with [ghis] heady[end if] saliva";
	if dorbb is true:
		now dorbb is false;
		...
		if dorvordur is 0:
			...
			say ". Amused by your little digression, [ghe] gets up and struts around the cave with you in tow, doubtless [if dorvornpcf is true]sloshing[else]jostling[end if] you about ever so slightly.";
			...
			If dorvordur is 1:
				...
				if dorvordis is 0:
					...
					say ".";
					say "[dorvorpdis]";
			else if dorvordur is 3:
				say ". Eventually, [ghe] chooses to go about [ghis] business while you still remain inside [ghim].";
				say "[dorvorlpa]";
				say "[dorvorlpb]";
	else if dorvordur is not 0:
		say ". Eventually, Doran climbs back to [ghis] feet, clearly feeling somewhat amused as [ghe] struts around the cave, [if dorvornpcf is true]sloshing[else]jostling[end if] you about ever so slightly.";
		say "     '[one of]Oh[or]Yes[or]Heh[at random], ";
		if dorroleint is 0:
			say "[if dorvornpa is true][one of]maybe next time my [dorgr] will perform better[or]at the very least my [dorgr] makes for a good meal, in spite of themselves[or]I have half a mind to keep my [dorgr] in there, for their failures[at random][else][one of]my slutty little [dorgr] is just as good a meal as they are at servicing their [gdragon][or]I have half a mind to keep my [dorgr] in there if it means I can keep them with me[or]my [dorgr] is just as good a toy as they are a snack[at random]";
		else:
...
to say dorvorpdis: [Doran Vore Protocolled Discharge]
	if dorvordis is 2 or (dorvordis is 3 and a random chance of 1 in 2 succeeds):
		...
		say "     Laying in a puddle of saliva[if dorvornpcf is true] and some spunk[end if], Doran[if dorroleint is 0] grins at you, looking you over to see if everything is in order before giving you a final nip of approval, leaving you a complete, dizzy[else]'s snout descends upon you, tongue gently caressing along your disoriented form as [ghe] takes the time to clean you of your embarrassing[end if] mess.";
		say "     '[one of]Heh[or]Ah[or]Hey[at random], [if dorroleint is 0][one of]my [dorgr] should probably be glad I chose to let them go[or]I think my [dorgr] looks appropriate like this, as a messy little slut[or]what a mess, I should put my [dorgr] back in after making all this[at random][else][one of]so that's where my [dorgr] was all along[or]I hope my [dorgr] had a nice trip[or]I'm certain my [dorgr] would like to go on another ride like that, eventually[at random][end if]!' Teasing you for a moment, [ghe]'s no doubt a bit worn out from the whole ordeal, and returns to [ghis] campfire to rest whilst you[if dorroleint is 0] clean yourself off and[end if] recover as well, before eventually going about your business.";
...
to say dorvorcdis: [Doran Vore Clean Discharge]
	...
	say "     '[one of]I see my [dorgr] has awoken[or]Rise and shine, my [dorgr][or]Ah, welcome back, my [dorgr][at random]!' Grinning, [ghe] gives you a playful [if dorroleint is 0]nip[else]lick[end if]. '[one of]That was fun[or]I enjoyed that[at random], [if dorroleint is 0][one of]I look forward to making a meal of them again, when I get the chance[or]I'm certain they're eager to be abused by their [dorgr] again, when they feel up to it[at random][else][one of]I'm honoured that my [dorgr] would let their [gdragon] make a meal of them[or]I hope my [dorgr] enjoyed that as much as their [gdragon] did[at random][end if]!' Nudging you off, [ghe] lets you go to attend your own matters[if dorroleint is 0], the first of which probably being to clean yourself off[end if].";
...
carry out dorreq:
	...
	if dorfirsttalk is false:
		...
		while trixieexit is 0:
			...
			say "(1) [link]Sexual Position[as]1[end link] - [bold type][if dorrolereg is 0]Dominant[else if dorrolereg is 2]Submissive[else]Neutral[end if][roman type][line break]";
			say "(2) [link]Your Regarded Gender[as]2[end link] - [bold type][if dorgenreg is true]Female[else]Male[end if][roman type][line break]";
			say "(3) [link]Doran's Regarded Gender[as]3[end link] - [bold type][if doransgender is true]Female[else]Male[end if][roman type][line break]";
			say "(4) [if dorroleint is not 0]Interaction Locked - Requires Max Domination[else][link]Sex Auto-fire[as]4[end link] - [end if][bold type][if dorautint is 1 and dorroleint is 0]Strict[else if dorautint is 2 and dorroleint is 0]Loose[else if dorroleint is 0]Off[end if][roman type][line break]";
			say "[line break]";
			...
			say "(6) [if dorcusprog < 2]Interaction Locked - Try Discussing with Doran[else][link]Roleplay Session[as]6[end link][end if][bold type][if dorrpis > 0] - Roleplay Queued[end if][roman type][line break]";
			say "(7) [link]Body Modification[as]7[end link][if dortitmod > 0 or dorballmod > 0] - [end if][if dortitmod > 0][bold type]+Breasts [roman type][end if][if dorballmod > 0 and dorinternal is false][bold type]+Balls[roman type][else if dorballmod > 0 and dorinternal is true][bold type]+Cum[roman type][end if][line break]";
			say "(8) [link]Vore Extension[as]8[end link] - [bold type][if dorvorreg is 0]Uninquired[else if dorvorreg is 2]Active[else]Inactive[end if][roman type] [italic type][if dorvorreg is 2 and scalevalue of player > 3]Player too large![else if dorvorreg is 2 and dorrolereg is 2]Doran is Submissive![end if][roman type][line break]";
			say "[line break]";
...
to say doropt_6: [NEW! Body Modification]
	...
	while trixieexit is 0:
		...
		if dorballmod > 0:
			say "(1) [link]REVERT [if dorinternal is false]Balls[else]Cum Production[end if][as]1[end link][line break]";
		else:
			say "(1) [link]Augment [if dorinternal is false]Balls[else]Cum Production[end if][as]1[end link][line break]";
		if dortitmod > 0:
			say "(2) [link]REVERT Breasts[as]2[end link] - [bold type][bracket][if dortitmod is 1]STANDARD[else]FULL[end if][close bracket][roman type][line break]";
		else:
		...
		if calcnumber is 1:
			if dorballmod > 0:
				if nullifying powder is not owned:
					...
					if player consents:
						...
						say "     [gche] has you help [ghim] apply the powder, [if dorinternal is true]dripping gradually subsiding[else]balls gradually receding[end if] to [ghis] original state. Once it's all taken care of, [ghe] wheels around to meet you.";
			...
			if tanuki coin is not owned:
			...
			else if lastinternalmod - turns <= 24 and lastinternalmod is not -1:
				say "     [gche] still appears to be sore after being restored. You should give [ghim] more time before having another go.";
			else:
				if lastinternalmod is -1:
					...
					say "     'Hmm... So [dorgr] prefers my nethers [if dorinternal is true]exposed[else]hidden[end if]? I don't see any issue with that... Are you sure you want this?'";
					now lastinternalmod is 555;
				else:
					say "     'Ah, yes. I can have [if dorinternal is true]exposed[else]hidden[end if] naughty bits with this coin, if that's what [dorgr] wants. Is it?'";
				if player consents:
					say "[dormod_3]";
				else:
					say "     'Very well, [dorgr]. Don't be afraid to ask me about it again if you change your mind.'";
...
to say dormod_1: [Modify Balls/Cum]
	...
	if dorinternal is true:
		...
		if dorrolereg is not 0:
			...
			say "     'Tsk, such a naughty [dorgr]...!' As if to 'punish' your actions, the [gdragon] gets up and forces you onto your back, [if scalevalue of player > 3]climbing on top of[else]moving over[end if] you before forcing [ghis] smooth and already precum-slick nethers against your face. '...Taste what they have done.'";
		...
		if dorrolereg is not 0:
			...
			say "     'Tsk, such a naughty [dorgr]...!' As if to punish your actions, the [gdragon] gets up and forces you onto your back, [if scalevalue of player > 3]climbing on top of[else]moving over[end if] you before forcing [ghis] already-growing ballsack against your face. '...Taste what they have done.'";
...
to say dormod_2: [Modify Tits]
	...
	if dorrolereg is not 0:
		...
		say "     'Oh, [if dortitmod > 1][dorgr], their [gdragon] is better-equipped to care for their little plaything[else]my plaything certainly gives me all sorts of fun new toys[end if]";
		if dortitmod > 1:
...
to say doropt_1: [Demeanour Option]
	if dordomtop < 2: [Explanation for non-submissive players who didn't get that intro in his first talk]
		...
		say "     'In a different mood? Would [if dorrolereg is 0]my [end if][dorgr] prefer to [if dorrolereg is 0]be on top again[else if dorrolereg is 2]be on bottom[else]alter our arrangement[end if]?'";
		let trixieexit be 0;
		while trixieexit is 0:
			say "[bold type]Sexual Position:[roman type][line break]";
			say "(1) [link]'[if dorrolereg is 0]Stay[else]Go[end if] on top.'[as]1[end link]";
			say "(2) [link]'[if dorrolereg is 2]Stay[else]Go[end if] on bottom.'[as]2[end link]";
			say "(3) [link]'Let's [if dorrolereg is 1]keep[else]make[end if] things random.'[as]3[end link]";
			say "[line break]";
			say "(0) [link]'Nevermind.'[as]0[end link][line break]";
			while 1 is 1:
				say "Choice? (0-3)>[run paragraph on]";
				...
				if calcnumber >= 0 and calcnumber <= 3:
					...
					say "'I'm sorry, could [dorgr] repeat that?'";
			if calcnumber is 1:
				say "     '[one of]Very well[or]No problem[or]Okay[or]Certainly[or]Of course[at random], my [dorgr] will [if dorrolereg is 0]be kept[else]now be put[end if] in their place.'";
				now dorrolereg is 0;
				now trixieexit is 1;
			else if calcnumber is 2:
				say "     '[one of]Very well[or]No problem[or]Okay[or]Certainly[or]Of course[at random], [dorgr]'s [gdragon] will [if dorrolereg is 2]keep[else]learn[end if] [ghis] place.'";
				now dorrolereg is 2;
				now trixieexit is 1;
			else if calcnumber is 3:
				if dorneutroll is 0, now dorneutroll is a random number between 1 and 2;
				say "     '[one of]Very well[or]No problem[or]Okay[or]Certainly[or]Of course[at random], [dorgr]'s [gdragon] will [if dorrolereg is 1]keep surprising[else]surprise[end if] them.'";
				now dorrolereg is 1;
				now trixieexit is 1;
			else:
...
to say dorreq_1: [Food requisition/NEW! Breastfeeding]
	if dorlastfed - turns >= 16:
		...
		say "     'My sincerest apologies, [dorgr], [if dortitmod > 1]I haven't recovered[else]I haven't come across anything[end if] yet! But please, do check again later.'";
...
to say doropt_3: [Doran Auto-Fire]
	if dorautr is false:
		...
		while trixieexit is 0:
			...
			say "(3) [link]Off[as]3[end link] [bold type][if dorautint is 0]Currently Inactive[else]Deactivate[end if][roman type].";
			say "[line break]";
			say "(0) [link]Nevermind[as]0[end link][line break]";
			while 1 is 1:
				say "Choice? (0-3)>[run paragraph on]";
...
to say doropt_4: [Doran Vore Extension]
	...
	if dorvorreg is not 0:
		...
		while trixieexit is 0:
			...
			say "(1) [link]State[as]1[end link] - [bold type][if dorvorreg is 2]Active[else]Inactive[end if][roman type][line break]";
			say "(2) [link]Duration[as]2[end link] - [bold type][if dorvordur is 0]Very Short[else if dorvordur is 1]Short[else if dorvordur is 2]Medium[else if Dorvordur is 3]Long[end if][roman type][line break]";
			say "(3) [link]Discharge[as]3[end link] - [bold type][if dorvordur is 0]None/Implicit [bracket]Locked via duration[close bracket][else if dorvordis is 0]None/Implicit[else if dorvordis is 1]Oral[else if dorvordis is 2]Anal[else if dorvordis is 3]Random[end if][roman type][line break]";
			say "[line break]";
...
to doranmodifyballs:
	...
	if player consents:
		...
		if dorrolereg is not 0:
			...
			say "     '...I imagine any additional consumption would have no more effect, or otherwise become exceedingly inconvenient. I certainly hope I'm much more pleasing to [dorgr]'s tastes!' Slowly sitting back down, it clearly takes [ghim] a while to adjust [if dorinternal is true][ghim]self[else][ghis] seating[end if] to best handle the alteration. Future engagements should be... Interesting...";
		else:
			say "     'Tsk, such a naughty [dorgr]...' As if to 'punish' your actions, the [gdragon] gets up and forces you onto your back, [if scalevalue of player > 3]climbing on top of[else]moving over[end if] you before forcing [ghis] ";
			if dorinternal is true:
			...
			say "     Before you have much chance to respond, [ghe] shifts and thrusts the slightly-exposed tip of [ghis] cock against your lips, forcing you to drink in [ghis] thick, powerful sexual fluids. Forced to 'Milk' the [gdragon] of the precum, there seems to be no end to the stuff! Eventually, [ghe] relents before you're completely stuffed by the fluid, but chooses to use [ghis] now-freed [if dorinternal is true]crotch[else]sheath[end if] to humiliate you further, rubbing and smearing the fountain of sticky liquid against your face and upper torso.";
...
to say dorcus_1: [Of One's Past]
	...
	while trixieexit is 0:
		...
		if calcnumber is 1:
			...
			now dorcustv_1 is 1;
			say "     'The answer to that question is fairly straightforward: We benefit from a simplistic relationship. You use me[if dorrolereg is 0]--On top though I may be--[else] [end if]and that brings me a joy. Anything else would get in the way of that.'";
			now calcnumber is -1;
			let dorcusexit be 0;
			while dorcusexit is 0:
...
to say dorrpsp:
	...
	else if dorrpis is 2 or dorrpis is 3:
...
to say dorrpovbs:
	...
	say " forced into your [if dorrp_tv3 is 2]depths[else]supple folds[end if], up to [ghis] knot, with a complete disregard for your well-being.";
...
to say dorrpovobss:
	...
	if dorrp_tv4 is 1 and (a random chance of 1 in 4 succeeds or ("Kinky" is listed in feats of player and a random chance of 1 in 3 succeeds)): [WS "Feeding"]
		...
		if dorcustv_1 is 1:
			...
			say "     Y";
		say "ou feel [ghis] tongue caress along your significantly cum[if dorrp_tv4 is 1] and piss[end if]-stained mess of a body. Though this tongue bath might bring you a moment of some reprieve, it's clear that the creature isn't swallowing the [if dorrp_tv4 is 1]noxious cocktail[else]seed[end if], and your soon met with [ghis] muzzle against your face, forcing your mouth open. Instinctively, [if tempnum is 0]you[else]you refuse [ghim], to which [ghe] lowly growls and pins [ghis] snout firmly against you until you can't help but[end if] oblige, the [gdragon]'s overpowering [if dorrp_tv4 is 1]fluids[else]cum[end if] flooding your maw. It's clear [ghe]'s attempting to feed you, but the smell and taste is so intense[if scalevalue of player < 4], and you're so small compared to [ghim][end if], that this continued, prolongued ritual eventually overwhelms you, causing you to black out.";
	if waiterhater is 0:
		wait for any key;
		say "[line break]";
...
to say dorrptks:
	...
	if dorrpis is 2:
		...
		if dorrp_tv1 is 0:
			...
			if player consents:
				say "     [if dorrp_tv2 is 1]Removing the bindings from [ghis] maw, y[else]Y[end if]ou pull [ghis] head forward to grind it against your [if dorrp_tv3 is 0]spent cock[smn][else]worn tool[end if]. [gche] is, of course, reluctant to attend such a demand, but after a bit of prodding (And additional threats of punishment), [ghe] eventually obliges. Large tongue enveloping your crotch, you bathe in [ghis] heated breath as [ghe]'s forced to deal with the mess you've made of yourself, cringing at the unfamiliar taste of your sexual fluids. Eventually, [ghe] leaves you positively spotless[if dorrp_tv2 is 1], which you reward by tieing [ghim] back up[end if]";
		say ".";
		say "     '[if dorrp_tv2 is 1][one of]Hfn, mnn[or]bfln, hff[or]Mnfn, nng[at random][else if dorrp_tv1 is 0][one of]P-please don't hurt me further[or]I-I promise I won't cause anymore trouble[or]I'll be good, just let me go[at random][else][one of]T-this changes nothing[or]I w-won't buckle so easily[or]I w-won't stand for your twisted abuse[at random][end if]...' [if dorrp_tv1 is 1]Barely threatening p[else if dorrp_tv2 is 1]Barely audible p[else]P[end if]rotests offered against you, there's no doubt that [ghe] won't be causing anybody any trouble. [gche]'s certainly learned [ghis] lesson, but [ghe]'s quite the intriguing prospect. Shall you claim [ghim] as yours? Else you'll leave the [gdragon] be.";
		if player consents:
			say "     Grinning, you grab the [gdragon] and [if scalevalue of player > 3]haul [ghim] off[else]pull [ghim] along[end if], [if dorrp_tv1 is 0]to which [ghe] meekly obliges[else]much to what paltry protests [ghe] might muster up against you[end if]. When you return to your place of rest, you go through the matter of properly training the [gdragon] as your new little slave, [if dorrp_tv1 is 0]a feat which is not all that difficult[else]stubborn though [ghe] might be[end if]. Eventually, [ghe] proves to be [if dorrp_tv1 is 0]an eager, slutty[else]a sufficient[end if] little 'mount' for you, and [ghe] [if dorrp_tv1 is 0]enjoys[else]learns to enjoy[end if] being paraded around as a trophy of past conquests...";
		else:
		...
		if cunts of player is 0 or cunt length of player < 5 or dorrp_tv3 is 1:
			now dorrp_tv3 is 2;
		say "     It very quickly becomes apparent that you bit off more than you could chew this time, and the [gdragon] eventually overpowers you, forcing you to the ground, where you're too exhausted to fight any further. As you slowly try to get to your feet, the [gdragon] [if dorrp_tv1 is 1]can't help but laugh at your pitiful state, nudging you about to tease and torment[else]firmly lashes you back to the ground with [ghis] tail, lowly growling at[end if] [ghis] failed assailant.";
		say "     '";
		if dorrp_tv1 is 1:
		...
		if dorrp_tv1 is 1:
			say ". Talon briefly testing [if dorrp_tv3 is not 2 and cunts of player > 1]one of your [cunt size desc of player] cunts[else if dorrp_tv3 is not 2]your [cunt size desc of player] cunt[else]supple anal ring[end if], [ghe] chuckles to [ghim]self as [ghe] enjoys watching you squirm and protest against [ghis] imminent abuse. Soon enough, however, [ghe] forces it into its depths more deliberately, the [gdragon] preparing";
		else:
			say ". Immediately, you feel one of [ghis] talons force its way into [if dorrp_tv3 is not 2 and cunts of player > 1]one of your [cunt size desc of player] cunts[else if dorrp_tv3 is not 2]your [cunt size desc of player] cunt[else]supple anal ring[end if], causing you to cry out. [gche] only growls in response, showing no restraint in subjecting you to this harsh treatment as the [gdragon] prepares";
		say " [ghis] new victim.";
		say "     '[if dorrp_tv1 is 1][one of]Ha[or]Hm[or]Heh[at random], [one of]it's as I've figured[or]I should've guessed[or]it seems obvious now[at random], [one of]you're just one little slut[or]you're much better suited as a mere toy[or]you just saw to make yourself my property[at random][else][one of]Hmpf[or]Tsk[at random], [one of]I suppose you'll suffice[or]this is the price for your trespass[or]you'll learn your place[at random][end if]!' Satisfied in [ghis] [if dorrp_tv1 is 1]brief[else]harsh[end if] foreplay, [ghe] frees your [if dorrp_tv3 is not 2]pussy[else]tailpipe[end if] of [ghis] scaled digit's influence, its slick departure echoing through the open air. You're given little time to recover before the beast [if scalevalue of player < 4]looms over[else]climbs on top of[end if] you, [if dorrp_tv1 is 1]irreverent[else]shameless[end if] prodding from [ghis] oozing dick. Much to you're embarrassment, you can't help but be aroused by [ghis][if dorrp_tv1 is 0] harsh[end if] abuse, [if cocks of player > 0]unattended cock[smn] grinding against the ground[else if dorrp_tv3 is not 2]portal aching to be stuffed[else]unattended cunt[sfn] left exposed in the open air[end if], which [if dorrp_tv1 is 1]compels [ghim] to grin in amusement[else]only compels [ghim] to snort in mind disdain[end if].";
		say "     The [gdragon] shows [if dorrp_tv1 is 1]little[else]no[end if] restrain in shoving [ghis] impressive tool into your hole, ";
		...
		if dorrp_tv2 is 0:
			...
			say "     '[if dorrp_tv1 is 1][one of]Oh, all this work has left me very hungry[or]I think I can make some better use of my guest[or]I'm certain my new friend would like to stay for dinner[at random][else][one of]Now to end this ordeal[or]Time to finish this[or]This will only end one way[at random][end if]...' Licking [ghis] lips, [ghe] lunges forward, jaws[if dorrp_tv1 is 0] firmly[end if] pulling your smaller form into the air, relinquishing you only to let gravity drop you into [ghis] slick, eager maw. Foot by foot, your body is engulfed within the [gdragon]'s supple confines, those same confines squeezing around you as [ghis] muscles plunge you deeper into [ghis] dark depths, [if dorrp_tv1 is 1]taking [ghis] time, clearly to torment you over[else]showing little restraint or care in subjecting you to[end if] your ultimate fate. You're eventually given the paltry reprieve of [ghis] more abiding stomach, loudly groaning with its new and struggling occupant.";
			if dorrp_tv2 is 1:
				say "     [if dorrp_tv1 is 1][gchis] muffled laughter the last thing you hear before[else]Regarding [ghis] meal with only a partially content snort,[end if] you eventually succumb to exhaustion, blacking out entirely as your quest ends with you as a mere meal for the beast you set out to defeat, and after being so thoroughly ravaged. No doubt others will try and fail as you have, concluding in a similar, dark fate...";
			else:
				say "     '[if dorrp_tv1 is 1][one of]I think I'll save you for later[or]Rest well, you'll be needing it[or]I think this is a fitting prison for my new consort[at random][else][one of]Consider yourself lucky, that I let you live, for now[or]I will abide no further protests from my new consort[or]It will be better for you if accept your new home[at random][end if].' [if dorrp_tv1 is 1]Laughing with perverse pleasure[else]Lowly growling in perverse satisfaction[end if], muffled from within these dark and damp, churning confines, [ghe] goes off to rest from the whole ordeal, [ghis] captive in tow. In the time that follows, you are occasionally relinquished from your confines, only to be ravaged by the [gdragon] time and time again, stowed toy for the twisted beast. Others come to try and challenge [ghim], failing as you do, and though they are abused all the same, you remain separated from the [gdragon]'s other victims, given 'special' treatment, and over time you eventually succumb to total obedience to [ghis] will...";
		if waiterhater is 0:
			wait for any key;
			say "[line break]";
