...
to say Doran_Beat:
	...
	if Player is not neuter:
		...
		[]
		if Player is female and Cunt Length of Player > 4:
			choose a blank row from table of fucking options;
...
to say DoranVictory_Vaginal: [cunt sex]
	...
	if scalevalue of Player < 4:
		...
		if Player consents:
			say "     You decide to consent to his aid, to which his taloned forepaws descend upon you, ever-so-gently lining up [if Cunt Count of Player > 1]one of your cunts[else]your cunt[end if] with his proudly jutting tool. Mindful of your regard, you're slowly forced upon rod, quickly contending with his flared head. [if Cunt Length of Player > 17]Your difficulty is only brief before you sink the head into your depths entirely, your subsequent ease much to the dragon's surprise[else]After much work and strained grunts, the two of you manage to sink the head into your depths, the rest taking somewhat less work, up to the knot[end if]. Done helping you, the beast lets you go, allowing you to continue on your own pace.";
		else:
			say "     You suggest against his aid, intent on taking care of this yourself as you line up [if Cunt Count of Player > 1]one of your cunts[else]your cunt[end if] with his proudly jutting tool. Taking a moment to breath, you slowly force yourself upon the rod, quickly contending with his flared head. [if Cunt Length of Player > 17]Your difficulty is only brief before you sink the head into your depths entirely, your subsequent ease much to the dragon's surprise[else]After much work and strained grunts, you manage to sink the head into your depths, the rest taking somewhat less work, up to the knot[end if].";
	else:
	...
	say "     'Oh, m-my victor is so [if Cunt Length of Player < 12]t-tight[else if Cunt Length of Player > 17]s-skilled[else]f-firm[end if].' His discourse is mixed with deep moans, elicited as you build a motion of riding the dragon's powerful dick, [if Cock Count of Player > 1]your own bouncing in the open air, staining the dragon's scales with their slick fluids[else if Cock Count of Player is 1]your own bouncing in the open air, staining the dragon's scales with its slick fluids[else]stuffed portal eagerly milking the beast for what he's worth[end if][if scalevalue of Player < 3], body visibly bulging to contain the organ[end if].";
...
to say DoranSexMenu: [Doran Sex Protocol]
	...
	if DoranRoleplayInSession > 0: [Is a Roleplay In Session?]
		...
		while Trixieexit is 0:
			...
			say "(4) [if Player is female][link]Ride [ghim][as]4[end link][else][italic type]Female-specific interaction[roman type][end if] [if Cunt Count of Player > 0 and Cunt Length of Player < 8][italic type]- Too small![roman type][end if][line break]";
...
to say DoranAutoSexProtocol:
	if DoranRole is 1 and a random chance of 1 in 2 succeeds:
		...
		say "[DoranDom_Anal]"; [anal sex scene]
	else if Player is female and Cunt Length of Player > 4 and a random chance of 1 in 3 succeeds:
		say "[DoranDom_Vaginal]"; [cunt sex scene]
...
to say DoranDom_Vaginal: [Cunt Sex] [BODY MODDED]
	...
	if scalevalue of Player < 4 and DoranRoleIntensity is 0 and a random chance of 1 in 2 succeeds:
		...
		if Libido of Player > 35:
			say "     The powerful dick's assault is more than enough to make you almost immediately cry out, ";
			if Player is male:
				say "cock[if Cock Count of Player > 1]s'[else]'s[end if] sputtering forcibly erratic against the [if Cunt Length of Player > 13]somewhat[else]immensely[end if] straining intrusion, staining the [gdragon]'s thigh and the ground with your [Cum Load Size of Player] load";
			else:
				say "puss[if Cunt Count of Player > 1]ies'[else]y's[end if] gushing forcibly strained against the[if Cunt Length of Player < 14] immense[end if] intrusion, staining the [gdragon]'s thigh and the ground with your sexual fluids";
			...
			if Player is male:
				...
				say "pussy[if Cunt Count of Player > 1]ies'[else]y's[end if] gushing forcibly strained against the[if Cunt Length of Player < 14] immense[end if] intrusion, staining the [gdragon]'s thigh and the ground with your sexual fluids.";
		say "     Shifting back onto [ghis] feet, [ghe] offers slight care in pulling you free of [ghis] dick, knot audibly popping from your stretched hole before planting you on the ground once more, soon followed by [if DoranBallModStatus > 0]an excessive[else]a[end if] flood of the beast's thick, heady ejaculate thereafter. Feeling particularly devious, [ghe] revels in your pitiful state, aiming [ghis] [if DoranBallModStatus > 0]perpetually[else]still[end if] cum-oozing dick at your head and upper torso to [if DoranBallModStatus > 0]drench[else]sully[end if] them as well.";
	else:
		...
		if Libido of Player > 35:
			say "     The powerful dick's assault is more than enough to make you almost immediately cry out, ";
			if Player is male:
				say "cock[if Cock Count of Player > 1]s'[else]'s[end if] sputtering forcibly erratic against the [if Cunt Length of Player > 13]somewhat[else]immensely[end if] straining intrusion, staining the ground with your seed [Cum Load Size of Player] load";
			else:
...
to say DoranDom_CockHandjob: [Cock-Focused Handjob] [BODY MODDED]
	...
	if DoranRoleIntensity is 0 and ((DoranAutofired is true and a random chance of 1 in 2 succeeds) or (DoranAutofired is false and a random chance of 1 in 5 succeeds)):
		...
		if Cock Count of Player is 1 and (Cunt Count of Player > 0 or anallevel > 1) and titfuck is 0:
			...
			say ".";
		say "     Slowly pulling [ghis] talons free of you as things die down, you're given some opportunity to survey [if DoranRoleIntensity is 0]the devastation[else]what[end if] [ghe] has wrought, yourself a worn out and[if ball size of Player > 24] massively[end if] cum-stained mess";
		if DoranRoleIntensity is 0:
...
to say DoranDom_CuntHandjob: [Cunt-Focused Handjob] [BODY MODDED]
	...
	if Player is male:
		say "[if Cunt Length of Player < 7]slight, strained grunts escaping [ghis] lips[else]meeting only brief resistance[end if] as [ghe] forces [ghis] digit past the supple folds of your flesh, pumping your dick all the while in [ghis] endeavor to milk them, fueled by a twisted determination.";
	else:
		say "[if Cunt Length of Player < 7]slight, strained grunts escaping [ghis] lips[else]meeting only brief resistance[end if] as [ghe] forces [ghis] digit[if Cunt Count of Player > 1 or anallevel > 1]s[end if] past the supple folds of your flesh";
		if Cunt Count of Player is 1 and anallevel > 1:
			say ", [ghis] endeavors to penetrate your ass [if Cunt Length of Player < 7 and scalevalue of Player < 4 and player is not twistcapped]equally[else if scalevalue of Player < 4 and player is not twistcapped]noticeably more[else if Cunt Length of Player > 5]not nearly as[else]just as[end if] difficult";
	say ".";
	if DoranRoleIntensity is 0 and ((DoranAutofired is true and a random chance of 1 in 2 succeeds) or (DoranAutofired is false and a random chance of 1 in 5 succeeds)):
		...
		if Player is male:
			say " and[if ball size of Player > 24] massively[end if] cum-stained";
		say " mess";
		if DoranRoleIntensity is 0:
			...
			say "     '[if ball size of Player > 24 and Cock Count of Player > 0][one of]Such a messy [DoranPlayerRegard][or]My [DoranPlayerRegard], look at all this mess[at random][else][one of]Tsk[or]Hmph[at random][end if], [one of]start licking[or]be good and clean this up[or]I won't let them leave until they take care of this[at random]!' Intent on having you attend to this matter, you're forced to lick clean the beast's talons of your own [if Player is male]thick, virile cum[else]honey[end if], tongue caressing along the curvature of [ghis] scaled digits";
			if Player is male:
				if ball size of Player < 7 or scalevalue of Player > 3:
			...
			say ".";
			say "     Giving you a moment to rest, [ghis] maw descends upon you a second time, meeting you with [ghis] thick tongue's caress as [ghe] endeavors to clean you up in silence. [if ball size of Player > 24]This ritual persists for a fair amount of time, considering the substantial mess you've made[else][gchis] work need only take a moment before [ghe]'s complete[end if], leaving you with only a thin veneer of saliva.";
		if DoranVoreStatus is 2 and scalevalue of Player < 4:
			if Player is male and ball size of Player > 20 and DoranRoleIntensity is 0, now DoranPlayerCumFilled is true;
...
to say DoranSub_AnalFuck: [Anal Catching] [BODY MODDED]
	...
	if Player consents:
		say "     Before you are completely overtaken, you wrench your [cock size desc of Player] cock from the beast, quickly jacking it[if Cock Count of Player > 2] and one of its brothers[else if Cock Count of Player is 2] and its brother[end if] off before you cry out, coating [ghis] ass[if scalevalue of Player > 3] and back[end if] with [if ball size of Player > 24]voluminous [end if]gouts of your thick, virile seed[if ball size of Player > 24]; so substantial your payload that you render the beast a shameful, coated mess of cum[end if]. It takes you some time to recover, eventually [if scalevalue of Player < 4]climbing[else]pulling yourself[end if] off. The [gdragon] lays there for a bit, panting [ghim]self in the wake of being ravaged. [gche] eventually does crawl back to [ghis] feet, looking satisfied";
		if DoranRoleIntensity is 4:
		...
		if Player consents:
			...
			if DoranRoleIntensity is 4:
				...
				if Player consents:
					...
					say "[if ball size of Player > 24]. [gche] doesn't seem even remotely phased by the volume you surrender to [ghim], reveling in the sheer excess[end if].";
...
to say DoranSub_CatchCunt: [Cunt Catching] [BODY MODDED]
	...
	if scalevalue of Player < 4:
		...
		if Player consents:
			say "     Easing back, your companion softly embraces your smaller form within [ghis] taloned forehands, ever-so-gently easing you down on [ghis] throbbing organ. Ever mindful of you, [ghe] sink [ghis] flesh into you, inch by inch, slowing as [ghe] contends its flared head. [if Cunt Length of Player > 11]Your difficulty is only brief before you envelop it within your depths entirely, your subsequent ease much to the [gdragon]'s enjoyment[else]After much work and strained grunts, the two of you manage to envelop the head within your depths[end if], the rest taking somewhat less work, up to the knot. Once you're ready to continue on your own the beast lets you go, allowing you to set your own pace from here on.";
		else:
			say "     You cheekily reject [ghis] offer, dead set on taking care of this on your own terms as you begin to ease yourself down on [ghis] throbbing organ, slowing once you start to contend with its flared head. [if Cunt Length of Player > 11]Your difficulty is only brief before you envelop the head within your depths entirely, your subsequent ease much to the [gdragon]'s enjoyment[else]After some work and strained grunts, you manage to envelop the head within your depths[end if], the rest taking somewhat less work, up to the knot.";
	else:
		say "     Now ready, you move over the [gdragon]'s scaled undercarriage, carefully aiming the dripping rod for [if Player is female]one of your exposed cunts[else]your exposed cunt[end if]. Briefly teasing the tip against your slick lips, much to the groans of the [gdragon]'s approval, you slowly ease yourself down on [ghis] throbbing organ, flared head slowly sinking past your soft, yielding folds of flesh. [if Cunt Length of Player < 12]After much work and strained grunts, you manage to envelop the head within your depths, the rest taking somewhat less work, up to the knot[else if Cunt Length of Player > 17]Struck by a rather impish urge, you abruptly plunge down on [ghis] tool in its entirety, so pliable that you envelop [ghis] knot with almost no difficulty. This shock is enough to cause [ghim] to briefly howl out, a gout of pre spurting from [ghis] suddenly engulfed tool[else]Past [ghis] head your descent is a fair bit easier, up to [ghis] knot, which is large enough to make you avoid dealing with just yet[end if].";
	say "     '[one of]Oh[or]Nrgh[or]Nnf[at random], [one of][DoranPlayerRegard] is so [if Cunt Length of Player < 12]t-tight[else if Cunt Length of Player < 17]s-skilled[else]f-firm[end if][or]I-I can't hold back, [DoranPlayerRegard][or][DoranPlayerRegard][at random]...' [gchis] mutterings are mixed with deep moans, made all the more prevalent as you build a motion of riding the [gdragon]'s powerful dick, [if Cock Count of Player > 1]your own bouncing against the open air, staining the [gdragon]'s scales with their slick fluids[else if Cock Count of Player is 1]your own bouncing against the open air, staining the [gdragon]'s scales with its slick fluids[else if Cunt Count of Player > 2]unstuffed cunts aching against the open air, staining the [gdragon]'s scales with their honey[else if Cunt Count of Player is 2]unstuffed cunt aching against the open air, staining the [gdragon]'s scales with its honey[else]stuffed portal eagerly milking the beast for what [ghe]'s worth[end if][if scalevalue of Player < 3 or Cunt Length of Player < 12] while stretched to the limits of your ability to contain the organ[end if][if DoranBallModStatus > 0]. Each descent causes yet another gout of precum to shoot from [ghim], causing you to flinch. [gchis] supply almost seems endless![else].[end if]";
	...
	while Trixieexit is 0:
		...
		if tempnum is 1:
			...
			if Player is male and (((player is female or anallevel is 3) and a random chance of 1 in 2 succeeds) or (Cunt Count of Player is 0 and anallevel < 3)):
				...
				now tempnum3 is 1;
				if ball size of Player > 24 and DoranVoreStatus is 2, now DoranCumFilled is true;
...
to say DoranRP_VirileBeast: [Doran RP: The Virile Beast Query]
	...
	if DoranRole > 0 and anallevel is 1 and ((player is female and Cunt Length of Player < 5) or Cunt Count of Player is 0):
		say ", additionally,";
	else if anallevel is 1 and ((player is female and Cunt Length of Player < 5) or Cunt Count of Player is 0):
		say "I must caution [DoranPlayerRegard],";
	if anallevel is 1 and player is female and Cunt Length of Player < 5:
	...
	if Player consents:
		...
		if Player is female and Cunt Length of Player > 6 and anallevel > 1 and (scalevalue of Player > 1 or player is twistcapped):
	...
	if Player consents:
		...
		if Player consents:
			...
			if DoranVoreStatus > 1 and scalevalue of Player < 4:
				...
				now DoranRP_var2 is 0;
			if Player is female and Cunt Length of Player > 4 and anallevel > 1:
...
to say DoranRP_VirileBeastScene:
	if Player is not female or Cunt Length of Player < 5 or DoranRP_var3 is 1:
...
to say DoranRP_KnightScene:
	...
	if DoranRoleplayInSession is 2:
		...
		if DoranRP_var3 is 0:
			...
			if Player consents:
				say "     Right at the very height of ecstasy, you free your throbbing dick from the [gdragon]'s rump, grinding against [ghis] inviting curvature to push you past the last inch. Loudly groaning, you paint the [gdragon]'s ass[if ball size of Player > 12] and back[end if] with your [Cum Load Size of Player], virile fluids,[if ball size of Player > 24] positively drenching the reptile with your seed and[end if] leaving [ghim] an embarrassed mess.";
			else:
				say "     With one final effort, you hilt your throbbing dick into the [gdragon]'s abyssal depths,[if Player is knotted] tying with [ghim] and[end if] firing surge after subsequent surge of your [Cum Load Size of Player], virile fluids[if ball size of Player > 24] visibly bloating the reptile with your seed[end if][if Cock Count of Player > 2]its brothers staining the creature's crotch[else if Cock Count of Player is 2]its brother staining the creature's crotch[end if]";
