...
to say DoranDom_Anal: [Anal Sex] [BODY MODDED]
	...
	if DoranRoleIntensity is 0:
		...
		if DoranBallModStatus > 0:
			say ", even though [ghe]'s clearly relegating you to an impossible task. Tongue caressing the softened organ, your efforts pained by you already-stuffed state, it's made worse by the fact that any effort to clean [ghim] just makes a bigger mess.";
		...
		if player consents:
			...
			if DoranRoleIntensity is 4:
				...
				if player consents:
					...
					say "[if cock width of player > 24].[run paragraph on] [gche] doesn't seem even remotely phased by the volume you surrender to [ghim], reveling in the sheer excess[end if].";
