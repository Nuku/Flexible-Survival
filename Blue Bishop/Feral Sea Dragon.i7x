...
to say fsdm attack:
	...
	if vorelevel is not 1 and scalevalue of Player < 4 and (A random chance of 1 in 5 succeeds or (player is kinky and a random chance of 1 in 4 succeeds) or (vorelevel is 3 and a random chance of 1 in 3 succeeds)):
		...
		if bodyname of Player is "Feral Sea Dragon" or bodyname of Player is "Feral Sea Dragoness":
			if Player is female and Cunt Length of Player > 11:
			...
			if fsdfeedmemory >= 2 and lastfsdfeeding - turns >= 8 and ((player is kinky and a random chance of 1 in 4 succeeds) or (vorelevel is 1 and a random chance of 1 in 4 succeeds) or a random chance of 1 in 3 succeeds) and "Touched by Madness" is listed in feats of Player:
				...
				if ((player is kinky and a random chance of 1 in 3 succeeds) or a random chance of 1 in 4 succeeds) and FSDOPEN < 2:
					...
					infect;
					now FSDOPEN is 0;
			else if Player is female and Cunt Length of Player > 11:
				...
				if Player is male:
					say "     Your audible moans amusing to the dragon, it's not long before your [cock size desc of Player], [bodytype of Player] dick[if Cock Count of Player > 1]s explode with their[else] explodes with its[end if] inevitable release[if ball size of Player > 12]. You fire off gout after excessive gout of seed against yourself and the beast, so much so that it appears to mildly perturbed, clearly not that interested in your fluids at all, let alone in this much excess[else]. You convulse with every successive throb of your release, spilling your seed against both yourself and the dragon, not that he seems to particularly care[end if]. He continues to milk you dry of your fluids before both his tongue and his body relinquishes its hold on you.";
				else:
...
to fsdmvicmenu:
	...
	while Trixieexit is 0:
		...
		say "(4) [if Player is female][link]Ride his dick[as]4[end link][else][italic type]Female-specific interaction[roman type][end if] [if Player is female and Cunt Length of Player < 12][italic type]- Too small![roman type][end if][line break]";
...
to say FSDM_1: [Anal Pitching]
	...
	if Player consents:
		say "     Just at the cusp of ecstasy, you wrench your tool[if tempnum2 is 1]s[end if] free of the sea dragon's tight hole, grinding against his hide until you're set off, firing your [Cum Load Size of Player] against his underside, [if ball size of Player > 19]utterly drenching it[else]making an embarrassing mess of it[end if]. It doesn't seem to be enough to set him off, his unattended dick dribbling against the open air. Should you help him out?";
		if Player consents:
		...
		say "     In one final thrust, you [if Player is knotted]tie[else]hilt[end if] your tool[if tempnum2 is 1]s[end if] within the sea dragon's tight hole, crying out as you're set off, unleashing your [Cum Load Size of Player] load into his supple bowels[if ball size of Player > 19] and quickly beginning to sputter from him[end if]. This is more than enough to set the beast off, roaring out weakly as his unattended dick wastes its virile load across the sand and his underside, further making an embarrassing mess of himself.";
...
to say FSDM_2: [Oral Receiving Cock]
	...
	if Player consents:
		...
		say "     It's not long before you finally cry out in bliss, firing your [Cum Load Size of Player] load into the creature's[if boundmod is 1] obliging[end if] maw. [if boundmod is 1]Whining softly, he's gulps audibly in an eager desire[else]Rumbling in detest, he can only oblige being forced[end if] to swallow your cum[if ball size of Player > 20], the sheer volume of which quickly causing it to ooze from his lips[end if].";
...
to say FSDM_4: [Cunt Catching]
	...
	if Player consents:
		say "     Slowly forcing the dragon's pointed tip through the folds of[if Cunt Count of Player > 1] one of[end if] your cunt's lips, you're [if Cunt Length of Player < 14]fairly[else if Cunt Length of Player < 18]briefly[else]barely[end if] strained as you engulf the beast's flared head. Descent slicked by [if tempnum is 1]your prior attendance[else]his sexual fluids[end if], the sea dragon groans and writhes as each successive inch in engulfed. Once you manage to take the head, the rest of his shaft seems comparatively easy to tackle, your speed slightly rising as you begin to ride the creature proper.";
	...
	if Player consents:
		...
		if Player is male:
			say "     Your own bliss quickly following suit, your cock[smn] unleash your [Cum Load Size of Player] load on the beast, [if tempnum2 is 1 and ball size of Player > 19]further embarrassing him as he's now utterly drenched in cum[else if tempnum2 is 1]further embarrassing him as your cum also stains him[else if ball size of Player > 19]utterly drenching him in your cum[else]staining him with your cum[end if]";
		else:
		...
		if Player is male:
			say ". Your own cock[smn] unleashing your [Cum Load Size of Player] load, you [if ball size of Player > 19]utterly drench the beast[else]stain the beast[end if] with your cum, who can only rumble and [if boundmod is 1]moan in approval, lost in a haze of[else]groan in disdain, muffled under the throes of his[end if] tainted ecstasy.";
		else:
