	...
	if vorelevel is not 1 and scalevalue of Player < 4 and (A random chance of 1 in 5 succeeds or (Player is kinky and a random chance of 1 in 4 succeeds) or (vorelevel is 3 and a random chance of 1 in 3 succeeds)):
		...
		if BodyName of Player is "Feral Sea Dragon" or BodyName of Player is "Feral Sea Dragoness":
			...
			if fsdfeedmemory >= 2 and lastfsdfeeding - turns >= 8 and ((Player is kinky and a random chance of 1 in 4 succeeds) or (vorelevel is 1 and a random chance of 1 in 4 succeeds) or a random chance of 1 in 3 succeeds) and "Touched by Madness" is listed in feats of Player:
				...
				say ". You feel his weight press more and more against you, but all you can do take immense pleasure in the feel of his overwhelming warmth both on top of you and inside you, and you feel like you're about to pass out before you're jolted to attention when his cock immediately explodes with his release.";
...
To say fsdm loss:
	...
	if a random chance of fsdsub in 9 succeeds:
		now boundmod is 1;
		say "     Before you can land the final blow, the dragon suddenly backs away and concedes to you. His demeanor rather peculiar, you can surmise that you've defeated this one before, and he's succumbed to newfound, submissive inclinations. ";
...
to say FSDM_2: [Oral Receiving Cock]
	...
	if Player consents:
		...
		if Player consents:
			...
			if boundmod is 1:
				say "[if scalevalue of Player < 4]its size no doubt somewhat overwhelming, though he shows some restraint as[else]you're able to keep up with the eager beast with little issue as[end if]";
			else:
				say "[if scalevalue of Player < 4]its size easily overwhelms you, the beast no doubt reveling in his superior stance within this ritual as[else]you're able to keep up with the impressive beast with little issue as[end if]";
	...
	if boundmod is 1:
		...
		say "     [if tempnum is 1]Having warmed up to you slightly[else if tempnum is 2]Having warmed up to you by a bit now[else]With persistent chastising, along with your cock's persistent prodding against his snout[end if]";
...
to say FSDM_3: [Oral Receiving Cunt]
	...
	if Player consents:
		...
		if Player consents:
			...
			say "     His own slick, writhing appendage invading your maw, ";
			if boundmod is 1:
				say "[if scalevalue of Player < 4]its size no doubt somewhat overwhelming, though he shows some restraint as[else]you're able to keep up the eager beast with little issue as[end if]";
			else:
				say "[if scalevalue of Player < 4]its size easily overwhelms you, the beast no doubt reveling in his superior stance within this ritual as[else]you're able to keep up with the impressive beast with little issue as[end if]";
			say " your taste is overwhelmed by his overpowering saliva. Eventually, [if boundmod is 1 or scalevalue of Player > 3]you find yourself satisfied with your bit of fun and you[else]the foreplay becomes too much for you and you have to[end if] get back on track.";
			WaitLineBreak;
	...
	if Player is male:
		...
		say "Lost in a haze of your need, you cling tightly to the beast, driving him to attend you ever-harder.";
...
to say FSDM_4: [Cunt Catching]
	if boundmod is 1:
		...
		say "     Given the sheer weight of the sea dragon, it takes a fair bit of work just to get him to roll onto his back[if scalevalue of Player > 3], even as large as you are[end if]. Stubborn beast ill-inclined to so easily oblige his better, there's little he can actually do to contest";
	...
	if Player consents:
		...
		if Player is male:
			...
			say "     Your own bliss quickly following suit, your oozing cunt[sfn] aching against the beast, [if tempnum2 is 1]further embarrassing him as he's stained with your juices[else]staining him with your juices[end if]";
		...
		LineBreak;
		now tempnum2 is 0;
		say "     Unable to hold back for any longer, the sea dragon finally roars out in bliss, body wracked with wave after successive wave of thick seed from his sputtering dick, [if scalevalue of Player < 4]blatantly bloating to abide it, until it begins to sputter from your stuffed hole[else][bodytype of Player] body visibly bloating to abide it[end if]. Your ";
		if Player is male:
			say "own cock[smn] unleashing your [Cum Load Size of Player] load, you [if Ball Size of Player > 4]utterly drench the beast[else]stain the beast[end if] with your cum, who can only rumble and [if boundmod is 1]moan in approval, lost in a haze of[else]groan in disdain, muffled under the throes of his[end if] tainted ecstasy.";
		else:
			say "stuffed cunt throbbing against the powerful intrusion, you stain the beast with your juices, who can only rumble and [if boundmod is 1]moan in approval, lost in a haze of[else]groan in disdain, muffled under the throes of his[end if] tainted ecstasy.";
...
to say FSDM_6: [Oral Giving Cock]
	...
	if boundmod is 1 and Player is not neuter:
		...
		now tempnum2 is 0;
	say ". The dragon's breath becomes increasingly shallow, [if tempnum2 is 1]your crotch awash in his hot breath[else]body trembling[end if] with each powerful exhale, [if tempnum is 1]his tool now slick with a mix of saliva and sex[else if tempnum is 2]his stuffed hole aching against you[else]his tool's sputtering becoming more substantial with each passing second[end if], indicating his imminent bliss. Shall you ";
	if tempnum is 1:
		say "engulf the head within your mouth at the last second?";
		if Player consents:
			LineBreak;
			now boundsegment is 1;
		...
		say "aim his release with the specific intent of shaming him?";
		if Player consents:
			now boundsegment is 2;
		else:
			now boundsegment is 0;
	...
	if Player has a body of "Feral Sea Dragon" or Player has a body of "Feral Sea Dragoness":
		...
		if humanity of Player < 10:
			...
			if fsdbias is 4 or fsdbias is 3:
				...
				if fsddom > 5:
					...
					if Player is male:
						...
						if ublevel is not 1:
							say "the two of you soon learn a way around when a bout of fun with a trespasser leads to them getting stuffed up [if Player is not female]your mate's cunt[else if Cunt Count of Player > 1]one of your cunts[else]your cunt[end if], only to be born again as one of your kin. As time would pass, your children would spread to the far corners of the world, until all who fare the sea do so with an ever-present fear of your kin...";
						else:
				...
				if fsdsub > 5:
					...
					if Player is male:
						say "[if FemaleList is not banned and FemaleList is not warded and Player is not sterile]bestial need requiring you to sate a desire for offspring by collecting some females just for you[else]offering you barely a moment of free time[end if]";
					else:
					...
					if Player is male:
						...
						say "you subjugate more of them under your will, and though your bestial need[if Player is female and Player is not sterile] for offspring[end if] is plenty sated by your subjects";
			...
			if BodyName of Player is "Feral Sea Dragon":
				say "     Eventually, you find that you manage to strike up a job as lifeguard for a beach resort";
			else:
				say "     Eventually, you find that you manage to strike up a job as mascot for a beach resort";
			say ", which is especially helpful since you only really ever find yourself comfortable at sea. Moreover, the beachgoers find themselves increasingly fond of your presence, giving you quite a reputation.";
