...
to say ebwhelpvic:
	if ebneuterswitch is true:
		if anallevel is not 0 and (cunts of player is 0 or anallevel is 3) and (a random chance of 1 in 2 succeeds or (anallevel is 3 and a random chance of 1 in 3 succeeds)):
			now tempnum is 2;
		otherwise if cunts of player > 0 and a random chance of 3 in 4 succeeds:
			now tempnum is 1;
		otherwise:
			now tempnum is 0;
	otherwise:
		if (cunts of player is 0 and anallevel is not 1 and player is male_vacant) and ((anallevel is 2 and ("Kinky" is listed in feats of player or "Mpreg" is listed in feats of player)) or anallevel is 3):
			now tempnum is 2;
		otherwise if cunts of player > 0 and player is fem_vacant:
			now tempnum is 1;
		otherwise:
			now tempnum is 0;
	if hp of player > 0:
	...
	say " [if birthedwhelp is true]your recent offspring[otherwise]the little whelp[end if], the creature chittering and chirping in a feeble expression of dominance at its[if hp of player > 0] willing[end if] victim";
	if tempnum is 2:
		say ". Completely without restraint the lizard makes a beeline for your ass. Nipping and clawing at your hips, it's little legs flit about in a weak attempt at removing any obstruction before properly positioning itself. [if hp of player > 0]Obliging the small dragon, you eventually help him get him properly positioned[otherwise]You weakly try to push the small dragon off of you, but this only invites him to nip and claw at you into submission, eventually getting himself into position[end if].";
		say "     With a tainted urgency, you immediately feel yourself [if scalevalue of player < 4]impaled[otherwise]stabbed[end if] by the tiny monster's instantly erect dick down your barely ready asshole, burying it's long, tapered length down your hole until he's slapping his smooth, scaled crotch between your thighs. You feel the thing, like a searing hot poker, worm its way around your innards, sputtering its molten-hot precum[if scalevalue of player < 4] deep[end if] inside you. Absolutely dead set on weakly pounding your ass into oblivion, you have to endure the sounds of its loud, wanton squealing filling air, hardly drowning out his increasingly wet motions.";
		say "     [if hp of player > 0]Barely given time to become aroused from[otherwise]Gradually aroused by[end if] this[if birthedwhelp is true], twisted[end if] embarrassing display, [if cocks of player > 0]your cock[smn] ache[smv] against the open air, driving you to stroke yourself through all this[otherwise if cunts of player > 0]your cunt[sfn] ache[sfv] against the open air, driving you to fondle yourself through all this[otherwise]much in spite of your lack of outlet[end if]. It doesn't take long for the whelp to finally howl out, clinging to you tightly as you're pumped with gout after successive gout of his scalding load. His release seems so excessive, you're soon made dizzy with the sheer volume[if scalevalue of player < 3], body visibly bloating to accommodate all of it[end if]";
		if libido of player > 30 and (cocks of player > 0  or cunts of player > 0): 
			say ". The whole ordeal is more than enough to set you off, staining the ground with your [if cocks of player > 0][cum load size of player] load[otherwise] juices[end if]";
		say ".";
...
to ebwhelphijackroutine:
	...
	if hijackgestation < 0:
		...
		otherwise:
			now tempnum is 2;
		say "     Unable to take it for any longer, you finally double over and cry out. You can feel the tainted offspring writhing and clawing itself through you [if tempnum is 2]bowels[otherwise]birth canal[end if] in desperate need for escape, wracking you with waves of pain for each successive inch until it finally breaks away, air suddenly filling with the low squealing of an Ebonflame Whelp.";
		if tempnum is 2:
			say "     [if scalevalue of player > 3 or player is twistcapped]You're thankfully stretchy enough for the monster to pull itself free of your[otherwise]Tears strained from your eyes, the monster stretches you to your limits to pull itself free of your tight[end if] ring";
		otherwise:
			say "     [if cunt length of player > 14]You're thankfully stretchy enough for the monster to pull itself free of your[otherwise]Tears strained from your eyes, the monster stretches you to your limits to pull itself free of your tight[end if] cunt";
