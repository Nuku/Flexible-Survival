...
to say flotlook:
	...
	otherwise:
		...
		say "     This odd thing raises a lot of questions. Why is it just floating there? Shouldn't it be the same as all the other infected here? Without having to touch it, you can see that the thing clearly has a hole for 'use', so it's clearly the product of this blight's influence. If you're so daring as to 'play' with it, you imagine the end result may be equally unpredictable...";
		say "     [italic type][if flotintense < 2]The thing looks completely inanimate.[run paragraph on] You're fairly certain there'll be little to no consequence in using it right now[otherwise if flotintense < 4]The thing appears to be mostly in a mundane state right now, but it seems to subtly become excited as you approach.[run paragraph on] You could probably get away with using it[otherwise if flotintense < 6]The thing seems tinged with a streak of wanton menace.[run paragraph on] You could probably get away with using it, but not for long[otherwise if flotintense < 8]You feel ill at ease near the thing, as though you're being drawn to the tainted object.[run paragraph on] You don't think it'd be particularly wise to use it right now[otherwise]The thing seems to undulate from the corners of your eyes, as though thrumming with an otherworldly need.[run paragraph on] You feel overwhelmingly drawn to the thing, but you'd have to be insane to use it now[end if]...[roman type][line break]";
...
to say flotfuck:
	...
	otherwise:
		flotdecay;
		say "     Are you sure? [if flotintense is -1]Who knows what will happen![otherwise if flotintense > 4]You imagine it won't end well, by this point![end if]";
		if player consents:
			if flotintense is -1:
				...
				if cocks of player > 0:
					...
					say "     Once you're satisfied, you fumble about to finally climb on top of the dolphin, [if scalevalue of player > 3]a somewhat embarassing sight, given how much larger you are than it, though it seems to manage your weight[otherwise]easily managing your weight[end if]. Awkwardly aiming[if cocks of player > 1] one of[end if] your [cock size desc of player], half-erect dick[smn] for the entrance, you carefully sink it past its lips, the toy [if cock length of player > 11]seeming to easily take your cock, in spite of its impressive size; it takes a bit of work, but the pliable material stretches to handle its substantive girth[otherwise]taking your cock without any difficulty, easily burying the organ within its pliable depths[end if].";
				otherwise:
					...
					say "     Once you're satisfied, you fumble about to finally climb on top of the dolphin, [if scalevalue of player > 3]a somewhat embarassing sight, given how much larger you are than it, though it seems to manage your weight[otherwise]easily managing your weight[end if]. Grinding your exposed, eager cunt[sfn] against the smooth, pliable vinyl of the toy's underside, the lewd sounds of your motion compelling you to persist.";
			...
			otherwise if flotintense < 4: 
				...
				if cocks of player > 0:
					...
					say "     Once you're satisfied, you fumble about to finally climb on top of the dolphin, [if scalevalue of player > 3]a somewhat embarassing sight, given how much larger you are than it, though it seems to manage your weight[otherwise]easily managing your weight[end if].  Awkwardly aiming[if cocks of player > 1] one of[end if] your [cock size desc of player], half-erect dick[smn] for the entrance, you carefully sink it past its lips, the toy [if cock length of player > 11]seeming to easily take your cock, in spite of its impressive size; it takes a bit of work, but the pliable material stretches to handle its substantive girth[otherwise]taking your cock without any difficulty, easily burying the organ within its pliable depths[end if]. Before you really begin, the inflatable trembles under your weight, engulfing your cock with a latex-like substance[if cunts of player > 0 and cocks of player > 2], as well as your cunt[sfn] and other rods[otherwise if cunts of player > 0 and cocks of player > 1], as well as your cunt[sfn] and other rod[otherwise if cocks of player > 2], as well as your other rods[otherwise if cocks of player is 2], as well as your other rod[end if].";
				otherwise:
			...
			otherwise if flotintense < 6: 
				...
				if cocks of player > 0:
					...
					if cunts of player > 0:
						say ". Feeling something wet flood your cunt[sfn][if anallevel is not 1] and ass[end if], [if cunts of player > 1 or anallevel is not 1]they quickly harden into thick, tapered appendages to pound you with[otherwise]it quickly hardens into a thick, tapered appendage to pound you with[end if]";
			...
			otherwise if flotintense < 8:
				...
				if cocks of player > 0:
					say ". Eagerly, it encases[if cocks of player > 1] one of[end if] your [cock size desc of player], half-erect dick[smn] where its hole would be[if cocks of player > 2] before taking the rest along with it[otherwise if cocks of player is 2] before taking the other along with it[end if].";
					say "     The toy continues to wobble against the waters, easily having consumed your[if cock length of player > 11] impressive[end if] meat as a latex-like substance quickly engulfs[if cockname of player is not listed in infections of internallist] your [ball size][otherwise]the rest of your crotch[end if] as well";
					if anallevel is not 1:
						say ". Soon, you feel the tainted material flood your [if cunts of player > 0]cunt[sfn] and ass, pounding your holes with a writhing series of tapered, worming tendrils[otherwise]your ass, pounding your hole with a writhing series of tapered, worming tendrils[end if]";
			...
			otherwise:
				...
				if cocks of player > 0:
					say "your partially-aroused, [cock size desc of player] cock[if cocks of player > 1]s are[otherwise] is[end if] firmly plunged within its vinyl depths,";
					if anallevel is not 1:
						say ". Moreover, you feel the tainted material flood your [if cunts of player > 0]cunt[sfn] and ass, pounding your holes with a writhing series of tapered, worming tendrils[otherwise]your ass, pounding your hole with a writhing series of tapered, worming tendrils[end if]";
					otherwise if cunts of player > 0:
				...
				otherwise:
					if anallevel is not 1:
						say "you feel the tainted material flood your [if cunts of player > 0]cunt[sfn] and ass, pounding your holes with a writhing series of tapered, worming tendrils[otherwise]your ass, pounding your hole with a writhing series of tapered, worming tendrils[end if]";
					otherwise:
		...
		otherwise:
			...
			if keychar in lower case exactly matches the text "s" or keychar in lower case exactly matches the text "1" or keychar in lower case exactly matches the text "return" or keychar in lower case matches the text "struggle":
				...
				if struggleatt < boundsegment:
					...
					flotlust;
				otherwise:
					say "      [if flotintense < 3]Easily[otherwise if flotintense < 5]With relative ease[otherwise if flotintense < 8]Eventually[otherwise]Finally[end if], you manage to wrench yourself free of the possessed toy and dive into the waters, [if flotintense < 3]though it quickly regresses to a state of complete inanimacy[otherwise if flotintense < 5]gradually cooling down and becoming inanimate once more[otherwise if flotintense < 8]quivering for a moment as it slowly begins to fill the void you left[otherwise if flotintense < 10]the mess of a thing trembling in distress over its now-missing rider[otherwise]the thing flailing in your general direction in hopes to be one with you once more, forcing you to swim even further[end if].";
					say "     You climb back onto the castle and clean yourself off. [if flotintense > 4]It's likely best that you leave the vinyl object alone until it 'cools down' a bit[otherwise]The vinyl object remains relatively harmless, for now at least[end if]...";
					say "[line break]";
					cleanboundmemory;
					wait for any key;
					now trixieexit is 1;
			...
			otherwise if (obliging is true and (keychar in lower case exactly matches the text "o" or keychar in lower case matches the text "oblige")) or (obliging is false and (keychar in lower case exactly matches the text "a" or keychar in lower case matches the text "abide")) or keychar in lower case exactly matches the text "2":
				...
				if 1 is 1: [if obliging is true]:
					say "You continue to [if flotintense < 5]ride the vinyl dolphin[otherwise]oblige the vinyl dolphin's influence[end if], ";
					if flotintense < 7:
...
to flotprogress: [passable]
	if flotintense is 2:
		...
		if cocks of player > 0:
			say "encase your dick[smn], [if cockname of player is not listed in infections of internallist]your [ball size], and coat[otherwise]coating[end if] [if cunts of player > 0]your cunt[sfn] and ass[otherwise]your ass[end if][if cunts of player > 0 or anallevel is not 1].[run paragraph on] It doesn't seem compelled to penetrate you just yet, though the undulating material certainly drives your need further[end if].";
		otherwise:
	...
	otherwise if flotintense is 4:
		...
		if cocks of player > 0:
			say ". The undulating material pumps your [cock size desc of player] dick[smn] with such force that you rock against the toy with each motion, slicked by your own cum and exaggerated by the waves. [if cunts of player > 0 and anallevel is not 1]Moreover, you feel something large and hard force its way into your cunt[sfn] and ass, the shape shifting around until it appear satisfied with a tapered, worming tendril to rail you with.[otherwise if cunts of player > 0]Moreover, you feel something large and hard force its way into your cunt[sfn], the shape shifting around until it appear satisfied with a tapered, worming tendril to rail you with.[otherwise if anallevel is not 1]Moreover, you feel something large and hard force its way into your ass, the shape shifting around until it appear satisfied with a tapered, worming tendril to rail you with.[end if]";
		otherwise:
			say ". The undulating material grinds against your [cunt size desc of player] cunt[sfn] with such force that you rock against the toy with each motion, slicked by your own honey and exaggerated by the waves. [if anallevel is not 1]Moreover, you feel something large and hard force its way into your holes, the shape shifting around until it appear satisfied with a tapered, worming tendrils to rail you with[otherwise]Moreover, you feel something large and hard force its way into your hole[sfn], the shape shifting around until it appear satisfied with a tapered, worming tendril to rail you with[end if].";
	...
	otherwise if flotintense is 8:
		say "     With one final pull, you're drawn entirely into the body of the inflatably dolphin, which seems to roll onto its belly now that your [bodytype of player] body anchors it. Form straightened out until, your new prison adjusts to fit you as tightly as possible without distorting its frame. Completely unable to move, your vision is limited to the translucent eyes of the vinyl construct. All you can hear is your pained panting and the wet sound of your continued, unrelenting abuse.";
		say "     Outwardly, the dolphin appears to have regressed to its inanimate state, though betrayed by your loud, muffled moans[if cocks of player > 0], its occasional twitching against the waves, and your throbbing, material-coated cock[smn] which now hangs exposed from its undercarriage[otherwise] and its occasional twitching against the waves[end if]. It's difficult to maintain even the slightest mental clarity at this point, the prospect of being milked by this toy eternally very close to a reality!";

to say flotbinddesc: [passable]
	if flotintense < 2:
		say "     Riding the inflatable dolphin, [one of]it creaks softly against your touch[or]it squeaks against your motion[or]it softly buckles under the weight of your motions[or]its motions are exaggerated by the waves[at random]. The thing appears to be completely inert right now, having only the slightest power. You imagine your only active option is to [bold type]S[roman type]truggle and climb off it, else you can [bold type]O[roman type]blige, or [if boundrecover is true][bold type]R[roman type]ecover from[otherwise][bold type]E[roman type]ndure[end if] these circumstances, though there is little need to.";
	otherwise if flotintense < 4:
		say "     Riding the inflatable dolphin, [one of]it creaks softly against your touch[or]it squeaks against your motion[or]it softly buckles under the weight of your motions[or]its motions are exaggerated by the waves[at random], having engulfed your [if cocks of player > 0 and cunts of player > 0]cock[smn] and cunt[sfn][otherwise if cocks of player > 0 and anallevel is not 1]cock[smn] and ass[otherwise if cocks of player > 0]cock[smn][otherwise if cunts of player > 0 and anallevel is not 1]cunt[sfn] and ass[otherwise]cunt[sfn][end if] within its tainted, vinyl grasp. It's influence is growing to a point of some concern, though it's still relatively weak. You imagine your only active option is to [bold type]S[roman type]truggle enough until you can pull yourself completely free, else you can [bold type]O[roman type]blige, or [if boundrecover is true][bold type]R[roman type]ecover from[otherwise][bold type]E[roman type]ndure[end if] these questionable circumstances.";
	otherwise if flotintense < 6:
		...
		if cocks of player > 0:
			say "having engulfed your cock[smn] [if cunts of player > 0 and anallevel is not 1]while pounding your cunt[sfn] and ass with large and writhing tendrils[otherwise if cunts of player > 0]while pounding your cunt[sfn] with large and writhing tendrils[otherwise if anallevel is not 1]while pounding your ass with large and writhing thendrils[otherwise]within its tainted, vinyl grasp[end if]";
		otherwise:
			say "pounding your cunt[sfn][if anallevel is not 1] and ass[end if] with large and writhing tendrils";
	...
	otherwise if flotintense < 8:
		say "     [one of]Now f[or]F[stopping]orced to ride the inflatable dolphin, [one of]it creaks loudly against you[or]it squeaks against your hard motion[or]it buckles under the weight of your constant motions[or]its motions are intensely exaggerated by the waves[at random], ";
		if cocks of player > 0:
			say "having engulfed your cock[smn] [if cunts of player > 0 and anallevel is not 1]while relentlessly pounding your cunt[sfn] and ass with large and writhing tendrils[otherwise if cunts of player > 0]while relentlessly pounding your cunt[sfn] with large and writhing tendrils[otherwise if anallevel is not 1]while relentlessly pounding your ass with large and writhing tendrils[otherwise]within its powerful, vinyl grasp[end if]";
		otherwise:
			say "relentlessly pounding your cunt[sfn][if anallevel is not 1] and ass[end if] with large and writhing tendrils";
		say ". Limbs stuck firmly against the tainted object's surface as you're forced to fellate a large and intoxicating tendril, it's power has reached dizzying proportions. You imagine your only active option is to [bold type]S[roman type]truggle enough until you can pull yourself completely free, else you can [bold type]O[roman type]blige, or [if boundrecover is true][bold type]R[roman type]ecover from[otherwise][bold type]E[roman type]ndure[end if] these questionable circumstances.";
	otherwise:
		...
		if cocks of player > 0:
			say "having engulfed your cock[smn] [if cunts of player > 0 and anallevel is not 1]while relentlessly pounding your cunt[sfn] and ass with large and writhing tendrils[otherwise if cunts of player > 0]while relentlessly pounding your cunt[sfn] with large and writhing tendrils[otherwise if anallevel is not 1]while relentlessly pounding your ass with large and writhing tendrils[otherwise]within its overwhelming, vinyl grasp[end if]";
		otherwise:
			say "relentlessly pounding your cunt[sfn][if anallevel is not 1] and ass[end if] with large and writhing tendrils";
...
to flotbindorgasm: [passable]
	if flotintense < 2:
		...
		if cocks of player > 0:
			say "cock pumps your [cum load size of player] load into the dolphin[if cocks of player > 2], its brothers staining its underside[otherwise if cocks of player is 2], its brother staining its underside[end if]. [if cock width of player > 23]Briefly bulging to contain your impressive release, it seems to recede and acclimate to the torrent[otherwise]Eagerly, it seems to consume your release, motion further slicked by it[end if]";
		otherwise:
			say "cunt[sfn] ache[sfv] against the toy, staining its underside with your juices";
		say ". Trembling subtly in the wake of this, it seems to feed off your [if cocks of player > 0]cum[otherwise]ecstasy[end if], [if flotintense < 2]gradually changing[otherwise]close to changing[end if] while you continue riding it.";
	otherwise if flotintense < 4:
		say "     Finally overtaken by bliss, your ";
		if cocks of player > 0:
			say "cock[smn] pump[smv] your [cum load size of player] load into the dolphin, engulfing vinyl milking it for as long as it can. [if cock width of player > 23]Briefly bulging to contain your impressive release, it seems to recede and acclimate to the torrent[otherwise]Eagerly, it seems to consume your release, motion further slicked by it[end if]";
		otherwise:
			say "cunt[sfn] ache[sfv] against the toy, engulfing vinyl prolonging your bliss for as long as it can";
		say ". Trembling further in the wake of this, it's further empowered by your [if cocks of player > 0]cum[otherwise]ecstasy[end if], [if flotintense < 4]gradually changing[otherwise]close to changing[end if] as you're driven to continue riding it.";
	otherwise if flotintense < 6:
		say "     Finally overtaken by bliss, your ";
		if cocks of player > 0:
			say "cock[smn] pump[smv] your [cum load size of player] load into the dolphin, engulfing vinyl milking it for as long as it can[if cunts of player > 0 and anallevel is not 1] while pounding your holes[otherwise if cunts of player > 0] while pounding your hole[sfn][otherwise if anallevel is not 1] while pounding your hole[end if]. [if cock width of player > 23]Briefly bulging to contain your impressive release, it seems to revel in the torrential flood[otherwise]Eagerly, it seems to consume your release, motion further slicked by it[end if]";
		otherwise:
			say "cunt[sfn] ache[sfv] against the toy, pounding vinyl milking you for as long as it can";
		say ". Groaning under you in the wake of this, it's further empowered by your [if cocks of player > 0]cum[otherwise]ecstasy[end if], [if flotintense < 7]gradually changing[otherwise]close to changing[end if] as you're forced to continue riding it...";
	otherwise if flotintense < 8:
		say "     Overtaken by bliss, your ";
		if cocks of player > 0:
			say "cock[smn] pump[smv] your [cum load size of player] load into the dolphin, engulfing vinyl milking it for as long as it can[if cunts of player > 0 and anallevel is not 1] while pounding your holes[otherwise if cunts of player > 0] while pounding your hole[sfn][otherwise if anallevel is not 1] while pounding your hole[end if]";
		...
		say ". Visibly trembling under you, it's even further empowered by your [if cocks of player > 0]cum[otherwise]ecstasy[end if], [if flotintense < 9]gradually changing[otherwise]close to changing[end if] as you're forced to continue riding it...";
	otherwise:
...
to say flotstrugglebar:
	...
	if flotintense > 3:
		say "[if struggleatt > 2]X[otherwise]-[end if]";
	if flotintense > 1:
		say "[if struggleatt > 1]X[otherwise]-[end if]";
	say "[if struggleatt > 0]X[otherwise]-[end if][bracket]";
...
when play ends:
	if bodyname of player is "Flotted":
		...
		say "     On occasion, the creature even seems inclined to have fun with some of those it encounters, often [if cocks of player > 0 and anallevel is not 3 and cunts of player is 0]used to ride[otherwise]made to be riden by[end if] them. You're unsure if inflatable toy particularly enjoys this, or if it likes to surpise you from time to time.";
