...
to say Yamato Dragoness vic:
	...
	if vorelevel is 3 and (a random chance of 1 in 3 succeeds or ("Kinky" is listed in feats of player and a random chance of 1 in 3 succeeds)):
		...
		if "Touched by Madness" is listed in feats of player and (("Kinky" is listed in feats of player and a random chance of 1 in 2 succeeds) or ("Kinky" is not listed in feats of player and a random chance of 1 in 4 succeeds)):
			...
			say ". These persisting motions quickly arousing you, [if cocks of player is 0 and cunts of player is 0]much in spite your lack of outlet[else if hp of player < 1 and player is not submissive]much in spite your better judgment[else]writhing impotently against these tainted confines[end if], it's not long before ";
			if lust of player > 33 and (cunts of player > 0 or cocks of player > 0):
				say "you cry out in orgasm, [if cocks of player > 0]your throbbing, sputtering cock[smn][else]your audible climax[end if] more than enough to bring her over the edge along with you. The [if daytimer is night]night[else]day[end if]'s light replaced with an oppressive blackness, your [if FYDTBMM is 0]uncertain[else]inevitable[end if] fate is punctuated by your sudden disappearance, pulled into the depths of her voracious portal in the wake of her bliss.";
			else:
				say "finally she cries out in orgasm, the [if daytimer is night]night[else]day[end if]'s light replaced with an oppressive blackness, pulled into the depths of her voracious portal in the wake of this bliss.";
			...
			if cocks of player > 0 and a random chance of 1 in 2 succeeds:
				...
				if libido of player > 33:
					...
					if cocks of player > 2 or (cocks of player is 2 and cock length of player > 24):
						say ", exposed dick[if cocks of player > 3]s[else if cocks of player is 3 and cock length of player > 24]s[else][end if] wasting your seed impotently across the open air, down to the trees below";
			...
			if cocks of player > 0 and a random chance of 1 in 2 succeeds:
				...
				if FYDM is 0:
					say ". Patiently lining herself up with you, [if cock length of player < 20]you're not rightly sure how she intends to get anything out of this exchange, there's no way anyone of your size could be reasonably equipped for this monster.[run paragraph on] This reality never seems to cross her mind as[else]you imagine it must be uncommon for the monster to encounter someone as well-endowed as you are, clearly eager to get things going as[end if] she sinks ";
					if cock length of player < 25:
						say "[if cocks of player > 2]two of your cocks[else if cocks of player is 2]both of your cocks[else]your cock[end if]";
					else:
						say "[if cocks of player > 2]one of your numerous cocks[else if cocks of player is 2]one of your twin cocks[else]your cock[end if]";
...
to say YamatF_1: [cunt pitching]
	...
	if cunt length of player < 22 and cocks of player > 1:
		say "     Given how large she is, you figure you could fit [if cocks of player is 2]both[else]two[end if] of your cocks down into her, shall you try?";
		if player consents:
		...
		say "     Given how large she is, you figure you could fit [if cocks of player is 2]both[else]two[end if] of your cocks down both of her holes, shall you try?";
		if player consents:
			now tempnum2 is 2;
		else:
			now tempnum2 is 0;
		...
		now tempnum2 is 0;
	say "     Ready to take the plunge, you expose[if cocks of player is 2 and tempnum2 > 0] both of[else if tempnum2 > 0] two of[else if cocks of player > 1] one of[end if] your [cock size desc of player], half-aroused dick[smn]. Grinding [if tempnum2 > 0]them[else]it[end if] against the eager vent, the dragoness faintly moans against your touch, [if tempnum is 1]your motion slicked by her eager juices[else]slowly becoming aroused by the tease[end if], before you slowly sink the[if cockname of player is listed in infections of bluntlist] blunt[end if] head[if tempnum2 > 0]s[end if] past its scaled lips.";
	...
	if player consents:
		...
		if cocks of player > 1:
			...
			else if (tempnum2 > 0 and cocks of player > 2) or tempnum2 is 0:
				say " while [if tempnum2 > 0]their[else]its[end if] exposed brothers stain her coarse undercarriage";
...
to say YamatF_3: [oral receiving cock]
	...
	if player consents:
		say "     Her grip on you is fairly tight, practically pinning you to a tree, but you manage to order her off just before you cry out, aiming your now free, saliva-slicked dick[smn] at the dragoness as you fire off your [cum load size of player] load, [if cock width of player > 24]utterly soaking[else]painting[end if] her snout with your thick cum. Her brief confusion allows you to spend your entire load on her.";
