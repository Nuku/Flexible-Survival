...
instead of going to College Walkway East while (A Wolf's Invitation is active and (hp of Sylvia > 3 or hpM of Sylvia > 2 or hpF of Sylvia > 2) and a random chance of 1 in 2 succeeds and LastCampusWalkin - turns > 2):
...
to AWolfsInvitationEvent: [Room invitation from Julian]
	...
	now Julian's Room Event is active;
	now A Wolf's Invitation is resolved;

Julian's Room Event is a situation.
Julian's Room Event is inactive.
Prereq1 of Julian's Room Event is A Wolf's Invitation.
The level of Julian's Room Event is 0.
The sarea of Julian's Room Event is "Nowhere".

instead of going northeast from Tenvale College Male Dorms while (Julian's Room Event is active and hp of Julian is 2):
	JuliansRoomEvent;
...
Suppressant Supply Run is inactive.
...
instead of going northeast from Tenvale College Male Dorms while (Suppressant Supply Run is active and hp of Julian is 4 and a random chance of 1 in 2 succeeds):
