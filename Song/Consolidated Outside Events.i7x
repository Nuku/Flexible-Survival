...
to say ResolveEvent Grumpy Old Men:
	...
	say "     'Harry, you're too old to be this cranky anymore,' the wolf says while walking next to his friend. The lupine seems a bit embarrassed by the feline, though he smiles all the while. 'I'll be too old when they put me in the grave. Until then, keep your fuzzy tail moving so we can find old Alma to see if we can borrow some of that ammunition of hers,' the tiger grumbles. 'Do you really think she survived all of this?' the wolf asks, waving his hand around to denote the mess that the town has become. The graying tiger gives the silver wolf a pointed look. 'I sure as hell do! Hell, knowing her, she's probably taking in refugees!' The tiger lets out a roar of laughter before hissing and leaning over to rub at his lower back. In a heartbeat, the wolf is helping the other by offering a shoulder, though the feline is a little too proud, or stubborn, to take it.";
	WaitLineBreak;
...
to say ResolveEvent Scavenging Dalmatian:
	...
	if food is owned:
		say "     A part of you wants to feel sorry for the mutant, but do you really want to spare some of your food to help?";
		if Player consents:
			LineBreak;
		...
		if humanity of Player < 10:
			...
			if Player is herm:
				...
				if anallevel > 1:
					...
					say "     Looking over all the warm, sticky spunk that surrounds you, you debate how to best debase yourself. Your body seems to decide the next course of action for you, and you reach below to take hold of your hardening [if Cock Count of Player > 1]cocks, stroking and frotting them together[else]cock, stroking and pumping it slowly[end if], your [cock size desc of Player] length[smn] throbbing and pulsing with excitement as you pleasure yourself";
			...
			wait for any key;
			turn the Player into a "Blue Gryphon Herm" silently;
	...
	if a random number between one and 20 < 12: [the felitaur heard]
		...
		if a random chance of one in 3 succeeds: [fight a monster instead of being raped]
			...
			repeat with X running from 1 to number of rows in Table of Random Critters:
				...
				if there is a Name entry and Name entry matches the text battleground, case insensitively:
					add x to Q;
