...
to say losetoinflatableorca:
	...
	say "     The orca slips its tail beneath your back, sandwiching you between the smooth vinyl of its underbelly and the broad surface of its caudal fin. All of a sudden, it lurches to the side, pulling you with it. You roll across the ground until you land atop a large, deflated bag of some kind, although you're quickly distracted again by a sudden kiss from the inflatable creature. As its thick, smooth tongue explores your mouth, the orca blows deeply down your throat, filling your lungs with a strange, intoxicating gas that makes you feel increasingly light-headed and aroused.";
	WaitLineBreak;
				...
				if keychar in lower case exactly matches the text "s" or keychar in lower case exactly matches the text "1" or keychar in lower case exactly matches the text "return" or keychar in lower case matches the text "struggle": [player picked struggle]
					...
					if struggleatt < boundsegment: [Determines if the cumulative "struggle attribute" is less than or equal to bound stage. If equal to, the player escapes. Each orgasm will remove a tick of struggleatt.]
						if orcaintense is 1:
							...
							say "     [one of]Breathing heavily, you try to find a grip on the orca's immaculate hide to push it away[or]Despite your arousal, you continue to try to widdle out from under the oversized toy, not intent on becoming its eternal plaything[or]You writhe and squirm beneath the dogged orca, clawing at its hide to coax it off of you[at random].";
					...
					if obliging is true: [oblige]
						if orcaintense is 1:
							say "     [one of]Eager to please[or]Too aroused to resist[at random], you [if cocks of player > 0]take hold of the orca's hips and thrust uo into the welcoming warmth of her sex, causing her to till and moan in delight[else]drag your tongue along the smooth, elastic walls of the cetacean's sex[end if].";
						else if orcaintense is 2:
							say "     [one of]Eager to please[or]Too aroused to resist[at random], you [if cocks of player > 0]squeeze the orca's hips and thrust up into the cum-slick depths of her cunt, causing her to trill and moan in delight[else]drag your tongue along the smooth, damp walls of the cetacean's welcoming sex[end if].";
						else if orcaintense is 3:
						...
						if orcaintense is 1:
							say "     You abide the orca's [one of]playful swaying[or]ministrations[at random], [if cocks of player > 0]rocking your hips to meet her every lustful descent[else]dragging your tongue along the smooth, elastic walls of her sex[end if].";
						else if orcaintense is 2:
							say "     You abide the orca's [one of]playful swaying[or]ministrations[at random], [if cocks of player > 0]thrusting up into her cum-slick cunt[else]dragging your tongue along the smooth, damp walls of her welcoming sex[end if].";
						else if orcaintense is 3:
							...
							if cocks of player > 0:
								...
								say "drawing your tongue in broad sweeps along the smooth, wet walls of her sex[if cunts of player > 0] while fingering your cunt[end if].";
						else if orcaintense is 4:
							say "     You abide the orca's [one of]assault on your senses[or]frantic fucking[at random], ";
							if cocks of player > 0:
					...
					if boundrecover is true: [recover]
						...
						if orcaintense is 1:
							say "     You attempt to keep your arousal in check while the orca [one of]rocks[or]rolls[at random] against your [if cocks of player > 0]groin[else]face[end if].";
						else if orcaintense is 2:
							say "     You attempt to keep your arousal in check while the orca [one of]rocks[or]rolls[at random] against your [if cocks of player > 0]groin. That warm, wet vise squeezes around your length,[else]face, her heady scent and lubricating wetness[end if] making it difficult to stay focused.";
						else if orcaintense is 3:
							say "     Head swimming with desire, you make an effort to keep your arousal in check. It's becoming increasingly difficult to control your libido, [if cocks of player > 0]her gripping cunt a hot, wet oven that pulls and squeezes at your cock without relent[else]her powerful musky aroma laced with intoxicating synthetics[end if].";
						else if orcaintense is 4:
							say "     Groaning and trembling beneath the sexy orca, you struggle to keep your libido under control, feeling your arousal spiral to ever-greater heights. [if cocks of player > 0]Your tongue seems drawn to her slavering pussy, unable to help yourself from lapping along her smooth, slick walls and the heady, addictive flavors of that slippery chalice[else]Your cock twitches and pulses inside of her slavering pussy, that perfectly-formed orifice rippling sensually around your length[end if].";
						else:
...
to orcabindorgasm: [Orgasm description. Progresses to next bound segment.]
	if orcaintense is 1:
		...
		if cocks of player > 1:
			...
			say "writhe beneath the orca[if cunts of player > 0], sullying your thighs with squirts of sticky nectar[end if].";
	else if orcaintense is 2:
		...
		if cocks of player > 1:
			...
			say "bury your face into that smooth and supple chalice, ravenously lapping at her walls.[if cunts of player > 0] You cum, hard, further slickening your increasingly cramped confines.[end if]";
	else if orcaintense is 3:
		...
		if cocks of player > 1:
			...
			say "Your eyes roll back into your head and back arches[if cunts of player > 0] as you splatter her underside in sticky femcum[end if], trembling beneath the overwhelming aroma of lust and sex above you.";
	else if orcaintense is 4:
...
to say orcabinddesc: [Displays at the top of the bound screen after every action. Modified by current bound segment and player genitalia.]
	if orcaintense is 1:
		...
		say "     You are pinned beneath the orca, her great maw pressed to your lips in a long, enduring kiss that fills your lungs with a mind-altering gas. The intoxicating essense tastes like the sweetest ambrosia in your hyper-aroused state, synthetic and musky and arousing all at once. You can only buck weakly in response, completely enclosed from the rest of the world[if cocks of player > 0] as you drive again and again into the sexy orca's slit. Every thrust pushes deep into her soaking wet channel, its smooth, hot bliss enveloping your achingly hard cock[end if].";
