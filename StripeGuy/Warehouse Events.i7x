...
to say ResolveEvent Dock Team:
	...
	say "[line break]You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]15[roman type] (Strength + Perception Check):[line break]";
	if dice + bonus > 14:
...
to say ResolveEvent Fresh Fish:
	...
	if Player consents:
		...
		let dice be a random number from 1 to 20;
		say "You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]13[roman type] (Charisma Check):[line break]";
		if dice + bonus > 12:
...
to say ResolveEvent Hot Stepping:
	...
	let dice be a random number from 1 to 20;
	say "[line break]You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]17[roman type] (Dexterity + Perception Check):[line break]";
	if dice + bonus > 16:
