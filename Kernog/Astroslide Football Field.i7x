...
to say AstroslideFootFielddesc:
	if Astroslide Football Field is unvisited:
		now tempnum is a random number from 0 to 1;
		if MaleList is banned or MaleList is warded or (Player is femaleinterested and Player is not maleinterested):
			now tempnum is 0;
		else if FemaleList is banned or FemaleList is warded or (Player is maleinterested and Player is not femaleinterested):
			now tempnum is 1;
