		...
		if calcnumber > 0 and calcnumber <= the number of filled rows in table of fucking options:
			...
			if player consents:
				...
				if nam is "Approve suit upgrades":
					...
					if player consents:
						LineBreak;
						say "     [italic type]Thank you, administrator,[roman type] the computer tells you. The cows start mooing franctically outside of the control room, as the milking machine gears up their ministrations.";
...
instead of going south from Milking Facility Operations Room while milkingFactoryPlayerTrap is 1:
	...
	if player is herm:
		say "     As if to confirm your dreadful suspicion, you feel something fix itself on your groin. You look down and see [cocks of player] metallic milker[smn] attached to your cock[smn]. [ittheym] start[smv] pumping air in-and-out, and [itstheirm] inner rubber sheath[esmn] vacuum[smv] against your length[smn]. At the same time, you feel your [cunts of player] cunt[sfn] spread apart by what feels like bull-shaped dildo[sfn]. [ittheyf] start[sfv] jackhammering your puss[yfn], keeping you busy.[if breast size of player > 0] With a loud [italic type]schlop[roman type], rubber cups attach themselves to your breasts and quickly vacuum, espousing your forms before going to work on you with an insistent [italic type]puff-pfff[roman type] noise.[end if]";
	else if player is male:
		say "     As if to confirm your dreadful suspicion, you feel something fix itself on your groin. You look down and see [cocks of player] metallic milker[smn] attached to your cock[smn]. [ittheym] start[smv] pumping air in-and-out, and [itstheirm] inner rubber sheath[esmn] vacuum[smv] against your length[smn].[if breast size of player > 0] With a loud [italic type]schlop[roman type], rubber cups attach themselves to your breasts and quickly vacuum, espousing your forms before going to work on you with an insistent [italic type]puff-pfff[roman type] noise.[end if]";
	else if player is female:
		say "     As if to confirm your dreadful suspicion, you feel something fix itself on your crotch. You feel your [cunts of player] cunt[sfn] spread apart by what feels like bull-shaped dildo[sfn]. [ittheyf] start[sfv] jackhammering your puss[yfn], keeping you busy.[if breast size of player > 0] With a loud [italic type]schlop[roman type], rubber cups attach themselves to your breasts and quickly vacuum, espousing your forms before going to work on you with an insistent [italic type]puff-pfff[roman type] noise.[end if]";
			...
			if humanity of player < 1: [Lost all sanity. Instant game over.]
				...
				if keychar in lower case exactly matches the text "s" or keychar in lower case exactly matches the text "1" or keychar in lower case exactly matches the text "return" or keychar in lower case matches the text "struggle": [player picked struggle]
					...
					if milkingCowSuitBindStage is 1:
						...
						say "     Taking pleasure in your captivity, you play with your mask's microphone, trying to make the most realistic moo, while you roll your hips in rhytmn with the machinery hooked to your suit. The AI turns a X-ray mode on the video feed, and allows you to vary between different angles by moving your eyes.";
					else if milkingCowSuitBindStage is 4:
						say "     You placidely watch the video feed provided by the vinyl mask, mooing in pleasure each time the machine hits the right spot. MOO diffuses farm scents and noises through your mask, helping your mind in becoming a nice little milk cow.";
...
to MilkingCowSuitOrgasm: [Orgasm description. Progresses to next bound segment.]
	if milkingCowSuitBindStage is 1:
		say "     You scream into the vinyl muzzle as the sexual stimulation brings you over the edge. Your hips buck against [if player is male]the milker[smn] attached to your cock[smn][else]the thick dildo[sfn] pilonning your cunt[sfn]. While your mind temporarily blanks out, your sexual fluids leave the suit through a series of tubes[end if], [if breast size of player > 0]joining the milk of your breasts[else]joining the rest[end if] inside the storage tank.";
	else if milkingCowSuitBindStage is 2:
...
to milkingCowSuitProgress: [Displays once when entering the next bound segment. 1 does not need to be checked since the bind starts at that value.]
	if milkingCowSuitBindStage is 1:
		...
		if player is herm:
			say "     With perfect timing, the various apparatus return to work on your body. And thus, you have a perfect angle on the milking pump[smn] sliding up and down your [cock size desc of player] cock[smn] and the thick, ox-shaped dildo[sfn] ramming your [cunt size desc of player] cunt[sfn][if breast size of player > 0], and the milking cups pressing on your breasts in order to extract their contents[end if].";
		else if player is female:
			say "     With perfect timing, the various apparatus return to work on your body. And thus, you have a perfect angle on the thick, ox-shaped dildo[sfn] ramming your [cunt size desc of player] cunt[sfn][if breast size of player > 0], and the milking cups pressing on your breasts in order to extract their contents[end if].";
		else if player is male:
			say "     With perfect timing, the various apparatus return to work on your body. And thus, you have a perfect angle on the milking pump[smn] sliding up and down your [cock size desc of player] cock[smn][if breast size of player > 0], and the milking cups pressing on your breasts in order to extract their contents[end if].";
...
to say milkingCowSuitBindDesc: [Displays at the top of the bound screen after every action. Modified by current bound stage.]
	if milkingCowSuitBindStage is 1:
		say "     You are down on your knees, trapped inside a black mass which threatens to engulf you[if player is male]. A milker is hooked to your cock[smn] and pumps slowly. It slurps down a bit of pre-cum here and then, as MOO tries to find the optimal rhytmn for your physiology.[end if][if player is female]. The fluid matter has formed an almost perfect reproduction of a bull cock, adapted to the size of your [cunt size desc of player] cunt[sfn]. It pushes, inch by inch in your puss[yfn] in an attempt to find your g-spot[sfn][end if][if breast size of player > 0]. Milking cups are hooked on each of your [breasts of player] breasts and stimulate them in an attempt to drow out milk[end if]. [one of]You turn your head just in time, as the muzzle part of the vinyl slime tried to fit on your [facename of player] head. You push it back with some difficulty.[or]The mass of vinly tries to stick your hands to the floor. You pull them away just in time, in a brief tug-of-war with the animated goop[or]The vinyl slowly creeps up your neck, trying to find the mask part in order to fuse with it.[purely at random]";
