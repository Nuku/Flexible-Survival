	...
	while sextablerun is 0:
		...
		if calcnumber > 0 and calcnumber <= the number of filled rows in table of fucking options:
			...
			if player consents:
				...
				if (nam is "Fuck Erin"):
					say "[ErinFuckTop]";
					[PCSexAftermath Erin receives "AssFuck" from player; ][What is this?]
...
to say ErinFuckBottom:
	...
	if player is male or anallevel is 3:
		PCSexAftermath player receives "AssFuck" from Erin;
	else if (player is female or player is herm):
		PCSexAftermath player receives "PussyFuck" from Erin;
	now isTwinHere is 0;
...
to say VioletFuck:
	...
	PCSexAftermath Violet receives "PussyFuck" from player;
	if cocks of player is 1:
		...
		PCSexAftermath Violet receives "AssFuck" from Violet;
	WaitLineBreak;
	if cocks of player is 1:
...
to say VioletStrapOn:
	...
	if cunts of player > 1:
		...
		PCSexAftermath Player receives "DildoPussyFuck" from Violet;
		...
		PCSexAftermath Player receives "DildoAssFuck" from Violet;
	else:
