...
[I'm unsure if -- if AndroFuck <= 3:
					break;
				otherwise:
					Now AndroFuck is 2;  -- Is an effective way to prevent AndroFuck from stacking infinitely. I think it works, but i'm not sure]
...
[Defining description variable]
AndroDesc is a number that varies.
...
[defining number variables for NPC scenes]
androtalk is a number that varies.
AndroFuck is a number that varies.
andronum is a number that varies.
AndroMenu is a number that varies.
calcnumber is a number that varies.
CollarNumber is a number that varies.
...
instead of conversing Andromeda:
	...
	otherwise if androtalk is 2:
		...
		now androtalk is 3;
	otherwise if CollarNumber is 1 and androtalk > 1:
...
instead of fucking Andromeda:
	...
	otherwise if player is female and androtalk < 2:
