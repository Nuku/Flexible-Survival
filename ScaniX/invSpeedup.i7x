	...
	if the number of owned grab objects > 0:
		...
		repeat with x running from 1 to the number of rows in the table of game objects:
			...
			if object entry is owned:
				...
				else if the number of trader in the Location of Player > 0:
					let tradeguy be a random trader in the Location of Player;
					say "|[link][bracket]T[roman type][as]give [Name entry] to [tradeguy][end link]";
				if ( ( ( object entry is armament or ( object entry is equipment and AC of object entry > 0 and effectiveness of object entry > 0 ) ) and object entry is not improved ) or the Name entry is "nanite collector" ) and the number of smither in the Location of Player > 0:
