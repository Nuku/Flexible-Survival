...
to say ResolveEvent Tide Pool Trouble:
	say "     As you meander down the beach, waves crash against rocks not too far ahead of you. The rocks litter this part of the shore. You cannot help but walk on some, but most of these are better described as boulders. Not much farther ahead, there are various boulders jutting out from the shore, towering above you the size of small buildings. You see the tide pools at your feet as you walk towards these towering boulders. Small fish swim as you move from rock to rock. As you approach the tallest rock, you notice something in one of the tide pools under that great boulder causes the water to splash out vigorously. The water seems to bubble out from the shallow pool. Could this be the entrance to some underwater cavern? The water seems to positively come alive as you approach.";
	say "[line break]     [bold type]Do you inspect the roiling tide pool?[roman type][line break]";
...
to say ResolveEvent Lingua Franca:
	...
	say "[line break]     [bold type]Do you approach the group?[roman type][line break]";
	...
	if Player consents:
		...
		let diceroll be a random number from 1 to 20;
		say "You roll 1d20([diceroll])[if bonus >= 0]+[end if][bonus] = [special-style-1][diceroll + bonus][roman type] vs [special-style-2]18[roman type] (Intelligence Check):[line break]";
...
when play begins:
	add PoliSci to BadSpots of FurryList;
	add PoliSci to BadSpots of DemonList;
	add PoliSci to BadSpots of MaleList;
	add PoliSci to BadSpots of FemaleList;
...
to say ResolveEvent PoliSci:
	...
	say "[line break]     [bold type]Do you approach the two arguing students?[roman type][line break]";
	...
	if Player consents:
		...
		LineBreak;
		say "     Deciding to leave the raging ass alone, you continue walking past the couple, giving them plenty of room. You overhear the donkey yell, 'He was my friend!' You hurry your step and try to move on before they notice you. When they are pretty far behind you, you hear the donkey yell, practically scream, 'You fucked my best friend!' How he has not noticed that everyone's standards have disappeared is beyond you, but thankfully you missed the bullet and continue exploring the campus.";
...
when play begins:
	add Sewer Stalker to BadSpots of FurryList;
	add Sewer Stalker to BadSpots of DemonList;
	add Sewer Stalker to BadSpots of MaleList;
	add Sewer Stalker to BadSpots of FemaleList;
...
to say ResolveEvent Sewer Stalker:
	...
	say "[line break]     [bold type]Do you try to run away from this stranger or stay around?[roman type][line break]";
	...
	if Player consents:
		LineBreak;
		say "     You close your eyes, turn around, and run as fast as you can. You open your eyes when you're a few feet away from the cloaked stalker, just in time to see a demon brute start to materialize. You dart past him. You turn a corner. There's another monster lurking in the darkness. Thankfully, it isn't paying attention. From behind you, light flashes and you hear a heavy thud. Did the demon brute just succumb to the light as you did? You continue running, trying to turn the same way each time, making a circle to leave the sewers where you came in. Eventually, you find a familiar path, where you first met that stalker. With your bearings in place, you head back to the mall, leaving the strange cloaked figure behind.";
	else:
		LineBreak;
		say "     You're too curious to leave. You smile nervously into the beautifully polished ball as your reflection shows off your [FaceSpeciesName of Player in lower case] face. Before you can do anything more than recognize yourself in the glass ball, images of the city begin to flash inside the orb before you, much like a TV screen flickering between channels. Your head feels heavy as you watch, but you can't seem to look away. Among the images, you see [one of]mutant horses fucking each other in the park, feline prostitutes tag teaming a succubus in the red light district, wyverns flying through the sky being shot at by military men[or]an alpha husky dominating a new member of his pack, orc warriors congratulating one another in a mess hall, a Spartan warrior wrestling a wolf in the college campus[or]a pink gelatinous woman having sex with a beaver in the red light district, an empty werewolf costume running around in the state fair, a large horned demon shoving a captured man into a large red sack[at random], and you see yourself standing in front of a strange figure in the sewers with your head canted slightly to the side. The images come to an abrupt halt. You're once again staring at your [FaceSpeciesName of Player in lower case] face in the glass orb.";
	...
	if scaleValue of Player <= 2:
		if a random chance of 1 in 4 succeeds:
			...
			if a random chance of 1 in 2 succeeds: [12.5% chance for rusty nail]
				say "a rusty nail! This [if scalevalue of Player > 2]may be useful if you could wield it properly[else]will make a useful weapon[end if][if searchesUtSit < 4]. There are probably useful items still in this pile of junk if you keep looking for them[else]. You doubt you'll find anything more than you already have in this pile of junk[end if].";
				ItemGain rusty nail by 1;
			else if a random chance of 1 in 2 succeeds: [6.25% chance for sharp screw]
				say "a sharp screw! This [if scalevalue of Player > 2]may be useful if you could wield it properly[else]will make a useful weapon[end if][if searchesUtSit < 4]. There are probably useful items still in this pile of junk if you keep looking for them[else]. You doubt you'll find anything more than you already have in this pile of junk[end if].";
				ItemGain sharp screw by 1;
			else: [increments variable]
				say "nothing particularly useful[if searchesUtSit < 4]. There are probably useful items still in this pile of junk if you keep looking for them[else]. You doubt you'll find anything more than you already have in this pile of junk[end if].";
