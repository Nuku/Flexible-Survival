...
to say Hawkman wins:
	...
	if Player is female or ( anallevel is 3 and a random chance of 2 in 5 succeeds ) or ( "MPreg" is listed in feats of Player and a random chance of 2 in 5 succeeds ):
...
"Hawkman Male"	"[PrepCombat_Hawkman Male]"

to say PrepCombat_Hawkman Male:
...
When Play begins:
	...
	now CumItem entry is "hawkman male cum"; [ Item to be given to the player if they have this infection and jerk off. ]
...
When Play begins:
	...
	now enemy title entry is "Female Hawkman"; [ Name of the encountered creature at combat start - Example: "You run into a giant collie." instead of using "Smooth Collie Shemale" infection name. ]
	now enemy Name entry is ""; [ Specific name of unique enemy. ]
	now enemy type entry is 0; [ 0 = non unique enemy; 1 = unique (unknown name); 2 = unique (known name) | Used to disqualify unique enemies from Vore/UB and showing the enemy name in encounters. ]
	...
	now MilkItem entry is "hawkman female milk"; [ Item to be given to the player if they have this infection and milk themselves. ]
	now CumItem entry is "hawkman female cum";
	now TrophyFunction entry is "-"; [ Function to generate a list of optional loot items, of which the player can choose one after victory. ]
	now scale entry is 3; [ Number 1-5, approx size/height of infected PC body: 1=tiny, 3=avg, 5=huge ]
	now body descriptor entry is "[one of]sleek[or]winged[at random]";
	now type entry is "[one of]avian[or]raptor[at random]"; [ one-word creature type. Ex: feline, canine, lupine, robotic, human... Use [one of] to vary ]
