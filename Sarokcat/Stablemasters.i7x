...
Cunt Depth of Slutslave is 12. [penetratable length in inches; some minor stretching allowed, or more with Twisted Capacity]
...
Carry out Slavelearning:
	...
	if LastSlutslavelearned minus turns < 10:
		...
		if charisma of Player < cha entry:
			increase Tc by 1;
			now Tcha is Tc;
		[now all numbers between 1 and Tc refer to a stat that can be increased]
		if Tc is 0: [until someone writes a scene for no stat being eligible for a raise, choose one at random]
			now Tc is 6;
			now Tstr is 1;
			now Tdex is 2;
			now Tsta is 3;
			...
			now Tint is 5;
			now Tcha is 6;
		let T be a random number between 1 and Tc;
		if T is Tstr:
