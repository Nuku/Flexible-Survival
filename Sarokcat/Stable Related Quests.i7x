...
to say ResolveEvent Ignored Memos:
	if MaleList is banned or FemaleList is banned or HermList is banned or FurryList is banned:
		...
		now fancyquest is 1;
		now Resolution of Ignored Memos is 1; [got the stuff]
...
to say ResolveEvent Hardware Fort:
	...
	if fancyquest is 4:
		if Recoveredhardware is 0:
			...
			let diceroll be a random number from 1 to 20;
			say "[line break]     You roll 1d20([diceroll])[if bonus >= 0]+[end if][bonus] = [special-style-1][diceroll + bonus][roman type] vs [special-style-2]17[roman type] (Dexterity Check): ";
			increase diceroll by bonus;
			if diceroll > 16:
...
to hardwarefortphase1:
	...
	if fightoutcome >= 10 and fightoutcome <= 19:
		challenge "Wolverine Guard";
		if fightoutcome >= 10 and fightoutcome <= 19:
			say "     Victorious over the guards of this little fort[if recoveredhardware > 0] at last[end if], you stagger over and take a look in at the store itself, only to find signs that a couple creatures have recently vacated the area, probably due to the sounds of combat. You wonder for a minute if the guards were here to protect the hardware store, or whoever was hiding here, still the question seems academic at this point, and having at least temporarily secured the store, you take a moment to rest, before activating the radio transceiver to signal the horsemen to come visit. Now you just have to protect the impromptu fort until the horsemen arrive and finish looting the place, unfortunately you are all too aware of the fact that any kind of large commotion like your recent combat, and the large amount of moving stuff the horsemen will be doing, is certain to attract plenty of attention from the creatures roaming the area. Indeed as you move back to the entrance of the area, you can already see several creatures heading your way to investigate the situation.";
			hardwarefortphase2;
					...
					if fightoutcome >= 10 and fightoutcome <= 19:
						...
						if fightoutcome >= 10 and fightoutcome <= 19:
							if recoveredhardware > 2:
								...
								now fancyquest is 5;
								now Resolution of Hardware Fort is 1; [success]
								now Hardware Fort is resolved;
							else:
								hardwarefortphase5;
		if fightoutcome >= 20 and fightoutcome <= 29:	[lost any fight]
...
to hardwarefortphase5:
	if recoveredhardware < 2:
		...
		say "     Looking around after the aerial assault is over, you realize that the horsemen have rather efficiently finished their looting and have either grabbing hand carts piled high with tools and equipment or are harnessing their brethren to makeshift carts equally laden. A few lucky ones are rather enthusiastically (if poorly) driving some heavily loaded forklifts, at least one of which seems to be loaded with a massive generator of some kind. As you look over the situation and find the leader of the horsemen looking at you, you certainly realize that they can't actually fight for themselves while so heavily laden with equipment, and it is up to you once again to deal with the creatures outside and lead them off while the equines make their way back to the Stables with their new loot. Sighing slightly, you patch your wounds the best you can, and prepare to lead the creatures off, only to be stopped by the head of the horsemen as he grasps your elbow and hands you a medkit.";
	...
	let bonus be ( Dexterity of Player minus 10 ) divided by 2;
	let diceroll be a random number from 1 to 20;
	say "[line break]     You roll 1d20([diceroll])[if bonus >= 0]+[end if][bonus] = [special-style-1][diceroll + bonus][roman type] vs [special-style-2]17[roman type] (Perception Check): ";
	increase diceroll by bonus;
	if diceroll > 16:
...
to say ResolveEvent Horsepowering:
	if generatorfixing is 3:
		...
		let diceroll be a random number from 1 to 20;
		say "[line break]You roll 1d20([diceroll])[if bonus >= 0]+[end if][bonus] = [special-style-1][diceroll + bonus][roman type] vs [special-style-2]15[roman type] (Intelligence Check):[line break]";
		increase diceroll by bonus;
		if diceroll > 14:
	...
	if provingstallionhood is 1:
		...
		if matriarchowned is 1:
			...
			while fightoutcome < 20 and HyenaPitFightCounter < 2: [runs for 2 times or until the player loses or flees]
				challenge "[if HyenaPitFightCounter is 0]Wolftaur[else]Leopardman[end if]";
			...
			if fightoutcome < 20: [player won]
				...
				say "     You barely have time to look up from the ground where your recent mating left you, before the hyenas descend on your already abused body, their hands teasing and stroking your [Skin of Player] skin";
				say "[HyenegotiationHyenaAbuse]";
				now provingstallionhood is 2;
				decrease humanity of Player by 50;
				now Resolution of contract hyenegotiation is 4; [lost]
				now Contract Hyenegotiation is resolved;
			else if fightoutcome is 30: [fled]
				say "     You manage to get away from your attackers, only to run right into the watching hyenas. They bowl you over and descend on your body, their hands teasing and stroking your [Skin of Player] skin";
				say "[HyenegotiationHyenaAbuse]";
				now provingstallionhood is 2;
				decrease humanity of Player by 50;
...
to say HyenegotiationHyenaAbuse:
	...
	MultiInfect "Hyena Herm" repeats 4;
