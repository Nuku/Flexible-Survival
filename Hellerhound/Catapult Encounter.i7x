...
instead of resolving a loaded catapult:
	...
	if the player consents:
		if pocketknife is owned:
			say "You slit open the bag to find a large compressed volume of cum, milk, and piss. Whoever did this was not on the side of the rescuers. It looks like it is infected thoroughly. ";
			say "You could drink some, but that is not advised.";
			if the player consents:
				say "You dip your [face of player] head into the mess, slurping it up.";
		...
		otherwise:
			say "You can't manage to open the bag. Maybe you need to slit it open with something?";
	otherwise:
	...
	otherwise:
		if gotcatares is 0:
			say "Upon close inspection, there appears to be bags of food and water inside the catapult. Maybe you could wriggle them free without discharging the catapult?";
		otherwise:
			say "You find that someone has reloaded the catapult., with food and water again. The ropes have changed, too. It looks more difficult to get on there.";
		...
		if the player consents:
			...
			if a random number between one and catadiff is greater than the dexterity of the player:[the catapult fires]
				say "You accidentaly bumb a trigger rope, and the contraption begins to swing. You get out of there fast. [catapult fire]";
			otherwise:
				say "You manage to get the resources free of the tangle, and bring them outside.";
				repeat with T running from one to six:
