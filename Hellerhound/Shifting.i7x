...
understand "shift into/to/-- [text]" as shifting.

check shifting:
	if Resolution of Secure Area < 2:
		say "You do not know how to do that!" instead;
	if Humanity of Player < 50:
		say "Your feral impulses prevent you from concentrating hard enough to change." instead;
...
carry out shifting:
	...
	repeat with X running from 1 to number of filled rows in Table of Random Critters:
		...
		if Name entry exactly matches the text critter, case insensitively:
			now tmonster is X;
			now ttransform is 1;
			break;
	if ttransform is 0:
		...
		repeat with X running from 1 to number of filled rows in Table of Random Critters:
			choose row X from the Table of Random Critters;
			[add Name entry to critter List;]
			if Name entry matches the text critter, case insensitively:
				now tmonster is X;
				now ttransform is 1;
				break;
	...
	repeat with x running through critter list:
		say "[x]";]
	say "You [if ttransform is 1]concentrate on becoming one with the [Name entry in lower case]s[else]You don't know any such beast[end if].";
...
To transform:
	...
	if BodyName of Player is not Name entry:
		say "[line break]Your body [one of]tingles[or]goes flush[or]vibrates with odd pleasure[or]goes cold[or]feels oily[at random] as [body change entry].";
	...
	if Libido of Player < libido entry:
		say "[line break]You can't help but [one of]feel your thoughts drifting towards sex[or]notice that the attributes of [Name entry in lower case] were very appealing[or]wonder if getting to know these creatures in the biblical sense would be all that bad[at random].";
...
This is the Shifter Check rule:
	if Player is shifter:
		...
		say "     (Being a shapeshifter, the normal ending for your body type is suppressed.)";
