...
The summondesc of Cute Crab is "     [if Snips is visible]Letting your cute crab Snips know that it's time to head out, Snips quickly climbs onto your foot, ready for adventure![else]Bursting from the ground near you, your cute crab appears. Snips clacks its pincers together in an excited fashion like a castanet player.[end if][line break]".
...
Scent of the Cute Crab is "     The crab's smell is faintly reminiscent of the beach.".
...
to say DismissSnips:
	...
	say "     [if Player is not in Computer Lab]Looking at you for a long moment, he finally relents and burrows under the ground[else]Snips quickly scuttles back to the home it has made out of one of the cubicles, clearly more than happy to be able to rest[end if].";

an everyturn rule:
	if Player is in Traveling and Snips is in Traveling and a random chance of 1 in 3 succeeds:
		say "[one of]Raising his eyestalks to look at you, Snips gives a few snaps of its claws, then starts to rush forward on many legs, doing its best to keep up with you[or]Raising both claws as if to take on the world, Snips moves to your side[at random].";
...
Cock Count of Snips is 1. [X cock]
...
Scent of Snips is "     The crab's smell is faintly reminiscent of the beach.".
...
instead of conversing the Snips:
	if Cute Crab is listed in companionList of Player or (Player is in Computer Lab and Snips is in Computer Lab):
		say "[SnipsTalkMenu]";
	else:
...
to say SnipsTalk2:
	if number of collected people < 4:
...
The summondesc of house cat is "     You [if Dinah is visible]call Dinah over to you, watching as she stalks over to your side, walking like a predator on the prowl[else]begin to call out for Dinah, only to stop in embarrassment as you realize she is already washing her paws nearby, watching you, obviously amused at your antics[end if].[line break]".
...
Scent of house cat is "     Your little kitty smells like a normal cat.".

to say DismissDinah:
	move Dinah to Computer Lab;
	say "     [if Player is not in Computer Lab]You begin to tell Dinah that she can head on home, only to find a pair of glowing eyes watching you from the shadows, right before they fade away completely[else]Reaching down to stroke Dinah's soft fur, you tell her that she can rest for now. Rubbing up against your leg, she then begins to make her way back to her cubicle to laze the day away[end if].";
...
Cock Count of Dinah is 0. [X cock]
...
Scent of Dinah is "     Your little kitty smells like a normal cat.".
...
instead of conversing the Dinah:
	if house cat is listed in companionList of Player or (Player is in Computer Lab and Dinah is in Computer Lab):
		say "[DinahTalkMenu]";
	else:
		...
		if there is an object in row 1 of table of itemselection:
			...
			if the player consents:
				...
				if the number of filled rows in table of itemselection is 1:
					...
					while chosenmilk is pocketknife:
						...
						if calcnumber > 0 and calcnumber <= number of filled rows in table of itemselection:
							...
							say "Invalid Option. Pick between 0 and [the number of filled rows in the table of itemselection].";
				if chosenmilk is not journal:
					...
					move Dinah to Computer Lab;
					say "[line break]     (Dinah the house cat is now a possible ally! You can make her your active ally by typing [link][bold type]ally Dinah[roman type][end link] or [link][bold type]ally house cat[roman type][end link]. You can see all the allies you have with the [link][bold type]allies[roman type][end link] command. Allies will lower the XP you gain from battle, but can gain levels themselves to be more useful in a scrap. Want to get rid of an ally? Use [link][bold type]ally dismiss[roman type][end link], or just [link][bold type]dismiss[roman type][end link].)";
...
Chapter 3 - Exotic Bird

Section 1 - Exotic Bird NPC/Pet
[Original content created by Sarokcat]
...
The summondesc of Exotic Bird is "     [if Chirpy is visible]Reaching out, you lightly pet Chirpy's brightly colored feathers. With a tip of your head, she instantly flutters over, landing softly on your shoulder[else]Hearing you whistle, Chirpy flaps over and takes up position overhead, keeping an eye out for any threats[end if].[line break]".
...
Scent of Exotic Bird is "     It smells like an exotic bird with a faint, tropical scent to it.".

to say DismissChirpy:
	move Chirpy to Computer Lab;
	say "     [if Player is not in Computer Lab]You let out a loud high pitch whistle, followed by a swish of your arm. Chirpy glances down at you from the air, knowing the meaning, and begins to fly back to the Library[else]Walking over to the little bird's cubicle, Chirpy happily flutters over to the desk to find ways to amuse herself till you have need of her again[end if].";
...
Cock Count of Chirpy is 0. [X cock]
...
Scent of Chirpy is "     It smells like an exotic bird with a faint, tropical scent to it.".
	...
	while sextablerun is 0:
		...
		if calcnumber > 0 and calcnumber <= the number of filled rows in table of fucking options:
			...
			if Player consents:
				LineBreak;
				now sextablerun is 1;
				if title entry is "Watch Chirpy":
					say "[ChirpyTalk1]";
		else if calcnumber is 0:
			LineBreak;
			now sextablerun is 1;
...
to say ResolveEvent Scared Bird:
	...
	if carried of food is 0:
		...
		if Player consents:
			...
			say "[line break]You roll 1d20([diceroll])[if bonus >= 0]+[end if][bonus] = [special-style-1][diceroll + bonus][roman type] vs [special-style-2]18[roman type] (Charisma Check):[line break]";
			...
			if diceroll > 17:
				...
				move Chirpy to Computer Lab;
				say "[line break]     (Chirpy the exotic bird is now a possible ally! You can make her your active ally by typing [link][bold type]ally Chirpy[roman type][end link] or [link][bold type]ally exotic bird[roman type][end link]. You can see all the allies you have with the [link][bold type]allies[roman type][end link] command. Allies will lower the XP you gain from battle, but can gain levels themselves to be more useful in a scrap. Want to get rid of an ally? Use [link][bold type]ally dismiss[roman type][end link], or just [link][bold type]dismiss[roman type][end link].)";
...
This is the Chirpy's Epilogue rule:
	if Exotic Bird is tamed:
		...
		say "     [if humanity of Player < 10]When you give in to your feral instincts, the exotic bird that you rescued seems to sense your new state of mind, and keeps at a safe distance, though she never strays too far, often helping you out of trouble or calling out to warn you of danger. In thanks, you often leave her little offerings of whatever is handy. It makes for a strange sort of partnership, but a happy enough one[else]After your rescue, you take the bird that you rescued to several specialists, who are amazed to find that a member of its rare species survived the city, and they thank you profusely for your help in bringing her back safely. Impressed at your ability to get the bird to trust you, not only do they offer you a reward for saving Chirpy, but they ask you to stop by occasionally not just to visit your now happy bird friend, but also to ask questions about any other strange birds and species that you might have encountered in the city[end if].";

Chapter 4 - Helper Dog
...
Scent of the Helper Dog is "     Hobo smells like a normal dog, surprisingly given the circumstances.".

to say SummonHobo:
	project Figure of Hobo_icon;
	say "     [if Hobo is visible]Leaning down, you ruffle one of Hobo's ears, telling him that it's time to get back out into the city. He happily wags his tail as he begins to trail[else]Coming obediently to your call, Hobo moves to your side, ready and eager to assist[end if] you.";
	now lastfuck of helper dog is turns;

to say DismissHobo:
	move Hobo to Computer Lab;
	say "     [if Player is not in Computer Lab]Softly patting Hobo's flank you tell him that it's time to head home. He happily starts to run off but stops when he notices you not following. With a curt nod to him, he tilts his head then resumes his run back to the Library[else]Walking over to the cubicle that Hobo has made into his own little house, you lean down, giving the faithful dog a tight hug. With a final ear rub that leaves Hobo's tail wagging, you tell him to take it easy at home for a while[end if].";

an everyturn rule:
	if Player is in Traveling and Hobo is in Traveling and a random chance of 1 in 3 succeeds:
...
Cock Count of Hobo is 1. [X cock]
...
Scent of Hobo is "     Hobo smells like a normal dog, surprisingly given the circumstances.".
	...
	if hdog is 0:			[first time finding]
		...
		if food is not owned:
			...
			if the player consents:
				...
				say "You roll 1d20([dice])[if total - dice >= 0]+[end if][total - dice] = [special-style-1][total][roman type] vs [special-style-2]22[roman type] (Charisma Check):[line break]";
				if total > 21:
					...
					add "Tamed" to Traits of helper dog;
					move Hobo to Computer Lab;
					say "[line break]     (Hobo the helper dog is now a possible ally! You can make him your active ally by typing [link][bold type]ally Hobo[roman type][end link] or [link][bold type]ally helper dog[roman type][end link]. You can see all the allies you have with the [link][bold type]allies[roman type][end link] command. Allies will lower the XP you gain from battle, but can gain levels themselves to be more useful in a scrap. Want to get rid of an ally? Use [link][bold type]ally dismiss[roman type][end link], or just [link][bold type]dismiss[roman type][end link].)";
	...
	if helper dog is listed in companionList of Player and skipturnblocker is 0:
		if lastfuck of helper dog - turns >= 4:
			if thirst of Player > 50 and carried of water bottle is 0 and carried of soda is 0 and hobo-water-gift is false:
				...
				if BodySpeciesName of Player is "Human" and FaceSpeciesName of Player is "Human":
				...
				else if FaceSpeciesName of Player is "Human":
					say "     While taking a break, your mind starts to wander into strange, confused thoughts focused on sex and depravity more than survival as your [BodySpeciesName of Player in lower case] instincts affect your mind. You hear a soft whine beside you before Hobo buries his nose in your pack. Thinking that he might be sniffing after some food, you go to grab him, but he instead pulls out your journal and drops it in your lap. Looking down at it, you pat the clever dog on the head in thanks for the reminder. You need to take better care of yourself if you want to get through this.";
				else if BodySpeciesName of Player is "Human":
					say "     While taking a break, your mind starts to wander into strange, confused thoughts focused on sex and depravity more than survival as your [FaceSpeciesName of Player in lower case] instincts affect your mind. You hear a soft whine beside you before Hobo buries his nose in your pack. Thinking that he might be sniffing after some food, you go to grab him, but he instead pulls out your journal and drops it in your lap. Looking down at it, you pat the clever dog on the head in thanks for the reminder. You need to take better care of yourself if you want to get through this.";
				else if BodySpeciesName of Player is not FaceSpeciesName of Player:
					say "     While taking a break, your mind starts to wander into strange, confused thoughts focused on sex and depravity more than survival as a jumble of [BodySpeciesName of Player in lower case] and [FaceSpeciesName of Player in lower case] instincts affect your mind. You hear a soft whine beside you before Hobo buries his nose in your pack. Thinking that he might be sniffing after some food, you go to grab him, but he instead pulls out your journal and drops it in your lap. Looking down at it, you pat the clever dog on the head in thanks for the reminder. You need to take better care of yourself if you want to get through this.";
				else:
					say "     While taking a break, your mind starts to wander into strange, confused thoughts focused on sex and depravity more than survival as a jumble of [BodySpeciesName of Player in lower case] instincts affect your mind. You hear a soft whine beside you before Hobo buries his nose in your pack. Thinking that he might be sniffing after some food, you go to grab him, but he instead pulls out your journal and drops it in your lap. Looking down at it, you pat the clever dog on the head in thanks for the reminder. You need to take better care of yourself if you want to get through this.";
				...
				now hobo-water-reminder is turns;
				now lastfuck of helper dog is turns;
			else if hunger of Player > 40 and carried of food > 0 and hobo-food-reminder - turns >= 12:
				...
				now hobo-journal is turns;
				now lastfuck of helper dog is turns;
			else if HP of Doctor Matt is 2 and carried of gryphon milk < 2 and hobo-grmilk is false and hobo-grmilkhelp - turns >= 8 and FurryList is not banned and HermList is not banned:
