...
"Football Gorilla Male"	"[PrepCombat_Football Gorilla Male]"

to say PrepCombat_Football Gorilla Male:
...
When Play begins:
	...
	now enemy title entry is "Male Football Gorilla"; [ Name of the encountered creature at combat start - Example: "You run into a giant collie." instead of using "Smooth Collie Shemale" infection name. ]
	...
	now face change entry is "it reshapes itself into a gorilla's simian visage, with a wide and very flat nose, pretty thin-lipped mouth and protruding brows over human-like eyes. Short fur sprouts everywhere but the innermost facial area"; [ Face change text, format as "Your face feels funny as [face change entry]." ]
	now body change entry is "it morphs into burly shape of a big gorilla, large-boned and with considerable bulk, but also a set of strong muscles to go with all that. Your arms end up being a bit longer than your legs, which allows for easy walking on all fours from, now on, if you so choose"; [ Body change text, format as "Your body feels funny as [body change entry]." ]
	now skin change entry is "dense black fur spreads rapidly over your form"; [ Skin change text, format as "Your skin feels funny as [skin change entry]." ]
	now ass change entry is "it gets meaty and well-cushioned, perfect to rest a heavy body on when you sit down"; [ Ass/tail change text, format as "Your ass feels funny as [ass change entry]." ]
	...
	now MilkItem entry is "football gorilla male milk"; [ Item to be given to the player if they have this infection and milk themselves. ]
	now CumItem entry is "football gorilla male cum"; [ Item to be given to the player if they have this infection and jerk off. ]
...
[Table of CombatPrep (continued)
name(text)	PrepFunction(text)
"Football Gorilla Cuntboi"	"[PrepCombat_Football Gorilla Cuntboi]"

to say PrepCombat_Football Gorilla Cuntboi:
	setmongender 8;]
...
When Play begins:
	...
	now enemy Name entry is ""; [ Specific name of unique enemy. ]
	now enemy type entry is 0; [ 0 = non unique enemy; 1 = unique (unknown name); 2 = unique (known name) | Used to disqualify unique enemies from Vore/UB and showing the enemy name in encounters. ]
	now attack entry is "[NonCombatError]"; [Text used when the monster makes an Attack]
	now defeated entry is "[NonCombatError]"; [ Text or say command used when Monster is defeated.]
	...
	now loot entry is ""; [ Dropped item, blank for none. Case sensitive. ]
	now lootchance entry is 0; [ Percentage chance of dropping loot, from 0-100. ]
	now MilkItem entry is "football gorilla cuntboi milk"; [ Item to be given to the player if they have this infection and milk themselves. ]
	now CumItem entry is "football gorilla cuntboi cum"; [ Item to be given to the player if they have this infection and jerk off. ]
	now TrophyFunction entry is "-"; [ Function to generate a list of optional loot items, of which the player can choose one after victory. ]
	now scale entry is 3; [ Number 1-5, approx size/height of infected PC body: 1=tiny, 3=avg, 5=huge ]
	now body descriptor entry is "[one of]strong[or]burly[or]powerful[at random]";
	now type entry is "simian";
