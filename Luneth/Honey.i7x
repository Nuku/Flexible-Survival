Version 3 of Honey by Luneth begins here.
[ Version 1 - Original by Stripes ]
...
to say ResolveEvent Smashed Hive:
	if HP of bee girl is 0:
		...
		if medkit is owned:
			say "[line break]     [bold type]Do you try to help the poor bee girl with your medkit or do you grab some honey while you can and make a run for it?[roman type][line break]";
			say "     ([link]Y[as]y[end link]) - Help the bee girl.";
			say "     ([link]N[as]n[end link]) - Grab some honey and run.";
			if Player consents:
				LineBreak;
				if ( "Expert Medic" is not listed in feats of Player and a random chance of 1 in 10 succeeds ) or ( "Expert Medic" is listed in feats of Player and a random chance of 8 in 100 succeeds ) or CurrentMedkitSupplies < 2:
					say "     Taking care not to appear threatening and trying to reassure her that you only want to help, you move slowly over to the bee girl. She buzzes angrily at you, but stumbles and cannot remain upright. Using your medkit, you try your best to patch her up, feeling sorry for this lone survivor. This uses up the last of that medkit, so you hope it turns out to be worth it.";
			...
			say "[line break]     [bold type]Do you try to help the poor bee girl with a healing booster or do you grab some honey while you can and make a run for it?[roman type][line break]";
		...
		if bee girl is listed in companionList of Player:
			say "     Your meandering through the park takes you back to the ruins of the shattered bee hive. The sticky rubble has been picked through since you were last here, the last of the honey having been taken. Even the bodies of the fallen drones are gone - something you don't want to think about too much. Looking down at Honey, who is clutching your side tightly, she could have shared their grisly fate had you not helped her. As you [if scalevalue of Player > 1]kneel down to [end if]hug her, you notice some bear tracks and dried dribbles of honey heading off into the woods. You might be able to track down one of the bears responsible for this, if you want to.";
			say "[line break]     [bold type]Shall you try?[roman type][line break]";
			...
			if Player consents:
				...
				say "[line break]You roll 1d20([dice])[if bonus + featbonus >= 0]+[end if][bonus + featbonus] = [special-style-1][dice + bonus + featbonus][roman type] vs [special-style-2]13[roman type] (Perception Check):[line break]";
				if dice + bonus + featbonus > 12:
					...
					if fightoutcome >= 10 and fightoutcome <= 19:
						...
						turn the Player into a "Queen Bee";
...
Scent of bee girl is "     Honey, unsurprisingly, smells of honey.".

to say DismissHoney:
	move Honey to Grey Abbey Garden;
	say "     Honey [if Player is not in Grey Abbey Garden]presses a quick kiss to your cheek before flying away[else]just shrugs her shoulders, going back to work in the garden[end if].";
...
Cunt Depth of Honey is 4. [penetrable length in inches; some minor stretching allowed, or more with Twisted Capacity]
...
[Conversation of Honey is { "Bzzz!" }.]
fuckscene of Honey is "[SexWithHoney]".
Scent of Honey is "     Honey, unsurprisingly, smells of honey.".
...
instead of conversing the Honey:
	if bee girl is listed in companionList of Player or (Player is in Grey Abbey Garden and Honey is in Grey Abbey Garden):
		say "[HoneyTalkMenu]";
	else:
...
to say HoneyTalk1:
	say "     '[if HP of bee girl is 5][one of]I'm so glad we were able to avenge my hive.'[or]What do you think about the garden? I think it looks just super now!'[at random][else][one of]I really hope we will be able to get justice for my hive soon...'[or]I'm gonna work really hard to get this garden sorted, we need more pretty flowers here!'[at random][end if]";
			...
			if calcnumber > 0 and calcnumber <= the number of filled rows in table of fucking options:
				...
				if Player consents:
					...
					if title entry is:
						-- "Eat out Honey":
							say "[HoneySex1]";
						-- "Get eaten out by Honey":
							say "[HoneySex2]";
						-- "Get blown by Honey":
							say "[HoneySex3]";
						-- "Get fingered by Honey":
							say "[HoneySex4]";
						-- "Fuck Honey":
							say "[HoneySex5]";
						-- "Queen bee fun":
...
to say HoneySex2: [player gets eaten out by Honey]
...
An everyturn rule:
	if bee girl is listed in companionList of Player and skipturnblocker is 0 and Player is not neuter:
...
to say SexWithHoney:
	if lastfuck of bee girl - turns < 4:
		...
		if Player is neuter:
			say "[beesexhoneypot]";
		else if HP of bee girl >= 5 and BodyName of Player is "Queen Bee" and a random chance of 1 in 3 succeeds:
			...
			if T < 3: [1-2]
				say "[if Player is male][beesexmale1][else][beesexfemale1][end if]";
			else if T < 5: [3-4]
				say "[if Player is male][beesexmale2][else][beesexfemale2][end if]";
			else: [5]
				say "[beesexhoneypot]";
		if HP of bee girl >= 5, infect "Queen Bee";
		now lastfuck of bee girl is turns;
...
This is the Honey's Epilogue rule:
	...
	if bee girl is tamed:
		...
		if humanity of Player < 10:
			if Player has a non-shifting body of "Black Equinoid":
			...
			else if Player has a non-shifting body of "Drone Wasp":
