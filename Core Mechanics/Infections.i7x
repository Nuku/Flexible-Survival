...
skipcockchange is a truth state that varies.[@Tag:NotSaved] skipcockchange is usually false.
...
This is the sex change rule:
	...
	if sex entry in lower case is "male" or sex entry in lower case is "both":
		...
		if Cock Length of Player < Cock Length entry and Cock Count of Player is not 0 and Player is not FemalePreferred:
			...
			if Player is MalePreferred or Player is HermPreferred or "Always Cocky" is listed in feats of Player:
		...
		if Ball Size of Player < Ball Size entry and Cock Count of Player is not 0 and Player is not FemalePreferred:
			...
			if prevcock2 < Ball Size of Player:		[did balls actually grow?]
				say "[run paragraph on]";
				follow the cock descr rule;
				say "[line break]You can [if Player is internalBalls]feel your internal[else]see your[end if] [one of]testicles[or]balls[or]orbs[or]nuts[at random] [one of]tingle[or]churn audibly[or]throb[at random] as they grow larger, [if Player is internalBalls]body straining to abide this[else]your skin becoming taut with the[end if] expansion, leaving you with a [one of]pair[or]set[at random] of [Ball Size Adjective of Player] balls!";
		else if Ball Size of Player > Ball Size entry and "One Way" is not listed in feats of Player:
			[Shrinks existing balls, unless "One Way" is selected]
			let prevcock2 be Ball Size of Player;
	...
	if Cock Count of Player is not 0 and "One Way" is not listed in feats of Player: [player currently male]
		if sex entry in lower case is "female" or sex entry in lower case is "neuter":
			...
			if Player is MalePreferred or Player is HermPreferred or "Always Cocky" is listed in feats of Player:
				if Cock Length of Player < 5, now Cock Length of Player is 5;
				if Ball Size of Player < 3, now Ball Size of Player is 3;
			if singlesexadjust is 1 and sex entry in lower case is "female":		[male/herm becomes female if female infection]
				remove manhood from Player;
		...
		else if Cock Count of Player > Cock Count entry and a random chance of 1 in 3 succeeds and "All The Things" is not listed in feats of Player:
			[removes cock if the player has more than the male/herm infection, no "One Way", no "All the Things"]
			say "[line break]Sudden pleasure runs through one of your doomed [Cock of Player] cocks as it sprays the last of its seed, dwindling down to nothing at all and vanishing, leaving only [one of]the powerful[or]that final[at random] orgasm to remember it by.";
			decrease Cock Count of Player by 1;
	if sex entry in lower case is "female" or sex entry in lower case is "both":
		...
		if Cunt Depth of Player < Cunt Depth entry and Cunt Count of Player is not 0 and Player is not MalePreferred:
			...
			if Player is FemalePreferred or Player is HermPreferred or "Always A Pussy" is listed in feats of Player:
		...
		if Cunt Tightness of Player < Cunt Tightness entry and Cunt Count of Player is not 0 and Player is not MalePreferred:
			...
			if prevcunt > Cunt Tightness of Player:		[did cock actually shrink?]
				say "[run paragraph on]";
				follow the cunt descr rule;
				say "[line break]You can feel a [one of]tightening[or]snugness[or]clenching[at random] from your accommodating [if Cunt Count of Player > 1][one of]cunts[or]pussies[or]vaginas[or]clefts[at random][else][one of]cunt[or]pussy[or]vagina[or]cleft[at random][end if] as you are hit by an unexpected orgasm. The squeezing does not release fully as your wet hole[sfn] shrink[sfv] somewhat to better suit your new infection by becoming tighter.";
		[CUNT ADDITION/SUBSTRACTION]
		if Cunt Count of Player < Cunt Count entry and Player is not MalePreferred and singlesexadjust is not 2: [player currently male, single sexed]
	...
	if Cunt Count of Player is not 0 and "One Way" is not listed in feats of Player: [player currently female]
		if sex entry in lower case is "male" or sex entry in lower case is "neuter":
			...
			if Player is FemalePreferred or Player is HermPreferred or "Always A Pussy" is listed in feats of Player:
				if Cunt Depth of Player < 5, now Cunt Depth of Player is 5;
				if Cunt Tightness of Player < 3, now Cunt Tightness of Player is 3;
			if singlesexadjust is 1 and sex entry in lower case is "male":	[female/herm becomes male if male infection]
		...
		else if Cunt Count of Player > Cunt Count entry and a random chance of 1 in 3 succeeds and "All The Things" is not listed in feats of Player:
	...
	if Player is FullyNewTypeInfected: [new parts]
		...
		if Nipple Count of Player is not Nipple Count entry:
			...
			if ( Nipple Count entry > Nipple Count of Player and "One Pair" is not listed in feats of Player ) or ( Nipple Count of Player is 0 and Nipple Count entry > 0 ):
				say "[run paragraph on]";
				follow the breast descr rule;
				say "[line break]Your chest tingles intensely as two new sensitive points form up, announcing the arrival of two new [descr] breasts, pressing out of your [Skin of Player] hide.";
				increase Nipple Count of Player by 2;
			else if Nipple Count entry < Nipple Count of Player and "Bouncy Bouncy" is not listed in feats of Player:
				say "[run paragraph on]";
				follow the breast descr rule;
				decrease Nipple Count of Player by 2;
				say "[line break]You look down just in time to see two nipples, [descr] breasts included, be reabsorbed into your body, leaving nothing but [Skin of Player] flesh behind.";
			increase Breast Size of Player by 2;
		if Nipple Count of Player is 0:
			increase score by 0; [do nothing]
		else if sex entry in lower case is "female" or sex entry in lower case is "both":
			let breast_change_text be "[line break]You [one of]groan and grab at your chest[or]give a loud moan, shuddering[or]almost tip forward in surprise[or]look down fearfully as sensation builds[at random], [Skin of Player] skin glistening as your ";
			if Breast Size of Player < Breast Size entry and ( ( "Male Preferred" is not listed in feats of Player or "Breasts" is listed in feats of Player ) and "Flat Chested" is not listed in feats of Player ):
				[breast growth if smaller than infection size, and "Breasts" and NOT either "Male Preferred" or "Flat Chested"]
				say "[run paragraph on]";
				follow the breast descr rule;
				say "[breast_change_text][descr] breasts [run paragraph on]";
				increase Breast Size of Player by 1;
				increase Breast Size of Player by ( Breast Size entry minus Breast Size of Player ) divided by 3;
				follow the breast descr rule;
				...
				follow the breast descr rule;
				say "become [descr] [one of]orbs[or]breasts[or]jugs[or]tits[at random]!";
		else if sex entry in lower case is "male":
			let breasttarget be male breast size entry;
			let breast_change_text be "You [one of]groan and grab at your chest[or]give a loud moan, shuddering[or]almost tip forward in surprise[or]look down fearfully as sensation builds[at random], [Skin of Player] skin glistening as your ";
...
To grow breasts by (x - a number):
	if "Flat Chested" is listed in feats of Player or Breast Size of Player > 25:
		continue the action;
	say "[line break]You [one of]groan and grab at your chest[or]give a loud moan, shuddering[or]almost tip forward in surprise[or]look down fearfully as sensation builds[at random], [Skin of Player] skin glistening as your [run paragraph on]";
	follow the breast descr rule;
	say "[descr] breasts [run paragraph on]";
...
To Infect:
	...
	if Libido of Player < libido entry:
		...
		raise Player Libido by ( libido entry minus Libido of Player ) divided by 3;
		if oldlib < 80 and Libido of Player > 79:
			say "[line break]You now have trouble thinking of anything but sexual satisfaction[if Player is male]! Your cock[smn] [ismv] fully erect constantly and drool[smv] precum steadily.[else if Player is female]! Your cunt[sfn] overflow[sfv] with hot juices that run down your thighs.[else]![end if]";
		else if oldlib < 50 and Libido of Player > 49:
			say "[line break]Your thoughts have sunk to almost constant depravity[if Player is male]! Your cock[smn] remain[smv] perpetually hard and leaking precum.[else if Player is female]! Your cunt[sfn] [isfv] hot and dripping juices as your arousal builds.[else]![end if]";

To Infect (x - text):
	MultiInfect x repeats 1;
