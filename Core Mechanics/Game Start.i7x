...
To regularstart: [normal start method]
	...
	while Trixieexit is false:
		...
		say "(9) [link]Hyperlinks[as]9[end link] - [bold type][if hypernull is not 1]On[else]Off[end if][roman type][line break]";
		say "(10) [link]Waiting for Input[as]10[end link] - [bold type][if waiterhater is 0]On[else]Off[end if][roman type][line break]";
		say "(11) [link]Screen Clearing[as]11[end link] - [bold type][if clearnomore is 0]On[else]Off[end if][roman type][line break]";
		say "(12) [link]Graphics[as]12[end link] - [bold type][if NewGraphicsInteger is 1]Inline[else if NewGraphicsInteger is 2]Side-Window[else]Disabled[end if][roman type][line break]";
...
to say gsopt_start:
	...
	if NewGraphics is true:
		...
		say "[line break]Please choose the proportion value now. Enter a number between 5 - 90. This will represent the percentage of your main screen that the graphics side-window will take up. We recommend somewhere around 30.";
		now calcnumber is 0;
	...
	if scenario is "Bunker":
		...
		say "[line break]     [bold type]By the time the dust clears, who has won?[roman type][line break]";
	...
	if scenario is "Running with Wolves":
		...
		if MaleList is warded or MaleList is banned or Player is FemalePreferred:
			turn Player into "Feral Wolf Bitch" silently;
		else if FemaleList is warded or FemaleList is banned or Player is MalePreferred:
			turn Player into "Feral Wolf Male" silently;
		else if Player is herm or Player is HermPreferred:
			turn Player into "Feral Wolf Male" silently;
...
to newplayercustomizationmenu:
	...
	if Name of Player is not "DebugTesting": [reduced menu until the new system goes live]
		while charactermenuexit is false:
			...
			say "(3) [link]Body Configuration Lock[as]3[end link] - [bold type][if GenderLock is 1]None[else if GenderLock is 2]Random[else if GenderLock is 3]Unchanging[else if GenderLock is 4]Always Cocky[else if GenderLock is 5]Always A Pussy[else if GenderLock is 6]Single Sexed[else if GenderLock is 7]Flat Chested[else]Simplified Masculine[end if][roman type][line break]";
...
to startgenderget:
	...
	if StartingGender is:
		...
		-- 4: [trans-man - flat chest + cock]
			...
			remove manhood from Player;
			now Breast Size of Player is 0;
		-- 5: [male herm - flat chest + cock + pussy]
			now Cock Count of Player is 1;
			now Cock Length of Player is a random number between 4 and 10;
			now the Ball Size of Player is 3;
			now Cunt Count of Player is 1;
			...
			now Nipple Count of Player is 2;
			now Breast Size of Player is 0;
		-- 6: [male herm - breasts + cock + pussy]
			now Cock Count of Player is 1;
			now Cock Length of Player is a random number between 4 and 10;
			now the Ball Size of Player is 3;
			now Cunt Count of Player is 1;
