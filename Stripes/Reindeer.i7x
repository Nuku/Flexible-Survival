...
When Play begins:
	...
	now Species Name entry is "Reindeer"; [ Name of the overall species of the infection, used so a "male x" and "female x" have "pureblood X" children. ]
	...
	now body entry is "that of an anthropomorphic reindeer with long, slender limbs. You have hoof-like hands and fully hoofed feet"; [ Body description, format as "Your body is [Body of Player]." ]
...
to say reindeerheat:
	...
	if heatturnskipper is true:
		...
		raise Player Libido by a random number between 2 and 5;
		if "Horny Bastard" is listed in feats of Player, raise Player Libido by a random number between 0 and 1;
		if "Cold Fish" is listed in feats of Player, lower Player Libido by a random number between 0 and 1;
		...
		raise Player Libido by a random number between 3 and 8;
		if "Horny Bastard" is listed in feats of Player, raise Player Libido by 1;
		...
		lower Player Libido by a random number between 5 and 10;
...
to say reindeerbreastheat:
	if Breast Size of Player < 2 or Nipple Count of Player is 0:
		...
		if "Cold Fish" is listed in feats of Player, lower Player Libido by a random number between 1 and 3;
