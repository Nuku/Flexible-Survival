Version 5 of Hospital by Stripes begins here.
...
to say ResolveEvent Ambulance Dock:
	...
	say "[line break]     [bold type]Do you proceed?[roman type][line break]";
...
to say ResolveEvent Staff Lounge:
...
to say ResolveEvent Radiology:
	...
	say "[if coonstatus is 0][hospfight1][else][hospfight2][end if]";
...
to say ResolveEvent Cafeteria:
	...
	say "     [if humanity of Player < 30 or Libido of Player > 50]The sight of this decadent meal is quite alluring, but you do manage to draw yourself away before giving in. With so many in there, you doubt you would emerge with your identity intact[else]You turn away from the decadent meal and move back down the hall quietly, eager to get some distance between you and such a large group of creatures[end if].";
	increase score by 1;
...
to say ResolveEvent Records Room:
	...
	say "[if coonstatus is 0][hospfight1][else][hospfight3][end if]";
...
Description of Locked stairwell is "     Off to one side are a pair of heavy, metal doors leading to a stairwell.[line break]".
...
to say hospprogress:
	if hospquest is 1:	[search still active]
		if hospnav is 1:
			say "[line break]     You are starting to learn your way around the hospital and are able to access more of it. You are making progress.";
		else if hospnav is 2:
			say "[line break]     You are figuring out how to get around in here better and better now that you've gotten to reach more landmarks. You still don't think you'll be able to reach that upper corner, though. Best keep at it.";
		else if hospnav > 2:
			say "[line break]     You have gotten quite good at circumventing the blocked rooms and fire doors in the hospital and should be able to get to most places[if hospstairs is 0], but you still need to find a way to get upstairs if you're going to get into those rooms. You should look around for an accessible [bold type]stairwell[roman type][else]. Between that and the open stairwell you've found to get upstairs, you should now be successful at [bold type]finding a way in[roman type] to reach those illuminated upper rooms with a little searching[end if].";
...
Description of Doctor Mouse is "     Dr. Mouse is a slender, white mouse with red eyes and a pink nose and ears. He's dressed in a white labcoat and doctor's attire, with a thin tail poking out behind his back. Most of the time he is busily working away in the lab, always scurrying about for equipment or checking on one of his many experiments.[line break]".
[Conversation of Doctor Mouse is { "Squeak!" }.]
icon of Doctor Mouse is Figure of DrMouse_clothed_icon.
...
Instead of conversing the Doctor Mouse:
	project Figure of DrMouse_clothed_icon;
	if hospquest is 0:
		say "ERROR 0: 'My dear [boygirl], I don't believe you should be here yet.'";
	else if hospquest is 1:
		say "ERROR 1: 'My dear [boygirl], I don't believe you should be here yet.'";
	else if hospquest is 2:
		...
		if FemaleList is banned or MaleList is banned or FurryList is banned or HermList is banned:
			...
			extend game by 16;
			say "[hospquestpt3]";
	else if hospquest is 5:
		say "[hospquestpt4]";
	else if hospquest is 6:
		say "[hospquestpt5]";
	else if hospquest is 7:
		say "[hospquestpt6]";
	else if hospquest is 8:
		say "[hospquestpt7]";
	else if hospquest is 9:
		say "[hospquestpt8]";
	else if hospquest is 10:
		say "[hospquestpt9]";
	else if hospquest is 12:
		say "ERROR 12: 'My dear [boygirl], I don't believe you should be able to get here right now.";
	else if hospquest is 13:
		say "ERROR 13: 'My dear [boygirl], I don't believe you should be able to get back here.'";
	else if hospquest is 14 or hospquest is 15:
		say "[hospquestpt11]";
	else if hospquest is 16:
		say "     'Please proceed to the Zephyr location and make the exchange for the data.'";
	else if hospquest is 17:
		say "     'I need you to finish that wretched task for Zephyr.'";
	else if hospquest is 18:
		say "[hospquestpt12]";
	else if hospquest is 19:
		say "[hospquestpt13]";
	else if hospquest is 20 or hospquest is 21:
	...
	else if hospquest is 22:
		say "[hospquestpt14]";
	else if hospquest is 23:
...
after going to 2F Trevor Labs while hospquest is 2:		[Examining other labs]
...
after going to Primary Lab while hospquest is 2:			[Searching the main lab]
	...
	if HP of Doctor Matt is 100:						[recording]
		...
		say "[line break]     [bold type]Do you explain?[roman type][line break]";
		say "     ([link]Y[as]y[end link]) - Yes.";
		say "     ([link]N[as]n[end link]) - No.";
		if Player consents:
			LineBreak;
...
scientific device is a grab object. It is not temporary.
...
to say hospquestpt9:
	if DoctorMouseProgress minus turns < 12:
		...
		now Staff Lounge is resolved;
		if "Raccoon" is a Name listed in Table of Random Critters: [removes Raccoon monster from Hospital]
		...
		say "[line break]     [bold type]Will you let Dr. Mouse give him the injection or do you want to take him as-is?[roman type][line break]";
...
to say hospquestpt10:
	...
	if HP of Doctor Matt is 100:
		...
		say "[line break]     [bold type]'So,' he asks with a grin, 'will you get that machine for me?'[roman type][line break]";
		say "     ([link]Y[as]y[end link]) - Yes.";
		say "     ([link]N[as]n[end link]) - No.";
		if Player consents:
			LineBreak;
		...
		say "[line break]     [bold type]'So,' he asks with a grin, 'will you get those research notes for me?'[roman type][line break]";
		say "     ([link]Y[as]y[end link]) - Yes.";
		say "     ([link]N[as]n[end link]) - No.";
		if Player consents:
			LineBreak;
...
instead of going to Hidden Lab while hospquest > 10:
	if hospquest is 11:
		...
		say "     You tell the jaguar orderly guarding the stairs that there's been a small snag and that you need to speak with Dr. Mouse. It seems the addition of primitive instincts to the strain has not helped their intellect and you are able to convince him to let you up.";
		now the player is in Hidden Lab;
		...
		say "[line break]You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]10[roman type] (Dexterity Check):[line break]";
		if bonus + dice > 9:
...
after going to Primary Lab while hospquest is 11:		[heading to the main lab to get the feat machine]
	[now the player is in Primary Lab;]
	if HP of Doctor Matt is 100:						[recording]
		...
		say "[line break]     [bold type]Shall you inject Dr. Matt?[roman type][line break]";
		...
		if Player consents:
			...
			if fightoutcome < 20:
				...
				if susan is visible:
					WaitLineBreak;
					say "     You glance back over to the other pair and find Susan finishing up as well. Her ballsack tightens up as she unleashes spurt after spurt of her seed into the dragoness. Her orgasm lasts quite a long time, apparently having a considerable load saved up from lack of use.";
					if HP of Orthas > 1:
					...
					say "[line break]     [bold type]Perhaps you can let her and the new Dr. Matt get reacquainted. Otherwise, you could try locking her up somewhere to keep her out of trouble, letting you keep the new fucktoy for yourself.[roman type][line break]";
					...
					if Player consents:
						...
						WaitLineBreak;
						say "     You set to work gathering up the research material and nanite adjustment system. You load the items onto a cart, adding the infection terminal as well. This work is broken up by occasional glances over at the lustful pair, watching as the huskybunny drives her cock into the muscled dragoness again and again. As you are heading out, you can [if HP of Orthas > 1]see that the dragoness's belly is starting to swell with eggs and her ovipositor is pushing those eggs into the furry slut's womb[else]hear the huskybunny yip loudly, cumming hard into dragoness and stuffing her full of even more cum in an attempt to breed with her[end if].";
						WaitLineBreak;
					else:
						LineBreak;
				...
				say "     While Orthas attempts to treat the doctor, [if susan is visible]Susan starts to recover and slips out of the room to join you in the hall, helping you to get up and get away[else]you manage to get back on your feet and start to head down the hall away towards the exit[end if]. As you make it to the stairwell down, you glance back. Orthas, catching sight of you out in the hall, growls and charges out after you. You run down the stairs quickly and rush out the front door with her in hot pursuit. As you rush down the steps, you are caught in a fiery blast that burns your flesh and sends you tumbling end over end. The black dragoness growls from the doorway and slams it shut violently, heading back up to check on the doctor again.";
				say "     In terrible pain, you struggle to rise, managing to pull out the radio and call the hospital for extraction. [if Susan is visible]Susan, only partially caught in the blast, is only a little better off than you. She helps you[else]With considerable effort, you[end if] get up and put some distance between you and the lab before the dragoness can return to finish the job. You manage to get about a block away before the helicopter arrives, taking you [if susan is visible]both [end if]back to the hospital.";
...
to say hosploss:
	...
	while ( there is a non-infectious entry and non-infectious entry is true ) or there is no Name entry or Name entry is TailName of Player:
		sort Table of Random Critters in random order;
		now MonsterID is 1;
		choose row MonsterID from the Table of Random Critters;
	...
	while ( there is a non-infectious entry and non-infectious entry is true ) or there is no Name entry or Name entry is TailName of Player or Name entry is FaceName of Player:
		sort Table of Random Critters in random order;
		now MonsterID is 1;
		choose row MonsterID from the Table of Random Critters;
	...
	while ( there is a non-infectious entry and non-infectious entry is true ) or there is no Name entry or Name entry is TailName of Player or Name entry is FaceName of Player or Name entry is SkinName of Player:
		sort Table of Random Critters in random order;
		now MonsterID is 1;
		choose row MonsterID from the Table of Random Critters;
	...
	WaitLineBreak;
	say "     You can see that the [if hospfight is 2]monstrous mouse is not immune to the increasing sexual energy in the room, having grown quite hard during the procedure. Another of the vixen nurses is on her knees, licking and kissing the large cock. Her muzzle is pulled onto the thick shaft as Dr. Mouse gets tired of foreplay and starts facefucking her with a low growl. She takes it in eagerly, playing with his heavy balls until he cums[else]albino mouse is not immune to the increasing sexual energy in the room, having grown quite hard during the procedure. His cock is small, perhaps about four inches to match his small height. But his balls are quite large, perhaps each the size of a peach. One of the vixen nurses is on her knees, licking and kissing his cock before taking it into her muzzle. She fondles his balls and sucks his shaft obediently until he sends his ample load[end if] down her throat. Pushing her off after he's done, the last few spurts of his thick seed coat her face and muzzle.";
	while ( there is a non-infectious entry and non-infectious entry is true ) or there is no Name entry or Name entry is TailName of Player or Name entry is FaceName of Player or Name entry is SkinName of Player or Name entry is BodyName of Player:
		sort Table of Random Critters in random order;
		now MonsterID is 1;
		choose row MonsterID from the Table of Random Critters;
	...
	WaitLineBreak;
	if Player is male:
		while ( there is a non-infectious entry and non-infectious entry is true ) or there is no Name entry or Name entry is TailName of Player or Name entry is FaceName of Player or Name entry is SkinName of Player or Name entry is BodyName of Player or Name entry is CockName of Player:
			sort Table of Random Critters in random order;
			now MonsterID is 1;
			choose row MonsterID from the Table of Random Critters;
...
to say posttlscene:
	...
	if HP of Doctor Matt is 101:		[fled/lost fight]
	...
	else if HP of Doctor Matt is 102:		[won fight - did not keep Sally]
		...
		say "     'Now, I have taken the liberty of perusing the hospital files and preparing a false identity from the patient records when the military finally do move into the city[if susan]. I will prepare one for your friend as well[end if]. They are physically similar to you, have no next of kin and won't be turning up to throw a wrench into things.' It seems that the good doctor has been thinking ahead for you.";
		now featunlock is 1;
	else if HP of Doctor Matt is 103:		[took from empty lab]
		...
		WaitLineBreak;
		now featunlock is 1;
	else if HP of Doctor Matt is 104:		[kept Sally]
...
to say ResolveEvent Dinosaur Nest:
	if nesteddino is 0:
		now nesteddino is 1;
		...
		let dice be a random number from 1 to 20;
		say "[line break]You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]12[roman type] (Perception Check):[line break]";
		if bonus + dice > 11:
...
lava lamp is a grab object. It is not temporary.
...
to say smellingthelavalamp:
	if lavalamplick is 0:
		...
		say "[line break]     [bold type]Do you follow your urges?[roman type][line break]";
		say "     ([link]Y[as]y[end link]) - Yes.";
		say "     ([link]N[as]n[end link]) - No.";
		if Player consents:
			LineBreak;
			say "     Following the strange compulsion, you lick the lava lamp a few times, tasting the saurian's female juices on it, finding them quite arousing. For a moment there, it's almost as if you can hear music.";
			...
			LineBreak;
			say "     Probably for the best that you don't do anything so foolish. You'd best be careful if you're starting to get ideas like that.";
	else:
...
to say ResolveEvent Viking Longboat:
	if longboatfind is 0:
		say "     Traveling through the museum, you come to the exhibits dealing with the Middle Ages in Europe and its centerpiece, the Viking longboat. It is a huge ship with a carved dragon at its prow and wooden shields and spears along each side. It is on display in the center of the room, taking up most of the space in the large hall, with a velvet rope barrier around it. You are about to hop the barrier when you realize that you need to find a way to scale the ship and get aboard.";
	...
	if fightoutcome > 29:		[flee]
		...
		if Player is female:
			if a random chance of 2 in 3 succeeds:
				...
				say "     Satisfied that he won't be interrupted, he quickly tears off your remaining clothes and presses his nose against your wet pussy, licking at it before rolling you over. He quickly mounts you, driving his large, brown cock deep inside you. He pants and growls as he pounds into you hard and fast, driving his thick meat in over and over again until he finally releases a hot blast of seed deep inside you.[fimpregchance]";
				say "     As he cums inside you, he grows more gentle, nuzzling you and snuffling at your ear. He gives your neck and shoulder a few nips before dismounting and firmly sending you on your way, forcing you to leave that section of the museum and to head back to the main foyer.";
			else:
		...
		let bonus be ( dexterity of Player minus 10 ) divided by 2;
		let dice be a random number from 1 to 20;
		say "[line break]You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]19[roman type] (Dexterity Check):[line break]";
		if bonus + dice > 18:
...
Instead of conversing the Nermine while nerminepackage is 3:
	...
	say "     [bold type]You gain a package for Dr. Mouse![roman type][line break]";
	ItemGain package by 1;
	if spear is owned:
		say "[line break]     'And you appear to be most lucky, little helper,' she adds, taking note of the spear you have. 'That is spear of great Viking chief who commanded the boat. A rare find, such a thing,' she says with a little greed in her eye, as if trying to come up with a suitable trade to tempt it away from you.";
	increase score by 10;
...
package is a grab object. It is not temporary.
...
instead of using package:
	say "     You ponder the old, string-tied box in your hands, wondering what the doctor's receiving.";
...
to say hospquestpt12:
	...
	say "     Overall, their forms are large and imposing, projecting strength and toughness. It's clear they'd be quite formidable in either a fight or a fuck. The [if Player is submissive]idea of being a big submissive chimera like these is quite alluring to you[else if the player is dominant]idea of being a big chimera like these excites your dominant urges[else]possibility of being such a powerful creature is quite enticing[end if]. You find yourself eager to try it out[if Susan is visible] and Susan seems excited by the prospect as well for some reason[end if].";
	say "[bold type](You've received a boost to your natural armor. You'll also get other combat bonuses and your natural armor boost will be greater while in Enhanced Chimera form, especially while pure.)[roman type][line break]";
	say "[line break]     [bold type]Shall you ask the murine doctor to activate the form?[roman type][line break]";
	say "     ([link]Y[as]y[end link]) - Yes.";
	say "     ([link]N[as]n[end link]) - No.";
	if Player consents:
		LineBreak;
	...
	if "Enhanced Chimera" is a Name listed in Table of Random Critters:
		now area entry is "Hospital";
		now non-infectious entry is false;
		now Cross-Infection entry is ""; [ Infection that this infection will give the player when they lose; can be left empty if they infect with the monster's own strain. ]
	if "Mismatched Chimera" is a Name listed in Table of Random Critters:
		now area entry is "Nowhere";
	now hospquest is 19;
	now DoctorMouseProgress is turns;
...
to say ec_activation:
	...
	if the remainder after dividing ec_fullcount by 4 is 0 and Player is male:
		...
		say "     Focusing your attention on your body, your watch as it shifts, becoming strong and well-toned even as brownish fur spreads across it. Even as the fur continues to spread outwards, that change is overwritten in places as armored plates form to cover the front of your torso. Feeling them, you find these tough, but with enough flex for movement. They [if Nipple Count of Player > 0 and Breast Size of Player > 0]cup your breasts like an armored bodice, both supporting and emphasizing your cleavage while also leaving your nipples exposed for fun[else if Breast Size of Player > 0]They cup your breasts like an armored bodice, both supporting and emphasizing your cleavage[else]They are placed on your chest such to emphasize your strong pecs and firm abs[end if]. Meanwhile, your arms and legs have become strong, growing to match your large, buff body. More toughened plates are added to your outer thighs and forearms as further protection.";
	turn the Player into an "Enhanced Chimera";
	if Player is male:
		if Cock Length of Player < 14:
	...
	if Player is female:
		...
		if Cunt Tightness of Player < 9:
			increase Cunt Tightness of Player by 2;
	if Breast Size of Player > 0 and Breast Size of Player < 6:
...
This is the Hospital Quest Epilogue rule:
	if hospquest is 13 and humanity of Player > 9:			[helped Dr. Matt]
		...
		else if HP of Doctor Mouse is 2:					[Dr. Mouse defeated]
