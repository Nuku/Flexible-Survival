...
to say ResolveEvent Tight Fit:
	...
	if bee girl is listed in companionList of Player:
		...
		say "     Even as you're pondering your chances of getting in there to reach it, Hobo moves around you and crawls into the tunnel. The clever dog comes back out with the [y] in his teeth, dropping it at your feet. Surprised at the dog's cleverness, you scritch his ears.";
		ItemGain y by 1;
		...
		say "     The tunnel looks like it would be [if difficulty < 8]easy[else if difficulty < 10]not too bad[else if difficulty < 12]hard[else if difficulty < 14]very difficult[else]extremely difficult[end if] [if scalevalue of Player is 3]to squirm in and out of[else]for a normal person to squirm in and out of. Being smaller, you've got a better shot at making it[end if].";
		...
		say "[line break]     You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2][difficulty][roman type] (Dexterity Check): ";
...
to say ResolveEvent Protected Supplies:
	...
	repeat with tt running from 1 to number of filled rows in Table of Random Critters:
		...
		if area entry exactly matches the text battleground, case insensitively:
			if BannedStatus entry is true or (DayCycle entry is 2 and daytimer is day) or (DayCycle entry is 1 and daytimer is night):
				next; [skips if banned creature or day/night doesn't match]
			add tt to q;
			if "Like Attracts Like" is listed in the feats of Player:
				if SkinName of Player is Name entry and a random chance of 1 in 2 succeeds:
					add tt to q;
				if BodyName of Player is Name entry and a random chance of 1 in 2 succeeds:
					add tt to q;
				if CockName of Player is Name entry and a random chance of 1 in 2 succeeds:
					add tt to q;
				if FaceName of Player is Name entry and a random chance of 1 in 2 succeeds:
					add tt to q;
	if q is not empty:
		sort Q in random order;
		now MonsterID is entry 1 of Q;
		...
		WaitLineBreak;
		let scavfight be false;
		if a random chance of 1 in 2 succeeds:
			say "     But as you move to get it, a [Name entry in lower case] moves to cut you off, probably wanting to take it for themselves. The creature charges to attack before you have a chance to consider falling back.";
			let scavfight be true;
		else:
			say "     But as you move to get it, a [Name entry in lower case] moves to cut you off, probably wanting to take it for themselves. The creature remains watchful, but keeps you from moving towards your desired supplies.";
	...
	if Player consents:
		...
		if T < 41: [1-40]
			...
			repeat with X running from 1 to number of filled rows in Table of Random Critters:
				...
				if area entry exactly matches the text battleground, case insensitively and ( there is no non-infectious entry or non-infectious entry is false ):
					now MonsterID is X;
					break;
			choose row MonsterID from the Table of Random Critters;
			infect;
...
to say ResolveEvent Dented Bike:
	...
	if "Three Bags Full" is listed in feats of Player, increase bonus by 1;
	let dice be a random number from 1 to 20;
	say "[line break]     You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2][difficulty][roman type] (Perception Check): ";
	if bonus + dice >= difficulty:
		if a random number between 1 and 3 is:
			-- 1:
				say "You spot something under one of the shredded rags - it's an unopened bottle of water in a holder on the bike's frame! Looking around, you find a few more tufts of fur and some messy splatters of cum leading away, but that's it.";
				ItemGain water bottle by 1;
			-- 2:
				say "You see a water bottle poke out from under one of the wheels. Its sports cap seems to have popped off during whatever happened here and part of the contents have run out. You look around a bit until you find it, then wipe the cap off as good as you can and put it back on the bottle. Looking around, you find a few more tufts of fur and some messy splatters of cum leading away, but that's it.";
				ItemGain dirty water by 1;
			-- 3:
				say "A small bulge in one piece of the shredded clothing attracts your attention. It's the pocket of a jacket, revealing a tasty chocolate bar. Looking around, you find a few more tufts of fur and some messy splatters of cum leading away, but that's it.";
				ItemGain chips by 1;
	if dentedbikecount < 3:
		increase dentedbikecount by 1;
	else:
		now Dented Bike is resolved;
...
to say ResolveEvent Looted Supermarket:
	...
	if a random number between 1 and 4 is:
		...
		-- 2:
			...
			if "Three Bags Full" is listed in feats of Player, increase bonus by 1;
			let dice be a random number from 1 to 20;
			say "[line break]     You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2][difficulty][roman type] (Intelligence Check): ";
			if bonus + dice >= difficulty:
		...
		-- 4:
			...
			if crowbar is owned:
				increase bonus by 3;
			let dice be a random number from 1 to 20;
			say "[line break]     You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2][difficulty][roman type] (Strength Check): ";
			if bonus + dice > difficulty:
				say "You break open the door to a small room in the back[if crowbar is owned] with the help of your crowbar[end if]. Aww - it's just the employees['] lockers and some cleaning supplies. At least one of them had a bag of chips in his locker.";
				ItemGain chips by 1;
			else:
				say "You wander elsewhere to look for supplies, but aren't able to find any on this trip.";
	if supermarketcount < 3:
		increase supermarketcount by 1;
	else:
		now Looted Supermarket is resolved;
...
to say ResolveEvent Plains Scavenging:
	...
	if a random number from 1 to 3 is:
		...
		-- 2:
			let difficulty be a random number from 6 to 16;
			if HardMode is true, increase difficulty by a random number between 0 and 3;
			say "     Sighing in annoyance you notice that there are some mutants lording over a few items, the [y] among them. Seeming peaceful enough folks, you head on over. The one in charge looks up at you with a grunt of annoyance at the interruption. From the looks of the creature, he should be [if difficulty < 8]easy[else if difficulty < 10]not too bad[else if difficulty < 12]hard[else if difficulty < 14]very difficult[else]extremely difficult[end if] to contend with.";
			...
			if "Three Bags Full" is listed in feats of Player, increase bonus by 1;
			let dice be a random number from 1 to 20;
			say "[line break]     You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2][difficulty][roman type] (Charisma Check): ";
			increase dice by bonus;
			if difficulty > dice:
				...
				say "Coming to a mutually beneficial agreement, you find that the other mutants are willing to give up the item to you in trade for some information. It would seem that there are still some decent [']people['] in the world after all[if petbonus is true]. Though having an intimidating guard by your side may have helped a little[end if]. You quickly add the [y] to your satchel and then bob your head gratefully to the others before going about your way.";
				ItemGain y by 1;
		-- 3:
			let difficulty be a random number from 6 to 16;
			if HardMode is true, increase difficulty by a random number between 0 and 3;
			say "     It is perched precariously among some rubble of a collapsing building. It looks [if difficulty < 8]easy[else if difficulty < 10]not too bad[else if difficulty < 12]hard[else if difficulty < 14]very difficult[else]extremely difficult[end if] to traverse.";
			if bee girl is listed in companionList of Player:
...
to say ResolveEvent Garden Veggies:
	if FemaleList is banned and MaleList is banned:
		...
		if fightoutcome >= 10 and fightoutcome <= 19:			[player victory]
			if MaleList is banned or (Player is FemalePreferred and FemaleList is not banned):
				say "     Having beaten the ewe, you are startled as a loud bang goes off and a chunk of the fence beside you is blown to pieces. Another sheep, a ram this time, comes out wielding a shotgun. 'Now you back away from my husband... er... wife... right now, or I won't miss next time.' You don't need to be told twice and flee the garden. You'll have to look elsewhere for food from now on.";
			else:
				say "     Having beaten the ram, you are startled as a loud bang goes off and a chunk of the fence beside you is blown to pieces. Another sheep, a ewe this time, comes out wielding a shotgun. 'Now you back away from my wife... er... husband... right now, or I won't miss next time.' You don't need to be told twice and flee the garden. You'll have to look elsewhere for food from now on.";
			...
			if MaleList is banned or (Player is FemalePreferred and FemaleList is not banned):
				...
				say "     As you struggle to get back up after having been beaten by the sheep, you are startled as another sheep, this time a ram comes out wielding a shotgun. Seeing the gun, you turn and flee before her angry mate can shoot you. You'd best not return there... for a whole bunch of reasons.";
				CreatureSexAftermath "Player" receives "OralPussy" from "Ewe";
			else:
				say "     As you struggle to get back up after having been beaten by the sheep, you are startled as another sheep, this time a ewe comes out wielding a shotgun. Seeing the gun, you turn and flee before his angry mate can shoot you. You'd best not return there... for a whole bunch of reasons.";
...
to say ResolveEvent Patient Rooms:
	...
	if patrooms is 0 and a random chance of 1 in 3 succeeds:	[blocked room]
		...
		if "Three Bags Full" is listed in feats of Player, increase bonus by 1;
		let dice be a random number from 1 to 20;
		say "[line break]     You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]14[roman type] (Strength Check): ";
		increase dice by bonus;
		if dice >= 14:
...
to say ResolveEvent Shattered House:
	...
	if x < 3: [1-2]
		say "     As you travel along, you are forced to cut down a side street to avoid a small cluster of creatures lustfully fucking in the road up ahead. Down this street, you pass a few houses, including one that seems to have been partially blown out from the inside. Given the lack of fire damage and excessive amounts of sticky fluids, you'd guess that whoever once lived in there became very large very quickly. Taking a quick look inside, you spot the partially destroyed kitchen. While large sections of it have been crushed, you do get lucky and find some food you can recover and take with you.";
		ItemGain food by 1;
	else if x is 3: [3]
		...
		if "Three Bags Full" is listed in feats of Player, increase bonus by 1;
		let dice be a random number from 1 to 20;
		say "[line break]     You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]16[roman type] (Dexterity Check): ";
		increase dice by bonus;
		if dice >= 16:
		...
		if "Three Bags Full" is listed in feats of Player, increase bonus by 1;
		let dice be a random number from 1 to 20;
		say "[line break]     You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]14[roman type] (Perception Check): ";
		increase dice by bonus;
		if dice >= 14:
