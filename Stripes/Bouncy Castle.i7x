...
to say bcfencingroom:
	...
	if bcswordplay is false:		[swords active - foam sword fight!] [FLOT - DONE]
		if flotmarked is true and a random chance of 1 in 2 succeeds:
			say ".  Scattered on the floor are several foam swords which, as you pause for a moment to look at them, start to rise up into the air on their own.  They wildly swing about the air for a bit, rarely ever getting close to you, before finally exhausting themselves and tumbling to the ground once more.";
...
to say bcgreathall:
	...
	if flotmarked is true and a random chance of 1 in 2 succeeds:
		say "     The orca tries to charge you again, but seems to have trouble aiming at you and misses completely, bouncing harmlessly against a wall! You have a quick moment to decide which direction you'd like to go.  Shall you head through the door to the [link]north (Y)[as]y[end link] or head back the way you came by going [link]west (N)[as]n[end link] back to the play room?";
		if the player consents:
		...
		otherwise:
			...
			if lastorcasex - turns < 4:
				...
				if cocks of player > 0:
					...
					say "     Once you're drained and released, she resumes her game of bumping you around, as if the sex and the shoving are all part of the same game to her.  Too dazed from the lustful ";
				otherwise:
					...
					decrease humanity of player by a random number between 0 and 3;
					say "     Once she's finished having her fun with you and released, she resumes her game of bumping you around, as if the sex and the shoving are all part of the same game to her.  Too dazed from the heady scent of her ";
				now lastorcasex is turns;
			say "sex and too many blows, you're eventually rammed out the door you just entered through.  You roll and come to a stop on the soft floor, knocked particularly hard by the rubber inflatable.  You are left a little dizzy and confused from your tumble.  It takes you a while to recover, but you seem otherwise unharmed.";
...
to say bcpunchingpillars:
	...
	if flotmarked is true and a random chance of 1 in 2 succeeds:
		say "start to rock [if bcseenpunchingpillars is true]again[otherwise]a little[end if] before thick tendrils of gooey latex flow out of them. They seem to fumble around a bit, but even as they occasionally graze past you, they seem wholly unaware of your presence... You should probably make a break for it before they finally lock onto your [']scent[']. You have two options for exit from this room.  Shall you head through the door to the [link]north (Y)[as]y[end link] or head back the way you came by going [link]east (N)[as]n[end link] back to the ball pit?";
		if the player consents:
...
to say bcpptendril:
	...
	otherwise:
		...
		if cunts of player > 0:
			say ".  You feel an uncontrolled wave of pleasure as liquid latex flows into your vagina[sfn] and asshole, as if trying coat you inside and out[if cocks of player > 0] while it ripples and flows over your hard manhood[smn][end if].";
		otherwise:
			say ".  You feel an uncontrolled wave of pleasure as liquid latex flows into your rectum, as if trying coat you inside and out[if cocks of player > 0] while it ripples and flows over your hard manhood[smn][end if].";
		let playernum be ( 150 + humanity of player + level of player + strength of player - libido of player);
		...
		otherwise:
			...
			otherwise if cocks of player > 0:
				say "squeezes and pumps at your [cock size desc of player] [cock of player] cock[smn], milking your man meat until you cum with a loud groan[if anallevel is not 1].  As you're cumming, you are also vaguely aware of the gooey flow working its way further into your bowels[end if].";
			otherwise if cunts of player > 0:
			...
			otherwise:
				say "pumps and pushes inside your rectum, sending debilitating waves of pleasure through you[if anallevel is not 1].  As this orgasmic rush has your body quivering, you can feel the gooey flow working its way further into your bowels[end if].";
...
to say bcpptendril_accept:
	...
	if cunts of player > 0:
		say ".  You feel an unrestrained wave of pleasure as liquid latex flows into your vagina[sfn] and asshole, as if trying coat you inside and out[if cocks of player > 0] while it ripples and flows over your hard manhood[smn][end if].";
	otherwise:
		say ".  You feel an unrestrained wave of pleasure as liquid latex flows into your rectum, as if trying coat you inside and out[if cocks of player > 0] while it ripples and flows over your hard manhood[smn][end if].";
...
to say bcwestparapets:
	...
	if flotmarked is true and a random chance of 1 in 2 succeeds:
		...
		say "     In any case you better bolt out of here, lest they change their mind. Shall you head [link]northeast (1)[as]1[end link] to the [if bcseenthroneroom is true]throne room[otherwise]next room[end if], [link]east (2)[as]2[end link] back to the upper hall or [link]take the slide (3)[end link] to leave this place?";
		now calcnumber is 0;
		while calcnumber < 1 or calcnumber > 3:
...
to chairboundstate:
	...
	while trixieexit is 0:
		...
		if lustatt > 99:
			if boundsegment < 2: [Legs and possibly arms bound]
				...
				if boundsegment is 1:
					say "[if cocks of player > 0]cock[smn] sputtering your [cum load size of player] load against the padded seat[otherwise if cunts of player > 0]needy cunt[smn] staining the padded seat with your honey[otherwise]writhing against the padded seat in insatiable lust[end if]. As if empowered by your sensitivity, the trap's grip tightens even further around you[if enduring is true], forcing you to try and keep it at bay[end if].";
				otherwise:
					say "[if cocks of player > 0]cock[smn] sputtering your [cum load size of player] load across the latexy floor[otherwise if cunts of player > 0]needy cunt[smn] staining the latexy floor with your honey[otherwise]writhing in insatiable lust[end if]. As if empowered by your sensitivity, the trap's grip tightens even further around you[if enduring is true], forcing you to try and keep it at bay[end if].";
			otherwise if boundsegment is 2: [Being gassed]
				say "     Arousing gas finally setting you off, you succumb to your need within this trap's grip, [if cocks of player > 0]cock[smn] sputtering your [cum load size of player] load across the latexy floor[otherwise if cunts of player > 0]needy cunt[smn] staining the latexy floor with your honey[otherwise]writhing in insatiable lust[end if]. As if empowered by your orgasm, the trap's grip tightens even further around you[if enduring is true], forcing you to weakly try and keep it at bay[end if].";
			otherwise: [being fully stimulated]
				say "     The trap's powerful influence too much to take, you quickly cry out in bliss, muffled by your mask. [if cocks of player > 0]Cock[smn] loudly being milked of your [cum load size of player] load by the greedy tubes[otherwise if cunts of player > 0]Stuffed cunt[smn] throbbing against the slick tendrils[otherwise]Genderless body constantly being teased by the vinyl tendrils[end if], your orgasm only empowers them in abusing you even harder, eager to set you off once more!";
			now lustatt is libido of player;
			if enduring is true:
		...
		otherwise if boundsegment is 2:
			if boundcounter > 2:
				say "     Blatantly aroused from breathing in the aphrodisiac, the motion of the strange chair increases[if cocks of player > 0]. Your stiffening, [cock size desc of player] cock[smn] slide into slick orifices which form in the seat beneath you, which squeezes and milks at your erection[smn], increasing your pleasure and making it harder to stay focused[end if][if cunts of player > 0 and anallevel is 3]. You feel some of the rubbery material bulge up and push its way into your damp puss[yfn] and asshole.[run paragraph on] Inflating slowly, you're filled with the pulsing dildos that shiver delightfully inside you from the chair's vibrations[otherwise if cunts of player > 0]. You feel some of the rubbery material bulge up and push its way into your damp puss[yfn].[run paragraph on] Inflating slowly, you're filled with the pulsing dildo[sfn] that shiver delightfully inside you from the chair's vibrations[end if][if cocks of player is 0 and cunts of player is 0]. The chair's material probes at your bare groin and, finding no genitals, forms rippling protrusions which stroke and rub against your flat crotch, sending soft shivers of delight through you[end if][if cunts of player is 0 and anallevel is not 1]. You feel some of the rubbery material bulge up and push its way into your quivering asshole. Inflating slowly, you're filled with the pulsing dildo that shivers delightfully inside you from the chair's vibrations[end if]. Now fully abused, you don't think you'll be able to last much longer!"; [penetrated]
				now boundsegment is 3;
		now enduring is false;
		say "     Legs[if boundsegment > 0] and arms[end if] lodged firmly within tightly-gripping holes[if boundsegment > 0] as you're forced to sit against a strange, blue [one of]chair[or]seat[at random][end if], [if boundsegment > 1]your head is buried in a latex-like hood, forced to inhale arousing gas as you fellate a phallic-shaped tendril[otherwise]they continue to tighten around your limbs[end if]";
		if boundsegment > 2:
			say ". [if cunts of player > 1]Holes persistently pounded by the rubbery material[otherwise if cunts of player > 0 and anallevel is 3]Holes persistently pounded by the rubbery material[otherwise if cunts of player > 0 or anallevel is not 1]Hole persistently pounded by the rubbery material[otherwise]Body caressed by the rubbery material[end if], your [if cocks of player > 1]cocks are perpetually squeezed and milked by the others[otherwise if cocks of player > 0]cock is perpetually squeezed and milked by the other[otherwise][bodytype of player] form is continually teased by the chair's vibrations[end if], driving you ever-closer to bliss";
		...
		say "Sanity: [humanity of player]/ 100	Lust: [lustatt]/100	Struggle: [chairstrugglebar][line break][run paragraph on]";
...
to say chairstrugglebar:
	...
	if boundsegment > 2:
		say "-";
	if boundsegment > 1:
		say "[if struggleatt > 4]X[otherwise]-[end if]";
	if boundsegment > 0:
		say "[if struggleatt > 3]X[otherwise]-[end if]";
	say "[if struggleatt > 2]X[otherwise]-[end if][if struggleatt > 1]X[otherwise]-[end if][if struggleatt > 0]X[otherwise]-[end if][bracket]";
			...
			otherwise:
				...
				if keychar in lower case exactly matches the text "s" or keychar in lower case exactly matches the text "1" or keychar in lower case exactly matches the text "return" or keychar in lower case matches the text "struggle":
					...
					if dolphinbundle is 0:
						increase boundmod by 1;
					otherwise if dolphinbundle is 1 and a random chance of 1 in 2 succeeds:
						increase boundmod by 1;
					wait for any key;
					next;
				otherwise if (obliging is true and (keychar in lower case exactly matches the text "o" or keychar in lower case matches the text "oblige")) or (obliging is false and (keychar in lower case exactly matches the text "a" or keychar in lower case matches the text "abide")) or keychar in lower case exactly matches the text "2":
...
to say bottlestrugglebar:
	...
	if dolphinbundle is 2:
		say "[close bracket][if struggleatt is 3]X[otherwise]-[end if][if struggleatt > 1]X[otherwise]-[end if][if struggleatt > 0]X[otherwise]-[end if][bracket]";
	otherwise if dolphinbundle is 1:
		say "[close bracket][if struggleatt > 1]X[otherwise]-[end if][if struggleatt > 0]X[otherwise]-[end if][bracket]";
	otherwise:
		say "[close bracket][if struggleatt > 0]X[otherwise]-[end if][bracket]";
...
to say dolcheckA:		[empty] 
	...
	otherwise:
		now tempnum2 is 5;
	say "     The strange suit starts pulling itself overtop of you, letting its slick inner surface slide across your [skinname of player] skin as it works to enclose your [bodydesc of player] body as best it can [if tempnum2 is 5]without assistance[otherwise]in spite of your protests[end if].  As the material is stretched over you, it feels very good and quite arousing, squeezing and rubbing against you as it does.  It seems to stretch and adjust itself to be just the right shape and size to contain you.";
	...
	if dolphinbundle is not 2 and dolescape is 0: [Standard Eject]
		...
		now dolcastleturn is 0;
		follow the turnpass rule;
	otherwise if dolphinbundle is 2 and dolescape is 0: [After third attempt, player succumbs]
		say ".";
		attempttowait;
...
[			say " [special-style-2]Fail[roman type].";]
		...
		if bcchairescape < 2:
			say "     Even as the vixen's capsule sinks lower, the sex chair stimulates you without stop - it's motion against you, the feel of its [if cunts of player > 0]probes[otherwise]probe[end if] throbbing inside you[if cocks of player > 0], those gripping holes around your maleness[end if] - all work without tire to pleasure you[if cocks of player > 0].  Your man meat pulses and throbs as the rubbery flesh squeezes and milks at you until you finally cum hard.  You groan around that phallic protrusion in your mouth as your balls are drained and the pleasure of orgasm overtakes you[end if][if cunts of player > 0].  Those throbbing probes inside you inflate to fill you to your limits and even a little further each time they pulse inside you, sending waves of delight through you.  You soak them and seat beneath you with your juices as you climax[end if][if cunts of player is 0 and cocks of player is 0].  Your body is rubbed and rocked against the seat beneath you, the smooth surface sliding wonderfully against your body while those stimulating bumps send shivers through your bare groin, making you cry out in delight[end if].  You pant and sag down atop the seat as the pleasure of your afterglow fills you.";
...
[					say " [special-style-2]Fail[roman type].";]
	...
	if bcchairescape is 2:	[escape]
		say "     Your effort finally pays off as you're able to pull your [one of]left[or]right[purely at random] arm free[if bcchairstage is 2].  You grab the back of the mask, stretching the rubbery straps until you're able to pull them off your head[otherwise].  You grab the back of the mask and pull it from your face, popping the thick, rubbery phallus from your mouth[end if].  Gasping in the fresh sea air, your head starts to clear and you turn your focus to getting out of this trap.  Grabbing the inflated tube around your other hand, you squeeze it firmly, partially deflating it.  When you release it, it is looser momentarily, allowing you to pull your arm free.  You do the same for your legs and push yourself off the seat before it can devise some new means to hold you";
		if bcchairstage is 2:		[fast escape]
			say ".";
			say "     You quickly rush over to the seat trying to engulf the vixen and repeat the same means of escape for her.  At first, she's too focused on trying to enjoy the chair to realize what's going on and you have to give her ass a squeeze and her muzzle a kiss after pulling off her mask to get her to focus on you long enough for you to remind her that you both need to leave before you're caught again.  Nodding in agreement, she pulls her limbs free after you deflate each of the pink tubes.  Once released, you help her off the seat, a wet, slick pop coming as the inflatable dildo inside her is pulled out[if compnumber > 1].  You then proceed to assist your companions, who have been making headway in freeing themselves thanks to the distraction your escape has caused and finish pulling them free[end if].";
			say "[bcfinalsuccess]";
		otherwise if bcchairstage > 2 and bcchairstage < 5:		[delayed escape]
			say "[if cocks of player > 0 or cunts of player > 0].  There is a wet, slick sound as you pull your groin free of the sticky seat and its stimulating devices[end if].";
			say "     You quickly rush over to the seat trying to engulf the vixen and repeat the same means of escape for her.  At first, she's too focused on trying to enjoy the chair to realize what's going on and you have to give her ass a squeeze and her muzzle a kiss after pulling off her mask to get her to focus on you long enough for you to remind her that you both need to leave before you're caught again.  Nodding in agreement, she pulls her limbs free after you deflate each of the pink tubes.  Once released, you help her off the seat, a wet, slick pop coming as the inflatable dildo inside her is pulled out[if compnumber > 1].  You then proceed to assist your companions, who have been making headway in freeing themselves thanks to the distraction your escape has caused and finish pulling them free[end if].";
			say "[bcfinalsuccess]";
		otherwise:		[too late]
...
to say bcchairfinal2:
	...
	say "     Those tubes pause briefly in their growth at her elbows and knees. For a moment, you think they're gathering pressure before continuing, but you notice that the activity's not stopped entirely.  Instead, the seat at her crotch is growing [if anallevel is 3]a pair of phallic bulges of its own, extending them into its victim's juicy cunt and stretchable asshole[otherwise]a phallic bulge of its own, extending it into its victim's juicy, stretchable cunt[end if], drawing fresh moans of delight from her.  These rubbery cocks pulse and vibrate as the chair fucks the latex vixen, keeping her lustfully distracted as it resumes encapsulating her.  You moan in pleasure at the sight, grinding your groin firmly down onto the seat, eager for some stimulation of your own.  In your lust-addled state, you find yourself hoping you'll get your turn soon.";
	attempttowait;
...
when play ends:
	...
	otherwise if bodyname of player is "Captured" and bcending is 2:
		...
		otherwise:
			say ".  Still sharing some connection with your body, you get to enjoy all the physical pleasures the castle provides, more directly sharing them with your united consciousness.";
