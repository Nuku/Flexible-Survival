...
to say losetotigress:
	...
	if hp of player > 0:									[DID THE PLAYER SUBMIT? - YES]
		...
		if cunts of player > 0 and cocks of player > 0:				[Yes = HERM Identified!]
			...
			if diceroll < libido of player:
				...
				otherwise:								[No = wants pussy, fucks the kitty]
					say "[line break]     With a soft moan, you rub over your [cockname of player] [if cocks of player is 1]cock, keeping it [otherwise]cocks, keeping them [end if]from growing fully soft and coaxing your erection back.  'Now there, sexy kitty.  I[']m not done with you yet.'  With a lustful smile back down at you, shi licks hir lips and moves to straddle your hips.";	[Male sex if fail check]
					if cock length of player < 16:			[Cock size check - not huge]
						...
						if cocks of player > 2:
							...
							say "     Hir breasts bounce as shi rides in your lap and hir cock slaps against your belly, drooling precum onto it.  Shi kneads hir left paw over your chest and rumbles louder and louder as shi rides you hard.  Hir right finds it way under hir to stroke your [if cunts of player is 1]pussy. [otherwise]pussies. [end if] You can feel hir hot juices leaking down over your [cock of player] and feminine folds as hir pussy and anus squeeze and milks at them.";
							say "     Having already blown your load once, you have a good long screw with the eager feline whore.  Hir cock blasts hot feline seed across your chest twice to add to the mess of you own seed already there, as if marking you as one of hir clients.  The quivering of hir vaginal and anal muscles around you as shi climaxes the second time is enough to finally push you over the edge as your hot cum into hir as well.  Shi yowls lustfully as your hot seed flows into hir, filling both hir womb and hir bottom with your warmth.  Your third cock shoots fresh cum across your chest, leaving you thoroughly soaked in a mix of the tigress[']s seed and your own.";
		...
		otherwise if cunts of player is 0 and cocks of player > 0:		[Yes = MALE Identified!]			
			...
			let diceroll be a random number from 1 to 125;			[lust check vs 125]
			if diceroll < libido of player:
				say "[line break]     As the tigress finally moves hir head back, shi smiles up at you.  You smile back at hir dazedly, still woozy in the blissful haze of the afterglow, but still feeling quite horny.  With a soft moan, you rub over your [cockname of player] [if cocks of player is 1]cock, keeping it [otherwise]cocks, keeping them [end if]from growing fully soft and coaxing it back to erection.  'Now there, sexy kitty.  I[']m not done with you yet.'  With a lustful smile back down at you, shi licks hir lips and moves to straddle your hips.";
	...
	if the player consents:									[Decides to fuck tigress]
		if cunts of player > 0 and cocks of player > 0:				[Yes = HERM Identified!]
			...
			otherwise:									[Cock size check: huge!]
				say "[line break]     As the tigress mewls hir need again, you decide to fill that lovely, striped muzzle and move to straddle hir chest.  Shi smiles up at you, licking hir lips even as you move into position.  Catching a flash of hir pointed teeth, you momentarily rethink your idea before shi grabs your hips and pulls you closer so hir raspy tongue can glide over your swollen member[if cocks of player > 1]s[end if].";
				say "     Given the size of your [cock size desc of player] [cock of player] member[if cocks of player > 1]s[end if], you won[']t be able to take full advantage of the tigress[']s muzzle, so you go for something almost as good.  Sitting across hir hips, you lay your lengthy cock[if cocks of player > 1]s[end if] across hir chest, nestling [if cocks of player is 1]it[otherwise]them[end if] between hir breasts.  Shi starts rubbing hir body against [if cocks of player is 1]it[otherwise]them[end if], rumbling with desire.  Shi licks and kisses at your tip[if cocks of player > 1]s[end if], lapping up your precum with a hunger for more.  Hir raspy, feline tongue is very stimulating, as is the warm, soft feel of hir bosom around your shaft[if cocks of player > 1]s[end if].  Hir well-trained paws stroke over the lower portion of your cock[if cocks of player > 1]s[end if] and fondle your balls, working to attend to as much of your as shi can manage.";
			...
			if diceroll < libido of player:
				...
				otherwise:								[No = wants pussy, does the fucking]
					say "[line break]     Deciding you want to pound hir dripping puss with your throbbing cock, you feel you[']re still ready for a second go and nudge the tigress over.  Shi rolls onto all fours eagerly, arching up hir tail and wiggling hir rear in anticipation.  You eye hir sexy rump, running your hands over it, looking at hir hot, dripping pussy.  That[']s more than enough to get you fully hard again and you move into position atop hir.";
					if cock length of player < 16:			[Yes = not huge cock]
...
to say paymentcheck:
	...
	otherwise if soda is owned:
		say "     Finding a can of soda, shi smiles happily.  'Hey!  This is my favorite flavour.  I haven[']t been able to find one for days.'  Keeping hir find as payment, shi sets your backpack aside.";
		delete soda;
	...
	otherwise if chips is owned:
		say "     Finding a bag of chips, shi smiles. 'Hey!  This is my favorite flavour.  I thought they[']d discontinued it, but you found one.'  Keeping hir find as payment, shi leans down and gives you a kiss on the cheek for bringing hir such a tasty treat.";
		delete chips;
