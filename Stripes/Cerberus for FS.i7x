...
to say beatthecerberus:
	...
	if facename of player is "Cerberus":
		...
		now godoggygo is 1;
	otherwise if a random number between 25 and 125 < libido of player:
		say ".  You find yourself oddly attracted to the lustful herm beast and the many possibilities for enjoyment hir body offers.  Shall you satisfy your lusts on hir wonderous body?";
	...
	if godoggygo is 1:
		...
		if cocks of player > 2 and cock length of player > 5:		[3+ COCKS ORAL]
			say ".  As your many cocks throb, you ponder hir three muzzles.  This would certainly be a rare opportunity to orally satisfy several of them at once.  Shall you have your three-headed bitch service your cocks with hir many muzzles?";
			if the player consents:
				say "[cerberus3oral]";
				continue the action;
			otherwise:
		...
		if cocks of player > 0 and cunts of player > 0:			[HERM]
			say ".  Being dual-equipped yourself, would you prefer to slake your urges by fucking hir or riding that hard cock between hir legs? (Y=fuck hir, N=ride cock)";
			if the player consents:
				say "[cerberusfuck]";
			otherwise:
				say "[cerberusride]";
		otherwise if cocks of player > 0:					[MALE]
			say ".  As you continue to stroke hir cock and finger hir pussy, enjoying listening to hir moan lustfully, you try to decide which of them you'd prefer.  Do you want to slake your urges by fucking hir or riding that hard cock between hir legs? (Y=fuck hir, N=ride cock)";
			if the player consents:
				say "[cerberusfuck]";
			otherwise:
				say "[cerberusride]";
...
to say cerberusbodytf:
	...
	say "The sensation of change continues through you, spreading into your limbs as they grow stronger and more muscled.  You are able to switch from standing upright to a quadrapedal stance easily.  Your hands become canine hands with wicked claws and your feet turn into large, padded paws to carry your heavy body.  Your strong limbs can carry you swiftly on all fours, allowing you to charge down and pounce upon your prey";

to say cerberustailtf:
	if cocks of player > 0:
		say "your ass suddenly emits a sharp, painful CRACK as the bones spontaneously and painlessly break. Your hips quiver a little, but you otherwise manage to hold your position as more cracks occur as a new tail forms behind you.  It shifts and grows into a thick, demonic tail.  At the same time, your groin burns with pain as does the end of your new tail as a small slit appears on the very end of it.  You look down and realize that your [if cocks of player is 1]penis[otherwise]penises[end if] as well as your testicles have vanished from your crotch, leaving a smooth surface behind.  You look at your tail again just in time to see your [if cocks of player is 1]penis[otherwise]penises[end if] emerge from the slit at the end of your tail, spurting virile seed ecitedly onto the ground as [if cocks of player is 1]it[otherwise]they[end if] settles into place.  [if cocks of player is 1]It is[otherwise]They are[end if] exactly the same size and form as before the shift in position.  You can feel your balls spreading out in the tail's tissue as well, equal in size as before, but spread out across the whole of your tail to hide them while keeping them ready to perform their duty when needed.  But you have a feeling that your [if cocks of player is 1]dick[otherwise]dicks[end if] will go back where [if cocks of player is 1]it[otherwise]they[end if] belongs if your ass gets mutated again. On the other hand, you could get used to having [if cocks of player is 1]a tailcock[otherwise]a set of tailcocks[end if]";
	otherwise:
...
When Play begins:
	...
	now skin entry is "a coat of short, coarse fur.  It is as black as coal, covering your dark, rough";
	...
	if bodyname of player is "Cerberus":
		if humanity of player is less than 10:
			...
			if facename of player is "Cerberus":
				...
				otherwise:
					say ".  You are taken as a bitch by most of the pack, filling your hot [if cunts of player > 1]pussies[otherwise]pussy[end if] over and over again.  You don't seem as popular as the fully herm Cerberuses that make up the bulk of the pack, but you are still bred often";
			...
			otherwise if cocks of player > 0 and cunts of player > 0:
				say ".  You feel as if something is missing, but you know not what nor where to find it.  You wander out into the city and find your way into the park.  Your powerful, canine body pounces upon one of the Painted Wolf Herms you find.  After a bit of a scuffly, you two are rutting wildly, swapping places until you've both been thoroughly filled with cum.  You head off together to her den and make plans for the future of the pack growing in both your wombs.";
			otherwise if cocks of player > 0:
		...
		otherwise:
			...
			otherwise:
				...
				say "     You get work as a manual labourer, with your strong body suitable for the work.  Your heavy-duty lungs and hearts are able to push you longer and harder any of the other workers, allowing you to do the work of two men easily.  Recognized for your physical ability, your boss quietly starts paying you more than the others and you are smart enough not to let them know.  And it really helps, as your big body comes with a big appetite.";
				say "     Your large, canine body draws the interest of several infected canines and lupines and soon you have a collection of lovers and mates[if cocks of player > 0 and cunts of player > 0].  You sire and give birth of several large, canine pups over the years from various lovers and breeds[otherwise if cocks of player > 0].  You sire a variety of large, canine pups with various lovers and breeds[otherwise].  You give birth to several large, canine pups over the yours for various lovers and breeds[end if].  And while you sometimes feel as if something is missing, your large family and numerous lovers help fill that longing for companionship.";
	otherwise if facename of player is "Cerberus":
