...
to say enhancedchimeradesc:
	...
	say "     [if showlocale is true]As you're snooping around in the hospital, you're[else]You are[end if] come across by one of those enhanced chimeras of Dr. Mouse's. The hybrid herm could be one of those from the photos or she might be an offspring of theirs. This one is [if lev entry < 13]a little over eight feet tall[else if lev entry < 15]roughly eight-and-a-half feet tall[else]almost nine feet tall[end if] with a powerful and imposing body typical of the designed infection.";
	say "     The creature's face and muzzle are [headdata]-like, but with a leonine mane that cascades over her broad shoulders and the horns of a [horndata]. Her chest is armored with thick, scaly hide that follows the curves of her big breasts to preserve their buxom form and show off her perky nipples. The remainder of her body and limbs are covered in [furdata] fur, save for some armored plates on the exterior of the arms and legs. Her paw-like hands and feet have plated digits ending in claws and thicker tufts of fur at the wrists and ankles. Growing from the base of her spine is a thick tail with scales and spiny ridges. Protruding from her sheath and resting above her plump ballsack is her big, meaty cock. It is blood red in color, with a spaded tip and several firm ridges along its shaft. There's a faint swelling at the base, signs of a knot as well. Thick precum leaks down the monstrous prick as she eyes you with arousal, moving closer.";
	WaitLineBreak;
	if BodyName of Player is "Enhanced Chimera" or FaceName of Player is "Enhanced Chimera":
		...
		say "[line break]     [bold type]Shall you have a romp with the horny herm?[roman type][line break]";
		...
		if Player consents:
			...
			LineBreak;
			say "     Not in the mood for sex, you instead give her some snuggles and scritches before separating and heading on your way. She seems a little disappointed, but does not press the matter, wandering off to find another to sate her needs.[ec_reset]";
		now dam is 0;
		now wdam entry is 0;
		now combat abort is 1;
	else:
		say "     The creature's cock throbs harder and she releases a lustful growl, rushing you with obvious intent.";
...
to say losetoenhancedchimera:
	say "     [if HP of Player > 0]Rather than fight the creature, you give in and accept its lustful advances[else]Having beaten the resistance out of you, it advances upon you with a lustful rumble[end if]. ";
	say "[ec_sex]";
...
to say ec_sex:
	...
	if Player is female and a random chance of 3 in 5 succeeds:
		...
		CreatureSexAftermath "Player" receives "OralCock" from "Enhanced Chimera";
	if hospquest >= 19:
		infect;
...
to say randombaseform:
	if a random number between 1 and 12 is: [head/original form]
		-- 1:
	...
	if a random number between 1 and 5 is: [fur color]
		-- 1: now furdata is "dusty brown";
		-- 2: now furdata is "dark yellow";
		-- 3: now furdata is "black";
		-- 4: now furdata is "dark brown";
		-- 5: now furdata is "deep gray";
	if a random number between 1 and 4 is: [horns]
		-- 1: now horndata is "triceratops";
		-- 2: now horndata is "bull";
		-- 3: now horndata is "ram";
		-- 4: now horndata is "rhino";

to say ec_reset:
	[reset creature stats for next encounter]
	choose row with Name of "Enhanced Chimera" from Table of Random Critters;
...
When Play begins:
	...
	now attack entry is "The [one of]enhanced chimera[or]powerful chimera[or]hybrid creature[or]lustful chimera[at random] [one of]strikes you with one of its powerful backhands[or]bashes you with an armored fist[or]charges you with its horns[or]head-butts you hard enough to make your head ring[or]slashes at you with its claws[or]gnaws at your arm with its mix of sharp teeth[or]growls and kicks you hard with a powerful leg[or]spins around sharply to bash you against a wall with its big tail[at random]!";
	now defeated entry is "     Having defeated the enhanced chimera, you drive the creature off.[ec_reset][line break]"; [ Text when monster loses. ]
	...
	now body change entry is "it goes through several shifts. As these progress, you become a large and buff creature. Your shoulders broaden and your muscles firm up, powerful while not bulging. You stand around 9 feet tall and have a well-built frame designed for the rigors of post-apocalyptic combat";
	...
	now CumItem entry is "enhanced chimera cum"; [ Item to be given to the player if they have this infection and jerk off. ]
