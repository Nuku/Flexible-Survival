...
to say paula_rescue:	[This is one of Good Alexandra's tasks.]
	...
	otherwise:
		...
		if fightoutcome >= 10 and fightoutcome <= 19:
			say "     With the first of the nurses dealt with, you glance over to see Alexandra still trying to deal with the other canine.  They seem pretty evenly matched, though the doctor does seem to have the advantage.  You consider going to her aid, but the other nurse has climbed off a her patient's juicy-covered muzzle and comes at you.";
			challenge "Vixen Nurse";
			if fightoutcome >= 10 and fightoutcome <= 19:
...
Instead of conversing the Paula:
	if debugactive is 1:
		say "DEBUG -> HP: [hp of Paula]. Location: [location of Paula]. Alexandra's hint: [if AT_Paula is true]Given[otherwise]Not given[end if]. <- DEBUG[line break]";
	...
	otherwise:
		say "     [one of]'Any patients for me?'[or]'You seem to go wandering around outside quite a bit.  Probably perving on the mutants, aren't you?'[or]'You know, you're going to get yourself into real trouble out there if you keep pressing your luck.  If you end up hurt, come see me for some [bold type]healing[roman type].'[or]'I wonder which of you two'll snap first and turn this place into a sex harem.  Probably [if hp of Alexandra < 60]Officer Doggy out there - she's wound pretty tight.  She should learn to relax and have a little fun.  Therapeutically ease the pressure and all[otherwise]be you, running around out there, probably getting up to all sorts of kinky stuff with those mutants out there.  You'll end up some kind of sex perv and come back here to ravage us all[end if],' she says, stretching her arms up, causing her quad breasts to press against the fabric straining to cover them.[or]'I'd made out alright on my own for a while, but my luck was bound to run out.'[or]'I think this vixen look's not bad.  Better than those itchy scaly patches I'd picked up.'[or]'Can you believe I was a part-time nurse at that hospital before all this?  This isn't what I had in mind when I was hoping to become full-time staff,' she says, fondling her double-bosom teasingly.[or]'So you thinking to get some [if hp of paula >= 5]more [end if]sex out of me in return for letting me stay here.  Well, I guess that should be expected,' she says, giving her booty a shake in your direction.[in random order]";
...
to say sexwithPaula:
	...
	otherwise if lastfuck of Paula - turns < 6:
	...
	while sextablerun is 0:
		...
		if calcnumber > 0 and calcnumber <= the number of filled rows in table of fucking options:
			...
			if player consents:
				...
				if nam is "Fellatio":
					say "[paulasex01]";
[				otherwise if nam is "Double-titty-fuck":
					say "[paulasex02]";		]
...
check paulahealing:
	If Paula is not visible, say "[if hp of Paula < 3]You're on your own for that unless you go see Paula for assistance[otherwise]Try using a medkit or another means of healing[end if]." instead;
...
carry out paulahealing:
	...
	otherwise if lastPaulahealing - turns is less than 8:
