...
to say ResolveEvent Aussie Pub:
	...
	if the BodyName of Player is "Red Kangaroo":
		say "[line break]     [bold type]You feel a longing to join them. Do you enter?[roman type][line break]";
		...
		if Player consents:
			...
			CreatureSexAftermath "Player" receives "[if Player is female]Pussy[else]Ass[end if]Fuck" from "Red Kangaroo";
			WaitLineBreak;
			...
			MultiInfect "Red Kangaroo" repeats 2;
			SanLoss 18;
			PlayerDrink 30;
			PlayerEat 12;
	...
	[NOTE:Consider adding an option for the player to get captured and end up in the position of the wolf eared man. without a bad end. full red kangaroo  tf sequeence.]

Part 2 - Down Under Pub
...
to say down under pub scene:
	if BodyName of Player is not "Red Kangaroo": [NOTE:Consider adding the option to become a red kangaroo again]
	...
	else if lastpubvisit - turns < 14: [Player can't visit the pub just yet]
		if HP of Gillian >= 4:
		...
		if HP of Gillian >= 4:
			...
			say "[line break]     [bold type]Which would you prefer, going to the Down Under or meeting with Gillian?[roman type][line break]";
	...
	if location of Player is Down Under Pub:
		say "[line break]     With your visit to the Aussie pub over, you head back to the nearby red light district to decide where to head next.";
		WaitLineBreak;
		move player to Entrance to the Red Light District;

to say downunderpubvisit: [NOTE:Consider adding options for the player to approach or be approached for sex depending on how forwardthey feel like being.]
	...
	if Player is male, add { 2, 3 } to randomroos; [girly joey, horny jill]
	if Player is female, add { 4, 5 } to randomroos; [lez jill, roo pair]
	sort randomroos in random order;
	if entry 1 of randomroos is:
		-- 1: say "[roopubsex1]";
		-- 2: say "[roopubsex2]";
		-- 3: say "[roopubsex3]";
		-- 4: say "[roopubsex4]";
...
Part 3 - Various sex scenes

Section 1 - Barmaid Scenes
...
to say roopubsex3:
	...
	say "[line break]     [bold type]Her breasts, you feel, would give a fine titty-fuck, though a ride in your lap may be more what you're in the mood for this time.[roman type][line break]";
	...
	if Player consents:
		...
		CreatureSexAftermath "Player" receives "Stroking" from "Red Kangaroo";
	else:
		LineBreak;
...
to say roofloorshow:
	...
	if Player is male and newgender > 2: [Partially changed human becoming male]
...
the scent of the Gillian is "     Her fine scent is a mix of pub fare, aroused roo and marsupial sex.".
...
instead of conversing Gillian:
	project Figure of Gillian_icon;
	if HP of Gillian < 4:
...
[		if anallevel is 3 and Cunt Count of Player is 0, add 6 to gilliansexList; [strap-on: anal]	]
		...
		if entry 1 of gilliansexlist is:
			-- 1: say "[gilliansex01]";
			-- 2: say "[gilliansex02]";
			[-- 3: say "[gilliansex03]";]
			-- 4: say "[gilliansex04]";
...
to say gilliansex02:	[69]
	say "     Gillian smiles happily at your suggestion of sex. 'Yeah, let's have some fun,' she says as she pushes you eagerly over to the bed. Pushing you down onto it, she moves atop you, face to crotch. Provided with an eyeful of her succulent pussy, you dive right in. She moans happily at your eagerness and soon has her muzzle pressed to your loins.";
	if Player is male:
...
[to say gilliansex03:	[blowjob]
	say "     Gillian smiles happily at your suggestion of sex. 'Yeah, let's have some fun,' she says as she pushes you eagerly over to the couch. ***";]

to say gilliansex04:	[fuck her]
	say "     Gillian smiles happily at your suggestion of sex. 'Yeah, let's have some fun,' she says as she pushes you eagerly over to the bed. While sharing a lustful kiss with you, she fondles your [Cock of Player] manhood. Once she's gotten you ready, she lays down on her chest and hikes her meaty ass in the air, tail held high and dripping cunny on open display. 'Come and get it, babe. I want a good, proper pounding from you this time. It's great having sex downstairs for the crowd, but it's always so rushed.'";
...
[to say gilliansex05:	[strap-on: vaginal]
	say "     Gillian smiles happily at your suggestion of sex. 'Yeah, let's have some fun,' she says as she pushes you eagerly over to the floor. ***";
...
definition: Down Under Pub is open:
	if TimekeepingVar > 5 or ( TimekeepingVar < 1 and TimekeepingVar > -3 ) or TimekeepingVar is -8:			[visits allowed from noon to midnight]
		no;
	else:
		yes;
...
to say randomdesc:	[male]
	if a random number between 1 and 15 is:
		-- 1: now slutname is "husky";
		-- 2: now slutname is "hawk";
		-- 3: now slutname is "panther";
		-- 4: now slutname is "Anthro Rabbit";
		-- 5: now slutname is "Brown Bear Female";
		-- 6: now slutname is "Blacktail Stag";
		-- 7: now slutname is "cheetah";
		-- 8: now slutname is "kangaroo";
		-- 9: now slutname is "skunk";
		-- 10: now slutname is "bull";
		-- 11: now slutname is "Zebra Stallion";
		-- 12: now slutname is "mouse";
		-- 13: now slutname is "tiger";
		-- 14: now slutname is "hyena";
		-- 15: now slutname is "wolf";
...
to say randomdesc2:	[female]
	if a random number between 1 and 15 is:
		...
		-- 2: now slutname is "hawk";
		-- 3: now slutname is "panther";
		-- 4: now slutname is "bunny";
		-- 5: now slutname is "Brown Bear Female";
		-- 6: now slutname is "doe";
		-- 7: now slutname is "cheetah";
		-- 8: now slutname is "kangaroo";
		-- 9: now slutname is "skunk";
		-- 10: now slutname is "cow";
		-- 11: now slutname is "Zebra Stallion";
		-- 12: now slutname is "mouse";
		-- 13: now slutname is "tigress";
		-- 14: now slutname is "hyena";
		-- 15: now slutname is "wolf";
