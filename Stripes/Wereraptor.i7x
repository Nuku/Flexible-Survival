...
to say ResolveEvent Paleontology Professor:
	if MaleList is banned and FemaleList is banned:
		...
		if daytimer is day:
			...
			say "[line break]     [bold type]While the voice is muffled, does it sound more like a man or a woman?[roman type][line break]";
			...
			if Player consents:
				...
				say "[line break]     [bold type]Shall you free him?[roman type][line break]";
				...
				say "[line break]     [bold type]Shall you free her?[roman type][line break]";
				say "     ([link]Y[as]y[end link]) - Yes.";
				say "     ([link]N[as]n[end link]) - No.";
				if Player consents:
					LineBreak;
			...
			say "[line break]     [bold type]While the voice sounds raspy, does it sound more like a man or a woman?[roman type][line break]";
			say "     ([link]Y[as]y[end link]) - It sounds like a man.";
			say "     ([link]N[as]n[end link]) - It sounds like a woman.";
			if Player consents:
				LineBreak;
				...
				say "[line break]     [bold type]Shall you let the guy out?[roman type][line break]";
				say "     ([link]Y[as]y[end link]) - Yes.";
				say "     ([link]N[as]n[end link]) - No.";
				if Player consents:
					LineBreak;
				...
				say "[line break]     [bold type]Shall you let the woman out?[roman type][line break]";
				say "     ([link]Y[as]y[end link]) - Yes.";
				say "     ([link]N[as]n[end link]) - No.";
				if Player consents:
					LineBreak;
...
to say losetowereraptor:
	if BodyName of Player is "Wereraptor":
		if wrmode is 1:
			...
			else if anallevel > 1 and a random chance of 2 in 3 succeeds and Player is not female:
...
to say beatthewereraptor:
	if wrcurseNermine is 9:
		...
		if wrmode is 1: [female]
			...
			say "[line break]     [bold type]Shall you give in to your primitive, lustful instincts and mate with your fallen foe?[roman type][line break]";
			say "     ([link]Y[as]y[end link]) - Yes.";
			say "     ([link]N[as]n[end link]) - No.";
			if Player consents:
				LineBreak;
...
to say PrepCombat_Wereraptor:
	...
	if MaleList is banned and FemaleList is banned:		[if both types are banned, the fight is aborted and removed from critter table]
		...
		say "(Currently requires at least one of Guy or Girl content to be available.)";
...
When Play begins:
	...
	now desc entry is "     You find yourself faced with a lizard-human hybrid which you quickly realize is some kind of velociraptor creature. Its torso and arms are much like that of a human, but with the muscled legs, tail and head of a velociraptor. And a [if wrmode is 1]female one at that. Leaning over in a stance typical for raptors, her breasts can clearly be seen at her chest. While her hands have smaller claws on them, those on her feet seem quite large, especially the long sickle-like one. Both have only three digits on them. Her scales are a deep blue over most of her body, shifting almost to black over her head, back and the top of her tail. She stares at you with a feral wildness to her yellow eyes and takes a step forward. She[else]male one at that. Leaning over in a stance typical for raptors, his dripping maleness can clearly be seen between his muscular legs. While his hands have smaller claws on them, those on his feet seem quite large, especially the long sickle-like one. His scales are a deep blue over his body, accented by green feathers on top of the head, on the forearms and at the end of his tail. He stares at you with a feral wildness to his yellow eyes and takes a step forward. He[end if] hisses at you before charging forward in a rush.[line break]"; [ Description of the creature when you encounter it. ]
	...
	now CumItem entry is "wereraptor cum"; [ Item to be given to the player if they have this infection and jerk off. ]
...
this is the wereraptor curse rule:
	...
	if wrcursestatus is 2:
		...
		if daytimer is night:
			...
			say "[line break]     [bold type]Shall you accept its arrival or try to hold it back?[roman type][line break]";
...
to wrcocksave:
	...
	say ". [if Player is male]Your groin feels a surge of wild power as [cock change entry][else]You feel a rush of hot excitement from your loins[end if]";
...
Chapter 4 - Cure Events

Section 1 - Greenhouse and Wolfsbane
...
to say ResolveEvent Greenhouse:
	say "     [if grhouse is false]Remembering Nermine's instructions, you keep your eyes open as you travel around the campus. Not recalling having seen one there before and unable to find out anywhere, you're about to give up when you swing back around towards the biology building again for another pass. And that's when you spot it as you come around the edge of the chemistry building. Linking it to the biology building, there is a connecting hallway and the topmost level of that passage is a greenhouse[else]Hoping to gain entrance to the greenhouse this time, you slip into the chemistry building. Moving carefully, you make your way up the stairs to the third floor and the pathway connecting it and the biology building[end if].";
	if daytimer is day:
...
wolfsbane is a grab object. It is not temporary.
...
to say ResolveEvent Getting the Knife:
	...
	if fightoutcome < 20: [player won]
		...
		if bonus + dice > 18:
			say "You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]19[roman type] (Intelligence + Perception Check):[line break]";
...
[chainmail vest moved to Core Mechanics/Equipment.i7x]

Section 3 - Dinosaur Skeleton
...
to say ResolveEvent Dinosaur Skeleton:
	if daytimer is day:
		...
		say "[line break]     [bold type]Do you go ahead with the cure or accept [if UtahGender is 1]his[else]her[end if] offer?[roman type][line break]";
		say "     ([link]Y[as]y[end link]) - Cure.";
		say "     ([link]N[as]n[end link]) - Accept [if UtahGender is 1]his[else]her[end if] offer.";
		if Player consents:
			LineBreak;
	...
	if wrcursestatus is 2:
		...
		if Player has a body of "Wereraptor": [NOTE: The player being a shifter is ignored here (@Stadler#3007)]
			if humanity of Player < 10:
				...
				say "     [if UtahGender is 2 and Libido of Doctor Utah > 0]When the eggs Dr. Utah left in the city hatch, they seems like other ferals and go[else]Seeming at first like other ferals, the spread of wereraptors goes[end if] largely unnoticed at first. But those who becomes wereraptors remain so, even if infected with other strains, keeping them among your ever-growing numbers. As the numbers of attacks from these 'night lizards' grow, you and Dr. Utah are pulled in as experts on prehistory and saurian creatures. But despite the many efforts and seemingly reasonable precautions you implement, the numbers continue to grow. With you both leading both sides, it is easy to ensure that your people rise in strength, pulling more people into your order and creating more sexy saurians to populate what will soon be your world.";
		else:
			if humanity of Player < 10:
