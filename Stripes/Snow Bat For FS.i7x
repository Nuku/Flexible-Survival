...
to say losetosnowbat:
	...
	if scalevalue of player > 0 and ( player is female or anallevel is 3 or player is mpreg_ok ) and daytimer is night and a random chance of 2 in 5 succeeds:
		...
		if player is female:
			...
			say "     Overwhelmed, [if cocks of player > 1]your cocks fire off their load into the air, causing you to pass out[else if player is male]your cock fires off its load into the air, causing you to pass out[else]you pass out[end if]; eventually, you awake. Back on the ground, exhausted, and thirsty, you gather yourself before slowly going back on your way.";
		...
		say "     Overwhelmed, [if cocks of player > 1]your cocks fire off their load into the dirt, causing you to pass out[else if player is male]your cock fires off its load into the dirt, causing you to pass out[else]you pass out[end if]; eventually, you awake. Now abandoned, exhausted, and thirsty, you gather yourself before slowly going back on your way.";
	else:
...
When Play begins:
	...
	now skin entry is "soft fur that is snowy white and ash gray in colour with a leopard-like pattern across your";
