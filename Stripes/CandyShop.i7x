...
to say ResolveEvent CandyShop:
	...
	say "[line break]     [bold type]Shall you step inside?[roman type][line break]";
	...
	if Player consents:
		...
		if fightoutcome < 20: [player won]
			...
			say "     [if fightoutcome is 21 or fightoutcome is 22]Unwilling to keep fighting, you give yourself over to the manic ferrets. Cheering, they bounce onto you, grabbing your limbs[else]As your strength wanes, you are tackled by several of the ferrets at once. They grab onto your arms, legs[end if] and body, pulling you down by sheer numbers. Even as you're going down, you're groped and fondled by the over-excited ferrets.";
			...
			CreatureSexAftermath "Player" receives "[if Player is female]Pussy[else]Ass[end if]Fuck" from "Sugar Ferret";
			CreatureSexAftermath "Player" receives "OralCock" from "Sugar Ferret";
			CreatureSexAftermath "Player" receives "OralPussy" from "Sugar Ferret";
...
to say Sweet Tooth scene:
	...
	if lastcaffeine of Sweet Tooth - turns < 8:
		say "     Swinging by the shops on the fairgrounds, you take a peek in the CandyShop. You can see the ferrets inside are still manic with excess energy. [if caffeinehigh of Player > 0]The thought that they won't have any cola there for you to get another fix pops into your head. Regardless of any plans you may have had, you turn and go looking for some soda elsewhere to satisfy your cravings[else]You decide it'd be best to not enter right now[end if].";
		...
		if Player is male:
			CreatureSexAftermath "Sugar Ferret" receives "OralCock" from "Player";
		else if Player is female:
			CreatureSexAftermath "Sugar Ferret" receives "OralPussy" from "Player";
		...
		if Player is male:
			...
			CreatureSexAftermath "Player" receives "AssFuck" from "Sugar Ferret";
		infect;
		increase ferretvisit by 1;
		...
		let randomferret be { 1, 2, 3 };
		if Player is male, add { 4, 5 } to randomferret; [M/F top, gay ferret (fuck/oral)]
		if Player is female, add { 6, 7 } to randomferret; [get share-fucked, 69 w/male]
		sort randomferret in random order;
		if entry 1 of randomferret is:
			-- 1: say "[CandyShopsex1]";
			-- 2: say "[CandyShopsex2]";
			-- 3: say "[CandyShopsex3]";
			-- 4: say "[CandyShopsex4]";
		...
		if soda is owned:
			...
			while carried of soda is not 0:
				...
				decrease humanity of Player by 3;
				raise Player Libido by 3;
			say "[bold type]You lose all your soda![roman type][line break]";
		else:
			say "[line break]     One of the ferrets pulls out a big bottle of cola and starts passing it around. Too excited to think clearly, you grab the bottle as soon as it gets close enough, taking a big swig and enjoying the wild rush the caffeine in it gives you. The others around you bounce around excitedly and more pop is pulled out from hidden stashes. You moan excitedly as a manic energy starts to suffuse you, filling you with a rush that's half lust and half a craving for more[if the player is sugarfaced]. Your eyes turn a bright red like those of the other ferrets around you as the caffeine makes you manic. Your teeth grow sharp and pointed and you gain a wide, toothy grin[end if][if the player is sugarskinned]. Your pastel fur grows richer in color, becoming a vibrant purple[end if]. You can't sit still at this point, filled with so much wild energy.";
			now lastcaffeine of Sweet Tooth is turns;
			PlayerDrink 6;
		sfcaffeine; [activates effects of Sugar Ferret caffeine high]
		follow the cock descr rule;
		...
		increase ferretvisit by 1;
	WaitLineBreak;
	say "     With your visit to the Sweet Tooth CandyShop over, you head back to the entrance area to the State Fair to decide your next course of action.";
...
to say CandyShopsex5:
	if caffeinehigh of Player is 0:
		...
		if Player consents:
			...
			CreatureSexAftermath "Sugar Ferret" receives "AssFuck" from "Player";
		else:
			LineBreak;
...
to sfcaffeine:
	...
	if Libido of Player < 52:
		now Libido of Player is 66;
	else:
...
an everyturn rule:
	if caffeinehigh of Player is not 0:
		...
		if caffeinehigh of Player <= 0 or BodyName of Player is not "Sugar Ferret":
			...
			say "     (With the caffeine buzz worn off, your stats have returned to normal, though you do feel weakened and tired.)";
