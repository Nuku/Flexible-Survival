...
to say libraryentrance:
	...
	if ( carried of water bottle > 1 and carried of food > 1 ) or jamesfed is 1:
		...
		now description entry is "Will you [if jamesfed is 0]try bribing him with some supplies[else]discuss a weapon trade[end if]";
	[]
	choose a blank row in table of fucking options;
	...
	while sextablerun is 0:
		...
		if calcnumber > 0 and calcnumber <= the number of filled rows in table of fucking options:
			...
			if Player consents:
				...
				say "Pick between 1 and [the number of filled rows in the table of fucking options], or 99/0 to exit.";
			...
			say "Invalid Option. Pick between 1 and [the number of filled rows in the table of fucking options], or 99/0 to exit.";
	wait for any key;
...
to say libguard1:						[Bluff]
	if BodyName of Player is "Wolverine Guard":
		...
		say "[line break]You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]14[roman type] (Charisma Check):[line break]";
		if bonus + dice >= 14:
...
to say libweaponoffer:
	...
	if x < 2:
		...
		if weaponpick is 0:
			...
			if Player consents:
				LineBreak;
				say "     You tell him he can have it if he'll let you go in for a bit. You flash [if journal is owned]your journal quickly, claiming you need to drop the book off and want to look for another[else]the corner of a notepad, claiming you need to write a report and need to check a book out from the library for it[end if]. He seems a little reluctant, but agrees. 'Well, I guess that'd be okay. You've been a good pal and this is a fine gift,' he says, hefting his new, blunt instrument around with a rather wild look in his eyes. 'This'll really help me guard my post. And it is a public library, so go quick and watch out for the librarians. I think they spent a little too long in the mythology section or something. They're not quite right anymore,' he adds cryptically as he pushes you in and closes the door behind you.";
				now jamesfed is 2;
				if weaponpick is 1:
...
to say libraryexplore:
	...
	WaitLineBreak;
	let randomlist be { 1, 2, 3, 4, 5, 6, 7 };
	repeat with x running through bookcollection:
		remove x from randomlist, if present;
	if randomlist is empty:
		...
		sort bookcollection;
	WaitLineBreak;
	if XP of Player > ( level of Player plus one ) times 10 or ( "Fast Learner" is listed in feats of Player and XP of Player > ( level of Player plus one ) times 8 ):
		level up;
	...
	if fightoutcome < 20: [player won]
		...
		say "     The harpies swoop in around you and grab you, carrying you aloft in their talons. With each of your limbs gripped in their strong talons, you cannot struggle, especially as they fly up several stories in the large, central area. James, drawn by the commotion, comes running in and spots you being carried off. He [if centrallib is 4 or jamesfed is 2 or jamessex is 3]shakes his fist at the harpies and growls. 'I told you not to upset them. I'm sorry, but they won't let me up there.' With no hope of rescue from him, you[else]growls and shakes his fist. 'Serves you right! I told you this library is closed!' You[end if] are carried up to the top floor and dropped onto a large nest made of books. Looking to the stairwell for an exit, you see that this is where the fire occurred and that several bookshelves have been knocked over to bury the only way down. You are trapped with them and at their mercy.";
		...
		if hellHoundLevel is 0:
			if Player is not MalePreferred and Player is not HermPreferred and "One Way" is not listed in feats of Player and "Always Cocky" is not listed in feats of Player:
...
carry out deweying:
	...
	repeat with x running from 1 to number of filled rows in table of library books:
		...
		if bookcode entry exactly matches the text topic understood, case insensitively:
			now found is x;
			break;
	if found is listed in bookcollection:
		...
		if found is:
			...
			-- 6: say "[libbook6]";
			-- 7: say "[libbook7]";
			-- otherwise: say "You look among the stacks, but don't find any books of interest.";
...
instead of conversing the Doctor Matt while 5 is listed in bookcollection and mattintel is 0 and HP of Doctor Matt > 0 and HP of Doctor Matt < 100:
	...
	say "[line break]     [bold type]Shall you relay this intelligence to the military?[roman type][line break]";
...
instead of conversing the Doctor Mouse while 5 is listed in bookcollection and mouseintel is 0:
	...
	say "[line break]     [bold type]Shall you make the offer to him?[roman type][line break]";
	say "     ([link]Y[as]y[end link]) - Yes.";
	say "     ([link]N[as]n[end link]) - No.";
	if Player consents:
		LineBreak;
...
instead of conversing the Gina while 5 is listed in bookcollection and hyenaintel is 0:
	...
	if Player consents:
		...
		if matriarchowned is 1:
			...
			say "     Telling Gina about your plan, she's very pleased with your idea. 'I'm really getting sick of some of these reports and stories I'm getting from the others out there. Useless stuff like [']Oh Gina, there was a bunch of dog-things out by the 7-11. You know, the one with the bad Slurpee machine. Well anyway, I saw them there a couple of days ago. Thought you should know.['] Got that one today. It took me ten minutes to get what little useful stuff she knew out of her, and even then, it probably won't amount to anything. Some quality reporting would help a lot. I'm sure the matriarch will be pleased with your reports,' she adds. Your recon information will help the hyena gang become a little stronger, opposing the other forces out in the city and delaying the military.";
			extend game by 12;
			increase score by 10;
			now hyenaintel is 1;
	else:
