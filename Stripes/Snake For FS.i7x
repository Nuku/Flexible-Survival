...
to say losetosnake:
	...
	if fsnakevalid is true: [Touched by Madness Female scene]
		...
		say "     Eventually, [if hp of player > 0]you recover enough from the ordeal[otherwise]the venom wears off[end if], allowing you to slowly get back to your feet, [if scalevalue of player < 4]overly bloated body making this a particularly difficult task[else if snakeocc > 0]forced to contend with your further exacerbated pregnancy[otherwise]forced to contend with your new and significantly progressed pregnancy[end if], to say nothing of the your unsated lust in the wake of such treatment.";
		if snakeocc is 0:
...
to snakecoresex:
	if cocks of player > 0: [Always goes for male genitalia first]
		...
		if cock length of player < 21:
			say "[if cocks of player > 2]two of them[else if cocks of player is 2]both of them[otherwise]the tool[end if] with little restraint or difficulty";
		else:
