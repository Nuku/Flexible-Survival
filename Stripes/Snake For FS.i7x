...
to say losetosnake:
	...
	if fsnakevalid is true: [Touched by Madness Female scene]
		...
		if breast size of player > 0: [Breast-particular Prelude]
			...
			say "     Eyeing its [if hp of player > 0]potential offering[else]new victim[end if] up for a moment, the serpent[if scalevalue of player > 4]partially [end if] wraps around you, [if scalevalue of player < 4]it's heavy weight keeping you pinned to dissuade protest[else]it ensuring that you can't struggle against its endeavours[end if] -- not that you [if hp of player > 0]are of any immediate desire to do so[else]could even if you wanted to, given the venom's effect[end if] -- before its curious, forked tongue tastes your ";
			snakecoresex;
...
to snakecoresex:
	if cocks of player > 0: [Always goes for male genitalia first]
		...
		say ".";
		say "     Tight, slick confines massaging your engulfed cock[if cock length of player < 21 and cocks of player > 1]s[end if], the serpents grip tightens more and more around you, as if to try and squeeze you into bliss. Its prolonged, audibly wet endeavours [if libido of player > 20]quickly[else]eventually[end if] forces you to cry out, constrained tool[if cock length of player < 21 and cocks of player > 1]s[end if] spurting gouts of [if cock length of player < 21 and cocks of player > 1]their[else]its[end if]  [cum load size of player] load into its voracious, slick maw";
		...
		say "[cunt size desc of player] cunt[sfn], [if breast size of player > 0]already quite aroused after its prior ritual[else if hp of player > 0]quickly made dripping and eager by the beast's affection[else]the beast's tainted infection leaving you dripping and eager[end if]. Satisfied, and with almost no restrain, it forces its slender head past the supple folds of [if cunts of player > 1]one of your pussies[else]your pussy[end if][if cunt length of player < 13], its substantial girth stretching you to your limits[end if]. Coiling slightly around you, [if scalevalue of player < 4]it's heavy weight keeps you pinned to prevent you from struggling[else]it ensures that you can't struggle against its endeavour[end if][if hp of player > 0], not that you have presented yourself in such a manner to give that sort of impression[end if].";
		say "     Its relentless, writhing endeavour to thrust its thick, scaled body in and out of your depths, the serpents grip tightens more and more around you, as if to try and squeeze you into bliss. Its prolonged endeavours [if libido of player > 20]quickly[else]eventually[end if] forcing you cry out. As your stuffed portal tightens around the creature, its movements become quicker and more erratic, the sensation of its tongue felt flicking along your beleaguered passage as it feeds on your emergent sexual fluids.";
