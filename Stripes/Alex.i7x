...
Description of Alex is "     [if alexbrunch < 4]A mostly humanoid male with ferret-like ears and a long, sinuous tail. He has a twitchy animal nose with whiskers around it. He has a slightly rumpled suit on, still looking good despite his changes. He is a pleasant enough looking person who looks even better in those moments when he's not stressed or worried by his clients[else]Alex is a sexy ferret male, with a cute mustelid head. His twitchy animal nose has an array of whiskers around it. He has a slightly rumpled suit on, still looking good despite his changes. You find his new look quite pleasing to the eyes and he certainly seems much happier and friendlier of late. It seems the changes he's undergone are for the better[end if].[line break]".
[Conversation of alex is { "Poing!" }.]
the scent of Alex is "     Alex smells [if alexbrunch < 4]like a mix of human and ferret[else]of male ferret and brunchtime fun[end if].".
...
instead of conversing the Alex:
	...
	if AlexProgress is 0:
		...
		say "[line break]     [bold type]Do you agree to help him?[roman type][line break]";
		...
		say "     Alex's whiskers twitch as he looks at you sternly. 'I need you back out there,' he grumbles before relaxing. 'Sorry, I'm just on edge lately. My clients may need your help out there, too. I'm worried about them.'";
...
to say ResolveEvent Meet Alex:
	...
	say "[line break]     [bold type]'I'll give you one of each. How's that sound?'[roman type][line break]";
	say "     ([link]Y[as]y[end link]) - Yes.";
	say "     ([link]N[as]n[end link]) - No.";
	if Player consents:
		LineBreak;
...
to say ResolveEvent Find Lorenda:
	...
	if lisacheat is 1:
		...
		say "     You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]17[roman type] (Perception Check): ";
		if bonus + dice > 16:
...
instead of going west from dirty sheds:
	if AlexProgress is 4:
		...
		say "[line break]     [bold type]Do you attempt to persuade him to come with you?[roman type][line break]";
		say "     ([link]Y[as]y[end link]) - Yes.";
		say "     ([link]N[as]n[end link]) - No.";
		if Player consents:
			LineBreak;
			...
			let dice be a random number from 1 to 20;
			say "You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]20[roman type] (Charisma Check):[line break]";
			if dice + bonus > 19:
			...
			LineBreak;
		if success is false:
			say "[line break]     [bold type]Do you attempt to break down the door?[roman type][line break]";
			say "     ([link]Y[as]y[end link]) - Yes.";
			say "     ([link]N[as]n[end link]) - No.";
			if Player consents:
				LineBreak;
				...
				let dice be a random number from 1 to 20;
				say "You roll 1d20([dice])[if bonus >= 0]+[end if][bonus] = [special-style-1][dice + bonus][roman type] vs [special-style-2]22[roman type] (Strength Check):[line break]";
				if dice + bonus > 21:
		...
		if success is true:
			...
			if gettinglee > 5 and alexleeinfo is 0:
				say "[line break]     This doesn't seem to be working for you. Perhaps [bold type]Alex[roman type] has more info to help you convince Lee to trust you.";
	else:
...
instead of fucking the Alex:
	...
	if AlexProgress < 5:
	...
	else if alexbrunch < 4:		[Had 0 to 3 brunches]
		if lastfuck of Alex - turns < 8:
			...
			if Player is herm:		[HERM]
				...
				[WaitLineBreak]
				say "     His nimble fingers and artful muzzle tease your [Cock of Player] shaft and [cunt size desc of Player] pussy until you can hold back no longer and shoot your hot load down the ferrety man's throat. He sucks it all down with soft moans until your balls are drained. His muzzle pulls back and he slides his slinky body up yours to kiss you. As your tongue dives between his lips, you can feel the mustelid's cum spraying across your body.";
				CreatureSexAftermath "Ferret" receives "OralCock" from "Player";
			else if Player is male:				[MALE]
				say "     The ferret runs his fingers over your cock[smn], holding [itthemm] up as he buries his muzzle against your hard meat, taking in your scent with a moan of pleasure. His tongue glides over your [cock size desc of Player] cock[smn] slowly, working over every square centimeter of [itthemm] and your balls to pleasure you[if anallevel is 3]. He continues lower, sliding his tongue across your tight pucker in several slow licks. His tongue wriggles against your back entrance and eases its way into your rectum as he eagerly rims you for increased pleasure[end if]. Having gotten you quite worked up, when he finally plunges his mouth down over your [Cock of Player] penis and starts sucking it firmly, you moan loudly. He smiles up at you and rubs one hand over your [Ball Size Adjective of Player] balls while the other strokes his animalistic member.";
				[WaitLineBreak]
				say "     His nimble fingers and artful muzzle tease your [Cock of Player] shaft until you can hold back no longer and shoot your hot load down the ferrety man's throat. He sucks it all down with soft moans until your balls are drained. His muzzle pulls back and he slides his slinky body up yours to kiss you. As your tongue dives between his lips, you can feel the mustelid's cum spraying across your body.";
				CreatureSexAftermath "Ferret" receives "OralCock" from "Player";
			else:								[FEMALE]
			...
			if TimekeepingVar > 5 or TimekeepingVar is -1 or TimekeepingVar is -2:	[early or mid-morning sex]
				WaitLineBreak;
				say "[alexbrunchtime]";
			follow the turnpass rule;
	else:				[4+ brunches]
		if lastfuck of Alex - turns < 4:
			...
			if Player is herm:		[HERM]
				...
				say "[line break]     [bold type]Do you want to mount him or get fucked?[roman type][line break]";
...
to say alexbrunchtime:
	...
	if alexbrunch < 4:				[First to third brunch]
		...
		increase score by 5;
	else if alexbrunch is 4:				[Fourth brunch]
		...
		WaitLineBreak;
		say "     [if Player is male]The smaller ferret climbs into your lap when you head to the dining room to eat, sharing one large plate. You snuggle, kiss and tease one another playfully while eating your breakfast, enjoying the ferret's special pancakes all the more for knowing what makes them special. The ferret playfully grinds his ass against your cock[else]You climb into his lap when you head to the dining room to eat, sharing one large plate. You snuggle, kiss and tease one another playfully while eating your breakfast, enjoying the ferret's special pancakes all the more for knowing what makes them special. The ferret's playful fingers tease your pussy[end if] as he happily informs you that Lorenda and Lee are getting along well. It seems she's helping [']him['] become more comfortable with being partly a [']her['] as well and they are making a rather cute couple.";
		...
		WaitLineBreak;
		say "     You slip off the robe and stretch out your body in front of the mirror, watching the changes in your reflection. Your body grows more lithe and flexible. You test out this flexibility, bending and contorting yourself with ease in front of the full length mirror[if Cock Count of Player > 1]. And speaking of [']full length['], the sight of your changing body excites your cocks to full erection. Like Alex's, they are smooth and slender with an upward curving glans[else if Player is male]. And speaking of [']full length['], the sight of your changing body excites your cock to full erection. Like Alex's, it is smooth and slender with an upward curving glans[end if][if Cunt Count of Player > 1]. You take a moment to examine your pussies as well, playing with your pink folds, rich with the scent of aroused mustelid[else if Player is female]. You take a moment to examine your pussy as well, playing with your pink folds, rich with the scent of aroused mustelid[end if].";
	...
	else:		[5+ brunches, breakfast in bed]
		project Figure of Alex_icon;
...
to say alexbodyreset:
	...
	if alexbrunch > 2 and FaceName of Player is not "Ferret":
		...
		now Face of Player is "that of an adorable mustelid with darting eyes that search for shinies. You have a cute, pink nose with plentiful whiskers on your ferrety muzzle.";
	if alexbrunch > 3:
		if BodyName of Player is not "Ferret":
			...
			now bodytype of Player is "[one of]mustelid[or]ferret-like[at random]";
			now SleepRhythm of Player is 0;
		if CockName of Player is not "Ferret":
...
This is the Ferret Infection rule:
	if Player has a body of "Ferret":
		...
		if humanity of Player < 10:
			...
			say "     Alex seems to be quite successful - certainly he's able to pay your playmates very well and he's soon buying an even larger home with a bigger playroom for you[if Player is impreg_ok]. It also comes complete with a lovely nursery for the growing kits in your womb[end if]. He's especially rambunctious after winning a big case and still surprises you with breakfast in bed at times.";
		else:
			...
			say "     Throughout your successful lives together, Lorenda and Lee are there for you both, good friends and occasional partners. Their hybrid kittens help to [if Player is impreg_ok]fill the large home you all share and are wonderful playmates to your own ferret kits[else]make your large house into a home[end if].";
			if humanity of Player > 49 and intelligence of Player > 15 and charisma of Player > 15:	[BEST ENDING ADD-ON]
				...
				say "     You watch as the first TF presidential candidate is announced while you and Alex snuggle with breakfast in bed. Lorenda and Lee, in the mood to [if coonstatus is 101]make a show of playing frisky maids with your pink coontoy[else]play frisky maids that day[end if], are there with you to see it. You all smile and agree he won't win, but you don't care. You know you will when you run in a few years['] time.";
