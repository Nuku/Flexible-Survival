...
Instead of resolving a CandyShop:
	...
	if the player consents:
		...
		if "soda" is listed in invent of player:
			...
			say "     At first, their antics over the cola are amusing, but you notice things getting more rambunctious as the ferrets start pushing and shoving to get another drink.  Their fur gets ruffled and scraggly and their colours grow richer, becoming vibrant reds, yellows, greens, blues and their eyes turn a bright, fiery red.  Sheaths swell and cocks start to poke free.  Pussies grow wet and dripping.  You start backing away towards the door when one of them notices you, turning to look at you with a demonic, pointy-toothed grin.  While several of the ferrets in here have already paired off into groups for lustful sex, the others turn their attention on you.  They poing towards you in a manner which would be cute if it weren't for the sinister, lustful looks and their fanged grins.";
		otherwise:
...
to say Sweet Tooth scene:
	...
	otherwise if sugarferretfight > 0:
		[puts Sugar Ferret as lead monster for impregnation]
		repeat with y running from 1 to number of filled rows in table of random critters:
			choose row y in table of random critters;
			if name entry is "Sugar Ferret":
	...
	otherwise:
		...
		if entry 1 of randomferret is 1:
			say "[candyshopsex1]";
		otherwise if entry 1 of randomferret is 2:
			say "[candyshopsex2]";
		otherwise if entry 1 of randomferret is 3:
			say "[candyshopsex3]";
		otherwise if entry 1 of randomferret is 4:
			say "[candyshopsex4]";
		otherwise if entry 1 of randomferret is 5:
			say "[candyshopsex5]";
		otherwise if entry 1 of randomferret is 6:
			say "[candyshopsex6]";
		otherwise if entry 1 of randomferret is 7:
			say "[candyshopsex7]";
...
to sfcaffeine:
	...
	if "Pure" is listed in feats of player, increase humanity by a random number between 1 and 5;
	if "Corrupt" is listed in feats of player, decrease humanity by a random number between 0 and 3;
...
to sfcaffeineboost:
	...
	if "Pure" is listed in feats of player, increase humanity by a random number between 1 and 3;
