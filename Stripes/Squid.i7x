...
to say losetosquid:
	say "     [if HP of Player > 0]You stand down and throw your hands up in submission against the tentacled lady. Cautiously, she advances upon you, and once sufficiently satisfied with your compliance she starts to gently relinquish you of your attire before having her fun[else]Quickly moving, you find you're too weak to react before her suctioned tendrils grab hold of you and throw you to the ground, getting a faceful of sand. She then roughly exonerates your clothing from its beleaguered owner, its lewd intent made transparent to your vulnerable form[end if].";
	if Player is male:
	...
	if Nipple Count of Player > 1 and Breast Size of Player > 0:
		say "     She bears down upon you, her suctioned 'hands' affectionately kneading your [breast size desc of Player] breasts, [if Nipple Count of Player > 4]occasionally switching between the numerous sets. The audible noise of her suckers as they a pulled from your vulnerable flesh makes[else if Nipple Count of Player is 4]switching between the main set and the second on occasion. The audible noise of her suckers as they a pulled from your vulnerable flesh makes[else]the audible noise of her suckers as they a pulled from your vulnerable flesh making[end if] you twitch and jerk about, much to her amusement[if scalevalue of Player > 3]. It's a little awkward for her to overpower a creature substantially larger than herself, but it's transparent that she enjoys the unique challenge of abusing such a sizable beast[end if].";
	else:
	...
	if Player is male:
		...
		say "     [if Player is female]With such extensive perversion imposed upon you, it's not long before you orgasm against the creature's ministrations, your abused cunt[sfn] tightening against the small creature's intrusion[sfn]. Satisfied, she slowly pries her limbs from your beleaguered form,[else]She watched you as you writhe against her perverse affections, very much amused by your protests as she arouses you, lacking any sexual outlet to sate her relentless assault. You're about to pass out from exhaustion before she finally relents, slowly prying her limbs from your beleaguered form and[end if] departing with a merry giggle and a gentle kiss.";
...
When Play begins:
	...
	now skin entry is "wet-looking [if squidcolor is 1]red[else if squidcolor is 2]blue[else if squidcolor is 3]yellow[else]purple[end if]";
	...
	now CumItem entry is "squid cum"; [ Item to be given to the player if they have this infection and jerk off. ]
...
to say squidskinchange:
	let clist be { 0, 1, 2, 3 };
	remove squidcolor from clist;
	sort clist in random order;
	now squidcolor is entry 1 of clist;
