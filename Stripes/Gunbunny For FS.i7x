...
When Play begins:
	...
	now face entry is "very human, save for the set of [if hp of doctor matt >= 12 and hp of doctor matt < 100]white[otherwise if hospquest < 14]grey[otherwise]black[end if] bunny ears atop your head.  Your face is [if cunts of player > 0]pretty and effeminate[otherwise]girlish[end if] with a cute, if slightly bucktoothed smile";
	now body entry is "largely human-looking, with a figure that is svelte[if cunts of player > 0] and effeminate[end if].  This illusion is marred by the paw-patterned gloves stuck over your decidedly paw-shaped hands.  They are cartoonish and [if hp of doctor matt >= 12 and hp of doctor matt < 100]white[otherwise if hospquest < 14]grey[otherwise]black[end if] with pink pawpads drawn on the underside.  Your limbs are slender and sexy, though you can feel a hidden strength and bounciness in your legs, allowing you to run faster and jump higher than you ever could before";
	now skin entry is "smooth, pink and normal-looking";
	now tail entry is "You have a cute bunny tail of [if hp of doctor matt >= 12 and hp of doctor matt < 100]white[otherwise if hospquest < 14]grey[otherwise]black[end if] fur attached to your toned bubble butt";
	...
	now face change entry is "it shifts and changes.  For the most part, you're hopeful you'll come out looking human as your features settle into what becomes a pretty [if cunts of player > 0]and[otherwise]if[end if] girlish face.  Your hopes for passing as human are dashed though as you gain a set of tall bunny ears covered in soft [if hp of doctor matt >= 12 and hp of doctor matt < 100]white[otherwise if hospquest < 14]grey[otherwise]black[end if] fur";
		...
		otherwise:
			if player is pure:
				...
				otherwise if hospquest < 14:
					...
					say "     Your first few missions are easy enough, guarding equipment shipments and escorting personnel around semi-safe zones.  You finally get some proper action about a month after joining up when some centaurs get the idea that feel you're passing through their territory and attack[run paragraph on]";
					...
					otherwise if level of player >= 8:
