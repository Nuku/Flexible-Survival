...
to say koballoondesc:
	if koballoonmet is 0:
		...
		say "     While individually, these odd kobolds don't look like they'd be much trouble to deal with given their size, there are about a half-dozen of them in their group.  Their numbers and the fact that they've encircled you [if level of player < 10]puts them at the[otherwise]does give them some[end if] advantage.  You try to sound confident, telling them that you don't want any trouble and to let you pass, but they pay your words no heed.";
		attempttowait;
		if mdasslevel > 0:
			say "     'Yes, this is the one I'd spotted with our former mistress.  She has taken a liking to this one.  We could use [if cunts of player > 0]her[otherwise]him[end if],' one says as he bounces in place.";
...
to say losetokoballoon:
	...
	otherwise if cunts of player > 0:
		...
		otherwise:
			...
			say "     The one beneath you cums first, though not by a great margin.  His inflated balls, mashed beneath those of the other, are forced to give up their seed - forcing it all into you.  As you [if scalevalue of player > 3]fill[otherwise]swell[end if] from the mighty output, the one atop you blasts a torrent of cum into your ass as well.  Between the two of them, your belly and womb are [if scalevalue of player > 4]overstuffed with hot, gooey spunk[otherwise if scalevalue of player > 1]overloaded with hot, gooey spunk[otherwise]bloated and rounded until they can take no more and the rest overflows from your mouth, cunt and ass[end if].  In the end, you can only moan and groan as this vigorous fucking and filling pushes you over the top.  You cum repeatedly through the process as your fall into a daze of orgasmic bliss.  This shameful display is made all the lewder as the other surrounding kobolds masturbate messily onto the rocking trio.  You're bathed in sloppy streams of their seed, left sore, sticky and sated by the fuckfest.  They leave you there for now, exhausted[if scalevalue of player < 3] and ballooned out by all the semen filling your once [body size of player] body[otherwise if scalevalue of player < 5]and rounded by all the semen stuffed into your well-fucked holes[otherwise]and plumped by all the semen stuffed into your well-fucked holes[end if].";
	otherwise:		[***temporary]
