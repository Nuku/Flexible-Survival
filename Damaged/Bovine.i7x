...
to say losetobovine:
	...
	if Bovine_type is 1:	[Anthro Cow]
		...
		if CowBully > 0:
			if Player is female and (Mooing > 0 or a random chance of 2 in 3 succeeds):
				...
				infect;
				now CowBully is 0;
			else if "MPreg" is listed in feats of Player and (Mooing > 0 or a random chance of anallevel in 5 succeeds):
...
to say BeatTheBovine:
	...
	if Bovine_type is 1:	[Anthro Cow]
		...
		if Player consents:
			LineBreak;
			say "     You take the defeated cow's hands and remove them from your waist, telling her to present herself. She lets out a lustful moo and drops to all fours, turning quickly around. As she does so, her breasts start to leak more of her milk, the sight making you rather thirsty. A thought comes to mind and you realize that you could not only satisfy your sexual urges, but you could also quench your thirst. (Y=Drink+Sex, N=Just Sex)[line break]";
			if Player consents:
				LineBreak;
				if Player is male:
				...
				if Player is male:
					...
					say "     With one last hard thrust, you bury your [cock size desc of Player] shaft in her depths and cum hard, [if Ball Size of Player < 5]dumping your seed into this cow girl, as she moos her[else]filling this cow girl up with every last bit of fertile seed you've got, making her moo loudly in[end if] orgasm. When you're finished with her, you get up and wipe your cock off on her ass, a few last spurts of cum splattering on her back. You take a bit of perverse pride at the state you've left the cow in and gather your things before heading back along the path. As you do, the bovine woman calls out after you, 'That was fun, but I don't think my bull is going to like you very much.' You stop in your tracks, wondering whether you should ask for more information on this bull of hers, but some loud crashing noises nearby send you running as fast as you can before the source of the crashing reaches you.";
					CreatureSexAftermath "Bovine" receives "PussyFuck" from "Player";
				else if Player is female:
		...
		if VictoryOverDaBull is not 3 and (VictoryOverDaBull > 0 or CowBully > 0):
			if BodyName of Player is "Bovine":
				...
				if Player consents:
					LineBreak;
					if Player is female and (Player is fpreg_able or a random chance of 2 in 3 succeeds):
						...
						fimpregchance;
						infect;
					else if "MPreg" is listed in feats of Player and (Player is mpreg_able or a random chance of anallevel in 5 succeeds):
	...
	if Bovine_type is 1:	[Anthro Cow]
		...
		if CowBully > 0:
			...
			if monsterHP < HP entry / 4:			[ < 20 for regular play ]
				...
				say "The [one of]big bull snorts softly, trying to eat grass and completely ignoring you[or]bull looks up from his grazing for a moment, making a mooing sound at you, before returning to his meal[at random].";
...
When Play begins:
	...
	add "Bovine" to infections of TailList;
	add "Bovine" to infections of AlwaysLacList;
	now Name entry is "Bovine";
	...
	now face entry is "drawn forward into a very cow-like muzzle, ending at a large, damp nose. Large ears flick back and forward above your head idly"; [ Face description, format as "Your face is [Face of Player]." ]
	now body entry is "a cross between a bovine and a human's, large, splayed hips leading down to hoofed feet"; [ Body Description, format as "Your Body is [Body of Player]." ]
	now skin entry is "[if looknow is true]a thick mat of rough fur covering your body, patterned like Holstein Friesian cattle over a thick[else]black-and-white splotched, furred[end if]"; [ skin Description, format as "Your body is covered in (your text) skin."]
	...
	now face change entry is "it draws forward, flowing into a large blunt muzzle even as your ears migrate up your head and stretch out to have a very bovine appearance. Slow thoughts begin to slowly push away your present needs. You find yourself mooing softly under your breath"; [ Face change text, format as "Your face feels funny as [face change entry]." ]
	now body change entry is "it grows and stretches, bones growing hugely into the frame of an anthro cow, forcing your muscles and posture to follow"; [ Body change text, format as "Your body feels funny as [body change entry]." ]
	now skin change entry is "it thickens and sprouts a pattern of black and white hair, slowly merging together into a very cow-like appearance"; [ Skin change text, format as "Your skin feels funny as [skin change entry]." ]
	now ass change entry is "it stretches and splays around new bones, forming up the wide hips of a cow"; [ Ass/tail change text, format as "Your ass feels funny as [ass change entry]." ]
	...
	now MilkItem entry is "cow milk"; [ Item to be given to the player if they have this infection and milk themselves. ]
	now CumItem entry is "bovine cum"; [ Item to be given to the player if they have this infection and jerk off. ]
