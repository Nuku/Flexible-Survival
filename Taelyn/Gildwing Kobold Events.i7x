...
[to say KoboldInfectionMenu:
	...
	if calcnumber is:
		-- 1: say "[KoboldEyeMenu]";
		-- 2: say "[KoboldScaleMenu]";
		-- 3: turn the Player into an "Avalon Kobold";
		-- 4: say "[KoboldSelectiveMenu]";
		-- 5: say "     ";]

to say KoboldEyeMenu:
	say "     What color will your eyes be?";
	say "     [link](1)[as]1[end link] Red[line break]";
	say "     [link](2)[as]2[end link] Blue[line break]";
	say "     [link](3)[as]3[end link] Green[line break]";
	say "     [link](4)[as]4[end link] Yellow[line break]";
	say "     [link](5)[as]5[end link] Orange[line break]";
	now calcnumber is 0;
	while calcnumber < 1 or calcnumber > 5:
		say "Choice? (1-5)> [run paragraph on]";
		get a number;
...
to say KoboldScaleMenu:
	...
	say "     [link](2)[as]2[end link] Blue[line break]";
	say "     [link](3)[as]3[end link] Green[line break]";
	say "     [link](4)[as]4[end link] Black[line break]";
	say "     [link](5)[as]5[end link] White[line break]";
	now calcnumber is 0;
	while calcnumber < 1 or calcnumber > 5:
		say "Choice? (1-5)> [run paragraph on]";
		get a number;
...
to say ClassMenu: [This is a one time use menu for this event only]
	...
	say "[line break]     'What class do you want to know about?'";
	say "     [link](1)[as]1[end link] Warrior[if TaniClassMemory is 1] - Tani's Recommendation[end if][line break]";
	say "     [link](2)[as]2[end link] Knight[if TaniClassMemory is 2] - Tani's Recommendation[end if][line break]";
	say "     [link](3)[as]3[end link] Rogue[if TaniClassMemory is 3] - Tani's Recommendation[end if][line break]";
	say "     [link](4)[as]4[end link] Bard[if TaniClassMemory is 4] - Tani's Recommendation[end if][line break]";
	say "     [link](5)[as]5[end link] Marksman[if TaniClassMemory is 5] - Tani's Recommendation[end if][line break]";
	say "     [link](6)[as]6[end link] Scout[if TaniClassMemory is 6] - Tani's Recommendation[end if][line break]";
	say "     [link](7)[as]7[end link] Tactician[if TaniClassMemory is 7] - Tani's Recommendation[end if][line break]";
	say "     [link](8)[as]8[end link] Companion[if TaniClassMemory is 8] - Tani's Recommendation[end if][line break]";
	now calcnumber is 0;
	while calcnumber < 1 or calcnumber > 8:
		say "Choice? (1-8)> [run paragraph on]";
		get a number;
		if calcnumber < 1 or calcnumber > 8:
...
to GildwingKoboldTest:
	say "     There are likely a lot of kobolds ahead. It could take some time. Are you ready?";
	say "     ([link]Yes[as]y[end link]) - Continue[line break]";
	say "     ([link]No[as]n[end link]) - Not Ready Yet[line break]";
	if Player consents:
		LineBreak;
		...
		else if resolution of Fall-O-the-Wisp is 2: [fought and won]
		...
		else if resolution of Fall-O-the-Wisp is 3: [fought and lost]
		...
		else if resolution of Fall-O-the-Wisp is 4: [ran away]
		...
		now calcnumber is 0;
		say "[line break]     'I'm Tyrin Redscale, level 20 Chieftain,' he continues. 'Why is it that you've come to the Gildwings?'";
		say "     [link](1)[as]1[end link] - Just Exploring[line break]";
		say "     [link](2)[as]2[end link] - Wanting to Help[line break]";
		say "     [link](3)[as]3[end link] - Just Scavenging[line break]";
		say "     [link](4)[as]4[end link] - Wanting to Become a Kobold[line break]";
		if resolution of Fall-O-the-Wisp is 4:
			say "     [link](5)[as]5[end link] - Invited to Join[line break]";
			while calcnumber < 1 or calcnumber > 5:
				say "Choice? (1-5)> [run paragraph on]";
				get a number;
				if calcnumber < 1 or calcnumber > 5:
		...
		LineBreak;
		if calcnumber is:
			-- 1: say "     'If you're able to move around Avalon so freely, then maybe we could use someone like you.'";
			-- 2: say "     'Is that so?' Tyrin asks, rubbing his scaly chin in thought.";
			-- 3: say "     'We have supplies, but it's only rationed to our clan and our allies,' Tyrin says, rubbing his scaly chin in thought. 'Maybe if you became our ally, we could help you out.'";
			-- 4: say "     'Really?' Tyrin asks with a look of pleased surprise. 'If that's what you really want, we can grant you that wish, but you need to prove that you're not an enemy first.''";
			-- 5: say "     'So my brother has told me. We'd be happy to have you as soon as we know we can trust you.'";
...
to say Tracking Test:
	...
	say "[line break]     You could track him and try to find signs of Tani's movements, or you could simply call out to him.";
	say "     ([link]Y[as]y[end link]) - Track[line break]";
	say "     ([link]N[as]n[end link]) - Call Out[line break]";
...
to say Ambush Test:
	...
	say "[line break]     Do you want to Ambush them now, or Report In?";
	say "     ([link]Y[as]y[end link]) - Ambush[line break]";
	say "     ([link]N[as]n[end link]) - Report in[line break]";
	if Player consents:
		LineBreak;
...
to say Class Question:
	if StrClass is 2 or IntClass is 2 or DexClass is 2 or PerClass is 2:
		...
		if StrClass is 2:
			...
			say "     ([link]Y[as]y[end link]) - For Myself[line break]"; [Warrior]
			say "     ([link]N[as]n[end link]) - For Others[line break]"; [Knight]
			if Player consents:
				LineBreak;
		...
		if DexClass is 2:
			...
			say "     ([link]Y[as]y[end link]) - Steal[line break]"; [Rogue]
			say "     ([link]N[as]n[end link]) - Charm[line break]"; [Bard]
			if Player consents:
				LineBreak;
		...
		if PerClass is 2:
			...
			say "     ([link]Y[as]y[end link]) - Take the Shot[line break]"; [Marksman]
			say "     ([link]N[as]n[end link]) - Report In[line break]"; [Scout]
			if Player consents:
				LineBreak;
		...
		if IntClass is 2:
			...
			say "     ([link]Y[as]y[end link]) - Leader[line break]"; [Tactician]
			say "     ([link]N[as]n[end link]) - Follower[line break]"; [Companion]
			if Player consents:
				LineBreak;
...
to say GildwingClanAccept:
	...
	if Player consents: [Become a Kobold]
		...
		if scalevalue of Player > 2 or scalevalue of Player < 2:
			say "     [if scalevalue of Player < 2]You are struck with dizziness as you suddenly start to expand, increasing in size until you reach around half the size of a human[else]You are struck with dizziness as the world seems to grow around you as you shrink closer to the ground, stopping at around half the size of a human[end if].";
		...
		if Player is male:
			...
			WaitLineBreak;
		[Womb]
		if (Player is female or "MPreg" is listed in feats of Player) and ovipregalways is false:
	...
	WaitLineBreak;
	say "     'When you got separated from him, you [if LostChoice is 1]had the skill to track him down on your own. This shows you were perceptive and careful to not alert the enemy[else]called out to him, which suggests you are willing to rely on others when you need to[end if].'";
	LineBreak;
	say "     'When it came to the Knight and Squire, you [if AmbushChoice is 1]didn't hesitate to take them down when you had the opportunity to ambush them[else]understood that there could be more, and by pulling back and reinforcing the area with traps, it would deal with the threat to greater effect in the long run[end if].'";
	LineBreak;
	...
	if TaniClassMemory is:
		-- 1: say "[bold type]Warrior[roman type], a true fighter. Sometimes you just need to punch a unicorn in the face to get the job done, and you're the one with the strength to do that.'";
		-- 2: say "[bold type]Knight[roman type]; honorable and strong, you strive to use your strength to protect others. A truly noble goal.'";
		-- 3: say "[bold type]Rogue[roman type], someone with skill in agility and combat. Someone who knows that the upfront way of doing things isn't always the best.'";
		-- 4: say "[bold type]Bard[roman type], someone who realizes that sometimes it's better to manipulate others with actions and words, and that avoiding a fight is best when possible.'";
		-- 5: say "[bold type]Marksman[roman type]; with a keen eye, you find and eliminate your target from a distance. Take them out before they can reach, or sometimes even spot you.'";
		-- 6: say "[bold type]Scout[roman type]; Tani was proud of your ability to track him down, and to have the patience to fall back and request the traps. He doesn't suggest someone becoming a scout lightly.";
		-- 7: say "[bold type]Tactician[roman type]; you're intelligent and consider yourself a leader. You can use those skills to help guide others or take out enemies with wit instead of brawn. It's better to fight smart than to run in blindly.'";
		-- 8: say "[bold type]Companion[roman type]; someone whose goal is more focused on the support of others. Companions use what skills they have to heal the body and soothe the woes of their allies. They have... indirect methods of dealing with things when they have to.'";
	LineBreak;
	...
	if Player consents:
		LineBreak;
		if TaniClassMemory is:
			-- 1: now PlayerClass is "warrior";
			-- 2: now PlayerClass is "knight";
			-- 3: now PlayerClass is "rogue";
			-- 4: now PlayerClass is "bard";
			-- 5: now PlayerClass is "marksman";
			-- 6: now PlayerClass is "scout";
			-- 7: now PlayerClass is "tactician";
			-- 8: now PlayerClass is "companion";
			now TaniRecommendation is 2;
	else:
		LineBreak;
		say "[ClassMenu]";
...
instead of going east from Avalon-Crossroads while Resolution of GildwingKoboldTest is 1:
	...
	if Player consents:
		...
		say "     ([link]Yes[as]y[end link]) - Join the Gildwings[line break]";
		say "     ([link]No[as]n[end link]) - Decline For Now[line break]";
		if Player consents: [Join Gildwings]
			LineBreak;
			say "[GildwingClanAccept]";
		else:
