...
to say losetoshadowbeast:
	if a random chance of 1 in 3 succeeds and inheat is not true:		[creature demands oral]
		...
		CreatureSexAftermath "Player" receives "[if Player is female]Pussy[else]Ass[end if]Fuck" from "Shadow Beast Male";
	else:
...
When Play begins:
	...
	now enemy title entry is "Male Shadow Beast"; [ Name of the encountered creature at combat start - Example: "You run into a giant collie." instead of using "Smooth Collie Shemale" infection name. ]
	...
	now desc entry is "     [one of]You see something shimmer out of the corner of your eye[or]You feel a strange presence behind you[or]You are overtaken by a sense of dread[at random] and you turn to see a black cat step out of the shadows. It looks like a regular panther on all fours, except for the two black tentacles which sprout from its back. They end in gripping pads, which you imagine are ideal for holding victims still. Between its legs you can see a hefty sheath and testicles, demonstrating the breeding ability of the beast. It seems to shimmer, like it's manipulating the light around it; suddenly, it disappears for a second before reappearing before you, claws extended![line break]";
	...
	now CumItem entry is "shadow beast male cum"; [ Item to be given to the player if they have this infection and jerk off. ]
...
to say ResolveEvent Hunted Survivor:
	...
	if the player is not facially human or the player is not bodily human:
		...
		say "     Uh-oh - what now? Maybe try to [link]help this man (Y)[as]y[end link]? Or would you rather [link]get out of here (N)[as]n[end link] before you're the next target of a hungry - or horny - monstrosity?";
		if Player consents:
			LineBreak;
