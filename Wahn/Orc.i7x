...
to say SubmitToOrcSlavers:
	if anallevel > 1:
		if "Dominant" is listed in feats of player:  
			...
			say "     [line break]";
			say "     Note: Since you've selected [']Dominant['], you won't be dragged off to the orc lair, as it's hard to avoid being used as a fucktoy there. This is to avoid the need for a 'player kicks and sreams, struggling against everything' variant in all scenes.";			
		otherwise: 		
...
to say OrcLairMainChamberWatching:	
	...
	if randomnumber is:
		...
		-- 4:
			...
			say "     All the bellowing and excitement about the close competition between those two totally holds your attention captive and you almost forget for a moment that you have to stay hidden. Only when the orc finally manages to get the upper hand and brings is opponent crashing to the floor with a loud thunk do you manage to shake your strange fascination with their exploits off and duck back into the dark shadows of the hallway. A cross between a bovine bellow and a wimper a moment later tells you that the orc decided to savour the pleasures of his victory right away...";
		-- 5:
