...
[  20: player suggested closing the door as they fuck             ]
	...
	if BrennanRelationship is 0:
		...
		if diceroll + bonus >= 15: [success]
			...
			if diceroll + bonus >= 15:
				say "     With a resounding crack, the doorframe around the lock gives out, sending a shower of splinters into the room beyond as you break in. The apartment is indeed in use - lived in, if empty of people right now. You set out to search it thoroughly.";
				say "[Apartment2bLooting]";
			else:
...
to ArmyConvoyEvent:
	...
	while calcnumber < 1 or calcnumber > 3:
		...
		if calcnumber is 1 or calcnumber is 2 or calcnumber is 3:
			...
			say "Invalid choice. Type [link]1[end link] to go on the trip, [link]2[end link] to postpone or [link]3[end link] to decline.";
	if calcnumber is 1:
		LineBreak;
		say "[ArmyConvoyTrip]";
...
to say ResolveEvent Wolf Whisperer:
	if BrennanRelationship is 7:
		...
		if Player consents:
			...
			say "     The wolf by Brennan's side is hurt badly too, missing a third of his front leg and licking forlornly at the remaining stump. It actually looks like the paw is on its way of re-growing, but that takes it out on the rest of him, as Remus looks rather starved and lean now that the nanites are shifting mass around to complete him again. 'Thank god, he found you!' Brennan groans out as he sees you come in. 'We got attacked by a drake! Man, that was one nasty lizard. Managed to drive it off in the end, but it was a close thing. And... well, you see what state we're in. I - we - need your help. No way we'd make it home safely, not in this state. Romulus and Remus have been on the mend like they usually do, but they've eaten everything I had with me and are still looking half-starved. And myself... Well, this bite hurts like a bitch and I don't heal as fast as the infected.'";
			WaitLineBreak;
			say "     After all the adventures you went through together, you tell Brennan that you will of course help him out. A smile crosses his bearded face as he lets out a relieved sigh, then packs his gear and eventually stands up slowly - wincing as he has to put weight on his wounded leg. Brennan starts using his baseball bat as a makeshift walking stick, allowing him to walk - not particularly fast, but at least he's mobile. The four of you set out onto the street, with yourself being the lead for the group of limping invalids. At first things go well, as apparently the fight between your companions and the drake frightened off the usual inhabitants of a block or so around the site of the action. Sadly, this doesn't last... a harpy spots your group from above, and decides to swoop down upon you, ready to attack!";
			challenge "Harpy";
			if fightoutcome < 20: [player won]
...
instead of going north from Smith Haven Mall Lot South while (Entrance Checks is active and Entrance Checks is not resolved and BrennanRelationship > 7 and BrennanRelationship < 90 and a random chance of 1 in 3 succeeds):
	...
	project the figure of Brennan_clothed_icon;
	say "     As you walk up to the entrance door of the mall, you see a familiar trio ahead of you - it's Brennan and his two wolves, Romulus and Remus! They are being held up from going inside by a wolverine guard with the nametag 'Logan' on his chest. [if LoganCommand > 0]He is the black sheep of wolverine security, relegated to guarding the outside of the mall. [end if]'What do you want?' the mustelid asks gruffly, eyeing the human survivor with a frown on his face. Brennan replies, 'Trade. I heard there are shops still open inside this mall. Got quite a bit to barter.' He pats the side of his bulging backpack lightly. Leaning forward to sniff at the man, Logan tells him, 'You got a weird scent for a human, but fine. Next! And no bitching if you grow a tail after moving through the crowd in there!'";
...
When Play begins:
	...
	add "Human Survivor" to infections of HumanList;
	add "Human Survivor" to infections of NatureList;
	add "Human Survivor" to infections of TransList;
	add "Human Survivor" to infections of BipedalList;
	...
	now MilkItem entry is "human survivor milk"; [ Item to be given to the player if they have this infection and milk themselves. ]
...
to say BrennanTalkMenu:
	...
	[]
	if (XP of Brennan is 1 or XP of Brennan is 2) and (Trickster's Masterpiece is resolved or Camp Bravo Entrance is known):
		choose a blank row in table of fucking options;
		now title entry is "Camp Bravo";
		now sortorder entry is 5;
		now description entry is "Tell him what you found at the Camp";
	[]
	if (XP of Brennan > 2 and XP of Brennan < 50):
		choose a blank row in table of fucking options;
		now title entry is "Tiny Tim";
		now sortorder entry is 6;
		now description entry is "Talk to him about the minotaur of Camp Bravo";
	[]
	if (XP of Brennan is 3):
		choose a blank row in table of fucking options;
		now title entry is "Tracking doctor 'Diego Garcia'";
	...
	while sextablerun is 0:
		...
		if calcnumber > 0 and calcnumber <= the number of filled rows in table of fucking options:
			...
			if Player consents:
				...
				if (nam is "Camp Bravo"):
					say "[BrennanSoldierTalk3]";
				if (nam is "Tiny Tim"):
					say "[BrennanTinyTalk]";
...
to say BrennanSoldierTalk3:
	if HP of Major Padgett is 100: [player got banned from Camp Bravo]
	...
	else if XP of Brennan is 1: [bringing this up the first time]
		say "     You quietly tell Brennan about what's been going on in Camp Bravo, relaying all the sordid details of their sexual escapades with a captured minotaur. The human survivor's face takes on a grim expression as he listens to your words. 'They do [italic type]WHAT?![roman type] That, that makes no sense at all! I - we - need to do something about this. There must be a way to convince their commander to stop this madness.'";
		say "     [bold type]Will you show Brennan the way to the army camp?[roman type][line break]";
		LineBreak;
		...
		if Player consents:
			...
			LineBreak;
			say "     Brennan looks rather disappointed when you refuse to set out right away. 'Fine,' he says after taking a deep breath, then looks you in the eye. 'Do what you must, but please hurry. When you come back, we can go there together. I firmly believe that we'll achieve more as a team than if I just turn up alone. Think of all the innocent young men that are being ordered to get fucked by that beast!'";
			now XP of Brennan is 2; [postponed]
	else if XP of Brennan is 2: [player postponed the trip previously]
		say "     Brennan's eyes light up the instant you mention Camp Bravo again. 'Finally! Let's get going then!' he says, full of elan to deal with the situation around the minotaur.";
		say "     [bold type]Will you show Brennan the way to the army camp?[roman type][line break]";
		LineBreak;
		say "     ([link]Y[as]y[end link]) - Sure, let's go right now.";
		say "     ([link]N[as]n[end link]) - Tell him that right now is a bad time.";
		if Player consents:
			...
			LineBreak;
			say "     Brennan looks rather disappointed when you refuse to set out right away. 'What? Again?!' he says, then forces himself to take a deep breath. With an urgent look into your eyes, he adds, 'Do what you must, but please hurry. When you come back, we can go there together. I firmly believe that we'll achieve more as a team than if I just turn up alone. Think of all the innocent young men that are being ordered to get fucked by that beast!'";
	else if XP of Brennan is 3: [player went to Camp Bravo with Brennan before]
		say "     Approaching Brennan and bringing up Camp Bravo again puts the bearded man into a somber mood. He strokes his chin and says, 'So - now we've been to the place and learned a bit more. Still, it irks me that we couldn't do anything about the matter when we were there. I do know that things aren't as dire as I originally thought, but hell, that still doesn't make it [italic type]right[roman type]. Tiny may be well-liked regardless, but their commanding officer is still basically whoring his men out. That's just... gah!' You put your hand on the man's shoulder and tell him that you understand his frustration, then suggest that some of the ideas Sergeant Alexander brought up might lead to a proper reckoning for the major commanding Camp Bravo. The two of you will need some good and solid evidence to show the 'Patient Zero' story built up around Tiny being a hoax.";

to say BrennanCampBravoTrip1:
	say "     After Brennan collects his gear and says goodbye to Romulus and Remus, the two of you set out into the streets of the city. Your way to Camp Bravo leads you through various back alleys, avoiding any open areas that might contain larger numbers of infected creatures. The few creatures that you come across do their best to avoid two such capable-looking survivors out on a mission, so you arrive at the army camp without any trouble. Seeing it from afar, Brennan looks over the cleared perimeter around the place, as well as the cars used as a blockade near the entrance. He gives a nod to himself, as if content that - whatever is happening inside the place - the soldiers are at least observing the usual safety precautions.";
	...
	say "     As you enter the camp, the hitching post comes into sight, the ground underneath still wet with a puddle of drying cum. Not too far away from it, the minotaur is stretched out on two stacked king-size mattresses, dozing contently. Brennan stares at the mythical beast for a few seconds, then shakes his head in disbelief, losing the last fleeting hope that you might just have been exaggerating. 'Come on, let's end this farce. The commander can't be completely devoid of reason,' the human survivor says, his tone rife with agitation. Struggling to keep up with his determined steps, you make it halfway across the parade ground (or rather 'breeding ground' these days) before a soldier steps into Brennan's path. 'I wouldn't do that if I were you,' the man tells your friend in an earnest tone, raising a hand to stop Brennan from just brushing past him.";
	say "     Frowning, Brennan does indeed stop moving, then glances at the man's nametag and replies in a somewhat accusatory tone, 'And why not, Sergeant Alexander?' The tough, grizzled soldier simply says, 'Not here. Come, follow me.' After exchanging a surprised look with you, Brennan follows the soldier, as do you. It does not take long for your little group to reach a secluded tent in the corner of the camp, which Alexander leads you into. Inside, there are quite a few boxes, all filled with various forms of non-edible supplies. The experienced soldier runs a hand through his hair, then leans back against a stack of boxes, crossing his arms. 'Well then, what do you think will happen if you go into that tent and talk to the major?' he asks in a gruff tone, looking at Brennan and raising an eyebrow.";
	WaitLineBreak;
	say "     The human survivor steps forward and starts to explain, 'I'll tell him how fishy that story about that minotaur being 'patient zero' really is - the nanites don't work that way. Everyone would just be minotaurs if it was true. I've seen enough out in the city to know that. And then-' With a single word, Alexander brings him to a halt: 'Bull-[italic type]shit[roman type].' Shaking his head, the man steps forward to say, 'Let me take you through what would happen. You go in, say your piece, and he has you thrown out. You've got no proof, and even though what you say makes sense, not even we here in this tent [italic type]know[roman type] if the minotaur thing really is a hoax. And what's more, the major bought the story, and now he is committed to it, no ifs or buts about it. It [italic type]has[roman type] to be true from his viewpoint, because if it's not, he'd be DONE - sent to the farthest reaches of Alaska to hide the embarrassment, most likely.'";
	say "     'Um, but...' Brennan starts, then pauses and bites his lower lip. He looks visibly deflated at Alexander's harsh words, his gaze wandering through the tent as he mulls over what the soldier just explained. 'So, it's hopeless then?' the muscular man asks, looking first to you, then to Sergeant Alexander again. 'I didn't say that. We just need to get actual proof, not just your best guesses at what the nanites will do. Are you a certified scientist or doctor? No? Then we're out of luck unless either of you know some qualified specialists who could give testimony. Or you could go check out the guy who sold the major this story - doctor Diego Garcia. Apparently he was working on something similar at the big hospital downtown? Could try finding him, or his office and papers.' Brennan nods slowly, his previous determination returning. Then he raises the point, 'But that'll still not help all the men and women here in the camp. We can't just leave things as they are.'";
	WaitLineBreak;
	say "     Sergeant Alexander blows out his cheeks and gives a shrug. 'Well, until you get us proof that this whole thing is nothing but a pipe dream, we've still got orders to follow. And who knows - all those strapping lads sacrificing their cherries to a bull-man might yet save the world. Besides, we got a huge delivery of immunity booster shots before the radio interference started... I think the men and women will be fine.' The grizzled soldier chuckles as he adds, 'For a nine feet tall mythical beast, Tiny is actually quite amicable. I know a few guys who visit him at night, just on their own accord. He's become more or less a beloved mascot of the camp. That's actually another reason why you shouldn't just try to expose the major flat out. He might snap and try to hurt Tiny or something.'";
	say "     'People like him? Really?' Brennan asks, surprise clearly visible on his face as he goes on to say, 'The minotaurs out in the city that I've seen are quite aggressive.' Alexander chuckles and pats the other man on the arm, 'Come on, you should meet him.' He leads the two of you outside again, then back to the parade ground and up to the dozing minotaur after collecting an apple from a supply crate and handing it to Brennan. The tough soldier bends forward and gives the sleeping beast-man a little shake as he says, 'Hey Tiny, got someone here who wants to say hello.' The large creature lets out a deep huff through his nostrils, then opens up his eyes, sitting up in a show of rippling muscles. 'New friend?' he asks his handler, still a bit sleep-drunk, glancing from Alexander to Brennan.";
	WaitLineBreak;
	project the Figure of TinyTim_icon;
	say "     The human survivor doesn't have to be prompted to offer the minotaur his apple, which Tiny accepts right from his hand, chomping down on it in one piece and soon swallowing it down. 'Nice breakfast,' the muscular brute says, smiling at his new acquaintance before pushing himself up to a stand, now towering over Brennan. He leans forward and sniffs at the man, then licks his face and proclaims, 'Friend smells good. Tiny likes new friend.' A throb goes through the massive manhood dangling between his legs and it grows a little bigger. 'Friend want to fuck?' comes the eager request right after, which Brennan hastily refuses with the words, 'Thanks, but not right now!' The minotaur turns his attention to Alexander, who explains to him that more sex isn't planned right now and that he can rest a bit more.";
	say "     After the memorable encounter, Sergeant Alexander accompanies you two to the camp entrance, sending you off with best wishes in your efforts to expose the minotaur cum sham. As you make your way back to Brennan's apartment, the man comments, 'Well, that was... educational. Guess I'll have to adjust my preconceptions quite a bit. We can talk about what our next plans are later.'";
	now XP of Brennan is 3; [evidence gathering suggested]
...
to say BrennanDiegoTracking:
	...
	say "No, you'll have to attack the supposed 'facts' that the story rests upon instead. Alexander passed along the info that 'Diego Garcia' was a prominent doctor at the City Hospital and worked on something possibly nanite-related, so that seems a good place to check. You explain your reasoning to Brennan, who nods in agreement. 'I've avoided the area until now, to be honest. When the outbreak hit, many people gravitated to the place to look for help, so you can assume the building is still packed with the infected even now. But hey, nothing we can't handle as a team, right? And we can take the wolves too.'";
	say "     [bold type]Do you want to travel to the hospital with Brennan?[roman type][line break]";
	LineBreak;
	...
	if Player consents:
		...
		say "     'How strange, the last time I scouted this area, there were lots of people flocking here. Well, and fucking in the streets. I wonder where they all went,' Brennan comments, looking around and prodding a few scraps of cum-stained clothing with his boot. 'At least the emergency power of the place is working. I had feared it would be pitch black in the depths of that building.' Your little group walks towards the lobby, finding the automatic doors sliding open for you. The waiting area is presently clear of people and creatures, through there is a lot of scattered clothes, as well as gurneys, crutches and other medical items. In some cases, these are piled up in an attempt to barricade the doors. Then suddenly, moaning can be heard from somewhere ahead, telling you that the place is not quite abandoned after all.";
		WaitLineBreak;
		say "     Both Romulus as well as Remus ears perk up and they start to walk forward, only to be stopped by Brennan, who quickly crouches down and holds onto the wolves. While he starts to whisper to them about having to stay quiet and not attracting the attention of everyone you four might meet in here, you sneak forward and peek over the admittance desk. On the ground behind it is a vixen nurse, riding a horseman stretched out on the ground. She obviously enjoys his equine shaft quite a lot, judging from the sounds of his cock squishing in and out of her. Seems you're in luck for exploring deeper into the hospital - these two certainly won't cause any trouble for quite a while. Stepping back quietly, you turn around and put your finger to your lips, then mime to Brennan that there is someone fucking behind the desk.";
		say "     The bearded man simply nods, then points towards the overview next to the (powerless) elevators, tapping the part that says 'Fourth Floor - Administration'. He nods towards the stairway not too far away, then shoos the wolves that way too. Joining them at the door a moment later, you walk through it into the stairwell, with Brennan closing the door behind you slowly, as not to make a sound. 'We got to be careful in here,' he says to the wolves and you in a quiet tone, then adds, 'There might be countless infected all throughout the building, one never knows what the next room might hold. Let's stay close together too.' With that said, the survivor leads you up the stairs. Arriving at the first floor landing before long, you have a short glance through the vertical view-slit in the door on that floor.";
		WaitLineBreak;
		say "     There is someone right on the other side, leaning against the door, so you mainly see white and just a little brown fur at the bottom end. The person seems to turn a little to the left and suddenly lets out a screech, gibbering manically and stumbling off to the right, at which point you can see that it is an anthro weasel, his arms stuck in a straight jacket. The infected man is clearly disoriented, most likely heavily medicated, so he doesn't get far before a sprinting jaguarman in blue-green hospital scrubs hunts him down. The muscular anthro grabs his captive and starts to guide him back towards your left - only to stop after a few steps and seemingly think better of it. Then he pushes the straight-jacketed weasel against the wall and pulls out his hard, feline cock to line it up with the man's bare butt. The wet splotches in his fur tell you that this isn't the first time he takes advantage like this.";
		say "     Brennan has also been watching through the little window, and you can see his knuckles whiten in their grasp of his bat - so you grasp his shoulder and shake your head. He was right about the danger in here earlier, so even if you 'saved' the weasel, there might just be more jaguarmen just around the corner. You can't save everyone, and your current concern is Camp Bravo, you quietly remind him, pulling Brennan away from the window as grunts and moans start up on the other side of the door. The landing one further floor up holds a new peril - here, you can't see anything through the window of the door, just greenish gunk splattered all over it from the other side, with a trickle of the fluid seeping through under the door. Wolves and survivors both, your little group jumps over the hazard, then continues onward.";
		WaitLineBreak;
		say "     On the third floor, there is little to see through the window of the door. Someone has thoroughly barricaded it from the other side, filling the hallway with beds, chairs and all sorts of medical devices. No way anyone is gonna get through that! Thankfully, you weren't intending to anyways. As for the door to the fourth floor - it is missing, completely. Even the hinges are gone, seemingly pried out of the wall through brute strength. Eyebrows raised, Brennan looks at you, then shrugs his shoulders and steps through first, his bat held at the ready. Following the man after his wolves, you find yourself in a dimly lit corridor, filled with further empty doorframes. Your human friend leads the way towards the left, glancing into the rooms quickly before waving everyone forward to walk past. Soon, he stops at one, nodding to the sign on the wall that reads 'Accounting'. Beyond lies a mid-sized office with four workstations, all of which have illuminated computer monitors.";
		say "     Brennan goes to sit down at the nearest computer and starts searching for what you need while you stand guard at the entrance, glancing up and down the eerily quiet corridor. It doesn't take all that long before the bearded man says in a quiet tone, 'I found it. D. Garcia actually did work here. But the D stands for 'Delores', and she was a dentist. Nanite specialist my ass! Let me just print this out on official hospital stationary and we got our first bit of evidence.' After a few more strokes of the keyboard and clicks with his mouse, Brennan gives you a thumbs-up. Then suddenly, an ungodly racket starts in the room - screeching, mechanical sounds, emanating from a device on a table in the back. 'You're kidding me, who has a [italic type]needle printer[roman type] these days?' Those words are as far as he gets before a misshapen creature sticks its head out of the rooms further back in the hallway, then comes running towards you, immediately followed by another mismatched chimera!";
		challenge "Mismatched Chimera";
		if fightoutcome < 20: [player won]
			say "     You overwhelm the weird, mismatched beast, landing a last blow to its face before the creature keels over backwards. Looking around for your compatriots, you see that Brennan has laid low his own opponent with a blow from his trusty bat, with the wolves standing guard over it now and looking for any twitch or groan that might indicate it will get up again soon. 'That was close,' the human survivor says, inspecting your unconscious opponent. ";
		else if fightoutcome > 19 and fightoutcome < 30: [lost]
			say "     The weird, mismatched beast throws you to the ground and is about to bring its foot down upon your face when suddenly, Remus jumps it and bites deeply into the creature's leg. It stumbles around, trying to shake the wolf off, which exposes the chimera to the baseball bat that Brennan swings with all the force in his muscular arms. With a solid *thunk* of the bat hitting the back of the disfigured being's head, it collapses on the floor, unconscious but still letting out rattling breaths. 'That was close,' the human survivor says, glancing over to where Romulus is standing over the similarly knocked out second beast. ";
		else if fightoutcome is 30: [fled]
			say "     You turn around to run from the weird, mismatched beast, sprinting down the corridor you came along while being chased by the creature. It is surprisingly fast for having such a lumbering, misshapen stature, so it almost catches you - until suddenly Remus jumps into the chase and bites deeply into the creature's leg. It stumbles around, trying to shake the wolf off, which exposes the chimera to the baseball bat that Brennan swings with all the force in his muscular arms. With a solid *thunk* of the bat hitting the back of the disfigured being's head, it collapses on the floor, unconscious but still letting out rattling breaths. 'That was close,' the human survivor says, glancing over to where Romulus is standing over the similarly knocked out second beast. ";
		say "'Look at them. This wasn't the infection! Someone [italic type]made[roman type] them into these monstrosities. God, that's fucked up. Let's get the documents and leave this vile place!' With that said, he hurries into the room, collecting what the printer has finished by now, then quickly laying the papers into a document folder and placing it in his backpack.";
...
to say BrennanScavTrip:
	...
	say "     Acknowledging his concerns, you agree to serve as a lookout, and after a moderately long trip through the chaotic streets of the city, you find yourself ";
...
to say BrennanScavPayout:
	...
	WaitLineBreak;
	say "     The four of you leave the building after that, moving out into the dangerous streets of the city. The trip back to the green apartment building is not without its own perils, including a few close encounters with aggressive infected, but between traveling in a group and taking stealthy side alleys whenever possible, you eventually arrive back at Brennan's home without having to fight again. A short elevator ride later, you are back where you started. 'Glad you joined us for scavenging today my friend,' the human survivor tells you with a smile, then yawns. 'I'm pretty beat though - so I hope you'll excuse me in going to bed now.'";
	move Brennan to Brennan's Bedroom;
...
to say BrennanSexMenu:
	...
	if Player is male and lust of Brennan > 20 and lust of Brennan < 99: [participated or watched wolf sex openly before]
		...
		now description entry is "Fill all of the trans-guy's holes";
	[]
	sort the table of fucking options in sortorder order;
	repeat with y running from 1 to number of filled rows in table of fucking options:
		choose row y from the table of fucking options;
		...
		if calcnumber > 0 and calcnumber <= the number of filled rows in table of fucking options:
			...
			if Player consents:
				...
				if (nam is "Take Brennan's cock in your pussy"):
					say "[BrennanSex6]";
				if (nam is "Take Brennan's cock in your ass"):
					say "[BrennanSex7]";
				if (nam is "Fuck Brennan's pussy and triple team him with the wolves"):
					say "[BrennanSex8]";
				now lastfuck of Brennan is turns;
				if Dexterity of Brennan is 0:
...
to say BrennanSex4: [pussy-fucking Brennan]
	if Libido of Brennan > 5 and Libido of Brennan < 100: [he has the magic dick]
		...
		WaitLineBreak;
		say "     Sliding yourself out of his sex and rubbing your dripping wet dickhead against his sensitive clitoris for a short moment, you then ram yourself - all of your hard cock - back into him, never stopping until your balls slap his crotch. Then, you do it again! 'Holy fuck!' Brennan almost shouts and bucks against you, the sudden change from your previous thrust and grind surprising his lust-fogged mind and pushing him straight over the edge. His leg bracing against the floor goes a little weak as the man's whole body trembles in climax, so you hold onto him, pinning your lover against the wall as you feel his inner muscles squeeze your cock. Moaning loudly and openly, Brennan leaks copious amounts of femcum all over your manhood and it starts to drip off your balls and trickle down the inside of his legs - a sensation and mental image that pushes you past the point of no return yourself in short notice.";
...
to say BrennanSex7: [taking Brennan's dick (ass)]
	...
	WaitLineBreak;
	say "     'Time to get this party started,' Brennan says in a teasing tone and finally frees his canine shaft from the loose shorts, letting them drop to the ground afterwards. This leaves Brennan standing fully naked before you, a breathtaking sight of well-trained masculinity, complete with a large canine cock between his legs. It stands large and erect over the tanned skin of his body, more reddish itself and coming with a pair of hairy (or rather furry) balls. Brennan closes the fingers of his right hand around the hard shaft, stroking it in obvious pleasure and milking a glistening drop of pre-cum from the cum-slit at the tip. Wiping that off with the index finger of his other hand, he looks at the sticky string he can form between his fingers with it, for a second lost in the wonder of what magic has given him, before he focuses on you again and gives you a broad grin.";
	say "     After collecting a little squeeze-bottle of lube, Brennan joins you on the bed, coming to kneel between your soon spread legs and leaning forward to kiss you deeply. 'Thank you for all of this, I still sometimes can't believe how much you've done for me,' he says with conviction, letting his hands move over your chest and sides, stroking and caressing them. Then he lubes himself up, making his cock good and slick, before reaching for your thighs and slipping his fingers under them, raising and spreading your legs. Moving his broad-shouldered body to cover yours, the human survivor rubs his erection up and down your butt-crack, then carefully lines himself up with the pucker. With a need-filled grunt, he pushes forward a second later, stretching your back passage around his invading member. 'Fuck yeah,' he adds in a breathless sigh, 'I love being inside you! This feels incredible!' Sinking himself fully into you - including the still-small bump of his knot that pops past your opening after a little extra push - Brennan revels in the tightness of your asshole or a little while, just taking in the feeling of such an intimate connection.";
	WaitLineBreak;
...
to say BrennanSex8: [pussy-fucking Brennan + wolves]
	if Libido of Brennan > 5 and Libido of Brennan < 100: [he has the magic dick]
		...
		say "     Brennan's eyes widen for a second, then a grin appears on his face and he says, 'Want to go to town down there, eh? Be my guest.' Chuckling, you crouch down and impatiently pop open the button of his pants and lower its zipper before wrenching the piece of clothing down, sliding it over his muscular thighs. The baggy boxer shorts beneath hang loosely over his crotch - not surprising, as the man doesn't have a cock after all. With an eager Brennan looking down upon you, you tug his pants off all the way while he slides his the tight shirt over his head and throws it aside. Glancing up, you watch the a glorious show of rippling abs and stretching muscles that creates. You then go for the final piece, grabbing the fabric of his shorts and tugging them down to his feet in one quick swoop. This leaves Brennan standing naked before you, a breathtaking sight of well-trained masculinity, contrasted by the fact that he has a pussy instead of a cock between his legs.";
		WaitLineBreak;
	say "     Crouched before your human friend with your face pressed against his crotch, you huff the heady scent of his sex and sweep your tongue deep inside, savoring the sweet nectar of his sexual fluids. Then you pull back and smile up at him while standing up, quickly peeling off your own clothes and then meeting Brennan's lips for a deep kiss. Making out and with his arms wrapped around you, the two of you stumble through the apartment to soon reach his bed. The trans-man gives you another quick peck on the lips, then lies down and spreads his legs invitingly, his pussy's glistening wetness in open view for you. 'Take me,' he calls out and lets out an eager sigh from the anticipation. His bearded face draws into a broad smile as you climb on the bed too, then slide yourself between legs and nudge the nether lips of your lover apart with your dickhead.";
	...
	WaitLineBreak;
	say "     Remus seems to be experiencing much the same in the other hole, and he lets out a victorious howl at full volume as his knot swells and ties his manhood inside Brennan's ass. Fur-covered canine balls throb hard as they erupt into the wolf's master, breeding the human survivor with a copious load of virile cum. You too can feel the rounded bulge of Remus knot, pushing against your cock inside of Brennan, and just like you two fucked Brennan together, you cum inside him almost at the same time too. With a deep moan, you push your hips up against Brennan and erupt into the depths of his pussy, spurting hot seed into your friend. The three of you slow down as you ride out your orgasms, only lightly grinding against one another in the blissful haze that fills your minds.";
...
after going to Survivor Refuge while (Soldier Questions is not resolved and XP of Brennan is 0 and (TimekeepingVar is -2 or TimekeepingVar is -3 or TimekeepingVar is -4 or TimekeepingVar is 6 or TimekeepingVar is 5 or TimekeepingVar is 4) and a random chance of 1 in 2 succeeds): [first time, Brennan awake and in the apartment, 50% chance]
	project the figure of Brennan_face_icon;
	say "     As you enter the main room of the apartment, Brennan walks up to you, a thoughtful expression on his face. 'Listen, I met some soldiers recently that were acting a little strange. The two of them were marching from the downtown area to the city's containment perimeter, and they started getting really... squirrelly when I asked them what they were up to.' Rubbing his bearded chin, the human survivor frowns a little as he goes on to say, 'You know, it wasn't the usual 'Please step back sir! We got orders to maintain a quarantine.' No, these guys immediately went beet-red in the face and murmured something about a 'special mission', then hurried along as if trying to shake me off. I kept up with them for a little while and asked more questions, but all I could get out of them was that they came from 'Camp Bravo'. Have you ever heard of that place? Seems like something weird is going on there, and I'd like to know more in case it's something we need to be worried about.'";
...
to BrennanRomulusRemusUndieSniff:
	...
	if Player consents:
		...
		LineBreak;
		say "     Shaking your head to throw off the mental images that the wolves behavior created, you step forward and snatch the shorts from the wolf's muzzle. Admonishing the beasts with some choice words, you then stroll into the bathroom and throw the underwear back into an open clothes hamper, following up by closing its lid securely.";
...
to BrennanRomulusRemusThreesome:
	say "     Standing in the main room of Brennan's apartment, you hear some unmistakable sounds from behind the bedroom door: moans, grunts, and the yips and barks of excited wolves. Curiosity makes you walk closer, and soon you see some movement through the leaned-to door - Brennan is lying naked on the bed, together with his two wolves.";
	...
	if Player consents:
		...
		say "     The wolf's cock penetrates Brennan deeply, sinking into the sweltering depths still so very wet from the man's previous orgasm. Clearly, it rubs the human survivor's insides in a [italic type]very[roman type] right way, making him groan - then shout - in lust as Romulus goes for a follow-up hump and sinks himself into him almost all the way, with just the knot outside Brennan's nether lips. 'Fuuck yeah! That's so, so good. I want it all!' Brennan gasps out, then lets his hands wander down to Romulus's hips. Planting them on the wolf's form, he gives a sudden yank downwards, forcing the full length of his erection into his vagina, knot and all. For a moment, Brennan just lies on the bed with his eyes closed and a broad grin on his face, taking in the full sensation of his beloved companion's hard shaft. Meanwhile, the bedroom is filled with excited barks as the wolves 'talk', both apparently shocked at their luck at serving this alpha and being allowed to fuck him.";
		WaitLineBreak;
		say "     Remus even jumps up from where he was lying, going to see and sniff between Brennan's legs and lick the human's pussy as well as his pack-mate's balls. Romulus twitches at the stimulation, his erection grinding against Brennan's insides and making him buck against his waist, opening his eyes and moaning loudly. The wolf still hesitates for a second, concern on his face as he gives a little questioning whine to his master. Brennan puts his hand on the wolf's neck in reply, petting him happily and saying, 'No need to be shy. Take me like the wild beast you are!' His master's permission is all the wolf needs to start rutting his human mate [italic type]hard[roman type], driving deep into his soaked, squeezing depths as his knot rams roughly against those achingly needy folds. The wolf's frenzied fucking comes as no surprise after having eaten Brennan out, his nose still full of those enticing feminine scents and arousal further spurred from all the earlier petting and kissing.";
...
to BrennanWolfRepeatThreesome:
	...
	if calcnumber is 1:
		...
		say "     [link](1)[as]1[end link] - You actually came in to join them. (Have some fun with the second wolf.)";
		say "     [link](2)[as]2[end link] - Tell him that you understand and it is kinda hot to watch them.";
		say "     [link](3)[as]3[end link] - Tell him that it is alright, but he should really shut the door the next time.";
		say "     [link](4)[as]4[end link] - Say that he's on a slippery slope and should better stop.";
		now calcnumber is 0;
		while calcnumber < 1 or calcnumber > 4:
			...
			if calcnumber is 1 or calcnumber is 2 or calcnumber is 3 or calcnumber is 4:
				...
				say "Invalid choice. Type [link]1[end link] to join, [link]2[end link] to watch, [link]3[end link] to tell him to shut the door or [link]4[end link] to tell him to stop.";
		if calcnumber is 1:
			LineBreak;
			say "     Clearing your throat and telling Brennan with an eager grin that you wanted to join their fun, the bearded man's reaction is, 'Oh? Oooooh! Wow, I didn't think that could be your reaction to all of this. What exactly did you have in mind though? I fear I'm a bit tied down right now.' Rubbing the side of the large wolf knotted with his pussy and lying on top of him, Brennan gives you a grin and a chuckle, then reaches out to Remus. 'Wanna take care of our friend over there?' he asks the feral beast and gets a happy bark as a reply. Tail raised high and wagging, Remus bounds over the bed to you and licks your hand eagerly. His hard shaft is dangling between the wolf's legs, ready for action.";
			say "[WolfRepeatThreesomeSexMenu]";
		...
		else if calcnumber is 3: [suggest closing the door]
			LineBreak;
			say "     Giving the human survivor a friendly smile and nodding understandingly, you tell him that you simply were curious about the noises coming from behind the only leaned-to door. As you go on to say that there is nothing to be ashamed about his current position, the trans-man relaxes visibly, then blushes a little as you suggest closing the door all the way next time. After a short glance at Remus, who by now has circled around the knotted-together lovers and his nuzzling Romulus's balls and Brennan's pussy, you give Brennan a wink and tell him to enjoy himself, then leave the room. As you gently close the door, a pleased bark followed by humping sounds starts to come from inside the room. Seems like the second wolf is getting some action too.";
			now lust of Brennan is 20; [player suggested closing the door as Brennan and the wolves fuck]
		else if calcnumber is 4: [tell him to stop]
			LineBreak;
...
to say WolfRepeatThreesomeSexMenu:
	project the figure of Brennan_face_icon;
	say "     What do you want to do with Remus?";
	LineBreak;
	now sextablerun is 0;
	blank out the whole of table of fucking options;
	[]
	...
	if Player is female:
		...
		now sortorder entry is 3;
		now description entry is "Get fucked by a canine dick";
	[]
	choose a blank row in table of fucking options;
	now title entry is "Take Remus's cock in your ass";
	now sortorder entry is 4;
	now description entry is "Get fucked by a canine dick";
	[]
	sort the table of fucking options in sortorder order;
		...
		if calcnumber > 0 and calcnumber <= the number of filled rows in table of fucking options:
			...
			if Player consents:
				...
				if (nam is "Fuck his muzzle"):
					say "[RemusThreesomeSex1]";
				if (nam is "Fuck the wolf's ass"):
					say "[RemusThreesomeSex2]";
				if (nam is "Take Remus cock in your pussy"):
					say "[RemusThreesomeSex3]";
				if (nam is "Take Remus's cock in your ass"):
					say "[RemusThreesomeSex4]";
				now lastfuck of Brennan is turns;
				wait for any key;
		else if calcnumber is 0:
			now sextablerun is 1;
