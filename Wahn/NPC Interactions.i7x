...
4	"Xerxes_Snow_Repeat1"	Xerxes_Snow_Repeat1	"[EventConditions_Xerxes_Snow_Repeat1]"	Grey Abbey Library	2500	16	50

Table of WalkInEvents (continued)
Priority	Name	EventObject	EventConditions	EventRoom	LastEncounterTurn	CoolDownTurns	EncounterPercentage
4	"Xerxes_Snow_Repeat1"	Xerxes_Snow_Repeat1	"[EventConditions_Xerxes_Snow_Repeat1]"	Grey Abbey Library	2500	16	50

to say EventConditions_Xerxes_Snow_Repeat1:
	if Xerxes is collected and Snow is collected: [list of conditions here]
		now CurrentWalkinEvent_ConditionsMet is true;

Table of GameEventIDs (continued)
Object	Name
Xerxes_Snow_Repeat1	"Xerxes_Snow_Repeat1"

Xerxes_Snow_Repeat1 is a situation.
ResolveFunction of Xerxes_Snow_Repeat1 is "[ResolveEvent Xerxes_Snow_Repeat1]".
Sarea of Xerxes_Snow_Repeat1 is "Nowhere". [standard walkins that cannot be hunted for are Nowhere, but walkin events can also be made huntable as an alternate access way]

to say ResolveEvent Xerxes_Snow_Repeat1:
	if debugactive is 1:
...
Table of NavInEvents (continued)
Priority	Name	EventObject	EventConditions	EventRoom	LastEncounterTurn	CoolDownTurns	EncounterPercentage
4	"Helen_Snow_Repeat1"	Helen_Snow_Repeat1	"[EventConditions_Helen_Snow_Repeat1]"	Grey Abbey Library	2500	16	50

Table of WalkInEvents (continued)
Priority	Name	EventObject	EventConditions	EventRoom	LastEncounterTurn	CoolDownTurns	EncounterPercentage
4	"Helen_Snow_Repeat1"	Helen_Snow_Repeat1	"[EventConditions_Helen_Snow_Repeat1]"	Grey Abbey Library	2500	16	50

to say EventConditions_Helen_Snow_Repeat1:
	if Helen is collected and Snow is collected: [list of conditions here]
		now CurrentWalkinEvent_ConditionsMet is true;

Table of GameEventIDs (continued)
Object	Name
Helen_Snow_Repeat1	"Helen_Snow_Repeat1"

Helen_Snow_Repeat1 is a situation.
ResolveFunction of Helen_Snow_Repeat1 is "[ResolveEvent Helen_Snow_Repeat1]".
Sarea of Helen_Snow_Repeat1 is "Nowhere". [standard walkins that cannot be hunted for are Nowhere, but walkin events can also be made huntable as an alternate access way]

to say ResolveEvent Helen_Snow_Repeat1:
	if debugactive is 1:
...
Table of NavInEvents (continued)
Priority	Name	EventObject	EventConditions	EventRoom	LastEncounterTurn	CoolDownTurns	EncounterPercentage
4	"Amy_Snow_Repeat1"	Amy_Snow_Repeat1	"[EventConditions_Amy_Snow_Repeat1]"	Grey Abbey Library	2500	16	50

Table of WalkInEvents (continued)
Priority	Name	EventObject	EventConditions	EventRoom	LastEncounterTurn	CoolDownTurns	EncounterPercentage
4	"Amy_Snow_Repeat1"	Amy_Snow_Repeat1	"[EventConditions_Amy_Snow_Repeat1]"	Grey Abbey Library	2500	16	50

to say EventConditions_Amy_Snow_Repeat1:
	if Amy is collected and HP of Amy > 1 and Snow is collected: [list of conditions here]
		now CurrentWalkinEvent_ConditionsMet is true;

Table of GameEventIDs (continued)
Object	Name
Amy_Snow_Repeat1	"Amy_Snow_Repeat1"

Amy_Snow_Repeat1 is a situation.
ResolveFunction of Amy_Snow_Repeat1 is "[ResolveEvent Amy_Snow_Repeat1]".
Sarea of Amy_Snow_Repeat1 is "Nowhere". [standard walkins that cannot be hunted for are Nowhere, but walkin events can also be made huntable as an alternate access way]

to say ResolveEvent Amy_Snow_Repeat1:
	if debugactive is 1:
	...
	if randomnumber is:
		-- 1: [telling stories]
			say "you spot Snow any Amy sitting together on one of the sofas between the bookshelves. Curious as you are, you wander closer to them and listen in for a moment - it's mostly a pretty innocent conversation, with Snow telling stories from before and Amy asking questions as she listens intently, her tail wagging happily. There are a few stories in which Snow tells a bit racier stuff about her previous boyfriends, but even that is pretty tame, compared to what's going on out on the streets every day now. In the end, you just leave the two of them to have their girl-talk in peace.";
		-- 2: [workshop lessons]
			say "you hear voices come from the small side room in which Snow has her workshop set up. Curious, you walk over to check out what is going on and find Amy and Snow inside, with the big squirrel just explaining something to the attentively listening husky. While the young woman then starts to loosen a screw of the item on the workbench in front of her, Snow turns her head to look at you and gives a nod in greeting. Then she says, 'Our sexy little doggie here isn't half bad with a wrench and screwdriver. Give her half a year of lectures and she could be up with the best of [']em - me, for example.' Snow smiles as Amy gives a shy 'Thank you' to answer her praise, then tells you 'Don't let us keep you from doing any other stuff - we're gonna be busy for quite a while, until Amy can take apart and assemble this blindfolded...'";

Table of NavInEvents (continued)
Priority	Name	EventObject	EventConditions	EventRoom	LastEncounterTurn	CoolDownTurns	EncounterPercentage
4	"Amy_Felinoid_Repeat1"	Amy_Felinoid_Repeat1	"[EventConditions_Amy_Felinoid_Repeat1]"	Grey Abbey Library	2500	16	50

Table of WalkInEvents (continued)
Priority	Name	EventObject	EventConditions	EventRoom	LastEncounterTurn	CoolDownTurns	EncounterPercentage
4	"Amy_Felinoid_Repeat1"	Amy_Felinoid_Repeat1	"[EventConditions_Amy_Felinoid_Repeat1]"	Grey Abbey Library	2500	16	50

to say EventConditions_Amy_Felinoid_Repeat1:
	if Amy is collected and HP of Amy > 1 and Felinoid companion is tamed: [list of conditions here]
		now CurrentWalkinEvent_ConditionsMet is true;

Table of GameEventIDs (continued)
Object	Name
Amy_Felinoid_Repeat1	"Amy_Felinoid_Repeat1"

Amy_Felinoid_Repeat1 is a situation.
ResolveFunction of Amy_Felinoid_Repeat1 is "[ResolveEvent Amy_Felinoid_Repeat1]".
Sarea of Amy_Felinoid_Repeat1 is "Nowhere". [standard walkins that cannot be hunted for are Nowhere, but walkin events can also be made huntable as an alternate access way]

to say ResolveEvent Amy_Felinoid_Repeat1:
	if debugactive is 1:
...
instead of navigating Grey Abbey Library while (Carl is in Grey Abbey 2F and HP of Carl > 9 and Helen is in Grey Abbey Library and a random chance of 1 in 10 succeeds):
	say "[NavCheck Grey Abbey Library]";
	if NavCheckReturn is false, stop the action;
	move player to Grey Abbey Library;
	say "[CarlHelenInteraction]";

instead of going up from Bunker while (Carl is in Grey Abbey 2F and HP of Carl > 9 and Helen is in Grey Abbey Library and a random chance of 1 in 10 succeeds):
	move player to Grey Abbey Library;
	say "[CarlHelenInteraction]";
...
instead of navigating Grey Abbey Library while (Carl is in Grey Abbey 2F and HP of Carl > 10 and David is in Bunker and a random chance of 1 in 10 succeeds):
	say "[NavCheck Grey Abbey Library]";
	if NavCheckReturn is false, stop the action;
	move player to Grey Abbey Library;
	say "[CarlDavidInteraction]";

instead of going up from Bunker while (Carl is in Grey Abbey 2F and HP of Carl > 10 and David is in Bunker and a random chance of 1 in 10 succeeds):
	move player to Grey Abbey Library;
	say "[CarlDavidInteraction]";
...
to say CarlDavidInteraction:
	...
	if randomnumber is:
		-- 1: [talking]
			say "you see Carl and David sitting together in one of the reading groups, regaling each other with loudly told stories about their experiences in the military, from basic training on till right now. Mentally adding in a couple of ice-cold beers and of course making Carl a human, this scene would have fit very well into the world before the nanite infection spread. It's nice to see that at least some people are able to keep their wits and do not go feral, even with everything that's been going on. You leave the two men to chat in peace, not wanting to disturb them.";
		-- 2: [training]
			say "Carl and David pass it, both dressed in shots and loose-sleeved shirts. They're jogging, from the looks of it, following the walls of the main library room in a big circuit. Something to be expected, being soldiers and all - they've got to keep in shape somehow. As they go along, the two of them chat merrily with each other, strengthening the bond that springs from being two soldiers brought together by crazy circumstances.";
