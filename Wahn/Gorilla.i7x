...
to say Gorilla Desc:
	...
	if inasituation is false: [regular scenes]
		if bodyname of player is "Football Gorilla" and player is pure:
			...
			say "     A burly and muscular gorilla steps into your line of sight. He is dressed in football shoulder pads and wears a helmet, both painted in the local team's colors. As the gorilla spots you, a smile spreads over his face and he drops on all fours, using his long arms as support as he quickly rushes towards you. [if cunts of player > 0 and cocks of player is 0]'Hey girl, nice to meet'cha. I was hoping to run into some hotties today - and there you are, lucky enough to meet one of your team's stars. [else]'Hey dude, nice to meet'cha. I was hoping to run into some hotties today - and there you are, lucky enough to meet one of your team's stars. [end if]Yup, I'm in the Tenvale Wildcats - well, Tenvale Gorillas now, anything else would be silly. So - wanna have some fun right now, or should I show you some moves first?'";
...
instead of navigating Astroslide Field Lockerroom while (GorillasMember > 1 and GorillasMember < 10 and GorillasMemberQuestCounter - turns > 8):
	...
	if GorillasMember is 7: [maximum points]
	...
	else if GorillasMember > 4: [okay results]
		...
		if player consents:
			...
			LineBreak;
			say "     ([link]Y[as]y[end link]) - Water [if player is female]Girl[else]Boy[end if].";
			say "     ([link]N[as]n[end link]) - Laundry [if player is female]Girl[else]Boy[end if].";
			if player consents:
				LineBreak;
				say "     <WIP>";
