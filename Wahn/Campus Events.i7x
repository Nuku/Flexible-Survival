...
to LibrarySexEvent:
	if (demon brute is companion of player and CollegeLibraryBrutusEncounter is 0):
		...
		if DBCaptureQuestVar is 5:
			...
			say "     You stumble upon Brutus, busy plowing a female student from behind. He is gently holding the mare over the ground with both arms, while he pumps inside her in a slow, but deep rythmn. The two of them are locked in a deep kiss, partly in order to quell their moans of pleasure. 'We're winning,' Brutus grins. 'You- you're still on this?' the student answers between two stiffled moans. The two go at it for another minute or so, until Brutus['] grunts get louder, and he unloads his balls inside the student, her belly bulging out visibly from the sheer quantity of demonic sperm.";
