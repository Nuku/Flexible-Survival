...
to say Françoisdesc:
	say "     François, the unusual hybrid before you, has an appearance that can only be described as a mutt. His short, stocky canine body and face are covered with a thick, mottled coat of fur made up of several different colors and patterns. There even appear to be small feathers poking out of his pelt in several places. Finally, a rather feline set of ears and tail complete the appearance. He wears a surprisingly clean white apron over his torso, which does a reasonable job of protecting his decency while still allowing his long tail to flit about unhindered.";
...
Instead of conversing the François:
	if HP of Karen is 6 and companion of player is Retriever Girl:
		...
		say "     [if daytimer is day]Bonjour[else]Bonsoir[end if], [one of]mon ami[if cunts of player > 0 and cocks of player is 0]e[end if][or]my friend[at random].  [one of]Comment ça va?[or]Comment allez-vous?[or]How are you?[at random]";
	else:
...
instead of fucking the François:
	...
	else if libido of François is 1:
		say "[françoissex1]";
...
to say muffin muffin Use:
	...
	if "Male Preferred" is listed in feats of player or "Modest Organs" is listed in feats of player or ( "Single Sexed" is listed in feats of player and cocks of player > 0 ):
		...
		Follow the cunt descr rule;
		say ". As you swallow the last bite, a warm tingling wells up in your belly, slowly spreading across your body. The sensation focuses on your groin, your [if cunts of player is 1][one of]cunt[or]pussy[or]vagina[or]cleft[at random] [one of]engorging[or]swelling[or]throbbing[at random] as it grows[else][one of]cunts[or]pussies[or]vaginas[or]clefts[at random] [one of]engorging[or]swelling[or]throbbing[at random] as they grow[end if], becoming [descr].";
	decrease hunger of player by 5;
	if hunger of player < 0, now hunger of player is 0;
