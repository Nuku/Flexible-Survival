...
Instead of resolving a Canine Patrol:
	...
	if gshepsquad is 1:
		...
		if the player consents:
			say "     As you approach the police dogs, one turns to face you while the other tends to the woman. He is wearing a policeman's cap, jacket, a pair of mirrored sunglasses and no pants. 'Are you [if catdogstate is 2]here to help?' he asks with a wag of his tail upon noticing your canine features[else if catdogstate is 1]a witness to this attempted assault?' he asks[otherwise]an accomplice to this attempted assault?' he barks upon noticing your feline features[end if]. You explain that you'd just arrived on scene and were about to help when they appeared[if catdogstate is 2]. 'Good dog,' he says with another wag, 'but it's best to leave the police work to us.' [else if catdogstate is 1]. 'I see,' he says, looking you over. [otherwise]. 'A likely story, cat,' he growls. [end if]He gets your statement and only has a few basic questions for you.";
			...
			say "     'That's a good girl,' the officer you were speaking to says, his own cock rising from its sheath as he rubs it. 'You're free to go, citizen. We'll take it from here,' he says in conclusion, turning momentarily back to you. [if catdogstate is 2]While a part of you wants to join in, you're[otherwise]You're[end if] overall a little stunned by the odd turn of events and decide to head off for the moment.";
			now gshepsquad is 2;
		else:
...
to say k9vansdesc:
	if gshepsquad < 4:
		...
		say "     'Hey! I'm glad you came. Sorry for all the questions earlier, but the fact that you even came shows me that you're [if catdogstate is 2]a good dog[else if catdogstate is 1]an alright person[otherwise]an alright person despite that feline taint you've picked up[end if],' he says, getting up and offering you his paw for a shake. 'Now, you were talking about wanting to get some help, but as you can see, we're not in much position to help out right now. We're cops and good dogs though, so we do our best.'";
...
to say sexwithBuster:
	catdogstatecheck;
	if debugactive is 1:
		say "DEBUG -> gshepsquad: [gshepsquad], HP: [hp of Buster], catdogstate: [catdogstate], Msg 1: [if busterstory1 is true]Y[otherwise]N[end if], Msg 2: [if busterstory2 is true]Y[otherwise]N[end if] <- DEBUG[line break]";
	if gshepsquad < 5:
...
[if catdogstate is 2][else if catdogstate is 1][otherwise][end if]
